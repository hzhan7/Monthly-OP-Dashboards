...
（tracked，2016-02 起，每月往右追一行 —— 期数别写死在这里），
`build/ibkr.py` 只查表、一个 PDF 数字都不解析；
...
哪些页已迁移：`python3 tools/exhibit_check.py status`（现读 data/*.js，名单不写死在这里）。
...
3. **未到披露期不下载。** 28 家的披露日从次月 1 号散到 21 号，要覆盖全部窗口就得
   ...
   **闸门比 LAG 提前 `EARLY = 5` 天开，不要改成和红点一样的 `+ GRACE`。** 两者共用
   ...
   它季末月的实测发布日一度在第 15 到第 41 天之间飘。**这条已经翻篇**：2026-01
   数据起官方改成「每月 15 日或顺延」的固定节奏，季末月不再是例外，LAG 与闸门
   都已跟着收，见 `fetch/spgi.py` 的发布节奏一节与 `monthly_run.EARLY_BY`）。
...
- 节奏表 `build/roster.py` 的 `LAG` 是 `(常规月, 季末月)` 两个值。SCHW / HOOD 的
  **季末月（3/6/9/12）没有独立月报**，数值随当季财报走；LPLA 的季末月机制不同
  （要等**下一个月**的 Historical File，见 `LAG` 里那条注释），但同样晚 2-4 周。
  用同一个日子判断会每季度误报一次红点，而每季度假一次的警报，人很快就学会无视了。
- **SPGI 曾在这份名单里，2026-08 复核后移出。** 公司在 2025Q4 财报 8-K
  （filed 2026-02-10）里承诺「2026-01 数据起，每月 15 日、撞非工作日顺延到下一个
  工作日发布上月数据」，季末月一并纳入；2026 年 6 个月逐月核对全中，其中 2026-03
  与 2026-06 两个季末月都比当季财报早 13 天出。`LAG['spgi']` 因此从 `(16, 46)`
  收成 `(18, 18)`。**两档不同不是越宽越保险**：旧的 46 会让真正缺货的季末月拖到
  第 51 天才变红。实测发布日全表在 `fetch/spgi.py` 的 docstring 里，只增不改。
...
- **SCHW / LPLA**：季末月（3/6/9/12）没有独立月报，数值取自当季季报。
  ...
  **月报**里 2026-01 那期才新增、只回填到 2025-01，在**季报**里却一直有（本地缓存的
  ...
  schw 页 Exhibit 10 / 11（紧跟各自的月频兄弟 Exhibit 8 / 9）。**教训**：一条列的「披露边界」要分「哪一列」和「什么频率」
...
- **时序图的窗口左端统一为 2016-01**（2026-08-18 定）。在此之前各页各定各的：
  ...
  db1 的序列从 2002-01 起、tmx 从 2001-12 起、ndaq 从 2005-09 起（**期数别写死在这里** ——
  `series/*.csv` 每月往右加一行；上一版这里写死了三个期数，2026-09-02 复核时
  已经有两个对不上），
  页面上除了每页那 1-2 张「全历史」图之外一律只画近两年。
...
- **全站同比只有一种口径：单月**（当月 ÷ 去年同月 − 1）。2026-09 页面所有者定的，
  原话是「我就需要直接的月度数据 yoy 同比折线图，不要给我搞 12 月滚动合计同比」。
  好处是**可核对**：拿这根柱除以 12 根柱之前那根，就是次轴金线上的这一点。
  （**例外全站两张，都在图上自己声明了**：`cost` Ex4 的线是公司报告 y/y、基期是
  同样周数的上年错位窗口，对不上的月份里最大的四个正是 53 周财年那四个月
  （Jan-18 / Jan-19 / Jan-24 / Jan-25，最大 26.6pp），其余都在 5pp 以内 ——
  **可比月与对不上的条数每月都在长，别在这里写死**；
  `ibkr` Ex2 的线用公司 Notes 还原后的账户增长，三个月对不上。
  逐张实测见 `build/CONTRACT.md` §6.1 第 1 条 —— 没有这句声明的对不上就是 bug。）
  代价没有消失，改由每张图自己承担 —— 图注里印出这条序列自己实测的毛刺
  （逐月标准差、相邻月最大跳变、与 12 个月滚动口径符号相反的月份数），
  由 `build/yoy.py` 的 `caliber_diff()` / `describe()` 现算。
  ⚠️ **这一条判据一条都不查**（2026-09-02 21:0x 逐条读过当时的 R0–R5、23:5x 连后加的
  R7 / R8 一起再读一遍，没有一条查图注有没有印代价），
  所以它坏了不会响：2026-09-02 21:0x 实测，回源确定命中流量列的 106 张图里
  还有 10 张欠着（9 张 `heat_matrix` + `spgi` Ex2），而判据报 `🔴 0 🟡 0`。
  同日 23:5x 照同一套数法重跑，欠账那两个数一格没动（**但它们是活数，引用前重跑**）；
  判据的输出那一半当晚就变了 —— 23:50 它长出一条 R8（近零基数序列没声明），当次报了
  10 张 🟡；23:59 再扫，同一条报 0 张（逐张核过的只有 `db1` Ex29：那一页当晚被重建，
  图注按 §6.1 第 5 条点了名、右轴也截到 +400%）。
  **而查「有没有印代价」的规则仍然一条都没有。**
  欠账清单、数法与建议的补法都在 `build/CONTRACT.md` §6.1 第 3 条与 §6.6。
  **这翻掉了 2026-08 全站审计定下的上一版默认（滚动）**，但没有翻掉那次审计的测量 ——
  只是那批数**必须连窗口一起引**：审计是在当时的图窗（近 25 个月）上量的。
  2026-09 在审计自己的分母上复算，对得上的是「65% 的可比序列至少有一个月两种口径
  符号相反」（复算 138/212 —— **分母是活数，那是 2026-09-02 20:4x 那份 `data/` 快照
  的读数**；同日 21:4x 重跑是 138/213，占比同为 65%。审计记 147/225。
  引用前照 §6.5 那段配方当场重跑，别拿这里的绝对条数去对今天的仓）；
  对不上的是「噪声是滚动的 2 倍以上」——
  同一批序列的标准差放大倍数中位复算只有 **1.9 倍上下**（20:4x 快照 1.87、
  同日 21:4x 重跑 1.86 —— 分母是活数；2.08 倍只在「全部 CSV 流量列」
  那个另一个分母上成立，`python3 build/yoy.py` 第 ⑦ 节当场印）。图窗 2026-08-19
  那次改动拉到 2016-01（到 2026-07 满打满算 127 个月，短序列从自己首月起）之后，
  这两个数变成 **1.40 倍 / 93%**。逐条复算与「哪个数
  复算不出来」记在 `build/CONTRACT.md` §6.5。变的是量级不是方向：单月口径毛刺更大、
  单看一个月能把方向读反，这件事没变，所以每张流量同比图都得把这笔账印在旁边。
  只是那次审计要解决的真正病根是「同一页混用两种口径」，统一成单月同样解决，
  而这是所有者要的那一种。
  例外共**两页五张**（都不是折线）：`exchanges-apac` Ex5 的三年连排分组柱与 Ex15 的
  量价分解桥、`exchanges12` Ex4/7/8；其中 Ex15 是由所有者对 Ex5 的回答**推出来的**、
  不是原话（`build/CONTRACT.md` §6.2），所以它是名单上最先该被重新审的一条。
  同比仍然只有一份实现（`build/yoy.py`）+ 一条契约
  （`build/CONTRACT.md` §6）；存量序列（OI / AUM / 账户数）走点对点同比、禁止滚动合计。
...
- 不同量纲一律拆成独立图表；确需双轴的，两轴零点画在同一高度 ——
  ...
  这时改为两轴各自缩放，并在图上写明「左右轴零点不同高」。对齐成功而柱全为正时，
  零线以下那一段只归右轴，不画左轴刻度与网格线（不印「−$100」这种不存在的左轴读数）
- **月度柱可按业务分部分色堆叠**（`gs_bar` 的可选 `stacks[]`），但**总额仍是那一根柱**：
  ...
  **没有改用堆叠柱专用图型（`stacked_dual`）是刻意的**：那个图型的右轴下界写死为 0，
  而这里的次轴是同比、会转负（单月口径下转负比滚动口径更频繁），负值会被顶到轴外 ——
...
- **「合计 + 分项」的组画成两张图，不画成一张多列折线**（`build/single.py` 的
  `groups[].mix`，2026-09 起；哪几页在用不在这里存，`grep -l "'mix'" build/specs/*.py` 现数）：第一张是**合计的水平值柱 + 次轴同比**
  （流量走**单月**同比、存量走点对点同比 —— 口径写进标题，
  图注里把单月口径的实测代价一起交代，见 `build/CONTRACT.md` §6.1 第 3 条），
  第二张是**分项的 100% 堆叠占比图**（`stacked_dual`，每根柱恒高 100%，只讲结构不讲规模）。
  两张回答的是两个问题：柱图答「这门生意有多大、在不在长」，占比图答「结构往哪边走」——
  而占比最有价值的场合恰恰是「总量在涨、某一块的占比反而在掉」。
  同一个合计若还有第二种成立的加总关系（例如衍生品成交既能按期货 / 期权切、也能按资产类别切），
  写进同一条 mix 的 `alt_splits`（2026-09-12 起）：合计柱仍只画一次，每一种切法各出一张占比图、
  分母相同，**跨图的段不许相加相减** —— 底座只复算每一刀自己的加总关系，不复算刀与刀之间的包含关系。
