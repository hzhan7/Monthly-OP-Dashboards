...
2. **`TEXT_OVERLAP` 不是一个根因，是好几类 —— 「修一处清一片」只在某一轮成立。**
   ...
   **第一行那个改动有一笔代价，当时的 commit message 把它算漏了，在此更正。**
   原文写「右轴每图恰好删 1 根，与消掉的 27 条压字一一对应」—— 前半句对，
   **后半句错**：那 27 张图是在 1280px **一个**视口下数的，而 27 条压字是跨
   1280/768 **两个**视口计的，两个分母不同，「一一对应」是凑巧撞上的假象。

   实际情况是：让位判据 `over()` 带 1px 容差（与左轴那段共用同一个判据），
   所以**只要刻度与标签的墨迹擦到边就删**，不管这处重叠够不够 8px² 的起报线。
   实测：在 8 个**QA 从未报过任何压字**的页（asx / cboe / guc / hkex / jpx /
   lpla / msci / ndaq，共 127 张图）上，仍有 **11 张各少 1 根右轴刻度**。
   全站量级约 39 张图各少 1 根，其中十几张并没有对应的已报压字。

   这笔代价可以接受，但要知道它存在：① 判据与左轴一直以来用的**是同一个**，
   属于口径一致，不是新加的激进；② 每张图只删 1 根，改后每图最少仍剩 4 根（右）/
   3 根（左），离「至少留 2 根」的下界还远；③ 左轴全站一根未删，既有行为没被污染。
   想收紧就把容差从 +1 改成 0 —— 但那会让左右两轴的口径分家，不建议。
   ...
   **刻意不含重叠面积** —— 含面积的话「同一条变小了」会被记成「消失 1 条 + 新增 1 条」，
   ...
   下面每一条都对应本文件里某一次**真实翻车**，并且都写成「要动手做什么」而不是
   「要注意什么」—— 之所以这么写，是因为 2026-08-19 那天上午刚把「别拿两个口径的
   数字对上就算验证过」当成道理写进本节，下午就又犯了一次（见上面那笔更正）。
   **写下来拦不住自己，动作才拦得住。**

   1. **两个数字相等，先问分母是不是同一个。** 我拿「1280px 单视口下 27 张图少了刻度」
      对上「跨 1280/768 两视口共消掉 27 条压字」，宣布「一一对应」—— 两个 27 毫无关系。
      **数字撞上是巧合的先验概率，比直觉里高得多**：一次改动里能数的量就那么几个，
      量级又都在同一个数量级上。动作：写下每个数字时，把它的口径也写下来
      （几个视口？单位是「图」还是「条」？分母是全站还是抽样？），再比。
   2. **区分「定点修复」与「普遍性收缩」。** 同样是「压字少了 27 条」，
      如果只有报过压字的那些图被动了，是定点修复；如果没报过压字的图也被顺带改了，
      那是普遍性收缩 —— **两者的风险画像完全不同**，而漂亮的说法会把后者藏起来。
      动作：拿「被改动的对象数」去比「被修复的缺陷数」，差额就是**过冲**，
      过冲必须单独报，不能并进「修好了 N 条」里。
   3. **声称「输出逐字节不变」就要真去比字节。** 别拿「FS=1 时 `fscale(8)` 恰为 8」
      这种推断当实测。动作：改动前后各渲染一遍，比对每张 `<svg>` 的哈希，
      报「N 张里 M 张相同」，并说清变了的那几张是什么共同特征
      （本轮实测 44 张里 40 张逐字节相同，变的 4 张 viewBox 宽全是 1172 = 通栏卡）。
...
4. **有一类压字是几何上无解的，验收标准要从「有没有重叠」换成「读不读得出」。**
   ...
   **两段都短于 165.7px**，所有候选位都被边界过滤掉。`lpla` Ex5 同型。
   ...
   评估时还出现过第三个选项：给 `visual_qa` 加一条「已被 z 序兜底救回的断点标签重叠」
   **豁免判据**，让这 4 条不再报出来。这个选项被否掉了，理由要单独记一笔，
   因为它和前两个选项不是一回事：

   > 我不主张动判据去让报表好看 —— 那等于让工具停止报告一个真实存在的几何事实，
   > 以后没人知道这 4 条还在。

   前两个选项（挪、缩字号）动的是**图**，这一个动的是**判据**：它不修问题，
   它修的是「问题被看见」这件事。护栏一旦学会替我们找理由，它就不再是护栏了。
   留着 🟡 + 在这里写清楚为什么不修，比让它从报告里消失诚实得多。
...
| 2.5 步长被印成整数 0/3/5/8/10/13/15（`ndaq` Ex10 —— 2026-09-16 重排前的号，冻结，§2.B） | `AXIS_UNEVEN` 🔴 | 偏离 33.3%（**在真站上抓到 7 张，见 §8**） |
...
| `cme` | Ex9（2026-09 重排前是 Ex12） | `AXIS_UNEVEN` | 右轴 `20% │ 18% │ 15% │ 13% │ 10% │ 8% │ 5% │ 3% │ 0%` —— 真步长 2.5%，**与当年 `ndaq` Ex10 那条一模一样**（那也是 `ndaq` 2026-09-16 重排前的号） |
