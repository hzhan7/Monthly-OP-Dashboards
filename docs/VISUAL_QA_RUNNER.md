...
2. **`TEXT_OVERLAP` 不是一个根因，是好几类 —— 「修一处清一片」只在某一轮成立。**
   2026-08-07 那一轮 40 条里 38 条同根因（竖排红色断点标签横穿柱顶数值），
   所以当时这么写是对的。但根因构成会随数据和窗口长度变，**不要把它当常识沿用**。

   2026-08-19 复查 52 条（🟡 48 + 🔵 4）的分解与处置，可作为分类方法的样板：

   | 根因 | 条数 | 代码点 | 处置 |
   |---|---:|---|---|
   | 「刻度让位给数值标签」只认左轴（`data-tick="l"`） | 27 | 让位循环 | 推广到左右轴，**全清** |
   | `lines_endlabels` 左端标签列写死 30px，装不下宽读数 | 8 | `M.l` 算式 | 改按内容定宽，**全清** |
   | `seasonality` 是唯一没接 `thinLabels` 的图型 | 2 | seasonality 分支 | 接上，**全清** |
   | `capSlot()` 截轴注记槽距写死 8px、不随 `fscale` 缩放 | 7 | `capSlot` | 改 `fscale(8)`，**全清** |
   | `ex.ylab` 从未接入 `fitVertical`，竖排长度无人守画布边界 | 1 | ylab 绘制处 | 接上，**全清** |
   | 「优先标签避让」按 `rotate` 一刀切，漏掉 `qtr_bar` 的竖排柱顶读数 | 3 | `priorityLabs` 的 others | 按来源判，**全清** |
   | 竖排断点注记 × 数值：几何不可解 | 4 | 见第 4 条 | **保留，带理由** |

   收尾状态：🔴 0 / 🟡 4 / 🔵 0，剩的 4 条就是最后一行那个。

   **验收方法比分类本身更要紧**：改完必须跑 `--all` 前后**逐条 diff `defects`**，
   判据是「只准减、不准增」。一条缺陷的身份取 `(page, viewport, type, ex, a, b)`，
   **刻意不含重叠面积** —— 含面积的话「同一条变小了」会被记成「消失 1 条 + 新增 1 条」，
   账面上永远新增大于零，久了就会开始容忍「新增几条正常」，这道闸就废了。

   **别按压字对的字面去分类 —— 这一轮我就分错了，而且错得很典型。**
   报告只给你两个字符串和一个 `kind`，照着分会得到「带 % 的 × 不带 % 的」「相邻数值互压」
   这种**现象分类**，它和根因不是一回事。上表第 4 行那个 `capSlot` 就是反例：

   | 报告里长什么样 | 报成的 kind | 我原先归的类 |
   |---|---|---|
   | `lpla` Ex7 「211.1」×「81.7」 | `lines_endlabels` | 折线末端读数互压 |
   | `schw` Ex7 「1,095」×「1,211」 | `gs_bar` | 相邻数值标签互压 |
   | `cost` Ex5 「32.5%」×「33.5%」 | `bar_line` | 相邻百分比标签互压 |

   三个页、三个 `kind`、三个「不同」的类 —— 实际是**同一个函数画的同一种注记**
   （`capSlot()` 排位的竖排红色截轴真值），改一行 `8` → `fscale(8)` 一起消失。
   反过来，同一个 `kind` 里也会混着不同根因（`gs_bar` 的 31 条里，27 条是刻度让位、
   4 条是 `capSlot`）。

   **真正的分界是「这两个 `text` 分别是谁画的」**，只有到 DOM 里看 `fill` / `transform` /
   父节点才分得出来。方法：把两个字符串在浏览器里选出来，看它们的属性，再回代码里找
   `txt(...)` 的调用点。别读报告猜。
...
4. **有一类压字是几何上无解的，验收标准要从「有没有重叠」换成「读不读得出」。**
   竖排断点注记压住数值标签时，`charts.js` 的两级机制是：先沿竖线平移到空白段，
   **挪不动就原地不动**，再由 z 序保命（把被压的数值 `appendChild` 回末尾，
   它自带的白色描边在红字上打个洞）。第二级只救可读性，**不改几何**，
   所以 QA 照旧记这条重叠 —— 这是设计如此，不是漏修。

   2026-08-19 实测 `enx` Ex22：注记「并入雅典交易所（Euronext Athens）」竖排长 165.7px，
   同一竖直条带上唯一的障碍「12.7」（y 126.4–138.4）把 259.7px 的绘图区切成两段，
   **两段都短于 165.7px**，所有候选位都被边界过滤掉。`lpla` Ex5 同型。
   两处的 z 序保命都已生效（数字画在红字之上，读得出）。

   曾评估过「挪不动就缩字号再挪」的两趟方案，**否掉了**，理由不是做不到：
   ① 那段 `brks.forEach` 被 **181 张图 / 24 个页 / 12 个 kind** 共用，
      拿这么大的回归面去换 2 张图上不影响读数的 🟡，代价与收益倒挂；
   ② 更要紧的是它引入**新的一类失败模式** —— 字号变成随图而异的隐式状态，
      以后没人能预测某张图的注记会是几号字，而这类问题 `visual_qa` 抓不到
      （它不判字号），只会在某天某张图上突然小一号。
   所以这 4 条的结论是：**几何不可解 + 可读性已由 z 序兜住 + 修复代价倒挂 ⇒ 保留**。
   报告里再看到它们，不必重新调查。

5. **合并一个老基线的分支时，它会把中间落地的特性静默删回去 —— 闸门全绿，没人会发现。**
   2026-08-19 摘 `backup/axp-2016-preRebase` 上那个「刻度让位推广到右轴」的改动时，
   同一份 `git diff` 里还躺着一个**反向 hunk**：把 `gs_bar` 的 `bar_labels: false`
   开关整段删掉。不是有人要删它 —— 是那条分支的基线**早于该特性入库**，
   于是 `git diff origin/main <branch>` 把「main 有、分支没有」如实报成了删除。

   这类事故在本仓最难查：它不报错，`verify_pages` / `check_yoy_caliber` /
   `visual_qa` 三道闸全绿（没有任何判据在看「某个 opt-in 开关还在不在」），
   受影响的图只是悄悄多出一排本该关掉的数值，要等人肉眼发现。

   **规矩：从旧分支摘改动一律逐 hunk 看，不要 `git cherry-pick` 整个提交、
   也不要 `git checkout <branch> -- <file>` 整文件覆盖。** 摘之前先跑
   `git log <branch>..origin/main -- <file>`，看清楚这条分支落后了哪些提交。
