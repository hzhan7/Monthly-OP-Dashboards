...
2. **`TEXT_OVERLAP` 不是一个根因，是好几类 —— 「修一处清一片」只在某一轮成立。**
   ...
   | 类 | 现象（压字对长什么样） | 条数 | 根因 | 处置 |
   |---|---|---:|---|---|
   | A | 带 `%`/`pp` 的数 × 不带的数 | 27 | 「刻度让位给数值标签」只认左轴（`data-tick="l"`） | 推广到左右轴，**全清** |
   | B | 七位数读数 × 轴单位标题 | 8 | `lines_endlabels` 左端标签列写死 30px，装不下宽读数 | 改按内容定宽，**全清** |
   | C | 相邻两期数值互啃末位 | 2 | `seasonality` 是唯一没接 `thinLabels` 的图型 | 接上，**全清** |
   | D | 竖排中文注记 × 数值 | 4 | 几何不可解（见第 4 条） | **保留，带理由** |
   | E | 其余（相邻标签、截轴注记、轴标题压 legend） | 11 | 若干互不相干的小根因 | 逐条处理 |
   ...
   **别只看压字对的字面去分类。** `lpla` Ex7 的「211.1 × 81.7」看着像两条折线的
   末端读数互压（`lines_endlabels` 的 `spreadY` 失效），实际到 DOM 里量才发现两个都是
   `rotate(-90)` 的**红色截轴真值注记**（`fill:#C00000`），相隔仅 8px —— 连续两期都被
   截轴就会并排画出两条竖条。根因、代码点、修法全都不同。分类要到浏览器里量，不能读报告猜。
