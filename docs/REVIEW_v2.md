...
> ⚠️ **2026-09 追记**：本节验的是 2026-08-06 那张图。同一个槽位（`/exchanges-apac/`
> Exhibit 5，连排三年的 `grouped_bars`）今天画的是「**12 个月滚动合计**的同比，连排三年
> （Jul-24 / Jul-25 / Jul-26，三段互不重叠）」—— 2026-09 全站同比改单月时，它是
> **所有者直接点名保留滚动口径**的一张（例外名单共两页五张 —— `exchanges-apac` Ex5 与
> Ex15、`exchanges12` Ex4 / 7 / 8，见 `build/CONTRACT.md` §6.2）。所以「柱高是各家与
> 自己去年同月比的 y/y」这句对今天的 Ex5 已不成立（对本节验过的那张图仍然成立）；
> 「柱高是增长率不是份额」那半句不受影响。
