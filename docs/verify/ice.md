...
| **欧洲衍生品** | ICE Futures Europe（Brent / Gasoil / TTF / Euribor / SONIA / Gilts / FTSE） | `adv_stir_kcontracts + adv_mltir_kcontracts`（欧洲利率）、`adv_energy_kcontracts`、`oi_*` | 仓内目前**没有同池对手**（Cboe 的欧洲业务是现货 ADNV，量纲与内容都不同）。等 Eurex 进池再配对；在那之前 ICE 单家成图。⚠ **更正（2026-08-07）**：原文写「等 Eurex / LSEG 进仓再配对」，LSEG 已于 2026-08-07 进仓，但**只有单公司页 `/lseg/`，未进任何横截面页，也未与本池配上对**。不是漏接：LSEG 的四条腿里没有一条是场内衍生品成交量 —— LCH 那 18 列是**清算**名义额与清算边数（三条服务线单双边口径各不相同，见 `docs/verify/lseg.md`），Tradeweb 那 27 列是 OTC 固收/信用的名义额 ADV，两者与 ICE 的 `adv_*_kcontracts`（张数）量纲与内容都对不上。要配对得先定跨口径规则 |
