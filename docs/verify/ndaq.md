...
| 序列 | 本文写作时（2026-08-07） | **今天（2026-08-19 实测）** | 说明 |
|---|---|---|---|
| 美股现货 matched 成交量 / 市占 / 交易日（`vol_us_cash_matched_nasdaq_sh` 等） | **2005-09** | 2005-09（251 个月） | nasdaqtrader.com 官方 xlsx，零断档 |
| 美股现货 IR 口径合计 `vol_us_cash_matched_mnsh` | 2025-01 | **2010-10（190 个月）** | 2026-08-18 用 B 组三盘口（Nasdaq + BX + PSX）之和回补；PSX 2010-10 才上线，所以拼接段最早到 2010-10。19 个重叠月里 18 个 \|差\| ≤ 0.0101% |
| 欧洲期权与期货成交量 `vol_nordic_derivs_mmcontracts` | 2025-01 | **2013-01（163 个月）** | 2026-08-18 用 D 组「Nasdaq Nordic 交易所公告档案」回补（162 册，发布月 2013-02..2026-07 零缺口），19 个重叠月**逐月全等** |
| 美股期权成交量 `vol_us_options_mmcontracts` | 2025-01 | 2025-01（**仍是 19 个月**） | 只有 IR 那份 PDF 有，滚动 19–24 个月窗口，没有档案可回补 |
| 欧洲现货成交额 `vol_nordic_cash_value_usdbn` | 2025-01 | 2025-01（**仍是 19 个月**） | 同上 |

~~也就是说：四条头条月度序列里只有一条满足「至少 2019 起」，另外三条今天只有 19 个月。~~
**2026-08-19 更正：四条里现在有三条满足「至少 2019 起」，只剩两条（美股期权、欧洲现货）
还是 19 个月。** 回补走的是**另外两条链路**，不是那份每月原地替换的 IR PDF ——
IR PDF 的历史版本仍然只在 Wayback 上（本机 hook 硬禁），这一条没变。
季度口径可回溯到 **1Q23**（PDF 第 2 页），能顶住同比但顶不住指数化。
⚠ `build/pools.py` 里 ndaq 两条腿的 `start` 仍写着 `'2025-01'`，池子还没吃上新历史 —— 待办。

⇒ ~~如果仓库对「四条腿都要 ≥2019」是硬要求，那三条腿单独看是 **C**~~ ——
2026-08-19：需要降级的只剩**两条**（美股期权、欧洲现货），且降级方案不变
（先用季度面板 1Q23 起，月度从 2025-01 起逐月长）。整页作为一个 dashboard 建，仍判 **B**。
