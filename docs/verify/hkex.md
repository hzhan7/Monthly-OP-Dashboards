...
> **2026-08-19 追记 —— 本文写作时序列只有 91 行，现在是 127 行。**
> 2026-08-18 那一轮把 `START_MONTH` 打开到 **2016-01**，本文「不新增早于序列首月的行」
> 那条规矩**已被显式推翻**（见下面「三条不能动的规矩」第 2 条的追记）。今天的逐列覆盖：
>
> | 列 | 覆盖 | 个数 |
> |---|---|---|
> | `adt_hkdbn` / `mktcap_hkdtn` / 7 个交易列（`trading_days_cash`、`vol_shares_*`、`trades_*`、`adv_shares_mn`、`adt_trades`） | 2016-01 → 2026-07 | 127 |
> | `derivatives_adv_contracts` | 2018-01 → 2026-07 | 103 |
> | `southbound_adt_hkdbn` | 2018-01 → 2026-07 | 61（中间有官方留白） |
> | `ipo_funds_hkdbn` | 2024-01 → 2026-07 | 31 |
> | `new_listings` | 2024-06 → 2026-07 | 26 |
>
> 本文正文里的 **90 / 91** 都是 2026-08-07 那天的样本量，作为当时的实测记录保留 ——
> 它们证明的结论（档案与 xlsx 同底稿）没有被推翻，反而被 2016-2017 那 24 个月
> 对 Fact Book 的 360 项逐位核对进一步加强了（闭合 ④，写在 `fetch/hkex.py` 模块 docstring）。
> 要现值别抄本文：`python3 -c "import pandas as pd; d=pd.read_csv('series/hkex.csv');
> print(len(d)); [print(c, d[c].notna().sum()) for c in d.columns[1:]]"`
...
**可用起止月（2026-08-19 实测）：2016-01 ~ 2026-07，127 个月，零断档。**
（本文写作时是 2019-01 起 91 个月；2026-08-18 把 `START_MONTH` 开到 2016-01 后前推了 36 个月。
逐日档案本身能回到主板 1986-04 / GEM 1999-11，2016-01 是与全站起点对齐的选择，不是源的上限。）
最新月的来源分工没变：档案覆盖到哪个月就用档案，档案尚未覆盖的当月用 B2 Bulletin 兜底。
...
2. ~~**不新增早于序列首月的行。**~~ —— **2026-08-18 已推翻，这条规矩不再有效。**
   旧理由是「往前加行会改变 `build/hkex.py` 里 `df.index[0]` 与全部窗口函数的输入，
   那不归 fetch 层决定」。那条理由的前提已经没了：`build/hkex.py` 的窗口同一天改成了
   `WIN_FROM='2016-01'` 的**现算式**（不再是写死的 `.iloc[-25:]`），窗口不再依赖首月。
   于是模块常量 `START_MONTH = '2016-01'`，`update()` 里的 `_backfill_rows()`
   **只在序列首月之前造行、且只造到 START_MONTH**；序列**内部**的空档仍然一格不填
   （那是口径留白，见坑 4）。这条路径是幂等的 —— 序列一旦从 2016-01 起，
   下个月再跑直接空转、连档案都不多下一份。
   ⚠ 那次人工决策的**代价**写在 `fetch/hkex.py` 模块 docstring 里，一并读：
   2016-01~2017-12 的 `derivatives_adv_contracts` 与 `southbound_adt_hkdbn`
   **一律留空**（衍生品的机器可读源全都很浅、PDF 试解析未闭合；南向 2016-12-05
   深港通才开市，是真实口径断点）；`new_listings` / `ipo_funds_hkdbn` 也在回补行里
   永远留空，否则会造出中间空洞、被 `GAPPY_OK` 体检直接停页。
