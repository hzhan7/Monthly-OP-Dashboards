...
真值 `series/lseg.csv`（**127 行 × 86 个数据列**，2026-08-19 实测）与四张 part CSV。

> **别照抄本文里的行数 —— 现算一遍只要一行。** 本文所有「N 行 / N 个月 / X → Y」
> 都是某天的快照，回补一跑就过期（2026-08-18/19 这一轮就把订单簿腿从 67 行推到 127 行、
> 一级市场从 97 行推到 115 行）。要现值：
>
> ```bash
> python3 - <<'PY'
> import pandas as pd, glob
> for f in sorted(glob.glob('series/lseg*.csv')):
>     d = pd.read_csv(f); c = [x for x in d.columns if x != 'month']
>     print(f"{f:34s} {len(d):4d} 行 × {len(c):3d} 列  {d['month'].iloc[0]}..{d['month'].iloc[-1]}")
>     for x in c:                      # 列各自的起点不同，这一层才是真相
>         s = d.loc[d[x].notna(), 'month']
>         print(f"    {x:44s} {s.min()}..{s.max()}  n={len(s)}")
> PY
> ```

> **2026-08-19 追记（本轮回补改掉的四处事实）**：
> ① **订单簿腿从 2021-01 前推到 2016-01**（67 → 127 个月），起点那条「主动取舍」的
>    理由已被证伪 —— 详见 §路 1 的追记；
> ② **一级市场按市场分开起点**：AIM 从 2017-01 起、Main Market 仍 2018-05 起，
>    两个官方空洞的键从「月份」改成「(市场, 月份)」，各自只砸一个市场（97 → 115 行）；
> ③ **最慢的那条腿换人了**：现在拖后腿的是 LCH RepoClear（最新 2026-05），
>    订单簿已与其余三路同步到 2026-07 —— 原「订单簿晚一个月」整节因此作废，已改名为
>    「慢腿与 `data_through`」并加追记；
> ④ 全站时序图窗口从「近 25 个月」统一改成「2016-01 起」。本文不写窗口，但
>    §横截面 那一节讲「短序列不进池」的判断要照 ① 重读：订单簿现在有 127 个月，
>    不再是短序列。
...
四个网站、四种格式、四种发布节奏、历史深度从 24 个月到 127 个月不等（2026-08-19 实测：
lch 24 / primary 115 行·AIM 114 个月 / tradeweb 115 / orderbook 127）。所以抓取拆成
...
2. ~~**LSE 订单簿那条腿比其余三路晚一个月**~~ —— **2026-08-19 已不成立**。
   现在宽表 2026-07 那一行只缺 6 列，全是 `repoclear_*`（RepoClear 自身滞后约两个月），
   17 个订单簿列已经写满（2026-07 那期月报 CreationDate = 2026-08-04，月末后第 4 天）。
   **今天最慢的腿是 LCH RepoClear，不是订单簿。** 空格仍然是事实、不是抓漏，
   只是空在别处 —— 详见「慢腿与 `data_through`」那一节的追记。
...
| 落地 | 17 列，`series/lseg_part_orderbook.csv`，**2016-01 → 2026-07**（127 行，17 列一格不缺；2026-08-19 实测，此前是 2021-01 起 66 行）[A] |
...
📌 ~~起点 2021-01 是**主动取舍**~~ —— **2026-08-18 已推翻，起点现在是 2016-01。**
旧理由的后半句站不住：份额行的旧标签 `UK Lit Orderbook trading` 一直在
`UK_SHARE_LABELS` 里，2016-2020 那 60 期 **17 列一列不缺**，实测 60/60 通过解析，
其中 2017-01..2020-12 共 48 期是零代码改动直接过的。前半句（更早的月报还印 Italian /
Derivatives / MTS / EuroTLX 等行）属实，但那些是**多出来的行**，模块按标签取行、
从不按行号，多几行不影响。为收下 2016 全年只加了两处兼容（交易日行 2016-12 及以前只有
2 列；2016-03 官方自己的日均与交易日对不上，放行恒等式但入库仍是官方原值）。
**再往前（2013-01..2015-12）没做**，理由见 `fetch/lseg_orderbook.py` 口径坑 3。

📌 两源交叉核对（**2026-08-19 本机用 `cache/lseg_orderbook/` 的 127 份 PDF + 副源工作簿
离线重算**，等级 [A]，不是转抄）：**126 个月全部对得上**（2026-07 副源尚无该月故不比、
2016-07 是官方真重述月按 `_RESTATED` 跳过）。最大偏差：

| 项 | 最大绝对偏差 | 出现在 | 最大相对偏差 |
|---|---|---|---|
| 笔数 | **313 笔** | 2019-01（20,025,721 vs 20,026,034） | 1.56e-5 |
| 成交额 | **5.41 £m** | 2016-02（113,828 vs 113,833.41） | 4.75e-5 |
| 日均成交额 | 0.71 £m | 2016-01 | **1.37e-4**（全表最大） |
| 日均笔数 | 14.27 | 2019-01 | 1.57e-5 |
| 交易日 | **0 天** | — | 0 |

⚠ 上一版这里写的「66 个月、最大差 8 笔 / 1.30 £m」只是 2021-2026 那一段。
偏差随年头变大**不是解析变差**，而是「副源 xlsx 持续重述、月报 PDF 是发布当时的快照」
—— 所以比对阈值改成了绝对与相对取宽者（`_XCHECK_REL = 5e-4`，是上表最坏值 1.37e-4 的
3.6 倍余量，同时比 2016-07 那个真重述的 5.6e-3 紧 11 倍，**刚好把重述漂移与真重述分开**）。
...
| 抓取方式 | `urllib.request` + 普通 UA；2026-08-07 实测 8 线程一次拉 201 个 xlsx 零失败（本轮 AIM 回补 16 期后，`cache/` 里现存 **215 个** factsheet：AIM 117 + MM 98，2026-08-19 数） |
| 落地 | 24 列，`series/lseg_part_primary.csv`，**2017-01 → 2026-07（115 行）**。⚠ **两个市场起点不同**，一行里各写各的那一段列：`aim_*` 11 列 **2017-01 起**（114 个月，缺 2019-09）、`mm_*` 13 列 **2018-05 起**（98 个月，缺 2022-12）。两个官方空洞的键是 **(市场, 月份)**，**各自只砸一个市场** —— 2022-12 的 AIM factsheet、2019-09 的 Main Market factsheet 都好端端地在。2026-08-19 实测；此前是「共用起点 2018-05、97 行、按月整行跳过」，那等于替官方多挖一个洞（白丢了 MM 2019-09 与 AIM 2022-12 两段）[A] |
...
（次月那份的年度块已经换成次月末的数）。**一个月一个文件，没有捷径** —— 现在这条腿的
115 行背后就是 215 个 xlsx（AIM 117 + MM 98，2026-08-19 数 cache）。
...
判据取工作簿 `docProps/core.xml` 的 `dcterms:created`，样本 = **2018-05 起**两个市场**全部 197 期**
（不是抽样）。下表 2026-08-19 本机从 `cache/lseg_primary_*.xlsx` 逐份重算，与模块 docstring
一致（只有 P90 因分位插值算法不同差 1-3 天：本次算得 AIM +10、MM +8），等级 [A]：
...
⚠ **AIM 2017-01…2018-04 那 16 期不进这个样本**（虽然 2026-08-19 起它们已经入库）：
它们是本轮一次性回补下载的，`created` 已经被官方重传污染 —— **2017-01 是 +97 天
（created 2017-05-08）、2017-02 +69（同样是 2017-05-08，两期一起重导）、2017-03 +25**，
而同一批里 2017-07 / 2017-10 / 2017-11 / 2018-02 / 2018-04 **五期又都是 +1**，中位 +5。
「有的 +1 有的 +97」不是当年的发布节奏，是重传时刻的分布；拿它去调闸门只会得到一条假长尾。
⇒ **样本口径仍是 2018-05 起 197 期，LAG / EARLY_BY 不动。**
（更极端的旁证：AIM 2016-11 那份 xlsx 的 `created` 写着 **2000-02-03** —— 模板日期，
那两期本来就因老版式读不了、不在库里。）
...
复算：`python3 fetch/lseg_orderbook.py --cadence`（它读 `cache/lseg_orderbook/mmr_*.pdf`，
不联网）。下面全部是 **2026-08-19 本机重算**，等级 [A]：
...
⚠ **节奏在 2024 年明显变慢，看这条腿只能看近两年**（上面逐年那一列就是理由）。

⚠ **2016-01…2020-12 这新回补的 60 期不进闸门样本** —— 与 AIM 那 16 期同一个坑：
`CreationDate` 被重传污染。逐年中位数看着更快（2017 中位 +3、2020 中位 +2.5），
但里面藏着 **2019-04 那期的 +1855 天**（CreationDate 2024-05-28，明显是五年后重导的文件）
与 2016-09 的 +49 天。一段既有 +1 又有 +1855 的样本描述的是重传时刻，不是当年的发布节奏。
⇒ **`LAG['lseg']` / `EARLY_BY['lseg']` 不因这次回补改动。**
...
## 慢腿与 `data_through`（原标题「订单簿晚一个月」，2026-08-19 已作废）

> **2026-08-19 追记**：本节旧版说「订单簿比其余三路晚整整一个月、2026-07 那一行缺 23 列」。
> **不再成立。** 2026-07 那期月报的 CreationDate 是 2026-08-04（月末后第 4 天），
> 17 个订单簿列已经写满；今天缺的 6 列全是 `repoclear_*`。
> **慢腿的身份换了：现在是 LCH RepoClear。** 下面的表与空格分布已按 2026-08-19 重测。

**今天的事实（2026-08-19，本机现算 [A]）：**

| | 覆盖到 | 说明 |
|---|---|---|
| `series/lseg.csv`（宽表） | **2026-07**（**127 行** × 86 列） | `data/lseg.js` 的 `data_through` = `2026-07` |
| `lseg_part_tradeweb.csv` | 2026-07 | 头条腿 |
| `lseg_part_primary.csv` | 2026-07 | `aim_*` 2017-01 起 / `mm_*` 2018-05 起 |
| `lseg_part_lch.csv` | 2026-07 | 但 RepoClear 那 6 列只到 **2026-05** ← **今天唯一的慢腿** |
| `lseg_part_orderbook.csv` | 2026-07 | **2026-08-18 起点前推到 2016-01，127 行零空洞** |
...
2026-07  缺 6 列   —— 同上；旧版这里是「缺 23 列」，那 17 个订单簿列现已写满
```

**`data_through` 为什么是 2026-07：**
...
（`docs/SINGLE_SPEC.md` §5）。

⚠ **2026-08-19：选 Tradeweb 的三条判据里有两条已经翻面了，头条没改，但理由必须重写。**
下表左栏是旧文原话，右栏是今天现算的值：

| 判据 | 旧文（2026-08-07） | 今天（2026-08-19 实测） | 还站得住吗 |
|---|---|---|---|
| 历史长 | tradeweb 115 > primary 97 > orderbook 66 > lch 24 | **orderbook 127 > tradeweb 115 = primary 115（AIM 114 / MM 98）> lch 24** | ❌ **翻面**：最长的现在是 orderbook |
| 无空洞 | 「tradeweb 那 27 列是 86 列里唯一零空格的一组」 | 宽表 127 行里零空格的列**只有 17 个，全是 orderbook 那一组**；tradeweb 27 列各有 **12 个前导空格**（2016 全年） | ❌ **翻面**：宽表前推到 2016-01 之后，零空格的换成了 orderbook |
| 发布快 | tradeweb 是唯一进得了「次月一周内」的腿 | 仍然成立：orderbook 近 30 期中位 +10、最晚 +24（2026-03 → 04-24），tradeweb 仍是 (8,8) 那一档 | ✅ **唯一还站得住的一条** |

⇒ **结论不变、依据只剩一条**：头条仍是 tradeweb，理由是**发布快**，不再是「历史最长」
或「唯一无空洞」。`lch` 依旧是硬性排除（连 `SINGLE_SPEC` §3「共同连续历史 ≥ 24 个月」都过不了）；
`orderbook` 若做头条，按 `docs/CRON_WIRING.md` §2.1「LAG 跟着决定 data_through 的那条腿走」，
`LAG['lseg']` 要从 (8,8) 抬到约 (26,26)，**整页每月晚 18 天上线** —— 这一条没变，
而且现在是留住 tradeweb 做头条的**唯一**理由。哪天 LSEG 把月报发布提速了，
这个选择就该重新评估一次。
...
募资额同时出现在 Summary 月度块与 New Issues / Further Issues **专表**。原始样本是
「2018-05 起两个市场 197 期」；**2026-08-19 本机把库里全部 212 期（AIM 2017-01 起 114 +
Main Market 2018-05 起 98）用 `_parse_factsheet()` 离线重跑了一遍**，结论不变：
**仍然只有 3 期不等，而且 3 期全都是 Summary 那一侧后来被改成专表的值，方向一次都没反过来**；
本轮新入库的 AIM 2017-01…2018-04 那 16 期**一处冲突都没添**（等级 [A]，不是转抄）。
这 3 处登记在 `cache/lseg_primary_conflicts.csv`，今天读到的仍是这 3 行：
...
证据是「1 期确凿出错 + 从未被验证过的其余各期」，不是「反复出错的统计」。
要把 YTD 列做进页面的人，得先自己把**全部 127 期**核一遍（2026-08-19：窗口前推到 2016-01
之后不再是 66 期；`cache/lseg_orderbook/mmr_*.pdf` 里 127 份全在，离线就能核）。
...
| 现有池 | LSEG 有没有对应物 | 判断 |
|---|---|---|
| 欧洲现货竞争（`/exchanges-eu/`，成交额 ADV） | `lse_orderbook_adv_gbp_m`、`turquoise_integrated_adv_gbp_m` | **最有希望的一组**，2026-08-19 起又更有希望了一点：① 币种是 GBP、池内是 EUR，折算会把 [A] 变成派生的 [C] —— **这一条没变，仍是主要障碍**；② ~~这条腿晚一个月，接进去会把整页共同最新月拖回 2026-06~~ **已不成立**：订单簿现已到 2026-07，且历史前推到 2016-01（127 个月，比 `/exchanges-eu/` 现有三家都长），「短序列 / 慢腿」两条反对理由都没了；③ 官方份额列（`lse_lit_uk_share_pct`）的分母是「英国 Lit 订单簿」，与池内「三家之和」的分母不是一回事，两个占比不能混画 —— **这一条也没变** |
