...
## 股票清算量整列重述（2026-08 版）—— 2026-09-12 核实并采纳

**结论。** 从 7 月数据版（2026-08 上旬挂出）起，Euronext 把 hist 工作簿 `Equity Markets` 的
`Shares (nb of contracts) (4)`（C13）及其 `Athex` 备注列（C14）在 2022-01..2026-06 整段重述。
8 月数据版（223,390 B，Last-Modified 2026-09-07 15:35:21 GMT，sha256 `2b47522c3c5d…`）沿用了这次重述，
并对 2026-06 另做了一处小更正。

这是**口径变更，不是更正旧数的错误**：
- legacy Euronext 那一块一格没动；
- 雅典那一块换成了 ATHEX 自己的单边成交笔数；
- 雅典还被 pro-forma 回填到并表前的月份，所以这一列全程含雅典。

处理：
- `fetch/enx.py` 的 `ACCEPTED_RESTATEMENTS` 按逐格指纹登记，采纳 108 格（sha256 `303812a12f0a…`）；
- `build/specs/enx.py` 不再在这一列上画 2025-11 的雅典红线（前提由 `_cleared_proforma()` 从 series 现算）。

以后官方再动这些格子，指纹对不上，就回到默认路径：不覆盖，写冲突文件等人核。

### 冲突量化（8 月版 vs 库内旧值；库内旧值来自 2026-08-07 建表那一版）

- 主列 54 个月全部上修 +3.29% ~ +15.19%（中位 +5.45%）；备注列 54 个月全部 ×6.84 ~ ×11.78。
- 2022-01..2025-10（46 个月）：新主列 − 旧主列 ≡ 新备注列（相对差 < 1e-9）⇒ 并表前的主列现在含雅典。
- 2025-11..2026-05（7 个月）：新主列 − 旧主列 ≡ 新备注列 − 旧备注列 ⇒ legacy 不变。
- 新备注列 ÷ 雅典现货成交笔数（双边计）：2022-01..2026-08 共 56 个月在 0.49850–0.49998；旧备注列在 0.04239–0.07304。
- 2026-06 在口径切换之外，legacy 另上修 +1.1274 千/日（×22 天 = 24,802.5，+0.09%）。
- 同一版里另有 15 处冲突，**不在这一条登记里**：逐列定性、采纳与不采纳见文末「2026-08 版其余 15 处冲突」。

### 官方旁证（均不经 hist 本身）

数字是本机从下载下来的原文里逐位读出、再对着新旧两版复算的。链接由检索代理提供。

| 旁证 | 官方印的数 | 旧口径 | 新口径 |
|---|---:|---:|---:|
| Q4 2024 业绩稿，FY2024 shares cleared | 234,777,332 | 234,777,332 | 243,918,062 |
| 同稿，FY2023 | 83,486,969 | 83,486,969 | 92,673,018 |
| Q2 2026 业绩稿（2026-07-30），Q2 2026 | 75,523,319 | 75,523,318.5 | 79,228,832.5 |
| 同稿，Q2 2025 备考（含雅典） | 76,120,584 | 旧主列 + 旧备注 76,120,583.5 | 78,960,013 |
| latest 工作簿（2026-09-07），2025-08 | 18,858,349 | 17,812,753 | 18,858,349 |
| 同上，Q3 2025 | 42,344,286 | 40,174,164 | 42,344,286 |
| 同上，YTD 2025 | 200,872,961 | 192,775,443 | 200,872,961 |
| 同上，2026-08 同比 | +8.2426% | +14.5964% | +8.2426% |
| ATHEX 2022 年报，Cash Market「Number of trades」 | 7.5m | 雅典备注 0.970m | 雅典备注 7.437m |
| ATHEX 2023 年报，同上 | 9.2m | 1.190m | 9.186m |

读法：
- 前四行说明**旧值当年是忠实的**：7 月底以前官方口径就是旧的，库里的旧值不是我们抓错的。
- 中间四行说明**官方现行口径是新的**。
- 最后两行说明**新的雅典数就是 ATHEX 自己的成交笔数**。

出处：
- Q4 2024 业绩稿 https://www.euronext.com/en/media/13054/download
- Q2 2026 业绩稿 https://www.euronext.com/sites/default/files/financial-event-doc/2026-07/2026.07.30_enx_pr_q2_2026_financial_results.pdf
- latest 工作簿 https://live.euronext.com/sites/default/files/statistics/ir/euronext_latest_month_volumes.xlsx（50,374 B，sha256 `54bd1ee92daa…`）
- ATHEX 年报 2023 https://athens.euronext.com/sites/default/files/2025-03/annual-report-2023-ENG-new-links.pdf（本机读的是 companiesmarketcap.com 的镜像）
- ATHEX 年报 2022 https://companiesmarketcap.com/annual-reports/101878.ar.en.2022.pdf

### 没解开的

- **官方没有为这次切换写任何说明。**
  - 两版 hist 的表头与脚注逐字相同；
  - 2026-07 与 2026-08 两期成交量新闻稿 PDF 里没有清算数字，也没有附注；
  - Q2 2026 业绩稿（07-30）用的还是旧口径，所以切换发生在 07-30 之后、7 月数据版挂出之前。
- **Equity Markets 脚注 (3) 与新口径矛盾。** 它仍写雅典 "since November 2025"。`series/enx_breaks.csv` 照抽不误，不画这一条是页面侧的决定。
- **旧雅典备注列是什么量，没找到。** 它约为新值的 1/7 ~ 1/12，ATHEX 年报与 FESE 年报里都没有对得上的量。
- **FESE 2023 年的数对不上。** FESE European Exchange Report 2023 的雅典 order-book trades 是 7,743,627，与 ATHEX 自己的 9.2m、与新口径都差约 19%；2022、2024 两年与新口径只差 1% 左右。疑为 FESE 自身问题，不影响结论。
- **Q2 2024 比较数对不上（与本列无关的旁支）。** Q2 2025 业绩稿印的 Q2 2024 比较数 55,211,959，与原发布的 58,879,480 以及新旧口径都对不上，原因不明。

### 逐月前后对照（千/日）

`legacy 新−旧` 这一列的算法：
- 并表前的月份 = (新主列 − 新备注) − 旧主列；
- 2025-11 起 = (新主列 − 新备注) − (旧主列 − 旧备注)。

除 2026-06 外，全部 |差| < 3e-13。

| 月份 | 主列 旧 | 主列 新 | 主列变动 | 雅典备注 旧 | 雅典备注 新 | legacy 新−旧 |
|---|---:|---:|---:|---:|---:|---:|
| 2022-01 | 381.868 | 412.536 | +8.03% | 4.340 | 30.668 | +0.0000 |
| 2022-02 | 435.656 | 472.053 | +8.35% | 5.017 | 36.397 | +0.0000 |
| 2022-03 | 519.822 | 557.286 | +7.21% | 5.307 | 37.463 | -0.0000 |
| 2022-04 | 343.277 | 372.703 | +8.57% | 3.735 | 29.426 | +0.0000 |
| 2022-05 | 335.635 | 375.138 | +11.77% | 4.258 | 39.503 | +0.0000 |
| 2022-06 | 307.808 | 334.877 | +8.79% | 3.457 | 27.069 | +0.0000 |
| 2022-07 | 292.555 | 316.497 | +8.18% | 3.270 | 23.942 | +0.0000 |
| 2022-08 | 227.314 | 251.197 | +10.51% | 3.105 | 23.883 | +0.0000 |
| 2022-09 | 286.715 | 314.010 | +9.52% | 3.352 | 27.295 | +0.0000 |
| 2022-10 | 289.262 | 311.739 | +7.77% | 2.796 | 22.477 | +0.0000 |
| 2022-11 | 296.181 | 322.907 | +9.02% | 3.417 | 26.726 | +0.0000 |
| 2022-12 | 213.772 | 236.029 | +10.41% | 3.234 | 22.257 | -0.0000 |
| 2023-01 | 264.675 | 295.403 | +11.61% | 4.220 | 30.729 | -0.0000 |
| 2023-02 | 318.927 | 362.087 | +13.53% | 5.738 | 43.160 | +0.0000 |
| 2023-03 | 328.589 | 370.165 | +12.65% | 5.415 | 41.576 | +0.0000 |
| 2023-04 | 283.077 | 308.796 | +9.09% | 3.466 | 25.719 | +0.0000 |
| 2023-05 | 274.309 | 310.139 | +13.06% | 4.402 | 35.831 | -0.0000 |
| 2023-06 | 247.841 | 283.594 | +14.43% | 4.925 | 35.753 | +0.0000 |
| 2023-07 | 258.716 | 293.635 | +13.50% | 5.102 | 34.918 | +0.0000 |
| 2023-08 | 233.881 | 269.409 | +15.19% | 4.376 | 35.528 | +0.0000 |
| 2023-09 | 269.413 | 308.914 | +14.66% | 4.670 | 39.501 | +0.0000 |
| 2023-10 | 266.348 | 300.731 | +12.91% | 4.634 | 34.382 | -0.0000 |
| 2023-11 | 391.426 | 431.112 | +10.14% | 4.586 | 39.686 | +0.0000 |
| 2023-12 | 852.860 | 886.718 | +3.97% | 4.299 | 33.859 | +0.0000 |
| 2024-01 | 882.322 | 920.147 | +4.29% | 4.980 | 37.826 | -0.0000 |
| 2024-02 | 927.995 | 971.935 | +4.73% | 6.297 | 43.940 | -0.0000 |
| 2024-03 | 977.375 | 1019.543 | +4.31% | 4.652 | 42.168 | +0.0000 |
| 2024-04 | 969.480 | 1012.656 | +4.45% | 5.099 | 43.176 | +0.0000 |
| 2024-05 | 872.285 | 907.792 | +4.07% | 4.026 | 35.508 | +0.0000 |
| 2024-06 | 966.507 | 1001.774 | +3.65% | 4.030 | 35.267 | +0.0000 |
| 2024-07 | 872.357 | 901.555 | +3.35% | 3.625 | 29.198 | +0.0000 |
| 2024-08 | 837.603 | 870.149 | +3.89% | 3.908 | 32.546 | +0.0000 |
| 2024-09 | 872.098 | 902.014 | +3.43% | 3.504 | 29.916 | +0.0000 |
| 2024-10 | 904.906 | 938.515 | +3.71% | 3.856 | 33.610 | +0.0000 |
| 2024-11 | 1016.950 | 1050.697 | +3.32% | 3.874 | 33.747 | +0.0000 |
| 2024-12 | 923.854 | 956.440 | +3.53% | 3.604 | 32.587 | +0.0000 |
| 2025-01 | 1066.912 | 1102.052 | +3.29% | 4.813 | 35.140 | +0.0000 |
| 2025-02 | 1212.185 | 1254.997 | +3.53% | 4.883 | 42.812 | +0.0000 |
| 2025-03 | 1387.330 | 1439.218 | +3.74% | 5.100 | 51.889 | +0.0000 |
| 2025-04 | 1610.339 | 1668.128 | +3.59% | 6.903 | 57.789 | +0.0000 |
| 2025-05 | 1095.485 | 1147.339 | +4.73% | 5.636 | 51.854 | -0.0000 |
| 2025-06 | 978.078 | 1023.969 | +4.69% | 5.360 | 45.890 | +0.0000 |
| 2025-07 | 972.235 | 1021.128 | +5.03% | 5.955 | 48.892 | +0.0000 |
| 2025-08 | 848.226 | 898.017 | +5.87% | 6.079 | 49.790 | +0.0000 |
| 2025-09 | 912.601 | 961.726 | +5.38% | 5.262 | 49.125 | -0.0000 |
| 2025-10 | 1032.927 | 1089.816 | +5.51% | 5.804 | 56.888 | +0.0000 |
| 2025-11 | 934.511 | 983.339 | +5.22% | 5.490 | 54.318 | +0.0000 |
| 2025-12 | 804.822 | 846.962 | +5.24% | 4.604 | 46.744 | +0.0000 |
| 2026-01 | 1068.198 | 1129.929 | +5.78% | 6.188 | 67.919 | +0.0000 |
| 2026-02 | 1203.602 | 1270.525 | +5.56% | 6.786 | 73.709 | +0.0000 |
| 2026-03 | 1394.827 | 1465.985 | +5.10% | 6.871 | 78.029 | +0.0000 |
| 2026-04 | 1239.864 | 1301.869 | +5.00% | 5.754 | 67.759 | +0.0000 |
| 2026-05 | 1227.966 | 1291.641 | +5.19% | 6.225 | 69.900 | +0.0000 |
| 2026-06 | 1189.397 | 1243.574 | +4.56% | 5.539 | 58.589 | +1.1274 |

---

## 2026-08 版其余 15 处冲突 —— 2026-09-12 逐列定性，采纳 11 格

同一版 hist（223,390 B，Last-Modified Mon, 07 Sep 2026 15:35:21 GMT，sha256 `2b47522c3c5d…`）在股票清算量之外，还有 15 处与库内不一致。
2026-09-12 在工作树里重下了两份工作簿：hist 的 sha256 没变；latest 是 50,374 B，Last-Modified 07 Sep 2026 15:35:04 GMT，sha256 `54bd1ee92daa…`。
`python3 fetch/enx.py` 复现出的冲突清单，与之前那一轮逐格相同。

### 结论

| 列 | 月份 | 库内旧值 → 官方新值 | 定性 | 处理 |
|---|---|---|---|---|
| `mktcap_eurtn` | 2026-06、2026-07 | 只差 1 / 3 个 ULP（相对 1.2e-16 / 3.6e-16） | 浮点表示 | `_same_number` 判为同一个数：不算冲突，库内值不动 |
| `listed_funds` | 2026-06 | 2178 → 2173 | 次月更正 | 采纳 |
| `csd_settlement_instructions_m`、`athex_csd_settlement_instructions_m` | 2026-06 | 13.801173 → 13.801191；0.570856 → 0.570874 | 次月更正，+18 笔全在雅典 | 采纳 |
| `money_raised_new_listings_eurm` | 2026-06、2026-07 | 177.902 → 193.010；303.791 → 308.226 | 次月更正，绿鞋补记回上市月 | 采纳 |
| `athex_money_raised_followon_eurm` | 2026-06 | 533 → 532.79 | 次月更正，雅典配股按实募改记 | 采纳 |
| `mktcap_eurtn` | 2025-11..2026-03 | −9,310 / −10,429 / −11,911 / −12,525 / −52 €m | 雅典并表后的双重挂牌去重 | 采纳 |
| `mktcap_eurtn` | 2026-04 | +€2.9m | 出处不明 | **不采纳** |
| `adv_bonds_retail_cleared_kcontracts` | 2026-06 | 37.600 → 44.893 千张/日（月合计 827,199 → 987,654） | 次月更正，但新值疑为占位数 | **不采纳** |

- **采纳：** 11 格分 4 条登记进 `fetch/enx.py` 的 `ACCEPTED_RESTATEMENTS`，每条只圈人核过的格子。
- **不采纳：** 两格写在那张表的文末，照旧每轮进 `cache/enx_restatements.csv`。冲突文件现在只剩这 2 行。
- **页面侧（`build/specs/enx.py`）：** 加了两条页尾注（市值去重、募资额含超额配售），释义「备注列」补了一句。
- **债券零售清算：** 页面没画这一列，pools 也不读它，页面不用改。
- **「次月更正」的意思：** 官方在下一期发布里改上一期的数。latest 工作簿的「上月」列，就是这类更正最先露面的地方。

### 什么时候改的

本仓 `cache/raw/enx/` 留有各版工作簿的存证，下面是逐版比对的结果。

- **7 月数据版 hist 已是新值**（220,465 B，sha256 `5f2fd7b0e696…`）：`listed_funds`、结算指令两列、`athex_money_raised_followon_eurm`、`adv_bonds_retail_cleared_kcontracts`。
- **8 月数据版 hist 才改：** `mktcap_eurtn` 全部、`money_raised_new_listings_eurm` 两格。
  - 其中 2026-06 的募资额，在 7 月数据版 **latest** 的「上月」列里已是新值 —— latest 先改，hist 晚一版才跟上。
- **旧值的来源：** 2026-08-07 建表（6 月数据版），以及 2026-08-08 入库（7 月数据版，只管 2026-07 那一行）时读到的值。

### 旁证一：两版 latest 工作簿（官方文件，但与 hist 同出一套数据）

| latest 版本 | 行 | 期间 | 印的数 | 与谁逐位相等 |
|---|---|---|---:|---|
| 7 月数据版（47,476 B，`1143b9710d68…`） | Funds | 2026-06 | 2,173 | 新值 |
| 同上 | Nb of Settlement instructions | 2026-06 | 13,801,191 | 新值 |
| 同上 | Money Raised - New equity listings | 2026-06 | 193.0097577893444 | 新值 |
| 同上 | 同上，YTD 2026 | 1–7 月 | 4,914.110985244195 | 新值加总（库内加总 4,899.003406994195，差 15.10757825） |
| 同上 | 同上 | 2026-07（当时的最新月） | 303.791169610157 | 旧值，即首发值 |
| 同上 | Bonds - Retail | 2026-06 | 987,654 | 新值；YTD 1–7 月 7,947,178 也按新值加总 |
| 8 月数据版（50,374 B，`54bd1ee92daa…`） | Money Raised - New equity listings | 2026-07 | 308.22587511015706 | 新值；Q3、YTD 也按新值加总 |
| 同上 | Money Raised - Follow-ons on equities，YTD 2026 | 1–8 月 | 20,114.055633982607 | 比「7 月版 YTD + 8 月当月」少 0.209997849997；Q3 与库内逐位相等 |
| 同上 | Nb of Settlement instructions | 2026-07 | 13,287,660 | **都不是**：hist 与库内都是 13,287,622，见「还在路上的冲突」 |

两版 latest 的图例里写着 "(R) Revised"，但格子里一个 (R) 标记都没有。

### 旁证二：旧值当年是忠实的

Q2 2026 业绩稿（2026-07-30，第 13 页）上，下面四项印的都是旧值：

- Nb of listed Funds 2,178；
- 新上市募资 Q2 178；
- 债券零售清算 Q2 2,950,200；
- 结算指令 Q2 38,060,160。

本文第 2 组复算过这几项，2026-09-12 检索代理又重读 PDF 确认了一遍。⇒ 这些更正都发生在 07-30 之后。

### 旁证三：不走同一套数据的出处

**CSD 结算指令多出的 18 笔全在雅典。** 出处是 Euronext Securities 的 key figures 工作簿（https://www.euronext.com/en/media/11654/download），它不含雅典；本机读的是 2026-09-12 取的副本，sha256 `ed046905f2d0…`。

| 月份 | 奥斯陆 | 哥本哈根 | 波尔图 | 米兰 | 四家合计 | hist Total − 雅典备注 |
|---|---:|---:|---:|---:|---:|---:|
| 2026-05 | 1,796,317 | 2,965,659 | 161,633 | 6,176,421 | 11,100,030 | 11,100,030 |
| 2026-06 | 1,921,495 | 3,746,524 | 174,780 | 7,387,518 | 13,230,317 | 13,230,317（按旧值、新值算都是） |
| 2026-07 | 1,792,649 | 3,991,260 | 174,058 | 6,647,759 | 12,605,726 | 12,605,726 |
| 2026-08 | 1,823,087 | 3,350,939 | 149,934 | 5,729,348 | 11,053,308 | 11,053,308 |

**新上市募资额的变动 = 绿鞋补记。** 依据是发行人的行权公告：检索代理读了原文，Bohus、Alia Mentis 两条本机又重开核对过。官方把绿鞋记回上市那个月。

| 上市月 | 发行人 | 市场 | 行权公告 | 绿鞋 | 金额 |
|---|---|---|---|---|---:|
| 2026-06 | Bohus ASA | Oslo Børs | 07-05 | 4,200,000 股（老股）× NOK 31 = NOK 130.2m，按 0.09005 €/NOK 折算 | €11,724,510.00 |
| 2026-06 | Alia Mentis | Euronext Growth Milan | 07-09 | 979,021 股 × €3.25 | €3,181,818.25 |
| 2026-06 | OPT | Euronext Growth Milan | 07-24 | 287,500 股 × €0.70 | €201,250.00 |
| | | | | **6 月合计** | **€15,107,578.25** = +15.10757825 €m |
| 2026-07 | Gens Aurea | Euronext Milan | 08-13 | 276,818 股 × €10（部分行使） | €2,768,180.00 |
| 2026-07 | Giunti Psychometrics | Euronext Growth Milan | 08-19 | 310,339 股 × €4.50（部分行使） | €1,396,525.50 |
| 2026-07 | First Point | Euronext Growth Milan | 08-25 | 120,000 股 × €1.00 | €120,000.00 |
| 2026-07 | Dipietro Group | Euronext Growth Milan | 08-31 | 100,000 股 × €1.50 | €150,000.00 |
| | | | | **7 月合计** | **€4,434,705.50** = +4.4347055 €m |

- 0.09005 €/NOK 是反推出来的（ECB 06-18 参考价 1/11.1050，取 5 位）；另外六笔都是股数 × 价格直接相乘。
- Gens Aurea 那一条，代理只读到检索摘要，没打开原文。
- 7 月合计去掉 Gens Aurea（主板）后余 €1,666,525.50，恰好等于 latest「SMEs」那一块 2026-07 的变动（202.791169610157 → 204.457695110157）。
- 出处：
  - Bohus https://mfn.se/ob/a/bohus/bohus-asa-exercise-of-greenshoe-option-in-full-2b6fbd4e
  - Alia Mentis https://www.teleborsa.it/News/2026/07/09/alia-mentis-esercitata-integralmente-l-opzione-greenshoe-221.html
  - OPT https://www.teleborsa.it/News/2026/07/24/opt-integrae-sim-esercita-integralmente-l-opzione-greenshoe-159.html
  - Gens Aurea https://www.borsaitaliana.it/borsa/notizie/teleborsa/finanza/gens-aurea-greenshoe-esercitata-parzialmente-145_2026-08-13_TLB.html
  - Giunti https://www.teleborsa.it/News/2026/08/19/giunti-psychometrics-esercitata-parzialmente-la-greenshoe-11.html
  - First Point https://www.teleborsa.it/News/2026/08/25/first-point-esercitata-integralmente-la-greenshoe-139.html
  - Dipietro https://www.teleborsa.it/News/2026/08/31/dipietro-group-esercitata-integralmente-la-greenshoe-37.html

**雅典再融资 533 → 532.79 = Interwood 配股按实募改记。** 依据是发行人公告：检索代理读了原文，本机复算。

- 当月雅典有两笔：
  - ADMIE Holding 增发 130,864,197 股 × €4.05 = €529,999,997.85（06-19 定价，06-23 获准上市）；
  - Interwood 配股：06-26 除权，07-01..07-14 认购；现金部分上限 12,000,000 股 × €0.25 = €3,000,000，07-16 公告实募 €2,792,835.50。
- 对账：
  - ADMIE + 上限 = €532,999,997.85 → 旧值 533；
  - ADMIE + 实募 = €532,792,833.35 → 新值 532.79。
- 上面 8 月版 latest 的 follow-ons YTD 少了 0.20999785 €m，正是 532.99999785 → 532.79 这一步。
- 集团合计（主列）在 hist 两版里都还是 1,434.4866826325715，也就是仍按 532.99999785 算 —— 见「还在路上的冲突」。

**市值去重。** 出处是 Euronext 自己的 Cash 月报工作簿（`https://live.euronext.com/sites/default/files/statistics/cash/monthly/Cash%20YYYYMM.xlsx`），它不含雅典。

- **版本：** 2026-09-12 取了 202511..202608 共 10 个月度版本；202608 版 Last-Modified Wed, 09 Sep 2026 08:15:05 GMT，sha256 `fe7d0bda230e…`。
- **合计：** `PM - Overview` 表「NUMBER OF ISSUERS & MARKET CAPITALIZATION - SHARES - LISTINGS」下 Euronext 那一列的 Market Capitalization (M€)。
- **三家：** `SM - Instrument level MTD` 里 Market Place = Brussels 的 TITAN S.A. / VIOHALCO / CENERGY 三行。
- **方法：** `Methodology` sheet 原文 "For all euronext statistics the multi-listed instruments are computed once"。

下表单位 €m。A = Cash 合计 + 雅典备注；B = A − 三家。

| 月份 | Cash 合计 | 雅典备注 | 三家 | A | B | hist 旧值 | hist 新值 | 读法 |
|---|---:|---:|---:|---:|---:|---:|---:|---|
| 2025-11 | 6,686,990.773 | 143,332.420 | 9,310.204 | 6,830,323.194 | 6,821,012.989 | 6,830,323.194 | 6,821,012.989 | 旧 = A，新 = B |
| 2025-12 | 6,706,063.448 | 146,799.112 | 10,429.058 | 6,852,862.560 | 6,842,433.502 | 6,852,862.560 | 6,842,433.502 | 旧 = A，新 = B |
| 2026-01 | 6,885,544.264 | 158,499.299 | 11,911.376 | 7,044,043.564 | 7,032,132.188 | 7,044,043.564 | 7,032,132.188 | 旧 = A，新 = B |
| 2026-02 | 7,155,584.760 | 157,625.272 | 13,047.600 | 7,313,210.032 | 7,300,162.432 | 7,312,687.824 | 7,300,162.432 | 旧 = A − 522.2，新 = B |
| 2026-03 | 6,640,938.779 | 143,125.100 | 10,852.434 | 6,784,063.879 | 6,773,211.445 | 6,773,263.478 | 6,773,211.445 | 旧 = B + 52.0，新 = B |
| 2026-04 | 6,877,802.872 | 157,697.326 | 12,434.311 | 7,035,500.198 | 7,023,065.887 | 7,035,500.198 | 7,035,503.113 | 旧 = A，新 = A + 2.9（仍未去重） |
| 2026-05 | 7,060,732.873 | 175,166.617 | 14,746.754 | 7,235,899.490 | 7,221,152.736 | 7,221,195.410 | 同左 | B + 42.7 |
| 2026-06 | 7,225,626.796 | 182,539.626 | 13,899.865 | 7,408,166.422 | 7,394,266.557 | 7,394,236.117 | 同左 | B − 30.4 |
| 2026-07 | 7,209,887.779 | 188,075.426 | 13,284.396 | 7,397,963.205 | 7,384,678.808 | 7,432,868.871 | 同左 | A + 34,905.7，A、B 两个口径都对不上 |
| 2026-08 | 7,247,871.246 | 194,023.027 | 13,495.717 | 7,441,894.274 | 7,428,398.556 | 7,428,278.702 | 同左 | B − 119.9 |

表里「=」指差 < €0.05m（Cash 表精确到 €1k）。

- **底层市值没改：** Cash 合计在各月度版本里逐位不变（唯一例外是 2026-05：202605 版 7,046,028.793，202606 版起 7,060,732.873）。改的是 hist 的加总方式。
- **不是 Euronext N.V. 自己：** 它不在雅典挂牌（检索代理读了 2025-11-19 要约结果公告，对价股只在阿姆斯特丹 / 布鲁塞尔 / 里斯本 / 巴黎上市），市值走势也与差额对不上。
- **独立复核三家市值：** 检索代理用 Yahoo 月末收盘价 × Euronext 行情页股数另算了一遍，2025-11..2026-01 与表里三家市值差 ≤ €2m。

**债券零售清算为什么不采纳。**

- **数字本身可疑：** 987,654 是整本 8 月版 hist 里唯一一格六位以上、数字连续递减 / 递增 / 全同的整数（本机扫了四张白名单 sheet 的全部数值格）。
- **latest 证明不了它：** latest 的「上月」列与两版 YTD 都按新值加总，但 latest 与 hist 同出一套数据，只能说明「官方现在是这个数」。
- **不走这套数据的出处只有 Q2 业绩稿，印的是旧值。** 另有两条间接估算，都落不到新值上：
  - Euronext Clearing 的 CPMI-IOSCO Q2 2026 定量披露（https://www.euronext.com/sites/default/files/2026-08/CCG_Quantitative_Disclosure%20Q2_2026.xlsx）：零售日均合约数 EUR 23,417.93 + USD 798.9，× 62 天 × 2。同一算法对 Q2 2025 / Q1 2026 / Q3 2025 的误差是 −0.5% ~ +0.7%，推出 6 月约 88–89 万张；
  - Cash 月报米兰债券成交笔数（6 月 456,986）× 历史上 2.05–2.11 的清算 / 成交比，推出约 94–96 万张。
- **幅度反常：** 这一列以往的更正是 0.2–0.3% 量级（例：FY2025 业绩稿印 2026-01 为 1,046,052，现在是 1,049,639），这回是 +19.4%。
- **结论：** 首发的 827,199 可能偏低，但 987,654 本身核不实。库内保留首发值，等官方给出能核的数。

**市值 2026-04 为什么不采纳。** A 逐位等于旧值，新值多出的 €2.9m 找不到出处。而且新旧两版这个月都还没去重（比 B 多 €12,434.3m），官方多半还要再改。

### 还在路上的冲突

下面这些 hist 下一版大概率会冒出来，本轮不处理。

- **`csd_settlement_instructions_m` 2026-07：** 8 月版 latest 印 13,287,660，hist 是 13,287,622（+38）。key figures 四家合计 12,605,726 = hist Total − 雅典，所以 hist 跟上时，应是雅典那一块 +38。
- **`money_raised_followon_eurm` 2026-06：** latest 已按雅典 532.79 算，hist 主列仍按 532.99999785 算 ⇒ 差 −0.20999785 €m。在 hist 跟上之前，本库 2026-06「主列 − 雅典备注」比 legacy 多 0.21 €m。
- **`mktcap_eurtn`：**
  - 2026-04 没去重（多 €12,434.3m）；
  - 2026-07 比 A 还高 €34,905.7m，原因不明。
- **`money_raised_new_listings_eurm`：** 这是结构性的 —— 只要最新月有带绿鞋的 IPO，下一版就会改它。

### 浮点表示差

`mktcap_eurtn` 有两格只差浮点表示：

- 2026-06 `7.39423611742153` → `7.3942361174215305`（+1 ULP）；
- 2026-07 `7.4328688714973` → `7.4328688714972975`（−3 ULP）。

官方那一格在 ULP 级别变了，数值没变。

**`update()` 的新判据：** 字符串不同时，先用 `_same_number` 判一次，容差 max(1e-9, 1e-12·|旧值|)，与 `fetch/lseg.py` 同一个式子。容差内既不算冲突也不改写，只打一行日志。同一版里最小的真更正是 2026-04 市值的 +2.9e-6（€2.9m），离容差三个数量级以上。

**离线测试**（hist / latest 用 cache 副本，series 用临时副本）：

- 1 个 ULP、`2188` 写成 `2188.0`、+5e-10：都不进冲突文件，库内字符串原样不动；
- +1e-8：进冲突文件；
- 采纳后重跑：`enx.csv` 字节不变，`update()` 返回 []；
- 篡改一格旧值、或少一格冲突：那一条整批不采纳。

### 过程说明

- **Wayback：** 被本机抓取规则挡住，没能取到 2025-12..2026-06 各版 hist 的首发值。本节的「旧值」一律指本仓建表时读到的值。
- **athens.euronext.com：** 在本机抓取黑名单里。雅典那两笔配股的数字来自发行人公告与希腊财经媒体。检索代理曾用浏览器面板打开该站的月度公报（公报里没有募资额），本节没用到该站的任何数字。
- **谁读的：** 外部原文由检索代理读取；凡标「本机」的，都是在本机对下载副本或 `cache/raw/enx/` 存证重算的。
