...
> 引擎**默认不许改**。图型不够用就换图型或改 Python 侧的数据组织，不要去动
> `assets/charts.js` —— 它同时服务**全站每一张已上线的页**（当前页数自己数：
> `find . -maxdepth 2 -name index.html`，减掉 hub 那一张；这里从前写死「14 张页」，
> 早就不是那个数了），改一行要把它们全部重新验收。
>
> ⚠️ **上面这句从前写的是「引擎不许改」，改成「默认不许改」+ 下面这套例外记录，是
> 2026-09 这一轮自行改写的**（`git show HEAD:docs/CHART_KINDS.md` 里还是老措辞），
> **尚未经页面所有者拍板**。所有者若不认这条放宽，连同下面那条例外一起退回即可。
>
> **例外只有页面所有者点名的时候，而且要在这里留一条记录**（否则下一个人读到上面
> 那句禁令，会以为引擎从来没被动过）。**待合并**的例外（改动还在工作树里、未合并，
> 所以这里写「要改」而不是「已经改了」—— 写成过去时就是把将来时说成既成事实）：
> - **2026-09 白 casing**：所有者点名 `/tmx/` 的堆叠图「线看不清」（**当时点的是两张**；
>   其中「加拿大现货成交额…各分项占比」那张的右轴线**此后已由所有者自己拍板删掉**，
>   理由是「同一条序列不许换个切法再画一遍」，见 `data/tmx.js` 页尾那条
>   「2026-09：本页改了四件事」的第（三）项 —— 所以今天照这条记录去找，
>   `/tmx/` 只找得到「股指期货：各分项占比」
>   那一张）。而这条线压在哪一段是数据决定的、靠挑线色救不回来（算式见 §3.14 那条
>   bullet），只能改引擎让线自带一层与背景无关的白轮廓。改的是 `polyline` 的第 7 个
>   参数 `halo`，**不传它的调用点一条属性都不加、输出逐字节不变**（合成 payload 逐属性
>   比对过）。
>   ⚠️ **授权口径要说清楚：所有者点名的是 `/tmx/` 那一张，而这一改会动到全站每一张
>   给了 `line` 的 `stacked_dual`**（现网横跨**多页**，具体几张几页自己扫 `data/*.js` 现算，
>   判据见 §3.14）—— 除 `/tmx/` 那一张之外的其余各张 / 其余各页**没有所有者指令**，
>   属于顺带生效的渲染改动，合并前请所有者一并拍板。
> - **2026-09-14 美元负号 + 零线以下的左轴刻度**：所有者点名 `/hood/` 的美元双轴图
>   （柱全为正、右轴 y/y 跨零）左轴印着「$-100」「$-2.0」、零线以下白占一截画布，并写明
>   「很可能还有很多别的页」「类似的双轴图型」，要求全站回归。要改两处：
>   ① `usd*` 格式器把负号挪到币符之前（`−$100`，U+2212），柱值 / 表格 / tooltip / 刻度同时生效；
>   ② 零点对齐把左轴拉进负区、而左轴自己的值全都 ≥ 0 时，零线以下不画左轴刻度与网格线
>   （判据见 §4）。量程、零点高度、浪费率、`ALIGN_WASTE_MAX` 兜底**一个都不动**。
>   ⚠️ 授权口径：点名的是 `/hood/` 那几张，②按判据对全站所有双轴 kind 生效（`gs_bar` 占绝大多数），
>   张数用 `tools/align_replica.py` 现算，这里不写。
...
| `usd0` `usd1` `usd2` `usd3` `usd4` | `$29` … `$29.2412`；负值 `−$29`（U+2212，在 `$` 之前；舍入成 0 不带号） | `pp0` `pp1`          | `+5pp` `+5.1pp`（自带符号）                      |
...
{ n: 2, kind: 'gs_bar', values: [...], legend: '合并月营收', ylab2: '% y/y',
  yoy: { name: 'y/y（单月，RHS）', values: [...], color: 'GOLD', yfmt: 'pct0' },
...
「默认关闭」的规矩；当时 IBKR 那 5 张 `gs_bar` 正走原路径，2026-09 起它们都给了 `yoy`）。
...
| 与 `ycap` / `yfloor` / `bar_marks` **不兼容** | 引擎对这三种柱**退回单色**：截轴的断口语义是「这根柱到不了顶」，分段后断口落在最上面那一段上、读者无从判断是哪一段被切（`yfloor` 与 `ycap` 走的是同一个 clampY，被下界钳住的柱同样如此）；斜纹是整根柱的标记，分段会把它切碎。退回单色本身是对的，问题是**图例仍列着各分部** —— 读者看到的柱与图例对不上 |
...
`stacked_dual`（§3.14）看上去正好是「堆叠柱 + 右轴线」，但它的**右轴量程压根不看
`line.values`**：`rtk = ticks(0, rc.ymax || 60, 6)`、`r0 = 0`（charts.js:984），
起点钉在 0，线上的负值点画不出来。零点对齐**可能**把 `r0` 扩到 0 以下，但那截负区
是按**左轴负段**的深浅算的，与右轴线自己能跌到多低无关；何况这里左轴是 `gs_bar` 的
分部堆叠、全非负，`r0` 一步都不会动。
（2026-09 起 `stacked_dual` 的**左**轴能画负段了，本条说的是**右**轴那条线，
两件事互不相干，这条结论一个字不变。反过来也别把 `gs_bar` 的 `stacks` 当成
「能装负段的堆叠」—— 它至今不能，`verify_pages.py` 对 `gs_bar` 的负段是硬 ERROR。）

而 `gs_bar` 这里的次轴是**同比，会转负**（2026-09 起全站单月口径）。今天页上那两条线
实测（`data/ase.js` / `data/guc.js` 的 Ex2）：日月光 87 个有值月里 **21 个为负**、
最低 **−19.7%**；创意 115 个有值月里 **37 个为负**、最低 **−52.3%**。
（同两条序列按 2026-09 之前的 12 个月滚动口径复算是日月光最低 −13.6%、创意最低 −23.0%；
那个口径今天页上一条线都不画，列在这里只为说明「换成单月之后负值更深」这件事本身
不是本节的争点 —— 无论哪一种口径，这条线都会转负，`stacked_dual` 都画不出来。）
...
{ n: 2, kind: 'stacked_dual', xlabels: [...],
  ...
  ylab: '千张/日', ylab2: '%' }
```
> **这个 kind 不只是占比图。** 它最早只服务「各段之和恒为 100 的份额堆叠带」，
> 所以本节（以及 `build/CONTRACT.md`、`build/engine_kinds.md`）从前一律按
> 「占比 / 每根柱恒高 100 / 轴自 0 起」来描述它。**2026-09 起段值可以为负**：
> 正段自 0 往上堆、负段自 0 往下堆，左轴下界跟着往下走。
> 冲着「合计 = 若干个可正可负的贡献项之和」的分解图去的，但**目前没有页面在用**：
> `/cost/` 的可比销售分解（FY24Q4 客单 −0.9%、客流 +6.4%、合计 +5.4%）最后选的是
> `bridge_bar`，拒用 `stacked_dual` 的两条理由记在 `build/cost.py:1933-1940`
> —— 柱顶画的是正向包络（那一格停在 +6.4，不是合计 +5.4）；段内标签写死 0 位小数。
> 判据：扫 `data/*.js`，全站 `stacked_dual` 的段值最小值**全部** ≥ 0 —— 现网**每一张**
> 都还在这一档。**这里不写张数**：这一处先后写过「23 张」又改成「33 张」，两次都是
> 图长了数没跟着长；张数、页数、给没给 `line`、占比 vs 绝对量各几张，全是可现算的，
> 改这一节时自己扫一遍 `data/*.js`，**别照抄上一版，也别再把当天的快照写回来**。
>
> ⚠️ **这类计数在全仓有好几份副本，分两类；判反了会把真话改成假话。**
> （副本有几份同样是可现算的，所以这里不写数。⚠️ **重扫命令不要按写死的数字找** ——
> 从前这里留的是 `grep -rn '23 张'`，把现在时那几处改成别的数之后，它只捞得到
> 「一个字都别动」的历史回归声明，该改的一处都捞不到：**写死的东西从数字变成了 grep
> 模式，一样会过期**。按「计数句式」找，不按具体的数找：
>
> ```
> grep -rnE '(全站|现网|现存|既有)[^。]{0,16}[0-9]+ 张' \
>      --include='*.py' --include='*.md' --include='*.js' . | grep -v '^./data/'
> ```
>
> 这一条会捞出全仓所有的「N 张」计数断言（含下面那两条历史回归声明），逐处按下面两类
> 判一次；里面绝大多数**与本节无关**（`223 张` 是 `gs_bar` 的同比审计快照，
> `build/verify_pages.py` 的「23 张末点不在顶部的图」、`build/specs/miax.py` 的
> 「本页 23 张图」、`docs/` 里的页数与图数各有各的事），别顺手一起改。
> 只要本节的规矩没被破坏，再叠一道 `| grep -iE 'stacked_dual|堆叠'` **应当是空的** ——
> 一有输出就说明又有人把 `stacked_dual` 的快照数写回去了。别信任何一份名单是全的。）
> - **现在时断言**（说的是「今天的存量」，过期就是假话；本轮已把它们**全部改成定性表述
>   + 判据**，一个快照数都不留 —— 下一轮再看到哪一处写回了数字，直接删数）：
>   本条；本节下面「都在这一档」那句；§4「几张没给 `line`」那句；
>   `assets/charts.js` 的「现网**每一张**…都还没有负段」与「现网**每一张**都是」；
>   `build/engine_kinds.md` §10「现网 `stacked_dual` **全部**在这一档」；
>   `build/CONTRACT.md` §3 的 `stacked_dual` 行（占比 / 绝对量那一句同样只留判据）；
>   `build/axisfmt.py` 的 `_neg_bar_guard` docstring「现存 stacked_dual 的列合计**全部**非负」。
> - **历史回归声明**（说的是「2026-09 改动当时那一批」，把 23 换成别的数反而变假，**一个字都别动**）：
>   `assets/charts.js` 里两句「既有 23 张…像素不变」、
>   `tools/align_replica.py` 的「既有 23 张一位都不变」。
>
> 要当期的全套数（张数 / 页数 / 给了 `line` 的几张几页 / 占比堆叠 vs 绝对量 / 段值全局
> 最小值），照下面这几条判据自己扫 `data/*.js` 现算，本文一个快照都不留：
> · 是不是本 kind —— `kind === 'stacked_dual'`；
> · 占比堆叠 vs 绝对量 —— 每期各段之和是否恒为 100；
> · 给没给线 —— 有没有 `line` 且不为 `null`；
> · 有没有负段 —— `stacks[].values` 的最小值是否 ≥ 0。
> 详见下面「负段」那一条与 `build/engine_kinds.md` §10。
...
- **`line` 是可选的**：不给就退化成**纯堆叠柱** —— 右轴刻度、图例里那一项、
  ...
  段数只是它的代理，而且会失准两次：两段业务各占四五成时（`/ase/`、`/guc/` 的
  「Revenue mix by business」那张，**按内容指认，不写 Ex 号** —— 九行之下就是这条规矩）
  线是纯冗余，该去掉 —— 现网这两张的 `line` **已经去掉了**，这件事做完了；
  两段里有一段常年 99%+ 时（`/tmx/`「股指期货：各分项占比」那张，SXF vs 其余；
  最矮那段现扫常年在个位数百分点以内，具体多少自己扫 `data/*.js` 现算）线反而是这张图的
  **全部内容**。
  给了线就在 `ylab2` 里照实写「同一条序列换个刻度」（`/exchanges-eu/`
  「Pool share of European cash-equity turnover」那张的措辞），别让读者当成第四个量。
- **引擎给这条线无条件加白 casing，payload 无从开关。**
  （`charts.js` 的 `polyline` 第 7 个参数 `halo`，`stacked_dual` 这一支传 `2.0`
  ⇒ 线两侧各露 1.0px 白边。）
  ⚠️ 这一条**不是 payload 契约** —— `line` 里没有任何字段能打开或关掉它，写在这一节
  只是因为它只作用于这个 kind。别读成「payload 要给一个 casing 字段」。
  （本条随引擎改动一起写，改动**尚未合并**，所以写「引擎会加」而不是「2026-09 起
  引擎已经加了」—— 合并前那样写是把将来时写成过去时。合并之后这个括号可以删。）
  ⚠️ **「线色与段色不同色」不是够用的判据，别拿它当验收标准。**
  这条线按构造压在堆叠段上（右轴 `0..ymax` 与左轴 `0..堆到顶×1.28` 两套刻度互不耦合，
  线心落在哪一段是数据决定的，画之前不知道），而**同亮度撞色与同色等价**：
  现网实测的两张 —— `/tmx/`「股指期货：各分项占比」那张的 GREEN 线**整条**压在 `NAVY`
  上（GREEN/NAVY 2.56:1）；`/exchanges-eu/`「Pool share of European cash-equity turnover」
  那张的 GREEN 线**多数期**压在 `MBLUE` 上（**GREEN/MBLUE 1.07:1**，灰度差 1.7%，
  等于没画；这一对正是 §3.6.1 记着的雷区）。两张都不同色，两张都读不出。
  ⚠️ 从前这里举的第二张是 `/tmx/`「加拿大现货成交额…各分项占比」，**那条右轴线已由页面
  所有者拍板删掉**（理由「同一条序列不许换个切法再画一遍」，见 `data/tmx.js` 页尾那条
  「2026-09：本页改了四件事」的第（三）项），所以 `/tmx/` 现在只剩「股指期货」那一张
  带线的堆叠图 ——
  **按内容指认也要核内容还在不在**，图名防得住图号平移，防不住「这张图的线被删了」。
  （**这里不写「多少期压在哪一段」也不写 Ex 号** —— 分子分母逐月都在长、图号还会因
  增删图整体平移，写一次就要过期一次；上面这两张实证本身也是**现在时**断言，要当期
  结论按下面那条判据自己扫 `data/*.js`。不会过期的只有 GREEN/NAVY 2.56:1 与
  GREEN/MBLUE 1.07:1 这两个调色板常量之间的比值。）
  换线色也换不出来：线色与柱顶那排百分比标签是同一个 `col(ex.line.color)`，标签恒落在
  柱顶之上的白底、要 ≥4.5:1，线压在深段上要 ≥3:1，两个约束的可行域是空的。
  casing 保证的是**线自己的那条边**（线色 vs 白，与背景无关：GREEN/白 4.54、GOLD/白 2.91）；
  背景那一侧仍看段色 —— 白/NAVY 11.62、白/MBLUE 4.84、白/GREEN 4.54 很强，
  白/GOLD 2.91、白/GRAY 2.43、白/BLUE 1.85 就弱。
  （以上全是调色板常量之间的比值，不会过期。）
  所以**不能写成「加了 casing 就处处 ≥3:1」**，只能说「线从此有一条与背景无关的自有轮廓」。
  逐张判「线落在哪一段」的判据（这条也不会过期）：把线换算成左轴等效值 `v / r1 * y1`
  再与该期各段的累计比对，`r1` 取 `ticks(0, line.ymax, 6)` 的**末项**而不是 `ymax`
  本身（`ymax` 90 / 70 / 7 会被 `ticks` 抬成 100 / 80 / 8，按 `ymax` 硬算会判反）。
- **casing 的边界：本轮只给 `stacked_dual` 的右轴线 —— 这不等于「别的线不需要」。**
  不传 `halo` 的调用点分两类，别读混：
  - 线**真的画在白底上**：`lines` / `lines_endlabels` / `year_lines` / `gs_line` /
    `gs_line_avg`。前三个里多条线互相交叠，给其中一条加白边等于把邻线擦断，所以**永远**
    不传。（`gs_line` / `gs_line_avg` 自己只有一条线 —— 它归这一类是因为画在白底上，
    不是因为「多线交叠」，别把理由记串了。）
  - 线**同样压在柱上**、和 `stacked_dual` 是同一个病，只是本轮没验收：`bar_line`、
    `bar_line_dual`、`gs_bar` 的 `yoy` 次轴线、`qtr_bar` 的右轴线、`grouped_bars`
    的右轴线。这五处现网张数合起来远多于 `stacked_dual`（各是多少自己扫 `data/*.js`
    的 `kind`）。其中 `gs_bar` 那条已经有 `build/verify_pages.py` 的硬 ERROR 顶着
    「不许与段同色」（§3.6.1）。**它们是另案，不是「不需要 casing」**；要动得连同各自的
    段内标签、markers 与既有守卫一起重新验收。
- ⚠️ **加了 casing 之后，仓里几处「那条线无描边」的记载要分开看**（2026-09-09 现扫；
  一律按**内容** grep，别按行号找 —— 这几个文件都在动）：
  - 说的是 `stacked_dual` 的右轴线、**本轮已一并改对**：`build/single.py` 里 stacked_dual
    配色那段从前写的是「线色不能与任何一段撞：`polyline` 无描边、画在柱之后，同色时
    它穿过那一段整段看不见」，现在已经换成 casing 口径（「理由 2026-09 换过一次…引擎
    现在给这条线打了一层白 casing，线**恒有一个形状**，『整段看不见』不再成立；但禁令
    照旧，换了个理由」），那条 SpecError 的文案也已经带上「引擎给这条线打的白 casing
    只镶边，不解决同色」。**别照着旧原文去 grep** —— 它已经不存在了。
    （「asx 拿到 `GOLD` 是退而求其次」那段说明本来就不以「无描边」为前提，没动。）
  - 说的是 `gs_bar` 的 `yoy` 次轴线（那条**确实**不传 `halo`）、**仍然为真，别顺手一起改**：
    §3.6.1 那段代码注释、`build/engine_kinds.md` 的 `stacks` 那一行、`build/mrbase.py`、
    `build/verify_pages.py` 的撞色守卫。
  - `build/ice.py` 的 `_color_guard`（docstring + 会打印给人看的 SystemExit 文案）
    本轮已改对：避同色的**理由**换了，规矩没换。
- 左轴 `min(0, 负向堆到底 ×1.15) .. 正向堆到顶 ×1.28`（无 `line` 时 ×1.06）；
  右轴**初值** `0 .. line.ymax`（**缺省 60**，给了 `line` 就几乎一定要显式给 `ymax`）
  —— 这只是量程的起点，后面「两轴零点同高」那一步还可能把它往 0 以下扩（见下条与 §4）。
  全部段值非负时下界恰好是 `0`、上界恰好是旧的「堆叠总高 ×倍数」，与 2026-09 之前
  逐字节相同（现网 `stacked_dual` **全部**在这一档；判据与「这里不写张数」的理由见本节
  开头那条 ⚠️）。
  ⚠️ **右轴量程只由 `0 .. line.ymax` 定、根本不看 `line.values`**（charts.js:984），
  **所以右轴线含负值的图不能用这个 kind** —— 负值点会被顶到轴外，画面不报错，
  读者只会读到「这条线从没转过负」。零点对齐扩出来的那截负区兜不住它：
  扩多深是按**左轴负段**的深浅算的，与右轴线自己能跌到多低毫无关系；
  左轴全非负时更是一步不动（`f = 0`，走的还是老路径）。
  要「堆叠柱 + 会转负的次轴线」，走 `gs_bar` 的 `stacks` + `yoy`（§3.6.1 / §3.6.2）。
  **段为负** 与 **次轴线为负** 是两件事，前者从 2026-09 起支持，后者仍然不支持。
- **负段（2026-09 起）**：正段自 0 往上堆、负段自 0 往下堆，一列两条独立的运行基线。
  - **柱的总高不再等于列合计**：一列 +6.4 与 −0.9 的柱从 −0.9 画到 +6.4，
    合计 +5.5 在图上是「柱顶到柱底之间被零线切开的那个差」，不是柱高。
    要让读者读到合计，靠表格视图 / 图注 / 右轴那条线，别指望柱。
  - **零线**由引擎那条通用的「量程跨零就画」逻辑给（`C.AXIS` 0.9px，画在网格层、
    被柱压住，只在柱与柱之间露出来），本 kind 不另画。
  - **段间 1.5px 留白缝留在朝零线那一侧**：正段在下缘、负段在上缘。
  - 段内标签仍钉在本段两条边的中点 ⇒ 负段的标签落在负段体内。
  - ⚠️ **有负段又要给 `line`，先算一遍浪费率**：左轴跨零、右轴初值零点贴底，
    「两轴零点同高」只能把右轴往下扩，扩出来的那一截正好等于左轴的零点比例 ——
    `waste = f = |y0| / (y1 − y0)`，`line.ymax` 在这里约掉了、不影响判定。
    代入本 kind 的 `y0 = 负向堆到底 ×1.15`、`y1 = 正向堆到顶 ×1.28`，
    阈值化简成一条纯比例：**最深负包络 ÷ 最高正包络 > 0.6822**
    （= `0.38 × 1.28 ÷ (1.15 × 0.62)`）才越过 0.38，才改走「两轴各自缩放」
    并在绘图区左上角印一行红色斜体「左右轴零点不同高」。
    远没到就照常对齐，右轴被扩进负区、刻度重算，红字不出现。
    `/cost/` 那组数（客单 −0.9、客流 +6.4、`ymax: 60`）用 `tools/align_replica.py`
    实测 `f = 11.2%`：左轴 `−1.035 .. 8.192`、右轴 `−7.5806 .. 60`、刻度 `−10 … 60`、
    对齐成功、**不印红字**（客单换成 −4.4 才有 `f = 38.2%` 越线）。
    ⇒ 判据是「跑一遍 `tools/align_replica.py` 看 `f`」，不是「有负段就一律别给 `line`」。
  - ⚠️ 旧行为（**别再退回去**）：`y0` 写死 0 + `Math.max(0, …)` 把负段钳成零高不画 +
    **单**条 `base` 照样往下走 ⇒ 上面那一段从负基线起画，看着是一整根颜色错的柱，
    不报错、`verify_pages.py` 也不抓（它只对 `gs_bar` 的 `stacks` 查负段）。
- `line.values` 的**数值标签硬编码成 `toFixed(1) + '%'`**，表格视图里也硬编码 `pct1`
  ...
  **负段把这条老毛病放大了**：−0.9 印成 `-1`、+6.4 印成 `6`，两位有效数字全丢。
  所以**百分数 / 百分点口径的堆叠不要开 `label`**（`/cost/` 的客单 × 客流就是这一类），
  数值给表格视图与 tooltip 去说。2026-09 那一轮**没有**顺手改成跟 `fmt`：
  exchanges-eu(`f2`) / exchanges-na(`f1`) / tmx(`pct1`) 那批页面已声明了 `fmt`，
  一改就不是逐字节不变了，要单独一轮回归。
...
- 触发条件（charts.js:768-770）：**只有 `bar_line_dual` 恒为双轴**；
  `qtr_bar` / `grouped_bars` / `stacked_dual` 给了 `line`、`gs_bar` 给了 `yoy` 才是双轴。
  `stacked_dual` 的 `line` 是可选的（§3.14），不给就是单轴 —— 现网**多数**张没给
  （给了 / 没给各几张、分布几页都是可现算的，判据与重扫办法见 §3.14 那条 ⚠️，
  这里不写快照数），`data/cost.js` 那张「Revenue mix by segment」就是一例
  （**按内容指认，不写 Ex 号** —— 图号会因增删图整体平移，见 §3.14）。
- **两轴零点默认画在同一高度**（只扩不缩，数据点一个都不会被挤出去）。
- 对齐代价 = 扩出来的量程 ÷ 对齐后的量程；超过 **0.38** 就放弃对齐，两轴各自缩放，
  并在绘图区左上角画一行红字「左右轴零点不同高（两轴独立缩放）」。
- 代价大多来自「柱全为正、线跨零」的组合 —— 左轴量程会被拉到负区，零线以下那一段
  一根柱都没有，只装右轴那条线的负值（浪费率就是这一段占绘图区的比例，封顶 0.38）。
  **不能接受就省掉 `line`**，把那条信息写进 `note` 或另起一张图。
- **那一段不画左轴刻度与网格线**（2026-09-14 起）。三条同时成立才不画：
  ① 零点对齐**之前**左轴下界 ≥ 0（`ycap` / `yfloor` 覆写之后算）；
  ② 对齐**之后**下界 < 0（没走兜底，且真被对齐扩进了负区）；
  ③ 左轴参与量程的值（§1.1 说的那个 `lv`）全部 ≥ 0。
  此前那几档照常印 —— 柱全为正的图上印着「−100」，美元轴印成「$-100」（`/hood/` 那几张
  美元双轴图就是这样被审出来的）—— 等于给一个不存在的左轴读数立标尺，而且那几条网格线
  与右轴刻度不在同一高度，顺着线去找右轴读数会读偏。
  **量程、零点高度、浪费率、兜底阈值一个都没变**，零线（`C.AXIS`）照画、右轴刻度照印，
  所以 `build/axisfmt.py` 与 `build/mrbase.py` 的 `align_sim` 这两份量程副本不用跟改；
  `tools/align_replica.py` 多给了 `left_ticks_labeled`（图上真印出来的左刻度）。
  ⚠️ 只管**左**轴。反方向（左轴有负值、右轴线全 ≥ 0 被对齐拉进负区，`stacked_dual`
  负段 + `line` 就是这一型）右轴的负刻度照印 —— 2026-09-14 扫 `data/*.js` 一张都没有
  （活数，引用前用 `tools/align_replica.py` 重扫）；真出现了再按同一判据补右轴。
  ⚠️ 图注里写「左轴被拉到 −XX」的仍然成立（说的是量程）；写「可见刻度自 −XX 起」
  「左轴刻度延到 0 以下几格」的就不成立了 —— 这两句在 `build/schw.py` / `build/spgi.py`
  里已随这次改动改掉。
  （本条随引擎改动一起写，改动**尚未合并**：合并之前线上页面仍是旧行为。合并之后这个括号可以删。）
- 2026-09 新增第二种来源，方向正好反过来：**`stacked_dual` 含负段 + 给了 `line`**。
  左轴跨零而右轴初值零点贴底（`r0 = 0`，见 §3.14），要对齐只能把右轴往下扩，
  浪费率**恰好等于**左轴的零点比例 `f = |y0| / (y1 − y0)`（`line.ymax` 约掉了）。
  所以它**不是**必然越线：对本 kind 化简成「最深负包络 ÷ 最高正包络 > 0.6822」才越，
  `/cost/` 那组数（客单 −0.9 / 客流 +6.4）只有 `f = 11.2%`，照常对齐、不印红字。
  先跑 `tools/align_replica.py` 看 `f`；真越线了同样的解：**省掉 `line`**。
...
| 不与 `ycap` / `yfloor` / `bar_marks` 同时出现 | 引擎对这些柱退回单色，而图例仍列着各分部 |
