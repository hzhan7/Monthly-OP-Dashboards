...
- **短了**：尾部按缺失处理 —— 对有 `isNum()` 守卫的 kind 是安全的（少画几根柱），
  对 `stacked_dual` 会算出 `NaN` 坐标（`posB[i] + undefined`），rect 直接画不出来；
  `NaN` 还会写回 `posB[i]`，这一列后面的正段也跟着全不画。（JSON 里的 `null` 不走这条路，见 §1.2。）
...
分水岭主要是「这条线要不要过 Catmull-Rom 平滑」：平滑函数直接做算术，`null` 被 JS 当成 `0`，
于是画出一条**塌到零的假线**，而且不报错。`stacked_dual` 也在不容忍那一侧，但它的堆叠段
**不走平滑**，是另一个理由（见下）。
...
- `gs_line` / `gs_line_avg` / `lines_endlabels`：平滑曲线先塌到 0（这一步三种都一样、都不报错）。
  **抛不抛 TypeError 不是一律的**，要看标数值那一步碰不碰得到 `null`、碰上的是哪个格式器
  （2026-09-14 用真引擎在浏览器里按 kind × fmt × 首尾 / 中段 `null` 逐个复核过）：
  - 碰不碰得到：`gs_line` / `gs_line_avg` **逐点**标数值（`fv(values[i])`），任何一格 `null` 都会碰上；
    `lines_endlabels` **只标首尾两端**（`fe(values[0])` / `fe(values[n-1])`），首尾为 `null` 才碰上 ——
    中间的 `null` 到不了格式器，**不抛**，只是把线画塌到 0，最难发现。
  - 碰上之后：先过一道 `Math.abs(v)` 的格式器**不抛**，印出一个假的 0 —— 现在是 `f0c` / `int`
    （`comma()`，印 `0`）与 `pct0z`（印 `0%`），图照画、不报错，读者看到的是数据里不存在的 0；
    其余格式器（含缺省 / 不认识的名字退回的 `f1`）都直接 `v.toFixed()`，**抛 TypeError** ——
    该卡片以下的渲染全部中断（同一页后面的 exhibit 都不会出现）。
- `stacked_dual`：**不是平滑图型，也不抛异常，只会画错**（2026-09-14 用真引擎在浏览器里复核过）。
  - 堆叠段为 `null`：`sneg = sv < 0` 为 false，`hi = lo + sv` 仍等于 `lo`（`lo + null === lo`），
    于是那一段按 **0 高**画，后面的段从同一条基线接着堆 —— 柱照画、只是矮一截，
    看上去就是一个真实的较小值。算出 `NaN`、柱画不出来的只有 `undefined`（数组比 n 短，见 §1.1），
    JSON 里的 `null` 不会。
  - 右轴 `line` 为 `null`：走 `polyline(…, doSmooth=true, …)`，`smooth()` 把它当 0，线被拽到 0
    （前导 `null` 还可能冲到 0 以下）；逐点的百分比标签按 `isNum` 跳过非数，不抛 TypeError。
