...
> 用例是「合计 = 若干个可正可负的贡献项之和」的分解图（`/cost/` 的可比销售
> = 客单价 × 客流：FY24Q4 客单 −0.9%、客流 +6.4%、合计 +5.4%）。
...
- 左轴 `min(0, 负向堆到底 ×1.15) .. 正向堆到顶 ×1.28`（无 `line` 时 ×1.06）；
  右轴强制 `0 .. line.ymax`（**缺省 60**，给了 `line` 就几乎一定要显式给 `ymax`）。
  全部段值非负时下界恰好是 `0`、上界恰好是旧的「堆叠总高 ×倍数」，与 2026-09 之前
  逐字节相同（全站 22 张 `stacked_dual` 都在这一档）。
...
- **负段（2026-09 起）**：正段自 0 往上堆、负段自 0 往下堆，一列两条独立的运行基线。
  ...
  - ⚠️ **有负段时不要再给 `line`**：左轴跨零、右轴钉死在 0，引擎「两轴零点同高」
    的对齐要把右轴一路往下扩，浪费率超过 0.38 的兜底阈值 ⇒ 改走「两轴各自缩放」
    并在绘图区左上角印一行红色斜体「左右轴零点不同高」。图没画错，但那行字会一直在。
...
- 2026-09 新增第二种来源，方向正好反过来：**`stacked_dual` 含负段 + 给了 `line`**。
  左轴跨零而右轴被写死钉在 0（`r0 = 0`，见 §3.14），要对齐只能把右轴一路往下扩，
  浪费率必然越过 0.38 ⇒ 必定走兜底、必定印那行红字。同样的解：**省掉 `line`**。
