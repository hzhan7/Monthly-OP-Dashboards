...
(function () {
  ...
  /* ── year_lines 的年份配色 ──────────────────────────────────────────────
     ...
     hkex 那张 2026-09-08 重排后是 Ex14，并已按所有者指令拓到全部 11 年 ——
     往年线 10 条，每档只差 5.9 L*，该页图注里现算印出了这个数并叫读者别靠颜色认年份）。
  ...
  function draw(host, ex, opt) {
    ...
    /* `halo`（可选，缺省 undefined）：在彩色线之下先打一条 `lw + halo` 宽的白色底线，
       于是线的两侧各露出 halo/2 的白边（制图里叫 casing）。道理与 txt() 给每个字加
       白描边同源：**这条线压在什么颜色上不由它自己决定**，靠挑线色躲不开。
       （⚠️ 同名不同义，别照搬 txt()：那边的 `o.halo` 是**布尔、缺省开**（`halo === false`
        才关），这里的 `halo` 是**白边总宽的数值、缺省关**。）
       只有 stacked_dual 的右轴线传它，理由与边界见那一支的注释。
       ⚠️ 必须做成一个参数，不能在调用处「先画一遍白的、再画一遍彩的」：本函数除了那条
       线，还画 markers 的圆点与越界钳位的空心红圈 + 竖排真值（下面 out 那一段），
       连调两次会把它们一并画两遍。
       这里的因果要说准：stacked_dual 的右轴线传的是 `Y2`，走的是下面
       `yfn !== Y && _rhsCap != null && r1 != null` 那一支，**与 capOn 无关** ——
       capOn 确实会因 `_rhsCap != null`（**只要这张图给了 line.ymax**；`ymax` 在契约里
       是可选的，见 build/CONTRACT.md §3）而为真，但它只守 `yfn === Y`
       那一支，这条调用永远走不到；末尾画红圈的循环也只由 `out.length` 决定。
       **闸门是开着的，今天恰好没有越界点**：判据是拿 line.values 与
       `r1 = ticks(0, line.ymax, 6)` 的**末项**比（不是与 ymax 比），逐月会变，
       要当期结论自己扫 data/*.js。一旦某月越界，连调两次就会把红圈与真值画两遍。
       不传 halo 的调用点一条属性都不加，输出逐字节不变。 */
    function polyline(vals, color, lw, doSmooth, markers, yfn, halo) {
      yfn = yfn || Y;
      ...
      /* 打底的白线：同一条 `d`，只是更粗，画在彩色线**之前**（SVG 后画的盖先画的）。
         markers 的圆点与越界红圈都不加白底 —— 它们是点不是线，本来就有自己的形状。 */
      if (halo) el('path', { d: d, fill: 'none', stroke: C.WHITE, 'stroke-width': lw + halo,
    ...
    if (kind === 'bar_line' || kind === 'diverging_bars' || kind === 'bars_labeled' || kind === 'bar_line_dual') {
      ...
      /* 一列**两条**运行基线：正段自 0 往上堆、负段自 0 往下堆（标准堆叠柱语义）。
         ...
         `posB` 与从前的 `base` 在全非负数据上逐字节相同（现网**每一张**都是；判据同
         上面 ylim 那一段：扫 data/*.js 取本 kind 的 stacks[].values，最小值 ≥ 0 ——
         不写张数，写死一次就要过期一次），所以既有页面走的仍是同一串浮点数。 */
      ...
      /* 右轴那条线是**可选**的（见上面 dual 的注释）。没给就到此为止：
         ...
         所以这一段整体跳过，不是「画一条空线」。 */
      if (rhsOf(ex) && Y2) {
      /* ── 这条线为什么必须带白 casing（第 7 个参数）────────────────────────
         它按构造压在堆叠段上：右轴是 0..ymax、左轴是 0..堆到顶×1.28，两套刻度
         没有耦合，所以线心落在哪一段完全是数据决定的，画之前不知道。
         **「线色与段色不同色」远远不够** —— 同亮度撞色与同色等价，现网两张实证：
         · /tmx/「股指期货：各分项占比」（NAVY+GRAY 两段）的 GREEN 线**整条**压在
           NAVY 上（GREEN/NAVY 2.56:1）；
         · /exchanges-eu/「Pool share of European cash-equity turnover」（GOLD/MBLUE/
           NAVY 三段）的 GREEN 线**多数期**压在 MBLUE 上（**GREEN/MBLUE 1.07:1**，
           灰度差 1.7%，等于没画；这一对撞色 docs/CHART_KINDS.md §3.6.1 早就记成雷区了）。
         两张都「不同色」，两张都读不出。
         （这里按**内容**指认而不写 Ex 号：图号会因增删图整体平移，写死号码下一次删图
          就变成假话。别改回写 Ex 号 —— 但按内容指认也要**核内容还在不在**：从前这里
          举的第二张是 /tmx/「加拿大现货成交额…各分项占比」，那条右轴线已由页面所有者
          拍板删掉（data/tmx.js 页尾那条「2026-09：本页改了四件事」的第（三）项白纸黑字
          写着），实例才换成上面这张。
          同理不写「多少期压在哪一段」—— 分子分母逐月都在长，两条实证本身也是**现在时**
          断言，要当期结论自己扫 data/*.js，判据见下面那段。真正不会过期的是
          GREEN/NAVY 2.56:1 与 GREEN/MBLUE 1.07:1 这两个调色板常量之间的比值。）
         而且换颜色换不出来：线色与下面那排柱顶百分比标签是同一个 col(ex.line.color)，
         标签恒在柱顶之上的白底、要 ≥4.5:1（相对亮度 ≤0.1833），线压在 NAVY/MBLUE 上
         要 ≥3:1（≥0.2212 / ≥0.6004），可行域为空 —— GREEN 卡在那个联合最优点上。
         所以只能让线自己带一层与背景无关的分离层，与 txt() 给每个字加白描边同一条道理。

         casing 保证的是**线自己的那条边**（线色 vs 白，与压在什么上无关）：
         GREEN/白 4.54:1、GOLD/白 2.91:1 —— 这条边恒定存在，线于是总有一个形状。
         背景那一侧另算，看段色：白/NAVY 11.62:1、白/MBLUE 4.84:1（深色段上从此彻底
         分开），但白/GRAY 只有 2.43:1、白/BLUE 只有 1.85:1 —— **浅色段上白边本身不显眼**，
         靠的是「线色 vs 白」那条内边。所以这不是「一加 casing 处处 ≥3:1」，
         别这么写进图注。（这一段全是调色板常量之间的比值，不会过期；会过期的是
          「哪一张图的线落在哪一段」那一半，那一半这里一个都不写。） */

      /* 宽度取 2.0（两侧各 1.0px），不取 1.2：
           · 1.0px 是能在 1x 屏上活下来的最小白边。0.6px 填不满一个设备像素，抗锯齿
             之后只剩一道灰雾，而这条线要压过去的正是 1.07:1 这种同亮度撞色。
           · 代价一侧是实测的，不是估的：同类图里**真开了段内标签的只有少数几张**，
             而段内标签画在这条线**之前**、会被白底盖住。
             ⚠️ 判据是 `stacks[].label` 的**值为真**（上面那句 `if (st.label && hgt > 8)`），
             **不是有没有这个键** —— 现网多数张都带着这个键、值却是 false，按「键在不在」
             数会数出好几倍、把结论数反。是哪几张、有几张，自己扫 data/*.js 现算。
             按本文件的几何（ph = H − M.t − M.b，stacked_dual 走 268 + XB 与 M.t = 30
             ⇒ 238px）逐期算「线心到段内标签字框」的距离：真接触只出现在
             /hood/「Transaction revenue mix by asset class」那张的少数几期，其余各张
             全期零接触、最近的一期也还差着这条线整宽（3.8px）的好几倍（而且那是
             **未计 thinLabels 抽稀**的模型下界 —— 真画出来的标签更稀、间距只会更大，
             照这个数去量页面会对不上；哪几期、差多少逐月都在变，要量就按当期数据自己
             重算）。而那几期**现在**就已经被 1.8px 的绿线划穿（不加 casing 也接触），
             1.2 → 2.0 只是把划痕从 3.0px 加宽到 3.8px。拿 0.8px 划痕换一条全站看得见
             的线，划得来 —— 结论不变：宽度仍取 2.0，不必为段内标签收窄。
             （这几期里有段内标签是白字的（`label_color: 'WHITE'`），白带压白字是
              「抹掉」不是「划过」
              —— 但根因是「段内标签画在线之前」这个 z 序，另案；本轮不动 z 序，
              动它要把开了段内标签的那几张重新验收一遍。）

         ⚠️ 边界：**本轮只有 stacked_dual 传 halo** —— 但这不等于「别的线不需要」。
         不传 halo 的调用点分两类，别读混：
           · 线**画在白底上**：lines / lines_endlabels / year_lines / gs_line /
             gs_line_avg。前三个里多条线互相交叠，给其中一条加白边等于把邻线擦断，
             所以**永远**不传。（gs_line / gs_line_avg 自己只有一条线，归这一类是因为
              它画在白底上，不是因为「多线交叠」——别把理由记串了。）
           · 线**同样压在柱上**、和 stacked_dual 是同一个病，只是本轮没验收：
             bar_line、bar_line_dual、gs_bar 的 yoy 次轴线、qtr_bar 的右轴线、
             grouped_bars 的右轴线。这五处现网张数合起来远多于 stacked_dual
             （各是多少自己扫 data/*.js），其中 gs_bar 那条已经有 verify_pages 的硬
             ERROR 顶着「不许与段同色」。**它们是另案，不是「不需要 casing」**——
             要动得连同各自的段内标签、markers 与既有守卫一起重新验收。
         别把 halo 提到 polyline 内部无条件生效。

         影响面说实话，别把它吹成「一次修好一批病图」：这一句一改会动到全站**每一张
         给了 line 的 stacked_dual** 的右轴线（纯渲染，payload 不动、不用重跑数据），
         但受益极不平均，取决于线落在哪一段：
           · 白边对背景本身就够强（白/NAVY 11.62、白/MBLUE 4.84、白/GREEN 4.54、
             白/RED 5.85）—— 线从此有形状，上面点名的 /tmx/ 那张（GREEN 压 NAVY）与
             /exchanges-eu/ 那张压 MBLUE 的那些期就在这一档；
           · 白边对背景弱（白/BLUE 1.85、白/GOLD 2.91、白/GRAY 2.43）—— 只多出
             「线色 vs 白」那条内边；
           · 线常年落在柱顶之上那片白底里的几张，几乎无变化。
         哪张图落哪一档逐月会变，这里一个存量清单都不留；判据本身不会过期，自己现算：
         把线换算成左轴等效值 `v / r1 * y1` 再与该期各段的累计比对，`r1` 取
         `ticks(0, line.ymax, 6)` 的**末项**而不是 `ymax` 本身（ymax 90 / 70 / 7 会被
         ticks 抬成 100 / 80 / 8，按 ymax 硬算会把好几张图判反）。
         柱顶之上那片白底有多高也是算出来的：`y1 = mx × 1.28` ⇒ 顶空 = 0.28/1.28 =
         **22% 的绘图区高度**（与下面那句同一个数；上面 ylim 分支里那句「28% 顶部留白」
         说的是同一块地方的**另一个口径** —— 相对最高柱高，不是相对绘图区）。 */
      polyline(ex.line.values, col(ex.line.color), 1.8, true, false, Y2, 2.0);
      /* 左右两轴各自缩放（左轴 min(0, 负向堆到底×1.15)..正向堆到顶×1.28，右轴 0..ymax），
         ...
         永远留着 22% 的绘图区高度），x 不动，仍看得出对应哪一列；抬上去之后是白底 ——
         但对比度**按 col(ex.line.color) 分两档，不是处处达标**：
           · GREEN `#548235` vs 白 = 4.54:1 —— 过 4.5，只过 0.04；
           · GOLD  `#BF9000` vs 白 = 2.91:1 —— **够不着 4.5**。那几张抬上去只是从
             「压在深段上读不出」换成「在白底上偏淡」，压字解决了，对比度没有。
         所以别写成「顺带把对比度问题一起解决」：这一步解决的是**压字**，
         对比度只在线色是 GREEN 的那一档成立。
         （原先这里写的是 5.4:1 —— GREEN 那档的 4.54 抄漏了一位。按 WCAG 相对亮度
          重算：#548235 的 L = 0.181071，(1.05)/(0.181071+0.05) = 4.5441 ⇒ 4.54。
          写成 5.4 会让人以为这条抬升还有余量可以再挪，其实一点都没有。
          当期哪些图用哪个线色，自己扫 data/*.js 的 line.color，别在这里写死张数。）
    ...
    /* 左右轴刻度 vs 逐点数值标签：同上一条规矩的另一半 —— 冲突时让刻度让位。
       ...
       db1 Ex31/34/35/37、enx Ex33、exchanges12 Ex9、hkex Ex3/Ex20（Ex20 是 2026-09-08
       重排前的号，现 Ex16）、ice Ex15/19/20/22、
