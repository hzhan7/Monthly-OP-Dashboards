...
#
# ━━ 与页尾 notes / 图注的分工 ━━
# notes 与图注说的是「这一张图**这个月**该怎么读」（含当月读数、当期实测的残差量）；
# 这一块说的是「这些词是什么意思」，一年到头是同一段 ⇒ 这里**不写当月读数**，
# 也不写任何随月份变的数（份额、分母大小、窗口的具体月份标签一个都不出现）。
# 出现的数只有两类：把定义钉住的**结构性**常数（合约乘数 100 股/张，出处是
# `series/contract_specs.csv` 的 US_MULTILIST_EQ_OPT 行，本文件已读成
# SHARES_PER_CONTRACT）与窗口年数（WIN_YEARS）—— 两个都是引本文件已有的常量，
# 一处硬编码都没有。自校验那条硬阈值**故意不写数**：它是本文件自己设的阈值、
# 不是官方定义里的常数，页尾口径说明里已经有它的具体值，这里再印一遍就是两处各写一份。
#
# ━━ 为什么是这 14 个词（选词判断）━━
# 判据只有一条：这个词出现在本页的图题 / 序列名 / 纵轴 / 汇总表行头 / 图注里，而且
# **不看定义就会读错**。本页是**横截面页**，最要紧的词恰恰是它自己造的口径 ——
# 读者在别处见不到，且每一个都是「分母是什么」的问题。按「读错会出什么事」分五类：
#   ① 分母到底是什么   真份额 / 行业分母 / 合并成交量 / 残差桶 —— 这一页的命门。
#      把「真份额」当成「池内四家之和里的占比」，现货池的读数会整体放大一倍以上
#      （分母从官方合并成交量缩成四家之和），而图形完全正常，看不出来。
#      「合并成交量含场外 TRF 内化」是现货池最容易读错的一点：不点破，读者会把
#      那一大段残差当成「别的交易所」，而它主要是券商与做市商的内部撮合。
#   ② 分子到底是什么   matched / ADV / 多重挂牌 —— matched vs handled 差一整块
#      路由出去的量；ADV 的分母（交易日数）全页只认一份；期权池的外延不含指数期权，
#      而 Nasdaq 的官方总量偏偏不拆，那一格只能当上界读。
#   ③ 两套互不相同的窗口   共同最新月 / 共同窗口、4 年同月窗口 —— 本页有两套窗口，
#      一套要求成员全齐（被披露最晚的那家卡死），一套不要求（因为分母是官方发的）。
#      混着读会得出「某家在这段窗口里凭空出现 / 消失」的结论。
#   ④ 本页自己轧出来、不是公司披露的派生量   池扩大 / 份额转移、季度份额（量加权）
#      —— 前者是归因桥的两块，后者的「量加权」是与「三个月份额取平均」的区别，
#      两者都必须点明「不是公司披露的数」与「恒等式长什么样」。
#   ⑤ 跨口径与不可比的那几处   定基名义额、自校验（锚点）、形式数断点 ——
#      定基名义额是全站主口径而本页是它的退化情形（这件事本身是本页的一条结论）；
#      形式数断点标的是「这条线在这一点左右讲的不是同一家公司」。
# **有意不收**：
#   · m/m、y/y、3Y %ile、pp/bp —— 全站通用的读图约定，summary.note 已经逐条讲过，
#     释义板再讲一遍就是两处各写一份（样板 /cost/ 的 9 条里一条都没有，是同一条判断）。
#   · 成交量、市值、交易所这类常识词 —— 本页对它们没有特殊口径。
#   · 「口径断点画在哪几张图上」「MIAX 深历史的拼接落差」「Nasdaq 为什么进残差桶」
#     —— 这三件事页尾 notes 与相关图注已经逐条写清落点与实测量，属于「这张图怎么读」，
#     不是「这个词是什么意思」。
#   · Centre Point / OI / RPC 这类别的页有、**本页没有出现**的词。
#
...
#      改成：只说线画在哪两张，并点破「没画线 ≠ 那一段可比」。
#      —— 这与 _brk_labels() 上面那条注释记的是同一类坑，第二次犯了。
#   ② 「matched」原来写「现货分子是 Tape A+B+C 的 matched 之和」，是把 CASH_MEMBERS 里
#      **NYSE 那一条成员 note** 的主语换成了整个现货池：实际只有 NYSE Group 走三条 tape
#      相加（`__nyse_cash__`），其余三家各取自己盘口的合计。
#   ③ 同条原来写「期权分子是各家自己披露的**多重挂牌**期权量」，在 Nasdaq 那一格是错的
#      （`vol_us_options_mmcontracts` 是六所当月总张数，官方不拆指数期权），
#      与本块「多重挂牌」那一条、与 notes[3] 正面打架。改成把口径统一交给「多重挂牌」。
#   ④ 「季度份额（量加权）」原来断言最末一格「**永远**落后于共同最新月」——
#      qshare(min_months=3) 只要三个月齐全就出数，共同最新月落在 3/6/9/12 月时最末一格
#      与它同季、并不落后（一年里有 4 个月这句话是假的）。notes[5] 是**当月**陈述、
#      每月重写，可以那样写；释义一年到头同一段，不能把它升格成恒真命题。
#      改成真正恒成立的那条：最末一格永远不含尚未走完的那个季度。
# ══════════════════════════════════════════════════════════════════════════════
# 名义额那一句随 contract_specs.csv 的 base_price_local 变：现在是空的（📌 已知缺口），
# 补上之后这句话必须跟着改，所以按当期实际取值分支，不写死「是空的」。
_GL_NOTIONAL = (
    f'📌 基期价格在 <code>series/contract_specs.csv</code> 的 {OPT_SPEC_ID} 行'
    f'目前是<b>空的</b>（2019-01 成交量加权标的均价没有官方单一字段），所以本页的名义额'
    f'只报到<b>股当量</b>（张数 × {SHARES_PER_CONTRACT:g} 股），<b>不报美元水平值</b> —— '
    f'这不影响本页任何结论，份额与增长率与那个常数无关。'
    if POOL_OPT.base_price is None else
    f'基期价格取 <code>series/contract_specs.csv</code> 的 {OPT_SPEC_ID} 行'
    f'（{POOL_OPT.base_price:,.2f}），名义额报到美元水平值。')

GLOSSARY = [
    ('真份额',
     '本页所有份额的口径：<b>分子</b> = 该家自己披露的撮合量，<b>分母</b> = '
    ...
    ('行业分母',
     '两个池各自的官方行业总量，<b>不是本页把成员加起来算的</b>：<b>期权</b>取 ICE 逐月'
     '披露的全美股票 / ETF 期权行业 ADV（<code>adv_us_equity_options_industry_kcontracts'
     '</code>），<b>现货</b>取 ICE 的 <code>adv_tapeA/B/C_consolidated_mnsh</code> 三条相加。'
     '期权那一条 MIAX 在自己的 IR 报表里也<b>独立报一份</b>，两家公司的数据链互不相干，'
     '本页把它们逐月比对（页尾口径说明「自校验（锚点 B）」那一条是实测结果）。'),

    ('合并成交量',
     '现货池的分母（consolidated volume）= Tape A + B + C 的全市场成交量。要紧的是它'
     '<b>含场外（TRF）内化成交</b> —— 券商与做市商在自己内部撮合掉、事后向 TRF 申报的'
     '那部分也在分母里。⇒ 现货几条份额线加起来离 100% 的那一大段<b>主要不是别的交易所</b>，'
     '而是内化：交易所真正的对手不只是彼此。期权池没有这回事（美股期权必须在交易所成交），'
     '<b>两个池的「其他」不是同一种东西，不要横向比这两个数。</b>'),

    ('matched（撮合量）',
     '各家份额的分子一律取 <b>matched</b> —— 该家<b>自己撮合成交</b>的股数 / 张数。'
     '现货侧 NYSE <b>不用 handled</b>：handled 含被路由到别家成交的量，而其余三家不披露'
     '对应口径，混用会让 NYSE 系统性偏高。⚠️ <b>四家各走各的官方列，不是同一套拆法</b>：'
     '现货侧只有 <b>NYSE Group</b> 的分子是本页把 Tape A+B+C 的 matched 股数相加得到的'
     '（NYSE / Arca / American / National / Texas —— 官方本来就没有「三个 tape 合计」这一行），'
     '其余三家取各自盘口的 matched 合计（Nasdaq 三个、Cboe 四个、MIAX Pearl 一个）；'
     '期权侧取各家自己披露的期权成交量（Cboe 只取 multiply-listed 段，'
     '⚠️ Nasdaq 官方<b>不拆</b>指数期权 —— 见「多重挂牌」）。'),

    ('ADV',
     '<b>日均</b>口径（average daily volume）：<code>当月成交合计 ÷ 当月交易日数</code>。'
     '本页凡是「千张/日」「百万股/日」的行与纵轴都是它。⚠️ 有几列官方只印<b>当月总量</b>'
     '（Nasdaq 的期权与现货撮合量），本页除以交易日数才换成 ADV；<b>美股交易日数全页只认'
     '一份</b>（ICE 的 <code>trading_days_us_equities</code>）—— 各家自己也发日历，混用会让'
     '同一个月出现两个 ADV，而差异小到不会有人发现。'),

    ('多重挂牌',
     '期权池的外延：<b>多重挂牌</b>（multiply-listed）股票 / ETF 期权 —— 同一只标的的期权'
     '在多家交易所同时挂牌、彼此可以抢单的那部分，<b>不含</b>只在一家挂牌的指数期权'
     '（官方的行业分母也不含）。⚠️ Nasdaq 的官方期权总量<b>不拆指数期权</b>，本页只能整块'
     '拿来当分子 ⇒ 它那一格应当<b>当作上界</b>读；这块污染有多大，页尾「锚点 C」那一条'
     '用它自报的季度市占量过。'),

    ('残差桶',
     '份额图上那一段「其他」：<b>桶 = 官方行业总量 − 图上已经单列的各家</b>，用<b>减法</b>'
     '得出，所以无论列几家，<b>加总恒等、不产生任何偏差</b>。这也是本页处理「某家在期初月'
     '还没有月度披露」的办法 —— 把它并进桶，而<b>不是</b>把整个窗口砍到那家的起点；'
     '代价只是它在那几张图上不单独拆出来。⚠️ <b>两个池的桶装的不是同一种东西</b>：'
     '期权池的基本就是别的交易所，现货池的主要是场外内化（见「合并成交量」）。'),

    ('共同最新月 / 共同窗口',
     '两件事，别混。<b>共同最新月</b>是<b>发布门槛</b>：四家都已发布的那个最新月，本页'
     '所有图与表一律截到它 —— 跑在前面的成员，其更新月份<b>不进本页任何一张图、任何一行表</b>。'
     '<b>共同窗口</b>是月度堆叠带与换算链自检的<b>取数区间</b>：它要求四家<b>全齐</b>'
     '（逐月 dropna），因此由<b>月度披露起步最晚的那个成员</b>决定；两个池的窗口长度差得很远，'
     '短的那个池短到不单画月度堆叠带（理由见页尾口径说明）。'),

    ('4 年同月窗口',
     f'起止对照 / Δ / 归因桥这三张图的取数口径：最新月与它 <b>{WIN_YEARS} 年前的同一个月</b>，'
     f'同月比同月。取同一个月份是刻意的 —— 份额本身带<b>月份效应</b>（到期周、假期月、'
     f'季末再平衡改变的不只是总量，还有订单流的构成），页尾口径说明用本页数据量过：'
     f'<b>一次季节性摆动就抵得上一年的结构性变化</b>。它<b>不要求成员全齐</b>'
     f'（与共同窗口正相反）：分母是官方发的，每家可以各自独立除，期初那个月凑不齐的'
     f'成员并进残差桶。'),

    ('池扩大 / 份额转移',
     '归因桥把每家的量变拆成的两块，<b>都不是公司披露的数</b>，是本页轧出来的：'
     '<b>池扩大</b> = 期初份额 × 池总量的增量（份额一点没变也本来就会拿到的那份）；'
    ...
    ('季度份额（量加权）',
     ...
     '其中月成交量 = 该月 ADV × 该月交易日数。<b>不是把三个月的月份额取平均</b> —— 源列'
     '全是日均值，简单平均会给交易日少的假期月过高权重。只认<b>三个月齐全</b>的季度，'
     '缺月的季度留空让线断开 ⇒ 那两张图的最末一格<b>永远不含尚未走完的那个季度</b>：'
     '共同最新月<b>不是</b>季末月时，最末一格停在上一个完整季，共同最新月本身的读数只在'
     '页顶汇总表里；共同最新月正好落在季末月时，它与最末一格同季，两者并不错位。'),

    ('定基名义额',
     '全站主口径：<code>张数 × 合约乘数 × 2019-01 基期价格</code>（汇率同锁基期）。存在的'
...
payload = {
    ...
    # 页顶「名词释义」。本页没有 brief，所以它紧跟 headline。选词判断与「有意不收哪些词」
    # 写在 GLOSSARY 上面那一块注释里。
    'glossary': gloss.render(GLOSSARY, where='exchanges-na glossary'),
