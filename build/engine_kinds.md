...
⚠️ 写死的只有 **右**轴：`stacked_dual` 的**左**轴 2026-09 起会跟着负段往下走，见 §10。）
...
>
> ⚠️ **左轴各 kind 的 `y0`/`y1` 分支另有三份副本**（与上面那三份不是同一批文件，
> 别只改一半）：`assets/charts.js` 的 `lv` + 那一长串 `else if (kind === …)`、
> `build/axisfmt.py` 的 `_left_vals` / `_left_range`、`tools/align_replica.py` 的
> `_left_series` / `compute_axes`。2026-09 给 `stacked_dual` 加负段支持时，
> 前两份已同改，**`tools/align_replica.py` 那一份还是旧的**（它的 `stacked_dual`
> 分支仍写 `y0 = 0.0`、左轴值仍只取列合计）—— 那是个诊断工具、不在构建链里，
> 现网也没有页面在 `stacked_dual` 上调它（只有 `build/spgi.py` 用它，spgi 无此 kind），
> 所以还没炸；谁先动到它，请顺手补齐。
...
| `stacks: [{name,color,values}]` | `gs_bar` | 无（本仓新增） | 把每根柱按业务分色堆叠，**总额仍取 `ex.values`**：纵轴量程、柱顶数值、12 个月均线、次轴 y/y、表格视图与 tooltip 一律照总额走。各段之和必须等于 `values`（`build/verify_pages.py` 硬校验，引擎**不替 payload 求和**）；各段必须显式给互不相同的 color，且**不能与 `yoy.color` 撞**（那条线无描边、画在柱之后，同色时穿过该段整段看不见）。与 `ycap` / `yfloor` / `bar_marks` 不兼容，命中的柱退回单色。⚠️ **不要改用 `stacked_dual`**：它的**右轴**被写死 `ticks(0, ymax, 6)`、`r0 = 0`，而 `gs_bar` 的次轴同比会转负（全站已统一成单月同比，`data/guc.js` Ex2 实测：创意 115 个有值月里 37 个为负、最低 −52.3%），负值会被顶到画布外。（2026-09 起 `stacked_dual` 的**左轴**能画负段了，见 §10 —— 但那解决的是「段」为负，不是「次轴线」为负，这条结论不变。反过来，`gs_bar` 的 `stacks` 至今**不能**有负段，见上面那条 `verify_pages` 硬 ERROR。）|
...
**2026-09 之前 `stacked_dual` 只能画非负段**，而且画错的时候不报错。旧行为是两处
...
- ⚠️ **右轴仍然写死 `r0 = 0`**：右轴那条 `line` 含负值照旧画到画布外。所以
  「堆叠柱里有负段」与「右轴线会转负」是两件事，后者依然要走 `gs_bar` 的
  `stacks` + `yoy`（见 `docs/CHART_KINDS.md` §3.6.2）。
- ⚠️ **负段 + 右轴 `line` 会触发零点对齐的兜底**（§9）：左轴跨零、右轴钉在 0，
  对齐要把右轴一路扩到 −13 才够，浪费率 >0.38 ⇒ 引擎改走「两轴各自缩放」并在
  绘图区左上角印一行红色斜体。不想要那行字，就**别给 `line`**（段高本身已经把
  每一块读出来了）。
