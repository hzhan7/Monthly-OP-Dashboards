...
（P&L 表头 "Q1'21"，KPI 表头 2021/Q1）。所以本脚本的 FIRST = 2021Q1，**不硬凑**。
⚠ 2026-09-14 更正：原文接着写「再往前只有 S-1 里的年度数，不是季度粒度」—— 这句是错的。
S-1 招股书（0001628280-21-013318）印着 2019Q1–2021Q1 逐季的交易 / 净利息 / 其他 / 总净收入
（千美元），与本 CSV 的 2021Q1 在凑整误差内逐格对得上。那一段由 build/basefill/hood_ipo.py
单独存进 series/hood_ipo_q.csv（单位与来源都不同，而本 CSV 是抓取器每季往右追加的台账，
不往里掺）。按资产类别的季度拆分只零散见于 2021 年 10-Q 的上年对比数，季度成交量 2021 年以前
一季都没有 —— 这两件事才是本 CSV 停在 2021Q1 的真实理由。
