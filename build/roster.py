...
LAG = {
    ...
    'hkex': (10, 10),    # 次月上旬
    # UMC 的台湾公告日 163 个月无一例外落在次月 4-10 日，但本页的源是 SEC 6-K，
    # 而 6-K 的 EDGAR filingDate 只有 **2025-05 之后**才与公告日同日（实测 15 个月零分歧）；
    # 2013-2024 年 UMC 把整月公告攒成一份 6-K 月底才报，最长滞后 33 天（2024 年仍有 14 天）。
    # 取 14 = 最近 24 个月的最坏值，不是照最近 12 个月（最大 8 天）定的。
    # 季末月与常规月同节奏（2025-03→04/10、2025-06→07/07、2025-12→01/07），故两个数相同。
    'umc':  (14, 14),    # 次月 4-10 日公告；LAG 照 6-K 上 EDGAR 的最坏滞后留余量
    'hood': (13, 30),    # 常规月次月 9-13 日；季末月随财报（实测 3 月→4/28、6 月→7/29）
    'schw': (17, 21),    # 次月第 10 个美股交易日：17 = 规则上界（9/1 周六那档），常规月实测第 12-16 天；季末月随当季季报，实测最坏 21
    # SPGI 的季末月**不再是例外**（2026-08 复核，原值 (16, 46) 已作废）：公司在
    # 2025Q4 财报 8-K 里承诺「2026-01 数据起，每月 15 日（顺延到下一工作日）发布
    # 上月数据」，季末月一并纳入。新节奏 6 个月逐月核对全中，其中 2026-03 与
    # 2026-06 两个季末月分别比当季财报早 13 天出（工作簿 04-15 / 07-15，财报 8-K
    # 04-28 / 07-28）—— 与财报脱钩是实测出来的，不是照公告推的。
    # 18 = 规则本身的上界（15 号撞周六、顺延到的周一又撞假日），实测最坏第 17 天
    # （2026-07 → 08-17）。取规则上界而不是实测最坏，是因为这里的 18 有出处，
    # 而实测新节奏只有 n=6，按 n=6 的最大值定等于把噪音当规律。
    # ⚠ 原来的 46 不是「更保险」而是**已经失效的保险**：它让真正缺货的季末月
    # 拖到第 51 天（46+GRACE）才变红。全部实测发布日见 fetch/spgi.py docstring。
    'spgi': (18, 18),    # 次月 15 日，撞非工作日顺延；季末月同档（2026-01 数据起）
    'axp':  (16, 16),    # 8-K 每月 15 日前后，撞周末顺延
    'msci': (17, 17),    # 次月中旬
    # ── 台湾半导体（2026-08 接入）—— 一行一条，删掉一家就删掉它这一行 ─────────────
    # ⚠ 全组共 **7 家**，但**不在同一处**：本表大体按 LAG 递增排（不严格），
    #   tsm(10) / guc(7) / umc(14) 早已夹在上面的非半导体各家之间，落在这一块的只有
    #   下面 4 家（nanya / mtk / ase / alchip）。要一次看全 7 家，看 GROUPS 里的 'semi'
    #   那一行 —— 那是这一组的**权威名单**；本注释块只是「排到一起的那几家」，
    #   别拿它当组花名册数家数（原先它写着「5 家」而下面只有 4 行，就是这么来的）。
    # 台湾《证交法》要求次月 10 日前公告上月营收，所以这 7 家的上界天然一致；
    # 差异全在**各家自订的惯例**与**本页数据源的额外滞后**上，逐家实测见各 fetch/<t>.py。
    'nanya': (13, 13),   # 新闻稿实测第 2-13 天、中位第 5；13 = 全历史最坏（2014-09 那期）
    'mtk':  (12, 12),    # 67 期实测：第 10 天 42 次；最晚第 12 天（撞假日顺延）
    'ase':  (15, 15),    # 99 期实测众数第 9-10 天；15 = 农历年那期（2024-01→2024-02-15）
    'alchip': (10, 10),  # 只有法定上限可依：公司不发月营收新闻稿、不预告日期
    ...
    'miax': (8, 8),      # 次月第 3-5 个工作日；5 期实测日历第 3/4/5/5/7 天，最坏排列落到第 8 天
...
# ⚠ 本表只登记**有自己页面**的家。三张公共表（fee_rates / mops_remarks / fx）
#   一条都不在这里，那不是漏登记：
#   · LAG 的取值处只有两个 —— main() 里按 GROUPS 遍历（喂红点）与 monthly_run.not_due()
#     （读 data/<t>.js 的 data_through 定闸门）。公共表两样都没有：没有页、没有卡片、
#     没有 data_through，写进来的那一行**谁都不会读**，只会让下一个人以为闸门归它管。
#   · 它们的闸门各自更准：fx 在数据月内就定稿（不套闸门），mops_remarks 问的是 TWSE
#     OpenAPI 的 `資料年月`（证交所汇总完全市场才翻），都是官方事实，不是日历推算。
#   · 真要连 GROUPS 一起加，首页会多出一张永远读不到 data/<t>.js 的空卡片，
#     monthly_run.check_registry() 还会因为两张名单对不上天天告警。
#   接线细节与失败语义见 monthly_run.py 的 mops_remarks() / fx() / fee_rates()。

META = {
    'cost': ('COST', 'Costco', '零售月结束后首个周三'),
    'ibkr': ('IBKR', 'Interactive Brokers', '次月首个美股交易日'),
    # 这三家季末月（3/6/9/12）没有独立月报，节奏另算 —— 标签里必须写出来。
    # 不写的话，页面显示「次月中下旬」而 LPLA 停在两个月前，看的人只会以为是坏了
    # （实际发生过：用户看到「LPLA 还是 5 月」来问是不是没更新）。
    # 原为「四家」，含 spgi：2026-08 官方改成固定「每月 15 日或顺延」后它移出本组，
    # 标签也一并改掉 —— 这句话是印在页面上给人看的，跟 LAG 一样会过期。
    'schw': ('SCHW', 'Charles Schwab', '次月第 10 个美股交易日（约 12-17 日）；季末月随当季季报'),
    ...
    'spgi': ('SPGI', 'S&P Global', '次月 15 日，撞非工作日顺延'),
    'axp': ('AXP', 'American Express', '次月约 15 日（8-K）'),
    'tsm': ('TSM', 'TSMC 2330.TW', '次月 10 日前'),
    'guc': ('GUC', '创意电子 3443.TW', '次月 5 日（公司日历预告），撞假日顺延'),
    'umc': ('UMC', '联电 2303.TW', '次月 4-10 日（源为 SEC 6-K，上 EDGAR 另有滞后）'),
    'mtk': ('MTK', '联发科 2454.TW', '次月 10 日，撞假日顺延至 11-12 日'),
    'ase': ('ASE', '日月光投控 3711.TW', '次月 8-15 日，众数第 9-10；农历年那期可延到第 15'),
    'nanya': ('NANYA', '南亚科 2408.TW', '次月第 2-9 天（新闻稿实测），法定次月 10 日前'),
    'alchip': ('ALCHIP', '世芯-KY 3661.TW', '次月 10 日前（法定期限）；无月营收公告，无发布日'),
    # 10 家新增交易所 —— 一家一行，cadence 与上面 LAG 的实测统计同源
...
GROUPS = [
    ...
    ('semi', 1, '半导体', ['tsm', 'ase', 'mtk', 'nanya', 'umc', 'alchip', 'guc']),
