...
ORDER = [
    ...
    'total-darts',        # Total client DARTs (disclosed) + 未被推导覆盖的占比
    'darts-per-account',  # Annualized cleared DARTs per account
    'commission-day',     # Implied commission revenue/day
    'cpt',                # Average commission per cleared order
    'cpt-product',        # Average commissions/trade（分产品）
    'margin',             # Customer margin balances
    'cash',               # Client cash
    'cash-margin-share',  # Client cash / margin loans as % of client equity
...
# brief 说「**这个月**这组读数该怎么读」、每月重写；图注说「**这一张图**这个月怎么读」
# （含当月读数、当月实测的毛刺量与占比区间）；这一块说「**这些词**是什么意思」，
# 一年到头是同一段 ⇒ 这里**不写当月读数、不写「最新一期」、不引任何随月份变的数**。
# 全表因此是一个**模块级常量**（不是函数）：里面没有一个需要现算的量，
# 也没有一处 Exhibit 号 —— 本页图号由页头 ORDER 决定（build/exhibits.py），
# 释义里写死「见 Exhibit 5」下一次挪图就是一句假话，所以一律**按图名指路**
# （真要指图号就写 ⟨ex:id⟩ 占位符，写盘时兑成当时的号）。
#
# 出现的数只有两个，两个都是**官方定义里的常数**、不是读数：
#   · 252 —— 官方那一列自己就叫 Cleared Avg. DART per Account (**Annualized**)，
#     年化按 252 个交易日折算（出处：页尾口径说明第 3 条与 cleared DARTs 推导式，
#     本文件 `cleared_all[1:] = ann_all[1:] / 252 * …` 用的就是它）。
#   · 「÷ 1,000」—— commission revenue/day 那条推导式里的**单位换算**（千笔/日 × $/笔
#     → $mn/日），是恒等式本身，不是数据。
# 其余一切带数的说法（覆盖率区间、期货费用占比、口径断点落在哪一期）**一概不写进来**：
# 它们逐月现算、已经印在各自的图注与页尾 notes 里，释义板再写一份就是两处各写一份，
# 且必然有一处先变旧。断点那一期尤其不能写死 —— 本页的规矩是由
# `series/ibkr_pr.csv` 的 `cpt_basis` 列现算（模块 docstring 的「口径」一段）。
#
# ━━ 为什么是这 13 个词（选词判断）━━
# 判据只有一条：这个词出现在本页的图题 / 序列名 / 纵轴 / 汇总表行头 / 核对表列头里，
# 而且**不看定义就会读错**。按「读错会出什么事」分四类：
#   ① 推导值 vs 披露值（本页最大的坑，占了五条）
#      DART / Implied / Total Client DARTs / 人均年化 cleared DART / Implied 佣金收入/日
#      —— 同一张页面上有三条单位相同（千笔/日）、量级也接近的 DART 序列：一条是公司
#      直接披露的，两条是本页轧出来的，而且**互相不是彼此的拆分**。不点破，读者会
#      拿推导值去和披露值逐格对数，把口径差读成估算误差；也会把「cleared」当成
#      修饰账户（那正是本页 2026-09 改正过的一处旧错，见 Total client DARTs 图注）。
#   ② 分母与单位   交易日 —— 本页有**两套**折日频的分母：按产品反推 DARTs 用当月
#      美股交易日数，人均年化 DART 折回日频用固定 252。两者互换会算出「推导值大于
#      披露总量」的月份。且官方那一列是**半天粒度**（19.5 / 20.5 / 21.5），核对表照印。
#   ③ 新闻稿那张表的两列   CPT / 平均订单规模 —— 都是「每笔订单」的均值而不是单价，
#      且只有 CPT 那一列改过口径。四张佣金图上那条红色竖虚线标的是 CPT 的断点，
#      不点破，读者会把它当成订单规模也断了。
#   ④ 存量 / 差分 / 派生比值   净新增账户（表内是账户差分，真实增长另有其数）、
#      客户现金（含扫到合作银行的受保存款）、融资余额（客户的借款，不是 IBKR 的杠杆）、
#      客户权益（两条占比共同的分母，而那两条占比是本页轧的、不是披露的一列）、
#      期货（含期货期权）。这一类读错不会让图形变形，只会让结论整条偏掉。
# **有意不收**：
#   · m/m、y/y、3Y %ile、pp/bp —— 全站通用的读图约定，汇总表的 note 已逐条讲过。
#   · 「单月同比」与「12 个月滚动」的取舍、截轴与斜纹柱这类**画法**约定 ——
#     页尾 notes 第 8、9 条与各图图注讲的就是它们在本页的具体落点。
#   · 账户口径的三次一次性调整（escheat / introducing broker 撤出 / 2024-11 存量下调）
#     的**逐条数字** —— notes 第 7 条与 Exhibit 2 的图注逐月点名，这里只说「差分 ≠
#     真实增长」这条定义性的事。
#   · 账户总数、成交量、市值这类本页没有特殊口径的常识词。
# ══════════════════════════════════════════════════════════════════════════════
GLOSSARY = [
    ('DART',
     'Daily Average Revenue Trades，<b>日均交易笔数</b>：平均每个交易日的客户交易笔数。'
     '⚠️ 本页以<b>千笔/日</b>计的 DART 序列有<b>三条，口径互不相同</b>：公司'
     '每月直接披露的 Total Client DARTs、本页推导的 cleared DARTs、以及按产品反推的'
     'product DARTs。三条画在同一种单位上、量级也接近，<b>但不是同一个数，也不是彼此的'
     '拆分</b> —— 后两条连一个共同的输入列都没有。比大小之前先看清读的是哪一条。'
     '⚠️ 另有一条名字里带 DART 的序列<b>不是笔数</b>：「人均年化 cleared DART」是按账户'
     '折算的<b>年化倍数</b>（单位 <b>x</b>），与上面三条<b>不同单位、不同轴</b>，见下条。'),

    ('Implied',
     '本页凡是图题以 <b>Implied</b> 打头的序列，都是<b>本页自己轧出来的推导值，'
     '不是公司披露的数</b>（cleared DARTs、product DARTs、commission revenue/day 三条）。'
     '每一条的算式、所用的假设、以及它离披露值有多远，都印在<b>各自的图注</b>里。'
     '⇒ 拿它去与公司披露的那一列逐格对数是对不上的，差的那一块是<b>口径差</b>、'
     '不是估算误差 —— 那张画 Total Client DARTs 的双轴图整张就是在量这件事。'),

    ('Total Client DARTs',
     '公司<b>每月直接披露</b>的全部客户日均交易笔数，一步推导都没有（页尾核对表里'
     '那一列写作「含未清算」）。它<b>含</b> non-cleared 客户的单：10-K 把客户二分为'
     ' cleared customers（用 IBKR 的执行<b>与清算</b>）与 non-cleared customers'
     '（只用 IBKR 的执行服务、在别的 prime broker 或托管行清算，典型是在 IBKR 自家 ATS '
     '上提供流动性的交易公司）。⇒ 它<b>大于</b>本页推导的 cleared DARTs；差的那一块是'
     '「执行但不由自己清算」的那部分单，<b>不是</b>清算失败率、未交收比例、市场份额，'
     '也<b>不是</b>客户流失。'),

    ('人均年化 cleared DART',
     '公司<b>直接披露</b>的一列（Cleared Avg. DART per Account, Annualized），'
     '<b>不是</b>推导值。分子只数 IBKR <b>自清算</b>的订单 ——「cleared」修饰的是'
     '<b>订单</b>、<b>不是</b>账户；分母是<b>全部账户</b>（10-K 把 Total Accounts 与'
     ' cleared customer accounts 当同一个数印）。年化按<b>固定 252 个交易日</b>折算，'
     '<b>不是</b>当月实际交易日 —— 本页由它还原 cleared DARTs 时沿用的就是这个 252。'),

    ('交易日',
     '官方历史指标表的 Trading Days 列，数的是<b>美股</b>交易日，而且是<b>半天粒度</b>'
     '（半日市会让它带 <code>.5</code>），页尾核对表照印、<b>不取整</b>。'
     '⚠️ 本页折日频有<b>两套</b>分母，不能互换：按产品反推 DARTs 用的是<b>当月</b>这个'
     '天数（对期货与国际股票也套用这一套<b>美股</b>天数，是本页写明的假设）；'
     '而把人均年化 cleared DART 折回日频用的是<b>固定 252</b> —— 换成当月实际交易日，'
     '会算出「推导值大于公司披露总量」的月份。'),

    ('CPT',
     '月度新闻稿 Key products 表里的 <b>Average Commission per Cleared Commissionable '
     'Order</b>（单笔清算订单平均佣金，$/笔），<b>含</b>交易所、清算与监管费用。'
     '⚠️ 它是<b>每笔订单</b>的均值，<b>不是</b>每股／每张单价。这一列的口径<b>改过一次</b>：'
     '此前是 per cleared <em>client</em> order，此后是 per cleared <em>Commissionable</em> '
     'Order —— IBKR LITE 上线后免佣订单退出分母。口径名逐月存在 '
     '<code>series/ibkr_pr.csv</code> 的 <code>cpt_basis</code> 列里，四张新闻稿口径的图上'
     '红色竖虚线由它现算（本页不写死日期），<b>跨线比 CPT 高低是跨口径比较</b>。'),

    ('平均订单规模',
     '同一张 Key products 表的另一列（<b>Average Order Size</b>）：每笔订单的'
     '<b>股数／张数</b>，<b>不是金额</b>。本页按产品反推 DARTs 走的就是它'
     '（<code>当月成交量 ÷ 平均订单规模 ÷ 当月美股交易日数</code>）。'
     '⚠️ 它与 CPT 印在同一张表上，但<b>本站没有为它登记过任何口径变更</b> —— '
     '那几张图上的红色竖虚线标的是 <b>CPT</b> 改词的那一期，<b>不要</b>把它读成'
     '订单规模的断点，跨线读订单规模的柱与占比是可以的。'),

    ('Implied 佣金收入/日',
     '本页推导的日均佣金收入（图题 Implied Commission Revenue / Day，$mn/day）：'
     '<code>cleared DARTs（千笔/日） × CPT（$/笔） ÷ 1,000</code>。'
     '⚠️ 它是<b>日均</b>：要月度总额还需<b>再乘当月官方交易日数</b>。'
     '两层近似复合 —— 乘数是全部 cleared DART 而 CPT 的分母只含 commissionable 订单，'
     '且 cleared DARTs 本身已经是推导值。月度<b>无</b>对应披露可比'
     '（季度有 —— 10-Q 的 Commissions 行，本页尚未接入）。'),

    ('净新增账户',
     '历史指标表披露的 Net New Accounts，它<b>就是账户存量的月度差分</b>。'
     '⚠️ 差分<b>不等于</b>真实开户增长：PDF 的 Notes 段披露过一次性调整（账户按法律'
     '要求上交政府的 escheat、一家 introducing broker 撤出带走的账户），公司会另给一个'
     ' real account growth。本页的分工是<b>柱与页尾核对表印表内披露值</b>，'
     '<b>汇总表那一行与所有同比走公司还原后的真实增长</b>（行头标着「已还原口径」）—— '
     '所以那张图的同比线<b>拿柱直接除是除不出来的</b>，调整月在图上以<b>斜纹柱</b>标出。'),

    ('客户现金',
     '官方行名 Client Credits（图题 Total Client Cash）：客户在 IBKR 的<b>贷方余额合计</b>，'
     '<b>含</b>扫到合作银行的受保存款（insured bank deposit sweeps）。'
     '它是<b>月末时点</b>的存量，不是当月流入，所以同比走月末对月末，'
     '也<b>没有</b>「当月开了几天市」这一层日历效应。'),

    ('融资余额',
     '官方行名 Client Margin Loans（图题 Customer Margin Balances）：<b>月末</b>客户融资'
     '借款余额，是<b>客户向 IBKR 借的钱</b>（IBKR 的资产），<b>不是</b> IBKR 自己的杠杆，'
     '也<b>不是</b>当月新放款。同为月末存量，没有日历效应。'),

    ('客户权益',
     '官方行名 Client Equity：<b>月末</b>客户账户权益合计，<b>不含</b>非客户余额。'
     '它是本页那两条占比（客户现金／权益、融资余额／权益）<b>共同的分母</b> —— '
     '分母取自同一列，所以两条占比同口径、可以直接比较。'
     '⚠️ 那两条占比<b>是本页轧出来的比值，不是公司披露的一列</b>。'
     '它也是 NII 的规模基数。'),

    ('期货',
     '本页凡是「期货」的张数与佣金，按公司口径<b>都含期货期权</b>。'
     '⇒ 页尾核对表「期货 千张」那一列、以及按产品反推 DARTs 的期货那一段，'
     '数的都是期货<b>与</b>期货期权的合计。'),
]
...
def main():
    ...
    ex = []
    # 左端裁决登记簿：{图的 id: 为什么它的左端比主窗口晚}。**在建图现场登记，
    # 理由字符串与交给 mrwin.Leg 的那一份是同一个变量** —— 页尾窗口说明照它现算，
    # 不再手抄一份图号名单（手抄的那份改一次图号就成假话）。
    # 建完之后有一道硬校验：实际左端晚了却没登记理由的，直接让构建失败。
    LATE_WHY = {}

    # 图注里要指**另一张图**时不写死图号：写 ⟨ex:id⟩，写盘时由 build/exhibits.py 兑成号，
    # 兑不出来就构建失败。本页原先自带一套 ⟨nav:…⟩ + NAV 回填（2026-08 的合并里各图前移过
    # 一号，手写的「见 Exhibit 16」当时全靠人肉跟着改），它回填的只有图号，2026-09 并进了全站那一套。
    ...
    # 「见 Exhibit 16」栽过一次，本页当年那套 ⟨nav:…⟩ 占位符就是那次的产物）。
    #
    ...
    ex.append({
        'id': 'net-new', 'kind': 'gs_bar', 'fmt': 'f1', 'xlabels': XL, 'xstep': 12,
    ...
    COST_NOTE[_id_nn] = mom_cost_zh(nn_real, ALL, WIN)
    ex[-1]['note'] += COST_NOTE[_id_nn]
    # 柱与线不同源，页尾那句「拿柱直接除就是线上这一点」在本图不成立 —— 登记在这里，
    # 页尾从登记簿现算，并由下面那道护栏用 payload 自己复核（多一张少一张都停机）。
    SPLIT_SRC[_id_nn] = ('柱是历史指标表内<b>披露</b>的 Net New Accounts（账户存量差分），'
    ...
    ex.append({
        'id': 'cleared-darts', 'kind': 'gs_bar', 'fmt': 'f0c', 'xlabels': XL4, 'xstep': 12,
        ...
        'note': 'We calculate cleared DARTs = Cleared avg. DART per account (annualized) / 252 trading days * '
                ...
                '差额是口径差（cleared ≠ total client），那张图与那条差额线见 Exhibit ⟨ex:total-darts⟩，'
                '不是估算误差。'
    ...
    ex[-1]['note'] += _w4.why
    LATE_WHY[ex[-1]['id']] = _lag4
    _id_cl = ex[-1]['id']
    # 逐图代价（§6.1 第 3 条）。窗口 = 本图横轴（左端被 resolve() 裁掉了首月），
    # 所以 win 走 `_w4.cut(WIN)` 而不是全历史：诊断只该量读者在图上看得到的那一段。
    COST_NOTE[_id_cl] = mom_cost_zh(cleared_all, ALL, _w4.cut(WIN), per_day=True)
    ex[-1]['note'] += COST_NOTE[_id_cl]
    ...
    ex.append({
        ...
        'id': 'product-darts', 'kind': 'gs_bar', 'fmt': 'f0c', 'xlabels': PXL, 'xstep': 12,
        'bar_labels': False,
        ...
        'note': 'Product DARTs estimated as monthly volume / average order size / US trading days. '
                f'<b>这三段是拿新闻稿的 Average Order Size 独立反推出来的，不是 Exhibit ⟨ex:{_id_cl}⟩ '
                ...
                '「披露总量里那道缺口不是推导误差」最硬的一条证据，写在 Exhibit ⟨ex:total-darts⟩ 的图注里。'
    ...
    _id_pd = ex[-1]['id']
    RHS_NOT_YOY.add(_id_pd)          # 本图右轴是占比，不是同比（见 RHS_NOT_YOY 的说明）
    ...
    ex.append({
        'id': 'total-darts', 'kind': 'bar_line_dual', 'x': 'long', 'xlabels': XL, 'xstep': 12,
        ...
        'note': ('<b>右轴那条绿线：公司披露的交易笔数里，本页那根推导出来的 cleared DARTs '
                 ...
                 f'线的分子来自 Exhibit ⟨ex:{_id_cl}⟩ 那根<b>推导值</b>'
    ...
    ex.append({
        'id': 'darts-per-account', 'kind': 'lines', 'fmt': 'f0', 'xlabels': XL, 'xstep': 12, 'zero_base': True,
        'end_label': True,
    ...
    ex.append({
        ...
        'id': 'commission-day', 'kind': 'gs_bar', 'fmt': 'f1', 'yfmt': 'f1', 'xlabels': PXL, 'xstep': 12,
    ...
    COST_NOTE[_id_cd] = mom_cost_zh(comm_day, PWIN, PWIN, per_day=True)
    ex[-1]['note'] += COST_NOTE[_id_cd]
    ...
    ex.append({
        ...
        'id': 'cpt', 'kind': 'lines', 'fmt': 'usd2', 'xlabels': PXL, 'xstep': 12,
    ...
    ex.append({
        ...
        'id': 'cpt-product', 'kind': 'lines', 'fmt': 'usd2', 'xlabels': PXL, 'xstep': 12,
        'zero_base': True, 'end_label': True,
        ...
        'note': f'纵轴从 0 起。三条线与 Exhibit ⟨ex:{_id_cpt}⟩ 的总额来自同一张 Key products 表。'
    ...
    _PR_NS = [_id_cd, _id_cpt, _id_pd, _id_pcpt]
    ...
    ex.append({
        'id': 'margin', 'kind': 'gs_bar', 'fmt': 'f1', 'xlabels': XL, 'xstep': 12,
        ...
        'note': f'公司披露的期末余额，{XL[0]} 起逐月；次轴金色折线是它的<b>单月同比</b>'
                ...
                f'{np.nanmax(mg_share):.1f}%，见 Exhibit ⟨ex:cash-margin-share⟩。',
    })
    _id_mg = ex[-1]['id']
    ...
    STOCK_YOY[_id_mg] = '融资余额是月末存量'

    ex.append({
        'id': 'cash', 'kind': 'gs_bar', 'fmt': 'f1', 'xlabels': XL, 'xstep': 12,
    ...
    ex.append({
        'id': 'cash-margin-share', 'kind': 'lines', 'x': 'long', 'xlabels': XL, 'xstep': 12, 'fmt': 'pct1',
        'zero_base': True, 'end_label': True,
        'title': _t16,
        ...
        'note': '<b>这是比值，不是公司披露值</b>：分子分母都取自官方历史指标表同一张表内的 Client Credits / '
                ...
                f'两条绝对额（Exhibit ⟨ex:{_id_mg}⟩ / ⟨ex:{_id_cr}⟩）与这里的占比是两件事：'
    ...
    ex.append({
        'id': 'equity', 'kind': 'lines', 'x': 'long', 'xlabels': XL, 'xstep': 12, 'fmt': 'f0c',
        'zero_base': True, 'end_label': True,
        ...
        'note': '公司披露值（期末口径，不含非客户余额）。'
                f'它是 Exhibit ⟨ex:{_id_mg}⟩ / ⟨ex:{_id_cr}⟩ 两条余额的分母，也是 NII 的规模基数。',
        'series': [{'name': 'Client Equity', 'color': 'NAVY', 'values': LN(eq_all)}],
    })

    # ── 图号：按页头 ORDER 就地排序、写上 n（build/exhibits.py）。ex.append 的先后不必
    #    等于图序；这之后 ex 就是页面顺序，下面的名单都按它走。写盘时 write_dash 按同一张
    #    表再核一遍，核对表的号也由它接在最后一张之后。
    exhibits.number(ex, ORDER, 'ibkr')
    _num = {e['id']: e['n'] for e in ex}

    # ── 页尾脚注要点名的几批图：**一律从建完的 `ex` 现算**，不在文案里手抄编号 ──
    # 这些名单从前是写死的图号串。写死的名单已经被撞过两次（2026-08 的合并、2026-09 的
    # 删并挪），任何一处漏改都会指到别的图上，而没有东西报错。现在名单里装的是 id，
    # 印出来的是 ⟨ex:id⟩ 占位符，写盘时兑成号。
    def _exlist(ids):
        ids = sorted(ids, key=_num.get)       # 登记簿按建图先后登记，印的时候按页面顺序
        return ('Exhibit ' + ' / '.join(f'⟨ex:{i}⟩' for i in ids)) if ids else ''

    # 排序不是可有可无：`_PR_NS` 按**建图先后**登记，而 2026-09 的重排把分产品 DARTs
    # 那张挪到了前面 —— 不排序就会印出「Exhibits 7／8／4／9」这种乱跳的图号。
    # 名单本身仍在建图现场登记，这里只按页面顺序取用；连号才印成区间（此刻的号现判，
    # 挪图之后不连号了就自动改印成逐个列出）。
    _pr_ns = sorted(_PR_NS, key=_num.get)   # 画在新闻稿口径上的那几张（建图现场登记）
    _pr_nums = [_num[i] for i in _pr_ns]
    _pr_span = (f'⟨ex:{_pr_ns[0]}⟩-⟨ex:{_pr_ns[-1]}⟩'
                if _pr_nums == list(range(_pr_nums[0], _pr_nums[-1] + 1))
                else '／'.join(f'⟨ex:{i}⟩' for i in _pr_ns))
    _xl_of = {e['id']: (e.get('xlabels') or XL) for e in ex}
    ...
    _ontime_ns = [e['id'] for e in ex if _xl_of[e['id']] == XL]
    _late_ns = [e['id'] for e in ex if _xl_of[e['id']][0] != XL[0]]
    _odd = [e['id'] for e in ex if e['id'] not in _ontime_ns and e['id'] not in _late_ns]
    if _odd:
    ...
    _gsbar_ns = [e['id'] for e in ex if e['kind'] == 'gs_bar']
    _zb_ns = [e['id'] for e in ex if e.get('zero_base')]
    ...
    _cap_ns = [e['id'] for e in ex if e.get('ycap') is not None or e.get('yfloor') is not None
               or (e.get('yoy') or {}).get('ymax') is not None]
    _nyears = len({k[:4] for k in ALL})
    _exn = {e['id']: e for e in ex}
    ...
    # 那句 `note += _w4.why` 什么也没加。Ex3/11/13 当时虽有 mrwin 那一段，但单腿时它的
    # `who` 退化成一个日期，印出来是「定住左端的是 12/17」这种同义反复 —— 登记在
    # LATE_WHY 里的真正理由一个字都没进图注。mrwin.py 是全站共用文件、那一轮不许改，
    # 所以改法留在本页：把登记的那句话 append 进对应图注，然后**逐张核对，对不上就停机**。
    # 页尾那句话从此是被守住的，不是被相信的。
    # （2026-09-14 mrwin 改成点名定住左端的那条序列，同义反复没了；但它只知道 Leg 的名字
    #  与 lag_zh，**数据上**为什么那几期不存在仍只有 LATE_WHY 知道，所以这一步照留。
    #  本页眼下只有 `_w4` 一处调 `resolve()`、还是 gs_bar，下面 `_said` 那一支暂时跑不到。）
    for _n in _late_ns:
        _e = _exn[_n]
    ...
    _yoy_ns = [e['id'] for e in ex if e.get('yoy') and e['id'] not in RHS_NOT_YOY]
    _MOM_DECL_FIELDS = ('title', 'ylab2', 'legend')
    _undeclared = []
    for _e in ex:
        if not _e.get('yoy') or _e['id'] in RHS_NOT_YOY:
            continue
    ...
    def _roll_scope(e, f):
        t = str(e.get(f) or '')
        c = COST_NOTE.get(e['id'])
        return t.replace(c, '') if (f == 'note' and c) else t

    _roll_left = sorted({e['id'] for e in ex for f in ('title', 'note', 'ylab', 'ylab2', 'legend')
    ...
    for _e in ex:
        ...
        if not _ok:
            _uncheckable.append(_e['id'])
        elif _bad:
            _split_found[_e['id']] = (_ok, _bad)
    if _uncheckable:
        raise SystemExit(
    ...
    _split_txt = ''.join(
        f'<b>Exhibit ⟨ex:{n}⟩ 是例外</b>（构建期实测：{_split_found[n][0]} 个可核月里 '
        f'{_split_found[n][1]} 个月柱除柱与线对不上，全是口径调整月）：{why} —— '
        f'所以那一条线拿柱直接除是除不出来的，该图图注里把调整月逐月点了名。'
        for n, why in sorted(SPLIT_SRC.items(), key=lambda t: _num[t[0]]))
    ...
    for i, w in enumerate(TWIN):
        ...
        trows.append(r)
    # 图号自查（2..N 连号、核对表接在最后一张之后）由 exhibits.number 与 write_dash 负责。
    ...
    # 同时补上人均年化 DART：抬头列的这六个读数里只有它是结构性下行（Exhibit ⟨ex:{_id_ann}⟩
    ...
    payload = {
        ...
        'subtitle': (f'{month_name} update — Exhibits ⟨ex:{ex[0]["id"]}⟩–⟨ex:{ex[-1]["id"]}⟩, '
        ...
        # 上面那一段注释里；版式与四道护栏在 build/glossary.py（本页只交二元组表）。
        'glossary': gloss.render(GLOSSARY, where='ibkr glossary'),
        'hub_line': hub,
        'source': SRC,
        'xlabels': XL,
        ...
        'notes': [
            ...
            '便于与披露逐条核对。',
            f'<strong>Cleared DARTs</strong>（Exhibit ⟨ex:{_id_cl}⟩）= Cleared avg. DART per account（年化）÷ 252 '
            '个交易日 × 期初与期末账户总数的平均值。这是券商研究里的标准还原口径，'
            '<strong>非公司直接披露值</strong>，故图标题一律以 Implied 打头。'
            f'它与公司披露的 Total Client DARTs 之间的差额画在 Exhibit ⟨ex:{_id_td}⟩ 的右轴上。',
            f'<strong>Commission revenue/day</strong>（Exhibit ⟨ex:{_id_cd}⟩）= cleared DARTs（千笔/日）× '
            '单笔清算订单平均佣金（$/笔）<strong>÷ 1,000</strong> → 百万美元/日（$mn/day）。'
            '要得到月度总额还需再乘当月官方交易日数。',
            f'<strong>Product DARTs</strong>（Exhibit ⟨ex:{_id_pd}⟩）= 当月成交量 ÷ 平均订单规模 ÷ 美股交易日数。'
            ...
            f'2024-11 Total Accounts 下调 9.1k）。Exhibit ⟨ex:{_id_nn}⟩ 的<b>柱</b>画表内披露值，'
    ...
    # 图号占位符 ⟨ex:…⟩ 由 write_dash 统一兑现，兑不出来就构建失败（build/exhibits.py）。
