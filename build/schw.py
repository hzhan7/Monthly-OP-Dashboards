...
数据源：series/schw.csv（Schwab Monthly Activity Report，次月第 10 个美股交易日，12-17 日）、
        ...
        series/fee_rates.csv（季报口径的 NIM；生息资产已改走 series/schw.csv 的月频腿）。
...
import exhibits       # 图号：顺序表编号 + 正文 ⟨ex:…⟩ 占位符（build/exhibits.py）
...
# 正文里指本页的图一律写 ⟨ex:id⟩，挪完图不用再改任何一处「Exhibit N」。
ORDER = [
    'nna',              # Core net new assets（月度柱）
    'nna-q',            # Core net new assets by quarter
    'organic',          # Annualised organic growth rate
    'bridge',           # What moved client assets: flows vs. markets
    'assets',           # Total client assets
    'accounts',         # New brokerage accounts opened
    'dats',             # Daily average trades（月度）
    'margin',           # Month-end margin balances
    'dats-q',           # Daily average trades by quarter（季报附表口径）
    'margin-q',         # Quarter-end margin balances（季报附表口径）
    'iea-share',        # Why there is no revenue bridge here（生息资产 / 客户资产）
    'nna-years',        # Core NNA path by year
    'accounts-years',   # New accounts path by year
    'dats-years',       # Daily average trades path by year
    'organic-heat',     # Annualised organic growth rate — 单月年化（热力矩阵）
    'nim',              # Net interest margin（季报口径）
]
...
def assert_quarterly(index, src):
    """季度必须逐季连号 —— 与 assert_monthly 同一条规矩，换成季频的三个理由：
    (1) 不相邻的两季会被画成相邻柱（CONTRACT §5.3）—— 季度柱图上一根柱就是一个季度，
        中间缺一季，读者看到的是一条从未存在过的连续线；
    (2) 断点索引 brk_idx() 按**位置**返回，缺季之后红色竖虚线会落在错的那根柱上；
    (3) 同比按 `p - 4` 取分母（按期号而不是按位置），缺季时它会静默变成 None ——
        图上是个断口，看不出是「缺数据」还是「小基数护栏放弃了」。
    """
    idx = list(index)
    gaps = [(str(idx[i - 1]), str(idx[i])) for i in range(1, len(idx))
            if (idx[i] - idx[i - 1]).n != 1]
    if gaps:
        raise SystemExit(f'{src} 季度不连续: {gaps}')
...
# in 2025; $10 billion in prior periods」——2025-01 起生效，且官方**没有**按新门槛回填
# 前期，所以断点左右两侧确实不可比。凡是画 core NNA 或其派生量（年化有机增速）的图都要带上。
# ⚠️ 措辞只能说**这一列**：这里 2026-09-16 之前写的是「月报不重述历史」，那是一句
# 被证伪的全称命题 —— 平均生息资产在 2025-12 那次口径变更里就被逐月改写过（见
# fetch/schw.py 口径坑第 5 条）。重述与否是逐列的，core NNA 这一列确实没回填。
...
# October 6, 2020."），并把 TDA 当作 2020Q4 的流入处理（脚注：$890.7bn 资产、
# 14.5mn 新开户）。推断的依据是反差：同一份稿里真做了重述的三处都写着
# "Prior periods have been reclassified to reflect this change."，TDA 这两条没有。
# 所以图注里不许把这句挂到官方名下（q_tda_note() 已按这条写）。
# 断点落在**并表当季 2020Q4**（不是 2021Q1）—— 那一季本身就已经含了 TDA 的近三个月。
# 跳幅见季报附表（本页 series/schw_q.csv 现读）：季度 DATs 2020Q3 → 2020Q4 翻了近 4 倍，
# 季末融资余额同期翻了 2.5 倍以上，两者都不是业务变化。
# ⚠ 本页**月度**图用不到它：月报的 DATs 与月末融资余额只回填到 2025-01，
# 整条月度序列都在这条线的右边，画上去反而是一句假话（brk_idx() 返回 None 即不画）。
BRK_Q_TDA = pd.Period('2020Q4', 'Q')
# 竖排标签同样要短（理由见上面 BRK_LABEL）：口径全称留图注，线上只留「谁并了谁」。
BRK_TDA_LABEL = 'TD Ameritrade 并表'
...
#   q_dats_k          = 附表 Clients' Daily Average Trades 的 **Total** 行（千笔/日）
#   q_margin_eop_usdbn = 附表 Growth in Client Assets and Accounts 里的
#                        **Margin loans outstanding** 行（$bn，季末；原表印成负数，
#                        因为它在那张表里是客户资产的减项 —— 落盘时已取绝对值）
# 每份申报给 5 个季度、相邻申报重叠 4 季，重叠值逐值核过才落的盘。
qdf = pd.read_csv(os.path.join(SERIES, 'schw_q.csv'))
qdf['quarter'] = pd.PeriodIndex(qdf['quarter'], freq='Q')
qdf = qdf.set_index('quarter').sort_index()
for c in qdf.columns:
    qdf[c] = pd.to_numeric(qdf[c], errors='coerce')
QNEED = ['q_dats_k', 'q_margin_eop_usdbn']
_qmiss = [c for c in QNEED if c not in qdf.columns]
if _qmiss:                                    # 规矩 5：失败要响，不静默写 NaN 上线
    raise SystemExit(f'series/schw_q.csv 缺列: {_qmiss}')
_qnan = [c for c in QNEED if qdf[c].isna().any()]
if _qnan:                                     # 季频腿两条线都不容忍空点（柱与右轴都要连号）
    raise SystemExit(f'series/schw_q.csv 有空值: {_qnan}')
assert_quarterly(qdf.index, 'series/schw_q.csv')
qdats = qdf['q_dats_k'].astype(float)
qmgn = qdf['q_margin_eop_usdbn'].astype(float)

# ── 月末融资余额与 long/short 策略：这一行的脚注会骗人，先把它钉死 ──
# 月报脚注 (4) 自 2026-04 那期起**首次**出现，原话是
#   "Balances include margin loans and short credits related to certain long/short strategies
#    from which the Company earns a fixed net yield."
# ⚠️ 这条脚注是 `Margin Balances at month end (4)` 与 `Transactional Sweep Cash (4,7)`
# **两行共用**的，两个半句各归各行：margin loans 归融资余额行，short credits 归 sweep cash
# 行 —— 脚注 (7) 明写 "Transactional sweep cash includes ... short credits related to
# certain client long/short strategies"。只读脚注 (4) 的字面、把 short credits 也算进
# 融资余额，是本页 2026-09-16 之前一直在犯的错（原话「口径含 short credits」）。
# 2026Q2 可以逐项对上：季末 165.1 − 与 long/short 相关的 margin loans 42.1 = 123.0，
# 与同期资产负债表 `Receivables from brokerage clients — net` 的 122.8 吻合（差 0.2 是
# 四舍五入与其他应收）；若再减 short credits 43.7 得 79.3，差了一个数量级。
# 所以本行 —— 以及季报附表那行 `Margin loans outstanding` —— **只含 margin loans**。
# 真正该提醒读者的是「其中与 long/short 相关的那一块有多大」，占比一律**现算**
# （分母取 series/schw.csv 同月的月末余额），月份滚过去也不会变成假话。
LS_FOOTNOTE_FROM = pd.Period('2026-04', 'M')
LS_LOANS = {pd.Period('2026-08', 'M'): 53.7}   # 该月末余额里与 long/short 相关的 margin loans，$bn
LS_2025_AVG_MN = 884                           # 2026Q2 稿给的 2025 年同期季均 margin loans，$mn
LS_RECON = ('2026Q2', 165.1, 42.1, 122.8)      # (季, 季末余额, long/short margin loans, 资产负债表净应收)

# ── 为什么 SCHW 没有「量→收入」桥 ──
# NII ≈ 净息差 × 平均生息资产。这两个因子现在**一个是月频、一个还是季频**：
#   · 平均生息资产 → **月频**，series/schw.csv 的 avg_interest_earning_assets_usdbn，
#     2013-09 起逐月，取自月报附表 "Selected Balances"（老版式是 "Selected Average
#     Balances"）块里的 Average Interest-Earning Assets。
#   · 净息差 → 仍是**季频**，series/fee_rates.csv（8-K Ex-99.1，2013-Q4 起）。
#     月报里没有它，这一条是**披露边界**，不是管道边界。
#
# ⚠️ 2026-09-16 之前这里写的是「这两个因子都只有季度腿」，那句话里生息资产那一半
# **是假的**：它在月报里逐月印了至少十二年，本仓拿不到只是因为 fetch/schw.py 的 COLS
# 没写这一行，而且老版式把数值印在标签**下面**那行，按标签行取值会读到 0 个数（详见
# fetch/schw.py 口径坑第 8 条）。这是同一种病的第四次（ab70cd7 → 47710ce → 963dc76 →
# 本次），病名是「把工具的边界当成了世界的边界」。**下面三句都是假话，别往图注里写：**
#   ·「Schwab 不披露客户现金」—— fetch/schw.py 口径坑第 7 条点名禁止。
#   ·「生息资产不按月披露 / 只随季报出」—— 它按月披露，本页现在就画着月频的。
#   ·「本页的生息资产只有季度腿」—— 从本轮起不再成立。
#
# 客户现金虽然按月可得，但**客户现金不是生息资产**（货基是客户表外持仓，sweep 是
# 负债端资金来源），顶不了这个分子 —— 这一条与上面那三句无关，至今成立。
# 而生息资产 / 客户资产这个比值不是常数（见下面现算的 _MONO13 / _UP_ALL），
...
_iea = rate_series('avg_interest_earning_assets', 1e-3)      # USD_mn → $bn（季频，只留作对账）
_nim = rate_series('net_interest_margin').dropna().sort_index()

# ── 生息资产占比：月频（2026-09-16 起）──
# 分子 = 当月**平均**生息资产，分母 = 当月**月末**客户总资产。两者的时点口径不同，
# 这一点本页一直存在、并未因改成月频而新增：季频那版的分母是「该季三个月月末值的
# 简单平均」，同样是拿月末存量去凑一个平均数；官方从不按月印客户资产的日均，
# 所以这是能拿到的最接近的分母，图注里明写。
# 与被它取代的季频画法的一致性是**实测**的，不是假设：把这条月度比值按季取平均，
# 与原来那条「季报生息资产 ÷ 季度客户资产均值」逐季相比，最大差 0.050pp、中位 0.003pp
# （51 个季度）—— 也就是说改成月频不改变这张图的形状，只是把 51 个点换成了 156 个。
_iea_m = df['avg_interest_earning_assets_usdbn'].astype(float)
if _iea_m.isna().any():          # 规矩 5：缺月要响。lines_endlabels 不容忍 null
    _g = [str(p) for p in _iea_m[_iea_m.isna()].index]
    raise SystemExit(f'series/schw.csv 的平均生息资产有空月: {_g[:6]}（Exhibit 12 不容忍空点）'
                     '—— 先跑 python3 fetch/schw.py --columns')
_share = (_iea_m / assets * 100).dropna()
_bs = pd.DataFrame({'iea_share': _share.values}, index=_share.index)

# ── 净息差仍是季频，所以它离开了 Exhibit 12，单独成图（见下方 Exhibit 17）──
# lines_endlabels **不容忍 null**（docs/CHART_KINDS.md §1.2：首尾为 null 直接抛
# TypeError、中间的 null 把线画塌到 0）。占比改成月频之后，两条线的频率不再相同：
# 把季度的净息差摊到月度轴上，每季只有 1 个点、另外 2 个月是 null，正好落进
# 「中间的 null 画塌到 0」那一档，而且**不报错**。合并画的唯一办法是把季度值沿用到
# 该季三个月 —— 那是补点，本页在 fee_period_note 里明写过「不外推、不补点」。
# 所以拆图，不是版式偏好，是这两条约束的交集里只剩这一个解。
if not len(_nim):
    raise SystemExit('fee_rates.csv 里 SCHW 的净息差一个季度都没有（Exhibit 17 画不出来）')
...
# 下面几个量全部现算：写死季度号的话，下一季页面上就是一句假话
# （本文件已经为「过去 32 个季度单边降」返工过一次，同一个坑不踩第二遍）。
# ⚠️ 2026-09-16 起 fee_rates.csv 在本页**只剩净息差一个用户**（Exhibit 17）——
# 生息资产占比已经整条走月频，所以这几个量说的都是净息差，不再是「两个指标都齐」。
_FEE_USED_Q = _nim.index[-1]                    # Exhibit 17 实际画到的最后一季
_FEE_HAVE_Q = _iea.index.max()                  # 表里 SCHW 季频生息资产的最新季（对账用）
_LATEST_Q = LATEST.asfreq('Q')                  # 本页月度数据所在季
_FEE_LAG = (_LATEST_Q - _FEE_USED_Q).n          # 费率落后本页月度数据几个季度
# 有月度数据、却还没有对应季度净息差的那几个月（季度线在此处收尾，不外推）。
...
# 流量类画的是**单月同比**（CONTRACT §6.1 第 1 条）。本页一条 12 个月滚动合计的同比都不画；
# ⚠️ 「全站」有五张明文例外，见 §6.2（exchanges-apac Ex5/Ex15、exchanges12 Ex4/7/8），都不是折线。这一段以前写的是反过来的话
# —— 「为什么流量类不再画单月同比」，后面跟着一整段替滚动口径说好话 ——
# 那是 2026-08 审计定的上一版契约，2026-09 已被页面所有者推翻，整段作废重写。
#
# 单月口径的代价没有消失，只是改由**每张图自己**承担并写明（§6.1 第 3 条）：
# 单月同比的分母是**去年那一个月**，而流量的月度分布本身带季节性与时点噪音
# （缴税季、月末结算日落在周几、大额单笔转入的到账月份），分母越小，同一笔绝对变化
# 被放大得越狠 —— 这不是业务信号，是除法的性质。所以本页每张同比图的图注里都印着
# 用**这条序列自己**实测出来的那笔账（caliber_gap_note / stock_note），数字一律现算，
# 不写死；对照的那一侧只以数字出现在图注里，**页上一条线都不画**。
# ⚠ 图注里只许说代价，不许替口径辩护，也不许说「滚动口径更好但我们没用」
...
# 所以：「12 个月均值同比」在数值上是个真实的量（走 yoy.ttm_mean_yoy，文案绝不能写
# 「合计」）—— 但 2026-09 起它**不许再上图**：§6.1 第 2 条把存量的口径定死成点对点，
# 并明写 ttm_mean_yoy「页面上不再用它画线，只在图注里当对照量出现」。
# 本页正是这个用法：stock_note() 拿它算出对照数字印进图注，图上画的仍是点对点。
def roll_yoy(s):
...
def caliber_stats(mono, roll, idx):
    ...
    口径都没有）。而本页要比的另外两对恰恰不是 FLOW：
      ...
      · 年化有机增长率这条**由流量推导的比率**，对「分子分母各自滚动 12 个月再相除」
        的那条比率（`organic_growth_roll`）—— 那条不是 12 个月比率的算术平均，
        共享模块里没有它。
    ...
    这里曾经写的是 `ddof=0`，于是同一个词「逐月标准差」在站上有了两种定义。两者之比
    恒为 sqrt((n−1)/n)，与数据无关。纯流量列不走这里，走下面的 flow_stats()。
    """
...
def flow_stats(s, idx):
    """**流量列**：单月 vs 12 个月滚动合计 —— 统计量整段由 `yoy.caliber_diff()` 出。

    上一版这里是 `caliber_stats(yoy.mom_yoy(s, FLOW), roll_yoy(s), idx)`：两条序列都
    来自共享模块，但**对齐与统计量是本页自己又写了一遍**，而 CONTRACT §6.4 白纸黑字
    「`yoy.caliber_diff()` 已经先取交集再比，别自己写」。于是 n、逐月标准差、相邻月
    最大跳变、符号相反的月份现在全部取自它返回的那一个 dict —— 与
    `build/single.py::mom_cost_zh()`、`build/yoy.py::describe()` 同一个来源。

    ⚠ 顺带修掉一个真差错：caliber_diff 的相邻月跳变只量「相邻两个月**都在交集里**」
    的那些对；上面 `caliber_stats` 那个 `jump` 是对 keep 列表直接 np.diff，交集有洞时
    它会把洞两侧接起来当成一次跳变（那不是跳变，是两段序列接在一起）。
    ...
    if len(keep) < 3:
        return None
    A = yoy.mom_yoy(s, yoy.FLOW).loc[keep].astype(float)
    B = roll_yoy(s).loc[keep].astype(float)
...
def stock_note(s, idx, what):
    ...
    这段被更正过两次。第一次（2026-08-07）改掉的是「12 个月末值相加不是任何东西，
    所以存量只能点对点」——前半句对、后半句错：12 个月滚动**均值**同比
    （yoy.ttm_mean_yoy）回答「去年一整年的平均余额 vs 前年」，是个真实的量。
    第二次（2026-09）改掉的是「本图不换口径的理由不是『不能换』，是实测下来换了没有
    收益」——那句话在新契约下是假的：§6.1 第 2 条把存量的口径定死成点对点，并明写
    ttm_mean_yoy 页面上不再用它画线，**现在确实不能换**。
    所以这里的结构是：先说口径由谁定，再把「合计是假名字、均值才是那个量的正确说法」
    说清楚，最后用**本序列自己的实测数字**把代价印出来（§6.1 第 3 条）——
    那些数字是代价，不是换口径的理由。
    """
    st = caliber_stats(yoy.mom_yoy(s, yoy.STOCK), mean_yoy(s), idx)
    base = (f'次轴是<b>点对点（单月）同比</b>：{what}是<b>期末存量</b>，'
            '而存量的同比口径由契约定死成点对点，<b>页面上不画任何 12 个月滚动口径的线</b>。'
            '这里顺带把一句常见的错话纠正掉：存量并非不能平滑 —— '
            '<b>12 个月滚动均值同比</b>（去年一整年的平均余额 vs 前年；数值上等同于'
            '滚动合计比，除数约掉了）是个真实的量，'
            '<b>不能叫的只是「12 个月合计同比」</b>：12 个月末余额相加不指代任何东西。'
            '它在本页只以<b>图注里的对照数字</b>出现，一条线都不画。'
            '拿它当尺子，单月口径要付的那笔账用本序列自己实测如下：')
    if st is None:
    ...
    return (base + f'对齐到同一批月份后（{st["n"]} 个月），点对点同比的标准差 '
            ...
            '而本图要回答的正是后者。金线抖到读不动时，'
            '契约给的补救是收窄轴范围（<code>ycap</code> / <code>yfloor</code>），'
            '不是换口径。')
...
#     改回单月是要求，不是统计结论 —— 2026-09 的 CONTRACT §6 把它定成了**全站唯一**
#     的同比口径（§6.1 第 1 条），本页 2026-08-15 只是先行了一步，不是页级特例。
#     代价必须写在图注里而不是留给读者自己撞上（§6.1 第 3 条）：单月同比的分母是
...
#     两种口径的实测差距仍然现算后印进图注，且**只作为代价出现**：对照的那一侧
#     只以数字进图注，页上一条线都不画，也不许写成「滚动更好但我们没用」。
#
...
_BRK_TXT = (f'红色竖虚线 = 口径断点（{BRK}）：单一客户流入的剔除门槛自该期起从 $10bn 提到 '
            '$25bn，官方<b>没有</b>按新门槛回填前期，线左右两侧不可直读，'
            '跨断点的同比与均值同样要打折扣。')
...
def ptp_axis_note(st, unit='%', d=0):
    ...
    return (f'次轴金色折线是<b>点对点（单月）同比</b>（本月 ÷ 去年同月 − 1，'
            f'比率序列取百分点差）—— <b>柱与线取自同一列</b>，'
            f'拿这根柱与 12 根柱之前那根一比就是线上这一点，读者可以自己核对。'
            + EXC_ZH +
...
ex.append({
    'id': 'nna', 'kind': 'gs_bar', 'full': True, 'height': H_BAR,
...
ex.append({
    'id': 'nna-q', 'kind': 'qtr_bar', 'full': True, 'height': H_BAR,
    ...
    'note': ('月度核心净新增资产按季汇总（恒等式可无损累加，见 Exhibit ⟨ex:bridge⟩）。'
             '右轴是<b>季度合计同比</b>（本季 3 个月合计 ÷ 去年同季 3 个月合计），'
             '不是 Exhibit ⟨ex:nna⟩ 那条单月同比 —— 两者当期读数并排见页尾「口径说明」。'
             ...
             '本窗口实测 waste 28.6% 未超阈值，所以引擎走的是<b>对齐</b> —— 左轴量程因此被拉到'
             '负区，下方那段空白正是对齐的代价（柱全为正，引擎在零线以下<b>不画</b>左轴刻度'
             '与网格线，那一段只归右轴 y/y）；右轴那条绿色零虚线与'
             '柱的基线<b>重合</b>，图内也<b>不</b>标「左右轴零点不同高」。真正超阈值触发'
             '兜底、并标出那句话的是 Exhibit ⟨ex:organic⟩（waste 50%）。'
             '⚠ 改窗口后这段必须重算：waste 随窗口变，'
             '跑 python3 tools/align_replica.py --note data/schw.js ⟨ex:nna-q⟩ 取新数。'
...
# 比率的点对点同比走共享模块（yoy.mom_yoy(kind=RATIO) 的定义就是 v − v.shift(12)）——
# 原先手写 `og - og.shift(12)`，数值逐月相同，但那是同一个口径的第二份实现。
ST4 = caliber_stats(yoy.mom_yoy(og, yoy.RATIO), yoy.mom_yoy(ogr, yoy.RATIO), d4.index)
...
ex.append({
    'id': 'organic', 'kind': 'gs_bar', 'full': True,
...
ex.append({
    'id': 'bridge', 'kind': 'bridge_bar', 'full': True, 'height': H_BRIDGE,
...
ex.append({
    'id': 'assets', 'kind': 'gs_bar', 'full': True, 'height': H_BAR,
...
ex.append({
    'id': 'accounts', 'kind': 'gs_bar', 'full': True, 'height': H_TALL,
...
ex.append({
    'id': 'dats', 'kind': 'gs_bar', 'fmt': 'f1', 'xlabels': xl(dm, ALL_N),
    ...
    'note': ('Client DATs first appear in the Jan-2026 report; the 13-month rolling table '
             ...
             '季报附表里的<b>整季</b>日均回得到 2016，本页画在 <b>Exhibit ⟨ex:dats-q⟩</b>，'
             ...
             '<b>单月口径只要 13 期就有第一个点</b>（当月与 12 个月前那一个月），'
             '所以这条只有 19 个月的序列一进页面就画得出同比 —— 这一句以前写的是'
             '「本序列还不满 24 个月，滚动 12 个月口径本来也算不出」，'
             '那是拿上一版契约的滚动口径（12 期填窗 + 12 期比较）反推的门槛，'
             '2026-09 全站改单月之后不再适用。口径也不会随历史变长而切换 —— '
             '单月是全站的口径，§6.2 点名的五张非折线例外不在本页。'
             + YOY_NOTE),
})
...
ex.append({
    'id': 'margin', 'kind': 'gs_bar', 'fmt': 'usd0', 'xlabels': xl(mb, ALL_N),
    'title': 'Month-end margin balances',
    'ylab': '$bn', 'ylab2': '% y/y (单月)', 'legend': 'Monthly',
    'values': L(d10.values), 'yoy': _y10,
    'mom_txt': oval(mom_of(mb), suffix=' m/m'),
    'note': ('Schwab only began disclosing month-end margin balances <b>at monthly '
             'frequency</b> in the Jan-2026 report; its 13-month rolling table reaches back '
             ...
             'Exhibit ⟨ex:margin-q⟩ 图注）。⚠️ 上面那句只管<b>月报</b>：'
             '季报附表里的<b>季末</b>余额回得到 2016，本页画在 <b>Exhibit ⟨ex:margin-q⟩</b>；'
...
def fee_period_note(head='<b>费率期间。</b>'):
    """本页唯一用到 series/fee_rates.csv 的地方是 Exhibit 17（净息差）。
    这句话回答读者的三个问题：图上的费率是哪一季的、本页月度数据走到哪个月、
    两者错开时页面怎么处理。整句从数据现算 —— 季度号一律走 qlab()，不写死。

    ⚠️ 2026-09-16 之前这句话由 Exhibit 12（生息资产占比 + NIM）用，并说「两条线都走
    季度腿」。生息资产已经整条走月频，所以这里只剩净息差一条，措辞同步收窄 ——
    留着「生息资产走季度腿」那半句，就是页面上一句新鲜出炉的假话。"""
    t = (f'{head}<b>净息差只有季度披露</b>（月报里没有这一行，这是披露边界不是管道边界），'
         f'本图取至 {qlab(_FEE_USED_Q)} 的公司披露值（<code>series/fee_rates.csv</code>；'
         '每季取自该季自己那份 8-K Ex-99.1 业绩新闻稿的原报值，官方后来的重述不回填）；'
    ...
    if _FEE_TAIL:
        ...
        t += (f'{who} 这 {len(_FEE_TAIL)} 个月已有月报数字、尚无对应季度的净息差，'
              f'季度线到 {qlab(_FEE_USED_Q)} 为止 —— 本页<b>不</b>把上一季的费率'
              '沿用到这几个月，也不外推、不补点。')
    else:
        t += '月度序列与季度线收在同一个季末，本图没有跨季沿用。'
    if _FEE_STALE:
        t += (f'<b>⚠ 费率已过期：</b>{qlab(_FEE_USED_Q + 1)} 起的净息差尚未进表，'
              f'已落后本页月度数据 {_FEE_LAG} 个季度（正常节奏是 0–1 季），'
              f'本图仍只到 {qlab(_FEE_USED_Q)}，请勿把它当作最新一季的水平读。')
    return t


# ── Exhibit 12：为什么这里没有「量 → 收入」桥（**月频**，全历史）──
# 2026-09-16：这张图从季度腿改成月度腿。分子由「季报的季均生息资产」换成
# series/schw.csv 的 avg_interest_earning_assets_usdbn（月报逐月印的那一行），
# 分母由「该季三个月月末客户资产的平均」换成**当月**月末客户资产，点数 51 → 156。
# 净息差因此离开本图（频率不同，lines_endlabels 不容忍 null），单独成 Exhibit 17。
# lines_endlabels **不容忍 null**（docs/CHART_KINDS.md §1.2：首尾为 null 直接抛
# TypeError、中间的 null 把线画塌到 0）——现在只剩一条线，且两列在 2013-09…最新月
# 逐月都有值，上面那道 _iea_m.isna() 的硬拦保证了这一点。
bs = _bs
_r0, _r1 = float(bs['iea_share'].iloc[0]), float(bs['iea_share'].iloc[-1])
# 与被取代的季频画法的偏差 —— 现算，不写死：把月度比值按季取平均，与「季报生息资产 ÷
# 季度客户资产均值」逐季比。这句话是这次换腿唯一的正当性证明，写死就成了不可复核的断言。
_Q_OLD = (_iea / assets.groupby(assets.index.asfreq('Q')).mean().reindex(_iea.index) * 100)
_Q_NEW = _share.groupby(_share.index.asfreq('Q')).mean()
_QD = (_Q_NEW.reindex(_Q_OLD.index) - _Q_OLD).dropna().abs()
ex.append({
    'id': 'iea-share', 'kind': 'lines_endlabels', 'fmt': 'pct1',
    # 156 个月标签必须抽稀：lines_endlabels 走的是与柱图同一套 x 轴标签循环
    # （charts.js `if (i % step) continue`），**不给 xstep 就逐个都画**，相邻月份名
    # 直接压字。改成月频那一轮这里漏了，visual_qa 当场报了 184 条 🔴（Ex12 一张图
    # 在两个视口上各 92 条）—— 同页 Exhibit 6 / 7 同样是 156 个月，早就在用 xstep_for。
    'xlabels': [mlab(p) for p in bs.index], 'xstep': xstep_for(len(bs)),
    'title': f'Why there is no revenue bridge here — {mlab(bs.index[0])} 起',
    'ylab': '%',
    'series': [
        {'name': 'Interest-earning assets / client assets', 'color': 'NAVY',
         'values': L(bs['iea_share'].values)},
    ],
    'note': ('Average interest-earning assets are published <b>monthly</b>, in the '
             '"Selected Balances" block of the Monthly Activity Report; this line '
             'divides that month\'s figure by the same month\'s month-end client assets. '
             f'It moved from {_r0:.1f}% to {_r1:.1f}% over {len(bs) - 1} months — treating '
             'it as a constant would be false precision, which is why this page carries no '
             'volume-to-revenue bridge.'
             f'<b>本图 2026-09-16 从季度腿换成月度腿</b>：分子改用月报逐月印的那一行'
             f'（<code>series/schw.csv</code>，{len(bs)} 个月），此前用的是季报的季均值，'
             f'点数从 {len(_Q_OLD.dropna())} 个季度变成 {len(bs)} 个月。'
             '⚠️ 换腿<b>不改变这张图的形状</b>，只提高分辨率：把月度比值按季取平均，'
             f'与原来那条季度线逐季相比，最大差 {_QD.max():.3f}pp、中位 {_QD.median():.3f}pp'
             f'（{len(_QD)} 个季度，现算）。'
             '⚠️ <b>分子分母的时点口径不同</b>：分子是当月<b>平均</b>余额、分母是'
             '<b>月末</b>时点余额。公司不按月印客户资产的日均，这是能拿到的最接近的分母；'
             '被取代的那条季度线同样是拿月末值凑平均（三个月末的简单平均），'
             '所以这不是换腿新增的问题。'
             f'⚠️ <b>口径变更：</b>官方自 2025-12 起把与客户 long/short 策略相关的'
             '客户融资贷款与 short credits 剔出平均生息资产，并<b>追溯调整了前期</b>'
             '（月报脚注原文 "Prior period amounts have been adjusted accordingly."）。'
             '本页 2025-01 起各月取的都是调整<b>后</b>的口径（最大改动 2025-11，−7.2$bn）；'
             '2024-12 及更早未被改动 —— 唯一能横跨这次变更的重叠月 2024-12 在新旧两版'
             '申报里逐字相同（431.2$bn），所以本图<b>不画断点线</b>。'
             '这条依据只有那一个点：13 个月滚动表的窗口决定了更早的月份不可能被重新印出来。'
             ...
             'PDF 版此处保留 2 位小数，网页图表引擎的格式器只到 1 位小数，'
             '切到「表格」视图可读到 2 位。本图不做窗口截取，画的是全部可得月份。'
             '另一个因子净息差仍只有季度披露，画在 <b>Exhibit ⟨ex:nim⟩</b>。'),
})
...
ex.append({
    'id': 'nna-years', 'kind': 'year_lines', 'fmt': 'usd0', 'xlabels': MONTHS,
...
ex.append({
    'id': 'accounts-years', 'kind': 'year_lines', 'fmt': 'f0c', 'xlabels': MONTHS,
    'title': 'New accounts path by year',
    'ylab': 'k accounts', 'series': y13, 'highlight': len(y13) - 1,
    'note': ('画的是<b>净除并购搬账之后</b>的开户量，与 Exhibit ⟨ex:accounts⟩ 的柱同一条序列：'
...
ex.append({
    ...
    'id': 'dats-years', 'kind': 'year_lines', 'fmt': 'f1', 'label_fmt': 'f1', 'xlabels': MONTHS,
    'title': 'Daily average trades path by year',
    'ylab': 'mn trades / day', 'series': y14, 'highlight': len(y14) - 1,
    'note': ('版式同 Exhibit ⟨ex:accounts-years⟩：每年一条线叠在 Jan–Dec 轴上，当年红色加粗，'
             ...
             '逐年对照图按定义要的是月点，接不进来 —— 见 Exhibit ⟨ex:dats-q⟩。）'
...
for y in hyrs:
    ...
    matrix.append(row)
ex.append({
    'id': 'organic-heat', 'kind': 'heat_matrix', 'full': True, 'fmt': 'pct1',
    ...
    'note': ('Green = faster organic asset gathering。色标取全部有限值的 5/95 分位，'
             ...
             '本图通栏（横跨两列），但仍按图号排在 Exhibit ⟨prev⟩ 之后，图序即阅读顺序。'
             '格内数字带 % 号，PDF 版是裸数字。'),
})
...
# 用 TDA 断点（而非 core NNA 门槛断点）的图 —— 现填 id，供页尾的 _BRK_DRAWN 区分两种断点。
# 不区分的话，页尾那句「Exhibit … 的红色竖虚线 = $10bn → $25bn 门槛」会把这两张一起认领。
_TDA_BRK_IDS = []


def q_ptp_yoy(s, brk=BRK_Q_TDA):
    """季频的点对点同比（本季 ÷ 去年同季 − 1，%），返回 (values, 被作废的季度)。

    ⚠ 与 Exhibit 3 那条绿线**不是同一个口径**：那条是「季度合计同比」（分子分母各是
    3 个月流量的**合计**），只对可加的流量说得通。本图两条序列都不可加 —— DATs 是
    **速率**（笔/日），季末融资余额是**存量**（时点数）—— 12 个月末值相加、3 个日均相加
    都不指代任何东西。所以这里直接本季比去年同季，那本身就是 CONTRACT §6 的点对点口径。

    跨 TDA 并表断点的那几季一律**作废**：分母那一季还是并表前的 Schwab，算出来的
    +600% 说的是「多了一家 TD Ameritrade」，不是交易活跃度涨了 6 倍 —— 与 YOY_BASE_MIN
    挡掉小基数月是同一条规矩（不画假点）。留着它还有第二笔账：右轴量程会被那四季
    撑到 600%+，其余三十几季全被压成一条平线，这条线就等于没画。

    ⚠ 但本图**故意不套** YOY_BASE_MIN 那条护栏本身。它的判据是「去年同季的基数小于
    本序列绝对值中位数的 25%」，那是给 core NNA 那种**结构性极小月**设计的
    （缴税季净流入几乎归零，算出来的 +569% 说的是分母有多小）。本图两条序列在十年里
    各长了近 20 倍 / 10 倍，2016–2019 的基数确实只有今天中位数的零头 —— 但那不是
    「基数异常」，那就是当年的真实水平，−5% / +8% 的同比完全说得通。
    照搬护栏的后果是实测过的：DATs 会把 2017Q1–2020Q3 **整段 23 季**的同比全部作废、
    margin 作废 9 季，右轴就只剩最近几年，长历史图的右轴等于白画。
    """
    out, void = [], []
    for p in s.index:
        prev = p - 4
        if prev not in s.index:
            out.append(None)
        elif prev < brk <= p:                 # 分子分母不是同一家公司
            out.append(None)
            void.append(str(p))
        else:
            a, b = float(s.loc[p]), float(s.loc[prev])
            # 两条序列都恒为正，下面这两条（分母为 0 / 分子分母异号）实际不会触发；
            # 仍然显式挡一下 —— 别把「恒为正」写成一个不写出来的假设。
            out.append(None if b == 0 or a * b < 0 else (a / b - 1) * 100)
    return L(out), void


def q_gap_note(vals, void):
    """右轴上哪些季度没有点、分别为什么 —— 图上是断口，必须交代（同 ptp_gap_note）。"""
    n_have = sum(1 for v in vals if v is not None)
    n_head = sum(1 for p in qdf.index if (p - 4) not in qdf.index)
    t = (f'右轴这条线有 {n_have} 个点：起头 {n_head} 季（{_QX[0]}–{_QX[n_head - 1]}）'
         '没有去年同季做分母')
    if void:
        t += f'，{"、".join(void)} 这 {len(void)} 季<b>跨并表断点、同比整段作废</b>'
    return t + ('，其余每一季都算得出。⚠ 本图<b>没有</b>套用本页月度图那条小基数护栏'
                '（去年基数小于全序列中位数的 1/4 就不算同比）：那条护栏是给'
                '「结构性极小月」设计的，而这条序列十年里长了近一个数量级，'
                '2016–2019 的低读数是<b>当年的真实水平、不是基数异常</b> —— '
                '照搬会把前二十多季的同比无谓地全部作废，右轴只剩最近几年。')


def Q_LAG_NOTE():
    """季频腿为什么比月度腿短一截 —— 披露节奏差，不是缺数据。月份/季度一律现读。"""
    lag = (LATEST.asfreq('Q') - qdf.index[-1]).n
    t = (f'<b>这条腿止于 {qdf.index[-1]}。</b>季报比月报慢一档：本页月度数据已到 '
         f'{mlab(LATEST)}（属 {LATEST.asfreq("Q")}），而季报附表最新只到 {qdf.index[-1]}')
    if lag > 0:
        t += f'，差 {lag} 个季度'
    return t + ('。这是正常的披露节奏差，<b>不是</b>数据缺失 —— 本页不外推、不补点，'
                '也不拿月度值凑一个不完整的季度（那会砸出一个假坑，理由同 Exhibit ⟨ex:nna-q⟩）。')


def q_tda_note(s, unit):
    """并表断点那句话 —— 跳幅从序列现读，不写死。断点滚出窗口就整句不出现。"""
    if _bkq is None:
        return ''
    a, b = float(s.loc[BRK_Q_TDA - 1]), float(s.loc[BRK_Q_TDA])
    return (f'⚠️ <b>红色竖虚线处不是业务，是多了一家公司。</b>'
            f'2020-10-06 Schwab 完成 TD Ameritrade 并表，<b>前期未重述</b> —— '
            '注意官方并没有写过「不重述」这四个字，用的是前瞻纳入式措辞'
            '（"Results of operations and metrics are inclusive of TD Ameritrade '
            'beginning October 6, 2020."），并把 TDA 当作当季<b>流入</b>处理'
            '（2020Q4 脚注：$890.7bn 资产流入、14.5mn 新开户）；'
            '同一份稿里真做了重述的三处都写着 "Prior periods have been reclassified'
            ' to reflect this change."，TDA 这两条<b>刻意没有这句</b>。所以 '
            f'{BRK_Q_TDA - 1} 的 {unit(a)} 与 {BRK_Q_TDA} 的 {unit(b)}'
            f'（{b / a:.1f} 倍）之间隔着的是一张资产负债表，不是客户行为。'
            f'线画在<b>并表当季 {BRK_Q_TDA}</b>（那一季本身已含 TDA 的近三个月），'
...
# 不能逐格对照。能核的是两条：季均必须落在该季三个月的区间内；三个月的简单平均与
# 季均只差各月交易日天数的权重。两条都现算 —— 写死结论，下一次回填就成了假话。
_QCHK = []
for _p in qdf.index:
    _ms = [m for m in dm.dropna().index if m.asfreq('Q') == _p]
    if len(_ms) == 3:
        _v = [float(df.loc[m, 'dats_k']) for m in _ms]
        _QCHK.append((_p, float(qdats.loc[_p]), min(_v), max(_v), sum(_v) / 3))
_QCHK_IN = [c for c in _QCHK if c[2] <= c[1] <= c[3]]
_QCHK_DEV = max((abs(c[4] - c[1]) for c in _QCHK), default=None)
if _QCHK and len(_QCHK_IN) != len(_QCHK):     # 规矩 5：对不上就响，别在图注里写一句假话
    _bad = [str(c[0]) for c in _QCHK if c not in _QCHK_IN]
    raise SystemExit(f'季度 DATs 与月度 DATs 对不上（季均不在该季三个月区间内）: {_bad}')
ex.append({
    'id': 'dats-q', 'kind': 'qtr_bar', 'full': True, 'height': H_BAR,
    # yfmt 必须显式给，与 Exhibit 6（:1137 那段注释）同一个坑、同一种修法。
    ...
    # （这条 🔴 是 54462c6 新增本图时带进来的，2026-09-16 两条支线各自发现、同一种修法。）
    ...
    'values': L(qdm.values), 'partial_months': 3, 'qtr_months': 3,
    'line': {'name': 'y/y (本季 vs 去年同季, RHS)', 'color': 'GREEN',
             'values': _y15, 'yfmt': 'pct0'},
    'break_at': _bkq, 'break_label': BRK_TDA_LABEL,
    'note': (q_tda_note(qdats, lambda v: f'{v / 1000:,.2f}mn 笔/日')
             + f'<b>这是 Exhibit ⟨ex:dats⟩ 的季频版：同一个量，换了一份源。</b>'
             f'月报里这一行是 2026-01 那期<b>新增</b>的，官方只回填 13 个月，'
             f'所以 Exhibit ⟨ex:dats⟩ 就只有 {len(dm.dropna())} 个月；'
             f'而<b>季报附表</b>里它回得到 {_QX[0]}，本图画的就是这 {len(_QX)} 个季度。'
             '⚠️ 两张图<b>不能逐格对照</b>：季度值是<b>整季</b>的日均、月度值是'
             '<b>单月</b>的日均，同一段时间里它们本来就不是同一个数。'
             + (f'能核的是另外两条，都现算：重叠的 {len(_QCHK)} 个完整季里，'
                f'季均<b>全部</b>落在该季三个月的最小/最大之间；'
                f'三个月的简单平均与季均最大只差 {_QCHK_DEV:,.0f} 千笔/日'
                '（差的就是各月交易日天数的权重）。' if _QCHK else '')
             + '<b>为什么锚在 Total 行。</b>2020Q1 起官方把附表里的 Revenue trades / '
             'Asset-based trades / Other trades / Total 四行塌成单行 '
             "<code>Clients' Daily Average Trades</code>，而它给的历史比较数<b>逐字等于</b>"
             f'旧的 <b>Total</b> 行（Q4-19 两处都是 {qdats.loc[pd.Period("2019Q4", "Q")]:,.0f}），'
             '所以取 Total 为锚跨过 2020Q1 是连续的。'
             '⚠️ <b>不能改用 DART</b>（= Revenue trades）：2019-10 零佣金之后那一行从 '
             f'Q3-19 的 395 塌到 Q4-19 的 151，而 Total 只从 '
             f'{qdats.loc[pd.Period("2019Q3", "Q")]:,.0f} 走到 '
             f'{qdats.loc[pd.Period("2019Q4", "Q")]:,.0f} —— 拿 DART 当锚，'
             '2019Q4 会砸出一个业务上不存在的断崖。'
             '<b>单位。</b>官方季报印的是<b>千笔/日</b>，本图除以 1,000 换成 mn，'
             '与 Exhibit ⟨ex:dats⟩ 同尺度（页尾核对表印的仍是官方原始的千笔/日）；'
             '柱顶标签保留 2 位小数 —— 2016–2019 各季都在 0.5–0.9mn 之间，'
             '按 1 位小数印会被四舍五入成一排相同的数字，长历史的形状就读没了。'
             '<b>右轴口径。</b>季频下「本季 vs 去年同季」就是<b>点对点同比</b>'
             '（CONTRACT §6 的全站口径），<b>不是</b> Exhibit ⟨ex:nna-q⟩ 那条「季度合计同比」—— '
             'DATs 是速率（笔/日），3 个日均相加不指代任何东西，本来也求不出合计。'
             + EXC_ZH
             + q_gap_note(_y15, _void15)
             + Q_LAG_NOTE()),
})
_TDA_BRK_IDS.append(ex[-1]['id'])
...
# 写死的话，哪天官方重述或解析出错，图注会替错数背书。
_OVL = [(p, float(mb.loc[p.asfreq('M', 'end')]), float(qmgn.loc[p]))
        for p in qdf.index if p.asfreq('M', 'end') in mb.dropna().index]
_OVL_MAX = max((abs(a - b) for _, a, b in _OVL), default=None)
if _OVL_MAX is not None and _OVL_MAX > 0.5:   # 规矩 5：两条腿真的对不上就别上线
    _w = max(_OVL, key=lambda t: abs(t[1] - t[2]))
    raise SystemExit(f'季末融资余额与月末口径对不上：{_w[0]} 月末 {_w[1]} vs 季报 {_w[2]}')
_ls_p, _ls_ml = next(iter(LS_LOANS.items()))
_ls_base = float(mb.loc[_ls_p]) if _ls_p in mb.dropna().index else None
_LS_Q, _LS_EOP, _LS_DEB, _LS_BS = LS_RECON
# 重叠期的举例串（前 3 个季度）单独拼好再进 f-string —— 嵌在 f-string 里要用第二层引号，
# 那是 3.12 才合法的写法，本仓不保证解释器版本。
_OVL_EG = '、'.join(f'{p}/{mlab(p.asfreq("M", "end"))} = {money(b, 1)}bn'
                    for p, _a, b in _OVL[:3])
ex.append({
    'id': 'margin-q', 'kind': 'qtr_bar', 'full': True, 'height': H_BAR,
    'fmt': 'usd0', 'label_fmt': 'usd0',
    'xlabels': _QX,
    'title': f'Quarter-end margin balances — {_QX[0]} 至今（季报附表口径）',
    'ylab': '$bn', 'legend': 'Quarterly (季报附表)',
    'values': L(qmgn.values), 'partial_months': 3, 'qtr_months': 3,
    'line': {'name': 'y/y (本季 vs 去年同季, RHS)', 'color': 'GREEN',
             'values': _y16, 'yfmt': 'pct0'},
    'break_at': _bkq, 'break_label': BRK_TDA_LABEL,
    'note': ('⚠️ <b>先说行名与脚注这个坑。</b>季报附表里这一行印的是 '
             '<code>Margin loans outstanding</code>，<b>至今没有任何脚注</b>；'
             f'月报那边要到 {mlab(LS_FOOTNOTE_FROM)} 那期才第一次挂上脚注 (4)'
             '（"Balances include margin loans and short credits related to certain '
             'long/short strategies"）。<b>这条脚注是两行共用的</b> —— '
             '<code>Margin Balances at month end (4)</code> 与 '
             '<code>Transactional Sweep Cash (4,7)</code>，两个半句各归各行：'
             'margin loans 归本行，short credits（客户卖空所得留在券商处的<b>贷方</b>'
             '余额，负债端）归 sweep cash 那行 —— 脚注 (7) 明写 sweep cash '
             '"includes ... short credits related to certain client long/short strategies"。'
             f'<b>所以本行只含 margin loans。</b>{_LS_Q} 可以逐项对上：季末 '
             f'{money(_LS_EOP, 1)}bn 减去与 long/short 相关的 margin loans '
             f'{money(_LS_DEB, 1)}bn = {money(_LS_EOP - _LS_DEB, 1)}bn，与同期资产负债表 '
             f'<code>Receivables from brokerage clients — net</code> 的 '
             f'{money(_LS_BS, 1)}bn 吻合；若按脚注字面再减一次 short credits，'
             '得数会差一个数量级。'
             '原表还把这一行印成<b>负数</b>（它在「客户资产与账户增长」表里是减项），'
             '本页取绝对值。'
             + q_tda_note(qmgn, lambda v: money(v, 1) + 'bn')
             + f'<b>这是 Exhibit ⟨ex:margin⟩ 的季频版，而且是同一条序列。</b>'
             f'季末就是该季最后一个月的月末，两边是同一个时点的同一个存量：'
             f'重叠的 {len(_OVL)} 个季度'
             + (f'<b>逐值相等</b>（{_OVL_EG}…，最大绝对差 {_OVL_MAX:,.1f}）。'
                if _OVL_MAX is not None and _OVL_MAX == 0 else
                f'最大绝对差 {_OVL_MAX:,.1f}bn。' if _OVL else '暂无重叠季可核。')
             + f'所以这两张图可以接起来读 —— 本页唯一<b>接不得</b>的是 '
             f'{mlab(avgm.index[0])}–{mlab(avgm.index[-1])} 那条<b>月度平均</b>口径'
             '（官方已停发，见 Exhibit ⟨ex:margin⟩ 的图注），它与月末/季末都不是一个量。'
             + (f'⚠️ <b>2026 起余额里有一块新东西在快速变大。</b>本行虽然只含 margin '
                'loans，但其中「与 long/short 策略相关」的那一块不是常态成分：'
                f'公司在 2026Q2 那份稿里给出 2025 年同期的季均读数只有 '
                f'${LS_2025_AVG_MN:,}mn，对当时 2025Q2 那 '
                f'{money(float(qmgn.loc[pd.Period("2025Q2", "Q")]), 1)}bn 的余额是零头；'
                f'而 {mlab(_ls_p)} 的月末 {money(_ls_base, 1)}bn 里它已经是 '
                f'{money(_ls_ml, 1)}bn，占 {_ls_ml / _ls_base:.0%}。'
                '这类头寸是券商赚固定净息差的撮合业务，不是普通保证金加杠杆，'
                '所以<b>末尾这几季的同比里混着它的膨胀</b>，只看方向不看大小。'
                if _ls_base else '')
             + '<b>右轴口径。</b>季末融资余额是<b>存量</b>，'
             '「本季 vs 去年同季」就是 CONTRACT §6 的点对点同比，'
             '<b>不是</b> Exhibit ⟨ex:nna-q⟩ 那条「季度合计同比」—— 12 个季末余额相加'
             '不指代任何东西（同 Exhibit ⟨ex:margin⟩ 图注里那条更正）。'
             + q_gap_note(_y16, _void16)
             + Q_LAG_NOTE()),
})
_TDA_BRK_IDS.append(ex[-1]['id'])


# ── Exhibit 17：净息差（季频，8-K Ex-99.1）──
# 它本来是 Exhibit 12 的第二条线。2026-09-16 生息资产占比改走月频之后，两条线的频率
# 不再相同，而 lines_endlabels 不容忍 null（把季度值摊到月度轴上，每季 2 个 null 会被
# 引擎当 0、把线画塌，且**不报错**），沿用到该季三个月又等于补点 —— 本页在
# fee_period_note 里明写过「不外推、不补点」。所以拆出来单独画。
# 图号取 17（追加在末尾）而不是插在 12 之后：页内有十几处图注写死了 "Exhibit 13–16"，
# 插号要连带改它们，而那种改动漏一处就是一句指错图的假话。核对表的图号是现算的
# （_ENS[-1] + 1）并有连号硬拦，所以末尾追加不需要任何人跟着改。
# 颜色用 NAVY 不用 RED：docs/CHART_KINDS.md §2 写着 RED 是断点与截轴离群值的专用色、
# **不做数据色**。它原先在 Exhibit 12 里就是 RED，那是一处既有的违规，顺手改掉。
_nim_x = [mlab(q.asfreq('M', 'end')) for q in _nim.index]
ex.append({
    # yfmt 必须比刻度步长更精确：净息差的量程让引擎选了 0.25 的步长，而缺省的 y 轴
    # 格式器只到 1 位小数，于是轴上印出 `3.3 │ 3.0 │ 2.8 │ 2.5`——**四舍五入把等距的
    # 刻度印成了不等距**，visual_qa 的 AXIS_UNEVEN 就是为这个写的。给到 2 位。
    'id': 'nim', 'kind': 'lines_endlabels', 'fmt': 'pct2', 'yfmt': 'f2',
    # x 标签沿用被取代的那张图的写法：标**季末月**（Dec-13），不标 2013-Q4。
    # 两个理由：与同页其余图的月份名同宽同形，且 qlab() 那种写法在 51 格上会压字。
    'xlabels': _nim_x, 'xstep': xstep_for(len(_nim)),
    'title': f'Net interest margin（季报口径）— {qlab(_nim.index[0])} 起',
    'ylab': '%',
    'series': [
        {'name': 'Net interest margin', 'color': 'NAVY', 'values': L(_nim.values)},
    ],
    'note': ('Net interest margin is the <b>other</b> factor in NII ≈ NIM × average '
             'interest-earning assets. Unlike the assets leg (monthly, Exhibit ⟨ex:iea-share⟩), the '
             'margin is disclosed only quarterly — that is a disclosure boundary, not a '
             'pipeline one: the Monthly Activity Report simply does not carry this line.'
             '<b>⚠️ 它的分母是平均生息资产，不是客户总资产</b>，两者不在一个量级'
             f'（{mlab(LATEST)} 分别是 {money(float(_iea_m.iloc[-1]), 1)}bn 与 '
             f'{money(float(assets.iloc[-1]), 1)}bn），把它乘在客户资产上估不出净利息收入。'
             '本页每一季取的是<b>该季自己那份</b> 8-K Ex-99.1 的原报值，'
             '官方后来的重述<b>不回填</b>，所以这条线是「当期原值」而非最终版。'
             f'与 Exhibit ⟨ex:iea-share⟩ 不能逐格对照：那张是月频 {len(bs)} 个点，这张是季频 '
             f'{len(_nim)} 个点。x 轴标的是各季<b>季末月</b>。'),
...
# 图号：按页头 ORDER 就地排序、写上 n（build/exhibits.py；从 2 起连号，核对表由
# write_dash 接在最后一张之后）。ex.append 的先后不必等于图序 —— 季频那两张要先有
# mb / dm 才能和月度对账，所以 append 在文件后段，排在哪由 ORDER 决定。
# 提前在这里编号而不是全留给 write_dash，是因为下面还有两处要按**最终图序**走一遍 ex：
# 热力矩阵图注里那句「排在谁之后」，与页尾断点那两串图号（_BRK_DRAWN / _TDA_DRAWN）。
exhibits.number(ex, ORDER, 'schw')

# 热力矩阵的图注说它「仍按图号排在 Exhibit ⟨前一张⟩ 之后」—— 前一张是谁由 ORDER 决定，
# 编号之后现填，挪图后不会指着原来的邻居。
for _i, _e in enumerate(ex):
    if '⟨prev⟩' in (_e.get('note') or ''):
        if _i == 0:
            raise SystemExit(f'{_e["id"]} 的图注说它排在前一张之后，可它现在是第一张')
        _e['note'] = _e['note'].replace('⟨prev⟩', f'⟨ex:{ex[_i - 1]["id"]}⟩')
...
table = {
    ...
    'cols': [['Core NNA ($bn)', 'nna'],
             ...
             # 2026-09-16 新增。列头照抄官方行名与官方单位：2026-01 那期起月报就是按
             # $bn 印这一行的（更早按 $mn，抓取侧已归一到 $bn，见 fetch/schw.py 口径坑 8）。
             ['Avg interest-earning assets ($bn)', 'iea']],
    'rows': [{'xl': mlab(p),
              ...
              'iea': tcell(r['avg_interest_earning_assets_usdbn'], 1)}
...
def _pair_txt(name, m, r, unit='%', d=1):
    ...
    if a and b:
        # 差值跟着读数取同样的小数位。写死 .0f 会把 0.46pp 印成「差 0pp」——
        # 一句自相矛盾的话：前面刚印了 +0.10pp 与 +0.56pp，后面说它们差 0。
        return f'{name}：{a}（对照 {b}，差 {abs(m - r):,.{d}f}pp）'
...
_CAL_ROWS = [t for t in (
    _pair_txt('Core net new assets（Exhibit ⟨ex:nna⟩ 次轴）', _y_nna, _last(_R_NNA)),
    _pair_txt('New brokerage accounts（Exhibit ⟨ex:accounts⟩ 次轴，已净除并购搬账）',
              yoy_of(nba_ex), _last(_R_NBA)),
    ('Core NNA 的另一种口径：季度合计同比（Exhibit ⟨ex:nna-q⟩ 的绿线）'
     f'{_Q_NNA:+,.1f}%' if _Q_NNA is not None else ''),
    _pair_txt('年化有机增长率（Exhibit ⟨ex:organic⟩，比率取 pp 差）',
              _y_og, (lambda s: (float(s.iloc[-1]) - float(s.iloc[-13]))
                      if len(s) >= 13 else None)(ogr.dropna()), unit='pp', d=2),
    f'Total client assets（存量，Exhibit ⟨ex:assets⟩）：单月 {_y_at:+,.1f}%'
    if _y_at is not None else '',
    f'Month-end margin balances（存量，Exhibit ⟨ex:margin⟩）：单月 {_y_mb:+,.1f}%'
    if _y_mb is not None else '',
) if t]

# Exhibit 12 是不是真的单边下行、全序列有几个月上升 —— 图注里那两句都得从数据现算。
# 该图现在画的就是全序列，所以窗口内与全序列是同一批点，两个判据仍分开算：
# 窗口一旦以后又被截短，这里不用跟着改。
# 单位从「季」变成「月」了（2026-09-16 换腿）：_UP_ALL 的量级会从个位数跳到几十，
# 页尾那句话里的量词必须跟着是「个月」，否则就是拿月数冒充季数。
...
# 串的是占位符 ⟨ex:id⟩（写盘时兑成号），ex 已按 ORDER 排好，所以串出来的号自然是升序。
_BRK_DRAWN = '、'.join(f'⟨ex:{e["id"]}⟩' for e in ex
                      if e.get('break_at') is not None and e['id'] not in _TDA_BRK_IDS)
_TDA_DRAWN = '、'.join(f'⟨ex:{e["id"]}⟩' for e in ex
                      if e.get('break_at') is not None and e['id'] in _TDA_BRK_IDS)
...
summary['note'] = (
    ...
    + 'Margin balances are margin loans only; short credits sit in transactional sweep cash.  '
    + (f'A second, unrelated break — the Oct-2020 TD Ameritrade consolidation, which the '
       f'company folded in prospectively without restating prior periods — is drawn at '
       f'{BRK_Q_TDA} on Exhibits '
       f'{_TDA_DRAWN.replace("、", ", ")} (the quarterly-supplement series).  '
       if _TDA_DRAWN else '')
    ...
    '表与图对不上是当时最常见的一类读者困惑。'
    '全页只剩一处口径不同：Exhibit ⟨ex:nna-q⟩ 的<b>季度合计</b>同比（分母是去年同季 3 个月的合计）。'
    '⚠ 本页现在有三条绿色的右轴线，只有 Exhibit ⟨ex:nna-q⟩ 那条是合计口径 —— '
    'Exhibit ⟨ex:dats-q⟩ / ⟨ex:margin-q⟩ 那两条是<b>季频的点对点</b>（本季 vs 去年同季），'
...
notes = [
    ...
    '（季报附表腿，Exhibit ⟨ex:dats-q⟩ / ⟨ex:margin-q⟩）与 <code>series/fee_rates.csv</code>，'
    '无任何估算或补插。'
    f'前两者是<b>月度/季度</b>表，<code>fee_rates.csv</code> 是<b>季度</b>表（随季报更新），'
    f'本页只有 Exhibit ⟨ex:nim⟩（净息差）用它，最新一季是 {qlab(_FEE_USED_Q)}。'
    '⚠️ 2026-09-16 起<b>平均生息资产已改由月度表供数</b>（Exhibit ⟨ex:iea-share⟩），'
    f'不再走 fee_rates.csv —— 那张表里同名的季频列仍在（最新 {qlab(_FEE_HAVE_Q)}），'
    '本页只拿它与月度腿逐季对账（<code>fetch/schw.py</code> 的 '
    '<code>_crosscheck_quarter_iea</code>，按日历日加权），不进图。'
    ...
    f'把序列从 99 个月接到了 {len(df)} 个月。结果分四档。'
    '⚠️ 这里此前写的是「档与档的边界是官方的披露史，不是抓取能力」，'
    '<b>2026-09-16 已证明这句话把两件事压成了一件</b>：边界其实有两条轴 —— '
    '<b>哪一列</b>（官方披露史）与<b>什么频率</b>（月频 / 季频）。'
    '第 (3) 档的边界是前者，第 (4) 档的存在就是后者的证据：'
    '同样两个量在季报附表里一直有，只是频率是季。'
    ...
    f'受此约束的是 Exhibit ⟨ex:nna⟩ / ⟨ex:nna-q⟩ / ⟨ex:organic⟩ / ⟨ex:bridge⟩。<b>(3) 日均交易笔数 / 月末融资余额 → '
    ...
    f'<b>(4) 同样两个量的季频长历史 → {_QX[0]}</b>（{len(_QX)} 个季度，Exhibit ⟨ex:dats-q⟩ / ⟨ex:margin-q⟩）：'
    ...
    f'生息资产占比（Exhibit ⟨ex:iea-share⟩）2026-09-16 起改走<b>月频</b>，与客户总资产同起于 '
    f'{mlab(bs.index[0])}；只有净息差（Exhibit ⟨ex:nim⟩）仍是季频，自 {qlab(_nim.index[0])} 起。'
    ...
    '（Exhibit ⟨ex:nna⟩、⟨ex:accounts⟩ 的图注均标了这一条，Exhibit ⟨ex:nna-q⟩ 的季度图按构造不受影响）。',

    '<b>市值变动是轧差项，不是披露值。</b>Exhibit ⟨ex:bridge⟩ 的滚存桥用的是恒等式'
    ...
    '（当月净新增 × 12 ÷ 上月末客户资产），见 Exhibit ⟨ex:organic⟩ 与 Exhibit ⟨ex:organic-heat⟩。'
    ...
    '<b>所有月度图的次轴金色折线</b>（Exhibit ⟨ex:nna⟩ 核心净新增资产、Exhibit ⟨ex:organic⟩ 年化有机增长率、'
    'Exhibit ⟨ex:assets⟩ 客户总资产、Exhibit ⟨ex:accounts⟩ 新开经纪账户、Exhibit ⟨ex:dats⟩ 日均交易笔数、'
    'Exhibit ⟨ex:margin⟩ 月末融资余额），Exhibit 1 汇总表的 y/y 列，'
    '页顶 brief 段里出现的全部同比读数，以及 Exhibit ⟨ex:organic-heat⟩ 热力矩阵的逐格读数。'
    '<b>图与表现在是同一个口径，可以互相验算</b>，这正是本轮改口径换来的东西。'
    '(2) <b>季度合计同比</b>（本季 3 个月合计 ÷ 去年同季 3 个月合计 − 1）—— '
    '仅 Exhibit ⟨ex:nna-q⟩ 的右轴绿线。另有<b>环比</b>（m/m）出现在各图的气泡里，那不是同比。'
    '<b>已从页面上取消的两种</b>：12 个月滚动合计同比（此前 Exhibit ⟨ex:nna⟩ / ⟨ex:organic⟩ / ⟨ex:accounts⟩ 的次轴）与'
    '12 个月滚动均值同比（存量序列的合法平滑口径，此前也只作对照）。'
    '这两种口径现在<b>一条线都不画</b>，只以数字出现在图注与本条里当对照量。'
    '页面上仍然并存的另一种同比只有上面 (2) 点名的那一处（Exhibit ⟨ex:nna-q⟩ 的季度合计同比）：'
    '它与单月同比的差别是<b>期长</b>（三个月 vs 一个月），不是同一段时间的两种算法，'
    '<b>两者不要跨口径比高低</b>'
    # 两个读数现算，不写死：下个月这两个数都会变，写死就是给自己埋一句下月生效的假话。
    + ((f' —— 本月 Exhibit ⟨ex:nna⟩ 读 {_y_nna:+,.1f}%、Exhibit ⟨ex:nna-q⟩ 读 {_Q_NNA:+,.1f}%，'
        '各自都对，比的不是同一件事。')
       if _y_nna is not None and np.isfinite(_y_nna) and _Q_NNA is not None else '。')
    ...
    '趋势看柱本身、看 Exhibit ⟨ex:nna-q⟩ 的季度图、看 Exhibit ⟨ex:nna-years,accounts-years,dats-years⟩ 的逐年对照。'
    f'{YOY_NOTE}'
    ...
    + f'当期各口径并排现算：{"；".join(_CAL_ROWS)}。'
    '<b>热力矩阵（Exhibit ⟨ex:organic-heat⟩）与逐年对照图（Exhibit ⟨ex:nna-years⟩ / ⟨ex:accounts-years⟩ / ⟨ex:dats-years⟩）本来就是逐月读数</b>：'
    ...
    '而官方<b>没有</b>按新门槛回填前期，因此断点左右的「核心」口径不完全可比。'
    '（⚠️ 这一句只管这一列：月报附表<b>会</b>重述，平均生息资产在 2025-12 那次口径变更里'
    '就被逐月改写过 —— 见释义板「13 个月滚动表」。）原始月报没有给出调整前后的'
    ...
    + 'Exhibit ⟨ex:nna-years⟩ 的断点分的是年份不是月份、Exhibit ⟨ex:organic-heat⟩ 的断点分的是行不是列，'
    ...
    f'<b>融资余额有两条口径不同的序列，本页只画其中一条。</b>Exhibit ⟨ex:margin⟩ 画的是<b>月末</b>'
    ...
    f'本页 2026-09-16 起把它们单独画成 Exhibit ⟨ex:dats-q⟩ / ⟨ex:margin-q⟩（<code>series/schw_q.csv</code>，'
    f'{len(_QX)} 个季度）并另起一个板块 —— 它们与 Exhibit ⟨ex:dats⟩ / ⟨ex:margin⟩ 是<b>同一个量</b>，'
    ...
    '这两个因子的频率<b>不一样</b>，别合着读：'
    f'<b>平均生息资产是月频</b>（<code>series/schw.csv</code>，{len(_iea_m)} 个月，'
    f'{mlab(_iea_m.index[0])} 起，取自月报 "Selected Balances" 块逐月印的那一行）；'
    f'<b>净息差是季频</b>（<code>series/fee_rates.csv</code>，8-K Ex-99.1，'
    f'{qlab(_nim.index[0])} 起），月报里没有这一行 —— 这一条是<b>披露</b>边界。'
    '⚠️ 本页 2026-09-16 之前在这里写「这两个因子进到本页时只有季度腿」，'
    '生息资产那一半<b>是假的</b>：它在月报里逐月印了十二年，本页取不到只是因为'
    '<code>fetch/schw.py</code> 的 COLS 没写这一行（而且老版式把数值印在标签'
    '<b>下面</b>那行，按标签行取值会读到 0 个数）—— 管道边界被当成了披露边界，'
    '同一种病本仓这是第四次。现在这一列已经进管道，本图也随之改成月频。'
    '客户现金也一样逐月可得 —— 「客户现金占客户资产的比重」自 '
    ...
    '（货基是客户表外持仓，sweep 是负债端资金来源），顶不了这个分子，这一条至今成立。'
    '生息资产 / 客户资产的比值在 Exhibit ⟨ex:iea-share⟩ 画的 '
    f'{len(bs)} 个月（{mlab(bs.index[0])}–{mlab(bs.index[-1])}）里从 {_r0:.1f}% '
    f'{"单边" if _MONO13 else ""}走到 {_r1:.1f}%（趋势，不是噪音），把它当常数会造出假精度。'
    f'其中 {_UP_ALL} 个月环比上升，高点是 {mlab(_bs["iea_share"].idxmax())} 的 '
    f'{float(_bs["iea_share"].max()):.1f}%，所以它不是一条单调下滑线。'
    '不搭桥，改把这个比值本身画出来（Exhibit ⟨ex:iea-share⟩）—— 它本身就是 NII 增长受限的原因。'
    '净息差因为频率不同，从该图拆出去单独画在 <b>Exhibit ⟨ex:nim⟩</b>：'
    '把季度值摊到月度轴上每季会多出 2 个 null，而 <code>lines_endlabels</code> '
    '不容忍空点（docs/CHART_KINDS.md §1.2，会把线画塌到 0 且不报错），'
    '沿用到该季三个月又等于补点 —— 本页不外推、不补点。'
    + fee_period_note(head='净息差的期间：'),

    f'<b>截轴不删点。</b>窗口拉到全历史之后有两张图需要截轴，都不删点：'
    f'（1）Exhibit ⟨ex:accounts⟩ 的纵轴截在 {comma(NBA_CAP)}k'
    ...
    '所以 Exhibit ⟨ex:accounts⟩ 与 Exhibit ⟨ex:accounts-years⟩ 画的都是净除之后的开户量，那两根柱另画成斜纹以示不同源，'
    '披露原值在 Exhibit ⟨ex:accounts⟩ 的图注里。'
    '<b>不这么做的后果是量化的</b>：留着 14,718k 那一根，纵轴要顶到 1,600k，'
    '2013–2019 年那一档（月度 70–165k）只占纵轴 6%，十几年的逐月差异在图上是一条平线。'
    f'（2）Exhibit ⟨ex:bridge⟩ 的滚存桥截在 ±{comma(BR_CAP)} $bn：市值变动的量级是核心净新增的'
    ...
    '（2）Exhibit ⟨ex:iea-share⟩ 的 PDF 版保留 2 位小数，网页图表引擎的格式器只到 1 位，表格视图仍是 2 位；'
    f'（3）Exhibit ⟨ex:bridge⟩ 的纵轴网页版截在 ±{comma(BR_CAP)} $bn（PDF 不截），'
    '超界值以红色真值标出；'
    ...
    '（6）Exhibit ⟨ex:nna-q⟩ 的柱顶标签加了 $ 前缀、Exhibit ⟨ex:organic-heat⟩ 的格内数字加了 % 后缀，PDF 是裸数字；'
...
def compose_brief(frame=None):
    ...
    ═══ 与同比口径的三轮改造（CONTRACT §6）的关系 ═══
    2026-08 那一轮把流量图的次轴改成 12 个月滚动合计同比，本段当时被要求逐处标「单月」，
    以免读者拿它去对图上的滚动金线。**2026-08-15 这一轮又把次轴改回了单月**，全页
    （次轴、汇总表 y/y 列、headline、本段）现在是同一个口径。**2026-09 那一轮**由页面
    所有者把单月定成了全站唯一口径（§6.1 第 1 条），本页因此不必再改一个字 ——
    但那些「单月」标注的职责又变了一次：从「防混淆」到「防再次改口径」，
    现在是「让这一段离开本页也自洽」（页面上另有 Exhibit 3 的季度合计同比）。
    ...
    ═══ SCHW 独有，别家不能照抄 ═══
      ...
      · **两组序列的样本深度差 8 倍**：客户资产有 155 个月（2026-08-16 回填到 2013-09），
        ...
        护栏两处都要带。
      · core NNA 的位次跨 2025 年的 $10bn→$25bn 剔除门槛（BRK），官方没有按新门槛
        回填前期，所以同月排名只读位次、不读幅度，句子里必须带这句限定。
    """
    ...
    if (B.need(pu['value']) and i >= 12 and B.need(r[i - 12]) and r[i - 12] > 0
        ...
        # 杠杆率这一句算不出。两种缺法：Jan-25 之前压根没有月末融资余额（Schwab 在
        # 2026-01 那期月报里才首次披露，13 个月滚动表把数回溯到 Jan-25），
        # 或者当月够不到 12 个月前那个基数月。**单月同比只要 13 期**，所以实际的第一个
        # 同比点落在 Jan-26 —— 这里不要按上一版契约的滚动口径（12 期填窗 + 12 期比较）
        # 去反推，那样会把第一个点算到 Jan-27，晚整整一年。
        # 缺值不是让整页失败的理由，也不该留一段空白 —— 改讲同样是
        ...
        if B.need(og_v[i]) and len(fin) >= 2:
            ...
            why = (f'{"、".join(absent)}本月尚无披露' if absent
                   else '月末融资余额还够不到 12 个月前那个基数月、凑不出单月同比')
...
# brief 说「**这个月**这组读数该怎么读」，每月重写；notes 与图注说「这一张图这个月
# 该怎么读」（含当月实测的毛刺量、当期各口径并排读数）；这一块说「**这些词**是什么
# 意思」，一年到头是同一段 ⇒ 这里**不写当月读数**、不写「最新一期」。
# 出现的数只有两类：把定义钉住的结构性量（剔除门槛、并购搬账的官方数量、各列的
# 披露起点）与恒等式本身。**一个都没有写死**：门槛取 BRK / BRK_LABEL，搬账数量取
# ACQ_ACCOUNTS_K（官方月报脚注 (4)，见该常量上方的原文引用），各列的起点全部从
# 序列现读（nna / dm / mb / avgm）—— 下个月官方再回填一格，这几句自己就跟着走。
#
# ━━ 为什么是这 10 个词（选词判断）━━
# 判据只有一条：这个词出现在本页的图题 / 序列名 / 纵轴 / 汇总表行头 / 核对表列头里，
# 而且**不看定义就会读错**。按「读错会出什么事」分四类：
#   ① 公司口径里带限定词的行  核心净新增资产（core 是**剔除**过的，与未剔除的 Net
#      New Assets 不是一条序列，官方对同一个月给过两个数）、月末融资余额（只含 margin
#      loans，short credits 在 sweep cash 那行；另有一条已停发的月度平均序列，且季报附表
#      还有一条季末的同口径长历史）。读串的代价是整条线系统性偏高或偏低，
#      而图形完全正常，看不出来。
#   ② 存量 vs 流量        客户总资产（月末存量，其环比变动**不是**资金流）、
#      新开经纪账户（当月开户量，不是账户存量，且本页画的是净除并购搬账之后的数）。
#      不点破，读者会把资产下跌月读成撤资月，或把并购搬账读成开户动能。
#   ③ 本页自己轧出来的派生量  年化有机增长率（当月流量 × 12 ÷ 上月末存量）、
#      市值变动（轧差项，公司不披露）、生息资产 / 客户资产（分子当月均值、分母同月月末）。
#      这三个都必须点明「不是公司披露的数」，否则读者会去官方文件里找它们。
...
# 这一条里以**定义的一部分**出现，不单独立条）；「季度合计同比」（notes 第 5 条把两种
# 同比的分工整段讲完了）；以及成交量、市值这类本页没有特殊口径的常识词。
# ══════════════════════════════════════════════════════════════════════════════
# 两笔并购搬账现读 ACQ_ACCOUNTS_K，不在这里再抄一遍数量：抄一遍就等于同一个数在
# 本文件里有两处定义，官方哪天修订脚注，改了一处漏了另一处不会有任何东西报错。
_G_ACQ = '、'.join(f'{mlab(_p)} 的 {_k / 1000:.1f}mn 户'
                   for _p, _k in sorted(ACQ_ACCOUNTS_K.items()))

GLOSSARY = [
    ('核心净新增资产',
     '官方行名 <code>Core Net New Assets</code>，本页的主口径，是<b>流量</b>不是余额：'
     '在净新增资产的基础上<b>剔除</b>单一客户的巨额流入/流出，以及表外 Schwab Bank '
     'Retail CD 的流量。'
     # 门槛是官方脚注里的常数（"generally greater than $25 billion beginning in 2025;
     # $10 billion in prior periods"），取自 BRK / BRK_LABEL 那一处定义，不另抄一份。
     f'「巨额」的门槛在 {mlab(BRK)} 调过一次（{BRK_LABEL}），而官方<b>没有</b>'
     '按新门槛回填前期（别把这句读成「月报从不重述」—— 见「13 个月滚动表」那一条）。'
     '⚠️ 它与未剔除的 <code>Net New Assets</code> <b>不是一条序列</b>'
     '（官方对同一个月同时给过两个数），本页<b>不拼接</b> —— 所以这条线的起点是 '
     f'{mlab(nna.dropna().index[0])}，比客户总资产与新开账户短一大截，'
     '左边那段空白是<b>披露边界</b>，不是当时没有净流入。'),

    ('年化有机增长率',
     '<b>不是公司披露的数</b>，是本页按 GS「LPLA monthly metrics」的规矩算的比率：'
     '<code>当月核心净新增 × 12 ÷ 上月末客户总资产</code>。'
     '流量类指标不算环比百分比（分母是上个月的流量，一个月的噪音会被放大成趋势），'
     '这条比率是它的替代品 —— 把流量放回存量的尺度上。'
     '⚠️ 分子是<b>当月这一个月</b>的流量乘 12 年化，<b>不是</b>过去 12 个月的实际合计，'
     '读的是「按这个月的速度跑一年会长多少」，因此逐月的季节性都留在里面。'
     '分母取<b>上月末</b>而非当月末，本月的流入不进自己的分母。'),

    ('市值变动（轧差项）',
     '汇总表行名 <code>Market gains, balancing item</code>：<b>不是公司披露的数</b>，'
     '是本页按恒等式<code>期初资产 + 核心净新增 + 市值变动 = 期末资产</code>轧出来的'
     '残差（= 客户资产环比变动 − 核心净新增）。'
     '⚠️ 既然是残差，它同时吸收了真实市场涨跌、口径调整与<b>并购转入的资产</b>，'
     '<b>不能</b>整段当成「市场贡献」读 —— 并购并表的那个月尤其不能。'),

    ('客户总资产',
     '<b>月末存量</b>（Total Client Assets）。官方单位是 $bn，本页月度柱图除以 1,000 '
     '换成 $tn 便于读轴，页尾核对表印的仍是官方原始的 $bn。'
     '⚠️ 它的<b>环比变动不是资金流</b>：其中绝大部分是市值涨跌（见上一条），'
     '所以资产下跌的月份完全可以同时是净流入月，反过来也一样，'
     '<b>拿资产的涨跌去反推客户在存钱还是撤资，方向会读反</b>。'
     '它同时还是本页两个比率的分母：年化有机增长率与生息资产 / 客户资产。'),

    ('新开经纪账户',
     '当月<b>新开户数</b>（流量），<b>不是</b>账户存量 —— 本页不画任何账户余额。'
     f'⚠️ 图上画的是<b>净除并购搬账之后</b>的开户量：{_G_ACQ}，都是把别家的存量账户'
     '整批搬进来，不是当月有人来开户；两笔的确切数量由官方月报脚注 (4) 明写，所以这里做的是'
     '<b>减法而不是估算</b>，减完与邻月严丝合缝。这两根柱画成<b>斜纹</b>以示与邻月不同源，'
     '披露原值在该图图注里；次轴同比走的同样是净除后的序列。'),

    ('DATs（日均交易笔数）',
     '官方行名 <code>Client Daily Average Trades</code>：公司披露的<b>本身就已经是'
     '日均</b>，拿到手<b>不要再除一次交易日</b>，也不能当成当月合计笔数读。'
     # 「2026-01 那期新增」是已经发生的一次性事实，与 fetch/schw.py「口径坑」第 3 条
     # 同源；随时间变的那部分（回填到哪个月）从序列现读，不写死。
     f'⚠️ <b>月度</b>披露史极短：这一行是月报 2026-01 那期<b>新增</b>的，官方只回填到 '
     f'{mlab(dm.dropna().index[0])}，更早的年份<b>月报里没有这一行</b> —— '
     'Exhibit ⟨ex:dats⟩ 左边没有柱是<b>披露边界，不是 0</b>。'
     f'但<b>季报附表</b>里这个量回得到 {_QX[0]}（那里是<b>整季</b>的日均，不是单月的），'
     '本页把它单独画成 Exhibit ⟨ex:dats-q⟩，与 Exhibit ⟨ex:dats⟩ 不可逐格对照。'
     '官方与核对表的单位是「千笔/日」，图上换算成 mn。'),

    ('月末融资余额',
     '官方行名 <code>Margin Balances (at month end)</code>：客户融资余额的<b>月末存量</b>，'
     ...
     'short credits 归 sweep cash（脚注 (7) 明写）。对账见 Exhibit ⟨ex:margin-q⟩ 图注。'
     ...
     f'⚠️ <b>季报附表</b>里同一个存量回得到 {_QX[0]}（Exhibit ⟨ex:margin-q⟩），那里的行名是 '
    ...
    ('平均生息资产',
     '官方行名 <code>Average Interest-Earning Assets</code>，月报 "Selected Balances" 块'
     '（2025-11 及更早的版式里叫 "Selected Average Balances"）里<b>逐月</b>印的一行，'
     '脚注定义是 "average total interest-earning assets on the Company&rsquo;s balance '
     'sheet"。⚠️ 它是<b>当月平均</b>余额，不是月末时点值，'
     '所以不能和本页其它「月末」口径的存量并排读。'
     f'本页取到 {mlab(_iea_m.index[0])} 起的 {len(_iea_m)} 个月。'
     '⚠️ <b>口径在 2025-12 变过一次，且官方追溯改写了前期</b>：与客户 long/short 策略'
     '相关的客户融资贷款与 short credits 被剔出这一行，脚注原文写着 '
     '"Prior period amounts have been adjusted accordingly."。'
     '本页 2025-01 起各月用的都是调整<b>后</b>的数（改动最大的 2025-11 是 −7.2$bn），'
     '2024-12 及更早官方没有改动。'
     '⚠️ 同名的量在<b>季报</b>里也有（<code>series/fee_rates.csv</code>，8-K Ex-99.1），'
     '那是<b>该季</b>的日均、且存的是当期原报值，与本页这条月度序列'
     '<b>不能逐格对照</b>；两者的一致性由抓取侧按日历日加权逐季对账'
     '（<code>fetch/schw.py</code> 的 <code>_crosscheck_quarter_iea</code>）。'),

    ('生息资产 / 客户资产',
     '<b>不是公司披露的一行</b>，是本页轧的比值：分子是上一条那个<b>当月平均</b>生息资产，'
     '分母是<b>同月月末</b>的客户总资产。'
     '⚠️ 分子是期间平均、分母是时点余额，两者口径不同 —— 公司不按月印客户资产的日均，'
     '这是能拿到的最接近的分母。'
     # ⚠️ 这条以前写「生息资产不按月披露（只随季报出）」「本页的生息资产只有季度腿」，
     # 两句都是假话 —— 前者是关于官方的，后者曾是关于本仓管道的事实，
     # 但 2026-09-16 那一列已经进了 series/schw.csv，后者也随之失效。
     # 「Schwab 不披露客户现金」同样是假话，fetch/schw.py 的「口径坑」第 7 条点名禁止：
     # 月报里的 Client Cash as a Percentage of Client Assets 自 2013-09 起每期都印
     # （同文件 _CASH_PCT_FROM；2015-04-15 报送及更早那一行没有 Client 前缀，该处有实证），
     # series/schw.csv 的 client_cash_pct 就是它。⚠️ 别写死它在哪个块下 —— 2026-01 起
     # 官方把这一行从 Client Activity 挪进了 Selected Balances（aug2026 表 r35）。
     # 释义板是「一年到头同一段」的定义性文字，三句都不能带。
     '（月报里逐月印着「客户现金占客户资产的比重」这一行，'
     '但<b>客户现金不是生息资产</b>，顶不了这个分子 —— 这一条与上面那两句无关，至今成立。）'
     '而这个比值本身在长期下行 —— 把它当常数就会造出假精度，'
     '所以本页不搭「量 → 收入」桥，改把比值本身画出来（Exhibit ⟨ex:iea-share⟩，月频）。'),

    ('净息差（NIM）',
     '公司披露的<b>季度</b>净息差（Exhibit ⟨ex:nim⟩）。月报里<b>没有</b>这一行，'
     '所以它至今只有季频 —— 这是<b>披露</b>边界，与上面两条不同。'
     '本页每一季取自<b>该季自己那份</b> 8-K Ex-99.1 '
     '业绩新闻稿的原报值，官方后来的重述<b>不回填</b>，所以这条线是「当期原值」而非最终版。'
     '⚠️ 它的分母是<b>平均生息资产</b>，不是客户总资产 —— 两者不在一个量级（见上面两条），'
     '把它乘在客户资产上估不出净利息收入。'),

    ('13 个月滚动表',
     '官方月报附表里的那张表：每期只印<b>当月与之前 12 个月</b>。本页的长历史是把相邻'
     '各期<b>逐期接续</b>出来的（重叠月份逐个对过），不是某一份文件里现成就有的。'
     '⚠️ 官方<b>新增一行</b>时也只把它回填到这张表的左端，所以新指标的历史一律止步于此，'
     '这就是本页各列起点互不相同的原因。'
     '⚠️ 本页 2026-09-16 之前在这里写「月报附表<b>不重述历史</b>，同一个月在新旧两期'
     '文件里是同一个数」，<b>那是错的</b>：官方 2025-12 改平均生息资产的口径时，'
     '把 2025-01 至 2025-11 已经发过的数逐月改写了一遍（最大 −7.2$bn）。'
     '重述与否是<b>逐列</b>的 —— 核心净新增资产的剔除门槛变更官方就没有回填'
     '（所以那条线上有断点），平均生息资产这一次回填了。没有一句话能同时罩住两者。'),
]
...
payload = {
    ...
    'brief': compose_brief(),
    # 名词释义：排在所有 exhibit 之前。选词判断与「有意不收哪些词」写在 GLOSSARY 上方。
    'glossary': gloss.render(GLOSSARY, where='schw glossary'),
    ...
    'summary': summary,
    'exhibits': ex,
    'order': ORDER,          # 图序；write_dash 编号、兑占位符之后拿掉（build/exhibits.py）
    'table': table,
    'notes': notes,
