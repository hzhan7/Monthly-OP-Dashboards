...
# 那是本仓「中期图」的标准窗口，审计量的也是图上画出来的那一段。
# ⚠ 别在这里写死期数：全站图窗 2026-08-18 起从「近 25 个月」改成 build/single.py 的
#   WIN_FROM = 2016-01，长度随月份增长；写死的期数当天就成了假话。
WIN_LONG_DEMO = 25
...
def _selftest():
    ...
    print('   → 「符号相反的序列占比」对窗口极敏感：几十年的序列总能找到一个相反的月，'
          '不切窗这个数会逼近 100% 而失去信息量。审计量的是图上那一段窗口，这里照办。')
