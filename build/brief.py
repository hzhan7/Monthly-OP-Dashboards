...
# ── 逐字引用官方原文 ─────────────────────────────────────────────────────
# 引文豁免的单一入口。为什么需要它、为什么它不是在偷偷取消分寸线，见 render() 的
# 「引文豁免」一节 —— 那段是这条机制的正式说明，改这里之前先读那一段。
QUOTE_MAX = 120        # 单页引文合计上限（去标签字符数）。理由见 render()。
_Q_TAG = 'mops-quote'
_Q_RE = re.compile(rf'<span class="{_Q_TAG}">(.*?)</span>', re.S)
_WS_RE = re.compile(r'\s+')


def quote(text):
    """把一段**逐字引用的官方原文**包成引文标记，返回可直接进 sentences 的 HTML 片段。

    这是 `render()` 承认引文、给出字数豁免的**唯一**方式：豁免的额度不是调用方报上来的
    一个数，而是 render 自己从这个标记里量出来的。两者的差别在于能不能说谎 ——
    传数字的写法允许「声明引用了 100 字、实际一个字没引」，那等于给自撰散文开后门，
    而护栏没有任何办法发现；标记的写法里，你只能为**确实印在页面上的那些字**免单。

    ⚠ 标记只包**引文本身**。引号「」、「公司在备注栏填的是」这类引导语、
    出处与日期，全都是**我们自己写的字**，必须留在标记外面，照常计入 230-380。
    这条不是形式主义：那些字是我们的编辑选择，分寸线管的就是编辑选择。

    只做一种改写 —— **空白折叠**（首尾去空、内部连续空白并成一个空格）。
    MOPS 那一栏常带尾随空格与换行，折叠的是排版不是内容，字还是那些字。
    其余一律不动：不转简体（繁体原文转简体之后就不是原文了，而且映射有损）、
    不截断（截断一句官方说明可能把它的意思截反，见 QUOTE_MAX 那段）、不改标点。

    `<` `>` 直接硬失败而不是转义：MOPS 的备注是中文散文，出现尖括号只可能是抓取
    串行或页面串了 HTML 进来 —— 那种时候要的是有人去看一眼，不是安静地转义上线。
    """
    if not isinstance(text, str):
        raise SystemExit(f'brief.quote() 只接受字符串，收到 {type(text).__name__}')
    s = _WS_RE.sub(' ', text).strip()
    if not s:
        raise SystemExit(
            'brief.quote() 收到空引文。备注栏为空（MOPS 常填 "-"）时不许调用它 —— '
            '空备注要走条件化叙述另写一句，而不是印一对空引号。')
    if '<' in s or '>' in s:
        raise SystemExit(f'brief.quote() 的原文里出现尖括号，疑似抓取串行：{s[:80]}')
    if len(s) > QUOTE_MAX:
        raise SystemExit(
            f'brief.quote() 的原文 {len(s)} 字，超过单页引文上限 {QUOTE_MAX} 字：{s[:80]}…\n'
            '  不要截断它 —— 截断一句官方说明可能把意思截反。正确处理是这一页**不引用**，'
            '改写一句「本月备注原文 N 字，全文见页注／原公告」，把全文放到不限字数的地方。')
    return f'<span class="{_Q_TAG}">{s}</span>'


def quoted_len(body):
    """一段 brief body 里被标记为逐字引文的字符数。render() 与调用方共用同一口径。"""
    return sum(len(m.group(1)) for m in _Q_RE.finditer(body))
...
def render(sentences, title=TITLE, lo=230, hi=380):
    ...
    **洞察保留，分解的层数收掉。**
    ...
    ═══ 引文豁免：`quote()` 包起来的官方原文不计入 230-380（2026-08 定）═══
    起因是「把 MOPS 月营收申报表『備註／營收變化原因說明』栏的公司原文写进 brief」。
    实测那时四家的净长是 ase 342 / guc 354 / tsm 349 / alchip 338，离 380 只剩 26-42 字，
    而备注原文动辄 30-120 字 —— 照原样卡，四家当场整页发不出去。

    ── 为什么这不是把分寸线偷偷放掉 ──
    380 这条线管的是**编辑裁量**：它防的是我们自己越写越多、写成 380 字的从句堆
    （上面那段说得很清楚，那是分寸失控的症状）。逐字引用的官方原文里**没有编辑裁量**
    —— 它有多长是公司填的，不是我们选的。拿一条为「约束我们自己的笔」而立的线去
    配给公司的原话，得到的不是克制，而是「为了凑字数把官方披露删掉」。

    所以豁免的实现是**减法而不是加法**：先把引文字数从总长里扣掉，再拿**剩下的、
    全部由我们自己写的那部分**去撞 230-380 —— 这条线一个字都没动。等价地说，
    我们自己的话仍然最多 380 字，一个字都没多。四条性质保证它不会变成后门：

      1. **上限对自撰散文一格不放宽。** 引文扣掉之后拿去比较的就是我们写的字数，
         阈值仍是 230-380 原值。想多写自己的话，一个字都借不到。
      2. **额度只能靠真引用挣，不能靠声明挣。** render 不收「引了多少字」这个参数，
         它自己数 `quote()` 标记里的字（见 `quote()` 的 docstring）。没有可以谎报的数。
      3. **下限跟着一起抬 —— 这才是这条护栏在引文场景下真正的作用。** 见下一段。
      4. **有天花板。** 引文合计上限 QUOTE_MAX 字，超了硬失败并要求改走「不引用 +
         指路页注」，页面不会因为公司写了长文就变成一篇文档转载。

    ── 下限：豁免必须是减法，不能是「只把上限放大」──
    这一条是整个设计的要害。假如做法是「有引文就把 hi 调到 500」，那么一个
    「引文 120 字 + 自撰 200 字」的页面总长 320，稳稳过关 —— 可它恰恰是这条护栏
    本来要拦的东西：自撰部分 200 字已经跌破 230，解读被引文顶掉了，页面看着满，
    实际没人在读数上说话，成了公告转载。
    减法写法下这一页会**当场失败**（自撰 200 < 230）。换句话说：引文不是配额，
    它既不给你多写的余地，也不能替你少写 —— 引 120 字，这一页的总长就必须
    达到 350 才够格，你欠读者的那 230 字解读一个字都不能少。**豁免只对引文成立，
    对两端同时成立。**

    ── 为什么 QUOTE_MAX 是 120、超了为什么不截断 ──
    2026-07 期四家的备注实测是 10-26 字（联发科 20 / 南亚科 10 / 世芯 26 / 创意 13），
    120 是它的约五倍：正常月份根本碰不到，它只管住尾部那种整段说明。
    超了不许截断 —— 官方增减原因常写成「本月營收較去年同期增加，係因…所致」，
    从中间截一刀可能把因果或转折（「惟…」）截掉，剩下半句的意思和原意相反，
    而它外面还套着引号、页面还说这是原文。那不是省字数，是造假。
    正确处理是这一页**不引用**，改印一句状态陈述并把全文放到不限字数的地方
    （页注 / payload 字段）—— 收回一句引用不是信息损失，印一句被截歪的引用才是。
    """
    body = ''.join(s for s in sentences if s)
    plain = re.sub(r'<[^>]+>', '', body)
    q = quoted_len(body)
    # q == 0（今天全部 19 页的状态）时下面这一支与改造前逐字节等价：ours == len(plain)、
    # 阈值原值、报文原文。引文豁免是一条**加出来的支路**，不动既有那一条。
    if not q:
    ...
    if q > QUOTE_MAX:
        raise SystemExit(
            f'brief 引文合计 {q} 字，超过单页上限 {QUOTE_MAX} 字。'
            '不要截断原文（会把意思截反），改走「不引用 + 指路页注」。')
    ours = len(plain) - q
    if not lo <= ours <= hi:
        raise SystemExit(
            f'brief 自撰部分 {ours} 字超出 {lo}-{hi}'
            f'（全文 {len(plain)} 字，其中逐字引用官方原文 {q} 字不计）'
            f'，模板可能拼坏了：{plain[:140]}…\n'
            + ('  引文不能替代解读：引了官方原文，仍然要自己写满 '
               f'{lo} 字。' if ours < lo else
               '  引文不给自撰散文额外额度：自己的话仍然最多 '
               f'{hi} 字。'))
    return f'<h4>{title}</h4><p>{body}</p>'
