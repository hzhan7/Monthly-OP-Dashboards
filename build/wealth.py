...
# 全站同比只有一种口径（CONTRACT §6）：流量走单月同比、存量走点对点同比，页上一条
# 12 个月滚动合计的同比都不画。本页画同比的图**全部是存量序列**（客户资产、客户权益、
# 平台资产、账户/客户存量），走的是 §6.1 第 2 条 —— 那一条与上一版逐字相同、不受
# 2026-09 那次改口径影响，所以本轮本页一条线都没动过。滚动合计对存量本来就是个假名字
# （12 个月末余额相加不指代任何真实的量）。但要说清楚：存量**并非不能平滑**，合法的
# 平滑口径是 12 个月滚动**均值**同比（去年一整年的平均资产 vs 前年），数值上等同于
# 滚动合计比（Σ12/Σ12′ 的除数约掉）；§6.1 第 2 条把它留作**图注里的对照量**，
# 页上不画线。本页用点对点，理由由**逐条实测**给出。
...
def stock_pair(cname, idx):
    ...
    ⚠ **为什么这一对只能在这里算**：`yoy.caliber_diff()` 只对 `kind=FLOW` 做两口径
    对比，`kind != FLOW` 时滚动那一侧一律返回 None，而这里比的正是**存量**对 12 个月
    滚动**均值**同比，共享模块里没有这一对。**但估计量必须与共享模块同一个**：下面用
    的是样本标准差（`ddof=1`），与 `build/yoy.py::_std()` 的 `np.nanstd(x, ddof=1)`
    是同一个估计量；这里写成 pandas 的 `.std(ddof=1)`，是因为 keep 已经把非有限值滤
    干净了，两种写法在同一批数上取值相同。
    这里曾经写的是 `ddof=0`，于是**同一条序列、同一个窗口，本页与单票页印出两个不同
    的数**（两者之比恒为 sqrt((n−1)/n)，与数据无关，所以差多少随窗口长度走、不写死）：
    Schwab 客户资产、LPL 客户资产、Robinhood 平台资产这三条本页与 /schw/ /lpla/ /hood/
    各自印一遍，读者跨页一核就对不上。改成 ddof=1 之后四页同数。
    """
...
def org_caliber_note(items, idx):
    """有机增速图的口径说明：画的是**单月年化**水平值，滚动口径只以数字出现。

    这类图画的不是同比而是一个**增长率的水平值**，所以 CONTRACT §6 的同比口径条文
    不直接管它；但「当月流量 x12」这一步同样会把一个月的噪音放大 12 倍，所以照 §6.1
    第 3 条的做法把这笔账印在旁边：两种口径的当期读数与波动并排给出，
    **对照的那一侧只以数字出现、页上一条线都不画**。
    ⚠ 不要写成「真要读趋势请看滚动口径」那类句子（2026-09 之前就是那么写的）：
    页上没有那条线，读者拿这里的一个数读不出趋势，而且那等于替页面上不存在的东西
    背书 —— §6.1 第 3 条明令禁止。这里该说的是**代价**，不是哪种口径更好。
    """
    rows = []
    for name, t in items:
        ...
        # 「逐月标准差」全站同一个估计量：样本标准差 ddof=1（build/yoy.py::_std()）。
        # 这里与上面 stock_pair 那一份从前写的都是 ddof=0，而 hood / lpla / schw / msci
        # 四页写的是 ddof=1 —— 同一个词在站上有两种定义，本页是落单的那个。一并改过来了。
        rows.append(f'{name} 当期单月年化 {A.iloc[-1]:.1f}% / 滚动 12 个月 {B.iloc[-1]:.1f}%'
                    f'（窗口内逐月标准差 {A.std(ddof=1):.1f}pp vs {B.std(ddof=1):.1f}pp，'
    ...
    return ('<b>口径：本图画的是「当月净流入 x 12」的<u>单月</u>年化增速</b>'
            '（GS LPLA 的流量口径规矩，与全站「一律单月」的同比口径同一个取向，'
            'CONTRACT §6）。它把一个月的流量乘 12，所以本身是条噪声很大的线 —— '
            '<b>这笔代价照实印在这里</b>，对照的那一侧是<b>滚动 12 个月有机增速</b>'
            '（滚动 12 个月净流入 ÷ 12 个月前的月末资产 —— 分子分母各自滚动再相除，'
            '不是把 12 个月的比率平均，后者对比率没有意义）：它<b>只在这行字里以数字'
            '出现，页上一条线都不画</b>。两种口径的当期读数与波动逐家并排：'
            + '；'.join(rows)
            + '。' + org_xref_txt(items))
...
def org_xref_txt(items):
    ...
    ⚠ 这半句翻过两次车，两次都是「抬头的外延比枚举宽」：
      ...
      （上面两条讲的都是 2026-08 那会儿的事：那时各单票页的次轴确实有滚动口径的。
       2026-09 全站统一单月同比之后 'roll' 这一档在本页成员上应当空掉，句子会自己
       变成「画单月同比的是 …」。函数留着不是为了那半句对照，是为了**它能自己发现
       某一页又变了**——写死任何一种说法都会在下一次改版静默变假。）
    ...
    if not named:
        return ''
    parts = []
    # 2026-09 全站统一单月同比之后，'roll' 这一档在成员页上**应当是空的**（§6.2 的两处
    # 例外都在交易所组的页上，不是本页成员）。分档保留而不是删掉：它是唯一一处会当场
    # 发现「某一页又挂回滚动次轴」的东西，删了这句就只剩人眼。措辞不再写「仍是单月」——
    # 那个「仍」是两种口径并存时代的对照语气，现在单月是全站唯一口径，没有对照面了。
    if buckets['roll']:
        parts.append('画 12 个月滚动口径同比的是 ' + '、'.join(buckets['roll']))
    if buckets['mono']:
        parts.append('画单月同比的是 ' + '、'.join(buckets['mono']))
    if buckets['none']:
...
def _lp_rank_txt():
    ...
    「LPL 的 y/y 高于 Schwab」。两个数由下面当场算（此处不写死示例读数：写死的那一版
    很快就与当期对不上，而它长得就像一句实测）。算不出来（缺月、缺 Schwab）就返回
    空串，不写半句。"""
    try:
...
# 的，窗口放宽之后它就不成立了。推导式与 build/ibkr.py 的 cleared_all 逐字相同
# （年化人均 cleared DART ÷ 252 × 期初期末账户均值），序列取 IBKR 的**完整历史**
# （RAW 未截到共同最新月）。
#
# ⚠ 但「完整历史」这个区间**不能直接印进某一张图的图注**。2026-09 修的就是这个：
#   Exhibit N_DATS 的图注说「约为<b>本图数值</b>的 84%–93%（Feb-16–Aug-26 全区间）」，
#   而本图横轴止于共同最新月（本轮 Jul-26）—— 区间右端那个 Aug-26 读者在这张图上
#   一格都找不到。凡是拿「本图数值」做主语的句子，区间必须按**那张图自己的窗口**现算。
#   （与本轮 build/single.py 修 `mom_cost_zh` 是同一类缺陷。）
#   完整历史仍然算着，但只作为「IBKR 单页那张图画的是完整历史」的对照出现，并当场
#   标明它是那一页的窗口 —— 两页数字不同时，差的是窗口不是口径。
...
def cleared_txt(win):
    """覆盖率的一句话，**按传进来的图窗现算** —— 句子里出现的月份都落在那张图的横轴上。

    win = 该 exhibit 的 x 轴月份（Period 列表）。窗口比完整历史短时，再把完整历史的
    区间挂在后面，并点明那是 IBKR 单页那张图的窗口，免得两页数字不同被读成口径分叉。
    """
    if CLEARED_COV is None:
        return '低于本图数值'
    w = _cv.reindex([p for p in win if p in _cv.index]).dropna()
    if not len(w):
        return '低于本图数值（本图窗口内没有可算的月份）'
    txt = (f'约为本图数值的 {w.min():.0f}%–{w.max():.0f}%'
           f'（{mlab(w.index[0])}–{mlab(w.index[-1])}，本图窗口内共 {len(w)} 个月，'
           f'中位 {w.median():.0f}%）')
    if len(w) < len(_cv):
        # ⚠ 这里**不印完整历史那两个月份的名字**：它们落在本图横轴之外，读者在这张图上
        #   找不到。只给「比本图多几个月」与那一侧的区间，两页对不上时够定位差在哪。
        txt += (f'；IBKR 单页那张图画的是这条推导列的完整历史（比本图窗口多 '
                f'{len(_cv) - len(w)} 个月），那边的区间是 '
                f'{CLEARED_COV[0]:.0f}%–{CLEARED_COV[2]:.0f}%、中位 {CLEARED_COV[1]:.0f}% —— '
                f'两页数字不同差的是<b>窗口</b>，推导式逐字相同')
    return txt
...
ex.append({
    ...
    # 标题里写明「单月同比」有两条理由，都与「别的页画的是别的口径」无关（2026-09 之前
    # 这里写的正是那一条：「schw Exhibit 2、hood Exhibit 3 已经有画滚动口径的了」——
    # 全站改口径之后一页都没有了，那句作废）：
    #   · CONTRACT §6.6 的判据认标题域里的「单月」，不写判 🟡；
    #   · 两边写的是同一个词，读者才知道跨页核到的确实是同一个数（本页这几条线与
    #     /schw/ /lpla/ /hood/ 同名图次轴的读数逐月一致，2026-09 对过两边的 payload）。
...
ex.append({
    ...
    **({'end_label': True, 'zero_base': True} if _k9 == 'lines' else {}),
    ...
    'note': (firms_note(inc9, exc9, 'LPL 不披露交易笔数。')
             ...
             f'（见 {IBKR_XREF_BOTH}），{cleared_txt(_i9)}，两条线不要混读；'
...
# 这是 CONTRACT §6.1 第 3 条那笔代价账的有机增速版：图上画的是单月年化（当月流量 x12，
# 一个月的噪音被放大 12 倍），对照的滚动 12 个月**只在文字里以数字出现，页上不画线**。
# ⚠ 从前这里写的理由是「不并排印，读者拿本页的线去核各单票页的次轴必然对不上」——
# 那半句现在既没必要也不准：各单票页那张同名图的主轴画的同样是单月年化，次轴是它的
# 单月同比（pp），两边根本不是靠这一段来对齐的。
def _org_now(t):
    a = df.get(f'{t}_org')
...
ORG_MIX_TXT = (
    ...
    + '单月年化把一个月的流量乘 12，噪声也跟着放大 12 倍 —— '
      '<b>这笔代价照 CONTRACT §6.1 第 3 条印在这里</b>：对照的那一侧是'
      '<b>滚动 12 个月有机增速</b>（滚动 12 个月净流入 ÷ 12 个月前的月末资产），'
      '它<b>页上一条线都不画</b>，只在下面这行字里以数字出现，当期读数逐家并排：'
...
# ── 各成员单票页上还有没有滚动口径的同比轴 —— 逐页现读那一页 payload 的轴标签 ──────
# 这句话的**前提**在 2026-09 换过一次。旧前提是「各单票页本轮都把流量类同比改成了
# 滚动口径」，那是一句写死的全称断言，先被 /schw/ 当场证伪（那一页把滚动口径取消了），
# 再被全站改口径整个作废：CONTRACT §6 现在规定全站同比一律单月，§6.2 的例外名单
# （exchanges-apac 的 Ex5 与 Ex15、exchanges12 的 Ex4/7/8，五张分布在两页上）
# 都不是本页成员，所以正常情况下这一扫
# 应当**一条都读不到**。判据本身一个字没改：仍是 _roll_axes 现读那几页的轴标签。
# 留着它是因为「读到了」才是新闻 —— 哪天某一页又挂回滚动轴，这句话会自己变，
# 而写死「大家都是单月」的散文不会。本页自己那半句留着，本页自己就能验、不必跨页。
...
_MEMBER_ROLL_TXT = (
    ('<b>各成员单票页也一样</b>（逐页现读那几页 payload 的轴标签，不是照抄说法）：'
     + ('，'.join(_MR_NOT) + ' 上一条 12 个月滚动口径的同比轴都没有。' if _MR_NOT else '')
     + ('<b>⚠ 但读到了例外</b>：' + '；'.join(_MR_HAS)
        + ' 仍挂着滚动口径的同比轴 —— CONTRACT §6.2 的例外名单只在 exchanges-apac 与 '
          'exchanges12 两页上，本页成员一张都不在名单里，'
          '跨页核这几张的数之前先看清是哪一种口径。' if _MR_HAS else ''))
...
notes = [
    ...
    # ── 同比口径：本页只有一种，与全站、与各单票页都是同一种（逐页现读复核）──
    f'<b>⚠ 同比口径：全站一律单月（CONTRACT §6）—— 流量走单月同比、存量走点对点同比，'
    f'页上一条 12 个月滚动合计的同比都不画。本页画同比的图<u>全部</u>是存量，'
    f'走点对点（单月）口径，原因逐图实测过。</b>本页画同比的是 Exhibit {N_YOY}（客户资产 y/y）、'
    f'Exhibit {N_ACCT}（账户 / 客户数 y/y）与 Exhibit {_exl(_HEAT_NS)} 四张热力矩阵，'
    '<b>它们画的都是存量序列</b>（客户资产 / 客户权益 / 平台资产 / 账户存量）。'
    '本页没有画流量同比的图 —— 净流入在本页是以<b>年化有机增速</b>的形式出现的，'
    '那是水平值不是同比。'
    + _MEMBER_ROLL_TXT
    + '<b>所以跨页核数不必先换算口径</b>：同一条序列在本页与在它自己那页上'
    '算的都是「本月 ÷ 去年同月 − 1」（两边同走 <code>build/yoy.py</code>），'
    '两边都有值的月份上读数相同。'
...
def compose_brief(df, latest):
    ...
    ═══ 与全站同比口径（CONTRACT §6）的关系 ═══
      **本页画同比的图全是存量序列**（客户资产、客户权益、平台资产、账户/客户存量），
      走 §6.1 第 2 条的点对点同比 —— 那一条不受 2026-09 那次改口径影响，所以本页
      本轮一条线都没动（理由与实测见页尾「⚠ 同比口径」与各图图注）。brief 引用的
      y/y 与汇总表、与下方各图因此是**同一个口径（单月）**，不存在「brief 单月 vs
      图上 TTM」的错位。
      **跨页也已经对得上了。** 2026-09 之前这里写着「同名的客户资产 y/y 在各单票页
      （/schw/、/hood/ …）的次轴已是滚动口径，读者跨页核数会对不上」—— 全站改口径
      之后那半句整句作废：各单票页一条滚动口径的次轴都不剩（§6.2 的两处例外在
      exchanges-apac / exchanges12，不是本页成员）。回源复算过，不是看措辞猜的：
      本页 Exhibit「Client asset growth, y/y」的三条线与 /schw/ 的 Total client
      assets、/lpla/ 的 Total client assets、/hood/ 的 Total platform assets
      各自次轴那条线，在**两边都有值的每一个月上逐格相同、一处不一致都没有**
      （2026-09 拿两边的 payload 对过；月份数逐月增长，所以这里不抄数）。
      句中凡同比措辞仍标「单月」，理由换成两条：§6.6 的判据认标题域里的「单月」；
      而读者跨页核数时看得见两边写的是同一个词，才知道那真是同一个数。
      有机增速画的是**增长率的水平值**不是同比：图上一律单月年化，滚动 12 个月
      （`{t}_org_roll` 列）按 §6.1 第 3 条只在图注与页尾**以数字**出现、页上不画线。
      brief 引用其名次时点名「单月年化」——名次是在单月年化序列上排的，拿图注里的
      滚动读数去核对不上，那不是错，是两个口径。
