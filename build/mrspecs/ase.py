...
【11】新落库的两列美元（`revenue_usd_mn` / `revenue_atm_usd_mn`）：来源、复核、陷阱
    **来源**：公司月度英文新闻稿正文的 `US$ Million` 表（与两张 NT$ 表同一份 PDF、
    同一页），公司自印的**整数**，不是任何人折出来的。99 个月零缺失。
    **复核**：两个独立 agent 各写一套解析器抽取，全表 99 个月 × 5 个数值列 = 495 格
    逐格 0 分歧；对照组明确表示「没能证伪对方任何一格」。
    **它们不是 NT$ 那两列的函数**（这条最要紧，见【5】末与【7】末）：
      · 反解隐含汇率（NT$ ÷ US$）落在 27.60（2022-01）~ 32.84（2025-04）之间，
        中位 30.68，99 个月无一逸出 27–33 —— 量级对，所以**看上去**像可以互推；
...
_SRC_NOTE = (
    ...
    '后者是本页美元腿的唯一来源。'
    '全表（99 个月 × 5 个数值列 = 495 格）由两套互相独立写的解析器各抽一遍、'
