...
【5】**有汇率线（Ex8），没有美元营收腿（Ex5／Ex6 显式跳过）**
    这两件事本轮起在底座里是分开的（`mrbase` §1.5 的 `fx_used` / `usd_leg_shown`）：
      · Ex8 画的是 **NTD/USD 汇率本身**，一条宏观序列 —— 挂同一份 `series/tsm_fx.csv`
        的每一页上它逐点相同，不需要日月光披露任何东西。它该出，理由见下面的 20-F 原文。
      · Ex5／Ex6 要的是「本币 ÷ 外部牌价」折出来的**公司**序列。本家不给，理由是
        下面 (a)(b) 两条 —— 那是 `skip` + `skip_note`，不是「没有 fx」。
    ⚠️ 旧版本这里写的是「不给 fx，Ex5/Ex6/Ex8 一起跳」。那是把两件事捆在了一起：
      跳掉 Ex8 等于因为「本家的美元数不好折」而连一条与本家无关的宏观汇率线都不画。
    理由不是「营收不以美元计价」——那句话与 20-F 直接冲突
    ...
    真正拦住**美元营收腿**的是另外两条（见 `_SKIP_FX_LINES` / `_SKIP_FX_CONTRIB`）。
    请注意这两条咬的都是「本币 ÷ 外部牌价折出来的那条**营收**线」，
    **一条都碰不到汇率本身** ⇒ 它们不构成跳掉 Ex8 的理由：
    ...
    ⇒ 挂 `fx`（Ex8 出），`skip = ['fx_lines', 'fx_contrib']` 并给逐条 `skip_note`。
    ...
    ⇒ **接管后第一顺位待办**：把新闻稿那两列美元落进 `series/ase.csv`
      ...
      月度美元实绩的一家，落库后它的美元腿会是**公司披露值**、比 TSM 的推导值更硬，
      届时 Ex5／Ex6 可以从 `skip` 里拿掉，并把 `fx.implied` 设为 False。
...
# 那是全序列的分位，而引擎的 5/95 只看 Ex9 那张 matrix。底座已按 matrix 现算并印在
# 该图图注里，spec 侧再留一份只会在窗口滚动后与它对不上。见 _HEAT_EXTRA 末尾。
...
# ── ⑧ 汇率：Ex8 出、Ex5/Ex6 不出。三段分工如下 ──────────────────────────────
#    · _SKIP_FX_LINES / _SKIP_FX_CONTRIB → skip_note，底座逐条印成
#      「本页不出「fx_lines」那张图：…」，是**这两张图为什么不画**的可复核理由。
#    · _FX_RATE_NOTE                     → Ex8 自己的图注末尾，说**这条线是什么、不是什么**。
#    · _USD_ROADMAP_NOTE                 → 页尾，说本家与其他六家不同的那件事实（有官方美元）。
#    「本页有汇率线但没有美元营收腿」这句总述由底座出（§1.5 的第 ② 支），这里不重复。
#
#    ⚠️ 下面 (a)(b) 两处引的百分数（隐含汇率偏离、汇率贡献 pp 差）**无法在构建期现算**：
#      公司自印的那两列美元不在 series/ase.csv 里，构建期读不到。它们是对**具名、有日期**
#      的官方文件（2025-06 / 2026-06 / 2026-07 三期月度新闻稿）的引证，与 _M_AND_A_NOTE 里
#      引 20-F 的并购贡献额同类，不是随窗口滚动的统计量。落库之后这两段应改为现算。
_SKIP_FX_LINES = (
    '<b>本家的月度公告<u>不是</u>只有新台币 —— 恰恰因为它有官方美元数，这张图才不能画。</b>'
    'ASEH 的月度新闻稿逐月印一张 <code>US$ Million</code> 表（合并与 ATM 各一张，'
    '2026-07 期：合并 US$2,309mn、ATM US$1,487mn），那是<b>公司披露的美元数</b>，'
    '不是谁折出来的；而底座的美元腿只会做「本币 ÷ 外部牌价」。两者并存 = 页上出现'
    '<b>第二个「美元营收」</b>，与我们引为数据源的同一份 PDF 打架，而且折出来的那条'
    '<b>没有任何官方数可以对账</b>。在源文件已经给了答案的地方再折一条自己的线，'
    '是造第二个答案。<b>正确的修法不是画，是把那两列美元落进 '
    '<code>series/ase.csv</code></b>（见页尾「ASEH 有官方月度美元实绩」一条）。')

_SKIP_FX_CONTRIB = (
    '<b>分歧已实测，而且不是四舍五入。</b>用公司自印的合并 US$ 与 NT$ 反解隐含汇率，'
    '与本页那条汇率线（NTD per USD）逐月不一致，<b>符号还在变</b>（2025-06 +0.63%、'
    '2026-06 −0.55%、2026-07 −0.81%），不是一个可以校准掉的常数偏移。传导到本该画的'
    '「汇率贡献 pp」上：2026-07 公司口径 +12.6pp vs 外部牌价推导 +12.8pp（差 0.2pp），'
    '2026-06 公司口径 +6.9pp vs 推导 +8.4pp（<b>差 1.5pp</b>）—— 一张整幅量程只有'
    '十几个点的图，误差能到一个半点，而正确答案本来就印在源文件上。'
    '（这几个数取自 2025-06／2026-06／2026-07 三期月度新闻稿；那两列美元尚未落库，'
    '所以它们是引证、构建期算不出来。）'
    '<b>这条理由碰不到那张汇率线</b>：它咬的是「本币 ÷ 牌价」折出来的那条<b>营收</b>线，'
    '汇率线画的是汇率本身，不进入本家任何一个营收算式。')

_FX_RATE_NOTE = (
    # 断点：本图**不画**本页登记的口径断点，底座 push() 的 _NO_BREAKS 名单挡住了。
    #    早先底座是无条件画的（TSM/alchip 的 breaks 为空，所以从没暴露过），
    #    本轮五家挂 fx 之后四家当场中招，页面等于声称公司并购打断了汇率口径。
    '<b>本图上没有红色竖虚线，这是刻意的。</b>本页登记的两个口径断点是'
    '<b>日月光自己的并表与处分</b>（2020-12 FAFG 并入、2021-12 中国四厂处分），'
    '而新台币兑美元是一条<b>宏观序列</b>，不会因为哪一家公司的合并范围变了而断 —— '
    '所以底座不把它们画到这张图上。那两条线出现在本页其余各图上，'
    '那里它们才成立（见页尾「口径断点与截轴」）。'
    '<b>其次，这条线不是日月光自己用的那个汇率。</b>本家每月自印美元营收，却<b>不披露所用'
    '汇率</b>；拿它自印的两个数反解隐含汇率，与本图这条外部牌价逐月不一致、符号还在变'
    '（2025-06 +0.63%、2026-06 −0.55%、2026-07 −0.81%）。所以本图只能读作'
    '「新台币兑美元的宏观走势」，<b>不能</b>拿它去乘本页任何一根新台币柱来还原公司印'
    '出来的美元数 —— 那正是本页不出「本币 vs 美元」与「汇率贡献」两张图的原因'
    '（逐条理由见页尾）。'
...
_USD_ROADMAP_NOTE = (
    '<b>ASEH 有官方月度美元实绩 —— 本仓七家里唯一的一家。</b>'
    '月度新闻稿逐月印 <code>US$ Million</code> 表（2026-07 期：合并 US$2,309mn、'
    'ATM US$1,487mn）。它现在<b>没有落进 <code>series/ase.csv</code></b>，所以本页的'
    '美元腿不是「画不画」的选择题而是「没有可对账的数」：唯一能画的只有'
    '「本币 ÷ 外部牌价」的构造值，而它与公司自印值的分歧已实测到 1.5pp（见页尾'
    '「本页不出「fx_contrib」那张图」）。'
    '<b>待办</b>：把那两列落库为 <code>revenue_usd_mn</code> / '
    '<code>revenue_atm_usd_mn</code>，之后把 <code>fx_lines</code>／'
    '<code>fx_contrib</code> 从 <code>skip</code> 里拿掉、并设 '
    '<code>fx.implied = False</code> —— 那时页上的美元线是公司披露值，'
    '比台积电那页的推导值更硬。在此之前，页上任何一处都不会出现折出来、'
    '冒充官方值的美元营收数字。')
...
SPEC = {
    ...
    # ── 汇率：**挂 fx（Ex8 出），但显式跳掉美元营收腿那两张（Ex5／Ex6）**。
    #    这是两件事，底座 §1.5 起也把它们分开判（fx_used / usd_leg_shown）：
    #      · Ex8 画的是 NTD/USD **本身**，一条宏观序列 —— 挂同一份 tsm_fx.csv 的每一页
    #        上逐点相同，不需要日月光披露任何东西，所以没有理由跟着 Ex5/Ex6 一起跳。
    #      · Ex5/Ex6 要的是「本币 ÷ 外部牌价」折出来的**公司**序列。本家不画，理由在
    #        skip_note 里逐条写着（公司自印美元数、隐含汇率分歧达 1.5pp），见文件头【5】。
    #    **理由不是「营收不以美元计价」**（20-F 说的恰好相反）。
    #    `implied` 不给（= True）：Ex8 的图注据此说明「本图与月营收公告无关」——
    #    这条线确实是外部牌价，不是公司随公告申报的换算汇率（本家根本不申报）。
    'fx': {
    ...
    # 跳掉的是**美元营收腿那两张**，不是汇率线。逐条理由（底座要求必须给）：
