...
【2】可加总性是**实测**，不是推定（`value.summable = True` 的依据）
    ...
    ⇒ 季度桥 / QTD / YTD / 12 个月滚动合计（汇总表「占 TTM 比重」行）
      这四道加总只在这一列上合法。
...
【12】**为什么不拼前身**（本轮的决定；页面上对应 `_PRE2018_NOTE` 那一条）
    ...
    **逐年的分部口径对照本轮没有做完** —— 所以本页不声称 `revenue_atm_ntd_mn`
    ...
    2017-05~2019-04 的读数）；汇总表「占 TTM 比重」那一行的 12 个月滚动合计
    11 个点跨口径（2018-05~2019-03，牵动 2017-06~2019-03）。
    换来的是 2016-01~2018-04 的 28 格柱。
    ⚠️ 上面这四个日期与两个计数**页面上不写死**：`yoy.TTM_WIN` 或序列首月一变，
      写死的版本会当场说谎而没有任何校验拦得住。
    ⚠️ 2026-09 起页上不再有 12 个月滚动合计**同比**（月营收柱右轴改成单月同比，
      页面所有者的指令），所以这里量的第二样东西换成了汇总表那一行的滚动**合计**
      —— 它只回溯 11 个月，不是 23 个月。给一条页面上不存在的线算损伤是白算。
...
def _splice_damage():
    ...
    **纯算式，不读任何数** —— 断点就是本序列首月，剩下的全由同比的定义决定：
      ...
      · 汇总表「占 TTM 比重」那一行的分母是 [m−N+1, m] 这 N 个月的合计，
        窗口罩住断点两侧（m−N+1 < 首月，即 m ≤ 首月+N−2）就跨口径
        ⇒ [首月, 首月+N−2] 这 N−1 个点跨断点。

    ⚠️ 这第二项原来算的是「N 个月滚动合计**同比**」（两个窗口、2N−1 个点）。
    2026-09 起月营收柱的右轴改成单月同比，页上没有滚动同比线了 —— 那一项等于在给
    一条不存在的线算损伤。真正还要过 N 个月窗口的是汇总表那一行的滚动**合计**。

    「牵动的读数」= 这些点吃进去的最早那个月 ~ 最晚那个月。

    之所以要现算而不是写死：`yoy.TTM_WIN` 哪天从 12 改成别的，写死的点数与日期
    会一起变成假话，而它们出现在页面正文里。
    """
    sp = _span()
    if not sp:
    ...
    d = {
        ...
        'ttm_win': n,
        'shr_lo': b, 'shr_hi': _mshift(b, n - 2), 'shr_n': n - 1,
        'shr_src_lo': _mshift(b, -(n - 1)), 'shr_src_hi': _mshift(b, n - 2),
    }
...
def _split_gap():
    ...
    try:
        ...
        return {
            ...
            'm_pos': int((gm > 0).sum()),
            # 单月一侧本来只报了区间。2026-09 起页上的同比就是单月口径，
            # 这一侧变成主口径，所以补上与滚动一侧同规格的计数，措辞才不用靠推断。
            'm12_n': len(gm[-12:]), 'm12_pos': int((gm[-12:] > 0).sum()),
...
_MAIN_LINE_NOTE = (
    ...
    '<b>金色在本图只有一处</b> —— 右轴那条<b>单月</b>同比线，它是比率不是金额，'
    '不属于任何一段柱。'
    '选合并的四条理由，每条都可复核：'
    ...
    f'汇总表「占 TTM 比重」那一行的 {_Y.TTM_WIN if _Y else 12} 个月滚动合计，'
    '这四道加总只有在这一列上才是合法的。'
    ...
    # ⚠️ 这两个读数原来把滚动口径摆在前面（那时页上的金线就是滚动）。2026-09 起
    #    金线是单月同比，所以先报单月 —— 页面上真的画着的那一个 —— 再把滚动摆在
    #    后面并注明「本页不画，只作对照」。数一个都没换，只换了顺序与标签。
    + (f'<b>代价：合并读不出封测景气。</b>最新月两条列的差已经很大 —— '
       f'<b>单月同比</b>（本图金线的口径）合并 {_GAP["mom_tot"]:+.1f}%、'
       f'ATM {_GAP["mom_atm"]:+.1f}%；'
       f'同一个月的 {_Y.TTM_WIN if _Y else 12} 个月滚动同比（本页不画，只作对照）'
       f'合并 {_GAP["ttm_tot"]:+.1f}%、ATM {_GAP["ttm_atm"]:+.1f}%；'
...
if _GAP and _SHARE_ATM:
    # 「这个差是不是双向的」现在按**单月**一侧判（页上画的就是这一侧），
    # 判据写在用到它的那一句里，这里不再另存一个只在滚动一侧成立的标志位。
    _SPLIT_NOTE = (
        ...
        # ⚠️ 主口径这一段原来量的是滚动同比之差（那时页上的金线就是滚动）。
        #    2026-09 起页上的同比一律是单月，所以主句改量**单月同比之差** ——
        #    读者拿页面上的线去核对的就是这一个；滚动那一侧仍然报，但明说本页不画、
        #    只作对照（它平滑掉月度毛刺，用来看「这个差是不是结构性的」）。
        f'把两列各自的<b>单月同比</b>相减'
        f'（口径统一走 <code>build/yoy.py</code>，与本页金线同口径），'
        f'{_GAP["m_n"]} 个可比月份上「ATM − 合并」落在 '
        f'<b>{_GAP["m_lo"]:+.1f}pp ~ {_GAP["m_hi"]:+.1f}pp</b>，'
        f'其中 {_GAP["m_pos"]}/{_GAP["m_n"]} 个月为正。'
        + (f'<b>注意这个差是双向的</b>：ATM 并非总是跑得更快，'
           f'历史上合并口径最多曾高出 ATM {abs(_GAP["m_lo"]):.1f}pp'
           f'（EMS 侧强、封测弱的那几年）—— 所以「把合并线当封测读一定会低估」'
           f'这句话在全历史上是假的，只在当下这一段成立。'
           if _GAP['m_pos'] not in (0, _GAP['m_n']) and _GAP['m_lo'] < 0 else '')
        + f'当下这一段：最近 {_GAP["m12_n"]} 个月里 '
          f'{_GAP["m12_pos"]}/{_GAP["m12_n"]} 个月 ATM 更快，'
          f'差 {_GAP["m12_lo"]:+.1f}pp ~ {_GAP["m12_hi"]:+.1f}pp。'
        + f'<b>本页不画的那个对照口径</b>（{_Y.TTM_WIN if _Y else 12} 个月滚动同比，'
          '它把月度毛刺抹平，用来看这个差是不是结构性的）在同一批数据上更窄：'
          f'全历史 {_GAP["t_n"]} 个月 {_GAP["t_lo"]:+.1f}pp ~ {_GAP["t_hi"]:+.1f}pp'
          f'（{_GAP["t_pos"]}/{_GAP["t_n"]} 个月为正），最近 {_GAP["t12_n"]} 个月 '
          f'{_GAP["t12_lo"]:+.1f}pp ~ {_GAP["t12_hi"]:+.1f}pp'
...
_FX_RATE_NOTE = (
    ...
    # ⚠️ 这句原来写的是「那两条竖线出现在本页其余各图上」—— **其余各图**是假的：
    #    热力矩阵那张一条都没有（`mrbase.apply_breaks()` 对 heat_matrix 直接返回空，
    #    引擎的 drawHeat() 也读不到 break_at），页尾那条清单同样只列画了线的那几张。
    #    改成指路页尾那份**现读 payload 生成**的清单，不在这里另开一个全称断言。
    '所以底座不把它们画到这张图上。那两条竖线画在本页哪几张图上，'
    '页尾「口径断点与截轴」那一条逐张点名（那份清单现读 payload，不是手抄的）。'
...
if _DMG:
    _PRE2018_NOTE += (
        ...
        f'汇总表「占 TTM 比重」那一行的分母是一个 {_DMG["ttm_win"]} 个月的合计窗口 ⇒ '
        f'<b>{_DMG["shr_n"]} 个点</b>（{_DMG["shr_lo"]}~{_DMG["shr_hi"]}）跨口径，'
        f'牵动 {_DMG["shr_src_lo"]}~{_DMG["shr_src_hi"]} 的读数。'
...
# 也不写「最新月占比」这种每期都要改的数（`_SHARE_*['cur']` 一次都没用）。
# 出现的数只有两类：把定义钉住的**结构性**量（分部占比的全历史区间、隐含汇率的
# 全历史区间与它超出舍入上界的月份数）与恒等式本身（ATM + 非 ATM ≡ 合并，
# 现算残差 = 0）—— 且**一个都不写死**，全部在 import 期从 series/ase.csv 现算
# （同本文件其余图注的做法：`_SPAN` / `_SHARE_ATM` / `_SHARE_NON` / `_ID` / `_IMP`）。
#
# ━━ 为什么是这 11 个词（选词判断）━━
# 判据只有一条：这个词出现在本页的图题 / 序列名 / 纵轴 / 汇总表行头 / 核对表列头里，
# 而且**不看定义就会读错**。按「读错会出什么事」分四类：
#   ① 这一页的主体划分（本页最贵的坑）  合并营收 / ATM（封测及材料）/ 非 ATM。
#      月度新闻稿只印**两个** NT$ 数（合并总额、ATM 分部），第三个是减出来的残差；
#      而柱图把两段并排堆叠、占比图把两条线并排画，视觉上像是两个对等的官方分部。
#      不点破，读者会把「合并」的增速当封测景气读（页尾实测：这个差是**双向**的，
#      并非「合并一定低估封测」），或把「非 ATM」当成公司披露的 EMS 分部去对季报。
#   ② 两条货币腿与它们中间那个**不存在的**恒等式  US$ revenue / 汇率贡献 /
#      隐含汇率 / 外部牌价。本家是全仓唯一「以新台币入账、却另外自印官方美元实绩」
#      的一家：两列都是官方值，中间却没有任何一个汇率把它们绑起来（公司从不披露
#      所用汇率，`fx.rate_filed = False`）。不点破，读者会拿页上那条外部牌价去乘
#      新台币柱来「还原」美元线（还原不出来，页尾有实证），或把 Ex9 那条红线
#      当成「公司申报的换算汇率」——而它只是两个印出来的数之商。
#   ③ 与页上主口径不同的那条时间轴  季度同比。本页的月度同比只有单月一种，
#      唯独季度桥那张图是 3 个月比 3 个月，两者**不可对读**；而且它的季度值是
#      月营收直接相加，与公司季报口径不完全相等（季报含其他收入项）。
#      释义里必须把「未满季」限定在**末季**：引擎的浅蓝柱 + 作废 y/y 只管末季，
#      左缘那个不满 3 个月的残季是**整季不画**（Ex3 图注里那句，两处不许打架）。
#   ④ 汇总表里三类容易被当成「另一个指标」的派生行  三月均值、QTD / YTD、
#      占 TTM 比重。三月均值的**水平值**是字面意思，但它在汇总表里带出来的 m/m 与
#      y/y 是**两个三月均值之比**，不是页面统一的单月口径 —— 与相邻那几行的同一列
#      不同时间基，页上没有第二处说这件事（页尾第 2 条把汇总表 y/y 列整列算成单月）。
#      QTD / YTD 是期内累计（m/m 与分位两列因此一律留空，那是口径原因不是缺数），
#      占 TTM 比重的分母是一个 12 个月的滚动合计（分母自己在动，占比掉了不等于营收掉了）。
# **有意不收**：
#   · m/m、y/y、3Y %ile、pp/bp —— 全站通用的读图约定，summary.note 已逐条讲过，
#     释义板再讲一遍就是两处各写一份（样板 /cost/ 的 9 条里一条都没有这类）。
#   · **单月同比** —— 它是 CONTRACT §6.1 给流量序列定的全站默认口径，本页不是特例；
#     页尾第 2 条把用到它的每一处逐个点了名、Ex2 的标题与图注印着它的代价（本序列
#     实测），这里再定义一遍只会与那两处争着当权威。季度同比要收，正因为它**是**
#     本页唯一的例外。
#   · **口径断点** —— 页尾「口径断点与截轴」那一条现读 payload 列出它画在哪几张图上，
#     另外两条（门槛 3%、跨断点是伪同比的幅度）把量化的部分说完了，图上还有红线本身。
#   · 未经查核、次月 10 日前公布这类**字面即定义**的词，以及成交量、
#     市值这类本页没有特殊口径的常识词。
#     （**三月均值本轮改为收**：早先把它归进「字面即定义」而不收 —— 那句话对它的
#     水平值成立，对它在汇总表里的 m/m 与 y/y 两列不成立，同一页上 QTD / YTD 的
#     同类坑却是收了的，两处标准不一致。）
# ══════════════════════════════════════════════════════════════════════════════
#: 滚动窗口宽度只此一处取值：`yoy.TTM_WIN` 哪天不是 12，下面三条释义一起跟着改。
_G_TTM = _Y.TTM_WIN if _Y else 12

_GLOSSARY = [
    # ① 主体划分 —— 三条连着读：先钉住主序列是哪一列，再说另外两列的来历。
    ('合并营收',
     '公司月度新闻稿印的 <code>CONSOLIDATED NET REVENUES</code>（NT$mn，'
     '未经会计师查核，台湾法定次月 10 日前公布），<b>含全部分部</b>。'
     '本页的<b>主序列就是它</b>：柱高、右轴同比、QTD／YTD、占 TTM 比重、季度桥、'
     '热力矩阵，全部由这一列派生。选它的两条硬理由：在<b>三个互相独立的源</b>上'
     '都取得到并对得上（公司新闻稿 PDF、TWSE MOPS 全市场月表、TWSE OpenAPI '
     '<code>t187ap05_L</code>），且逐月加总<b>无需任何调整</b>就等于 20-F 的'
     '年度合并营收（SEC XBRL，逐年对过账）。'
     '⚠️ <b>它是这家控股公司的增速，不是封测的增速</b> —— 差多远、什么时候'
     '反过来，见页尾「合并线与 ATM 线的差距」一条。'),

    ('ATM（封测及材料）',
     '公司月度新闻稿逐月印的<b>唯一一个分部数</b>（<code>ATM NET REVENUES</code>，'
     '2018-05~2018-12 期叫 <code>IC-ATM</code>）。它是<b>分部基础</b>、'
     '<b>含分部间交易</b>，因此<b>对不上</b>合并利润表里的任何一行，也不等于'
     '合并利润表的 Packaging ＋ Testing ＋ Others。'
     + (f'占合并的实测区间 <b>{_SHARE_ATM["min"]:.1f}% ~ {_SHARE_ATM["max"]:.1f}%</b>'
        f'、中位 {_SHARE_ATM["med"]:.1f}%（{_SHARE_ATM["n"]} 个月现算）。'
        if _SHARE_ATM else '')
     + '⚠️ 这一列<b>被追溯重述过</b>（2019 年年中那几个月，当期 ATM 表标题带星号'
       '说明），而合并列在同批新闻稿里一处没动 —— 所以主序列取的是合并那一条。'
       '公司每月也印 ATM 的<b>美元</b>数并已落库，但底座的外币腿只有一条，'
       '它<b>不在任何一张图上</b>。'),

    ('非 ATM',
     '页上写作「Non-ATM (EMS & others)」，<b>不是公司披露的数</b>，是本页减出来的'
     '<b>残差</b>：<code>非 ATM ＝ 合并 − ATM</code>'
     + (f'（{_ID["n"]} 个月上与这个减法逐月恒等，现算最大绝对差 '
        f'{_ID["max"]:.0f} NT$mn）' if _ID else '')
     + '，实质是「EMS 分部 ＋ Others − ATM 分部间抵销」。'
       '⇒ 它<b>不等于</b>合并利润表的 EMS 行，也不等于季报附注里的 EMS 分部基础数，'
       '叫它「EMS」省事但那是在口径上说谎。它的任何异动都可能只是 ATM 那一侧的'
       '口径动了（含上面那次追溯重述），与 EMS 的经营无关。'
     + (f'占合并的实测区间 <b>{_SHARE_NON["min"]:.1f}% ~ '
        f'{_SHARE_NON["max"]:.1f}%</b>、中位 {_SHARE_NON["med"]:.1f}%'
        f'（{_SHARE_NON["n"]} 个月现算）—— 「EMS 大约占三分之一」这句话'
        f'对本序列的多数年份是错的。' if _SHARE_NON else '')),

    # ② 两条货币腿 —— 先说那一列是什么，再说两条腿之差，最后说两条汇率线各是什么。
    ('US$ revenue',
     '公司在<b>同一份</b>月度新闻稿<b>同一页</b>自印的 <code>US$ Million</code> 表'
     '（合并与 ATM 各一行），是<b>第二次独立披露</b>的官方读数，'
     '<b>不是</b>本页拿新台币除以汇率折出来的 —— 页面一次折算都没有做。'
     'ASEH 的<b>功能货币是新台币</b>，所以这一列是公司另行申报的实绩，'
     '不是它的记账本位（与世芯-KY 那页正好相反，那家的美元栏是计量本身）。'
     '⚠️ 两列金额之间<b>没有恒等式</b>：公司从不披露自己用的换算汇率，'
     '「本币 ≡ 美元 × 汇率」对本家不成立，本页也没有任何一处算式依赖它。'
     '页上的美元腿<b>只有合并</b>那一条，美元那两张图不能按分部拆着读。'),

    ('汇率贡献',
     '<code>新台币单月同比 − US$ 单月同比</code>，单位是<b>百分点</b>不是百分比。'
     '因为两头的<b>金额都是公司自印</b>的官方值，这根柱是<b>观测值</b>，'
     '<b>不是</b>任何折算假设的产物（别家没有官方美元实绩时，只能拿本币除以外部'
     '牌价折一条出来，那种柱依赖「全部营收按当月平均汇率一次性折算」这个假设；'
     '本页把那种折法在本家的误差量级现算在图注里）。'
     '⚠️ 它读作「以新台币计的那个头条数被汇率抬高／压低了多少」，'
     '<b>不是公司受到的汇率冲击</b> —— 真正的暴露在成本端与对冲上，'
     '月营收公告看不见。'),

    ('隐含汇率',
     '「NTD per USD」那张图上的<b>红线</b>：把公司自印的合并 NT$ 营收除以'
     '同一份公告里的合并 US$ '
     '营收，逐月得到的<b>比值</b>（页上线名写作「NT$ ÷ US$ revenue as printed」）。'
     + (f'{_IMP["n"]} 个月落在 <b>{_IMP["lo"]:.2f}</b>（{_IMP["lo_m"]}）到 '
        f'<b>{_IMP["hi"]:.2f}</b>（{_IMP["hi_m"]}）之间、中位 {_IMP["med"]:.2f} —— '
        f'量级像汇率，但与外部牌价逐月对不上、符号还在翻：'
        f'{_IMP["n"]} 个月里有 <b>{_IMP["over_noise"]} 个月</b>的偏离'
        f'超出了「两列都是整数、各错半个单位」所允许的舍入上界。'
        if _IMP else '它与外部牌价逐月对不上，且符号在翻。')
     + '⚠️ <b>它不是「公司申报的换算汇率」</b>：公司从不披露所用汇率，本页也不替它'
       '申报一个。它只说明「每月一个统一汇率」这个假设与数据相容，而<b>相容不等于'
       '唯一</b> —— 公司也可能按币别或子公司分别换算后加总，那样它就是一个营收加权'
       '的复合比率，不是牌价表上的某一个数。'),

    ('外部牌价',
     '「NTD per USD」那张图上的<b>深藏青线</b>：'
     '美联储 H.10 台湾地区日度牌价按月算术平均'
     '（存于 <code>series/tsm_fx.csv</code>，本站挂同一份汇率的每一页上逐点相同）。'
     '它是一条<b>宏观序列</b>，与日月光披露什么无关，'
     '<b>更不是</b>公司申报的换算汇率（<code>fx.rate_filed = False</code>）。'
     '⇒ 页上那两条营收腿<b>都不由它折出</b>；拿它去乘新台币柱，'
     '<b>还原不出</b>公司自印的那条美元线（这不是精度问题，实证见那张图自己的图注）。'),

    # ③ 与页上主口径不同的那条时间轴。
    ('季度同比',
     '<b>本页唯一不是单月口径的同比</b>，只出现在季度桥那一张图：'
     '<code>当季 3 个月合计 ÷ 去年同季 3 个月合计 − 1</code>。'
     f'它既不是单月也不是 {_G_TTM} 个月滚动，'
     '<b>不与</b>页上其余同比（柱图右轴、本币 vs 美元、热力矩阵、两张表的 y/y）'
     '<b>对读</b>。季度值本身是<b>该季已公布月份的 NT$ 营收直接相加、不作任何调整</b>；'
     '<b>末季</b>未满 3 个月时柱画成浅蓝、同比整点作废（1~2 个月比上年完整 3 个月'
     '必然砸出一个假坑）。⚠️ <b>这套未满季处理只管末季</b>：序列<b>左缘</b>那个'
     '不满 3 个月的残季是<b>整季不画</b>，不是浅蓝柱 —— 所以这张图的首格晚于'
     '序列起点，别照着上面那条规则去左缘找一根浅蓝柱。'
     '⚠️ 它是「用月营收抢跑季报」，但<b>季报口径含其他收入项</b>，'
     '与本页这条加总不完全相等。'),

    # ④ 汇总表里三类派生行。
    ('三月均值',
     '汇总表的「3-month moving avg.」那一行：<code>含当月在内的连续 3 个月合并营收的'
     '算术平均</code>，用来把单月噪音抹平（水平值仍是 NT$bn，与上面几行的金额同一'
     '把尺）。⚠️ <b>但这一行的 m/m 与 y/y 两格是<u>两个三月均值之比</u></b>，'
     '<b>不是</b>本页其余各处的<b>单月</b>口径 —— 分子分母各自都已经是 3 个月的平均，'
     '所以它<b>不与</b>柱图右轴的同比、本币 vs 美元那两条线、热力矩阵，以及同一张表里'
     '按单月算的那几行 y/y <b>对读</b>；同一列上它与上面那几行差出几个点，是'
     '<b>口径差不是矛盾</b>。'),

    ('QTD / YTD',
     '<b>期内累计</b>：QTD ＝ 本季<b>已公布月份</b>的合并营收相加，YTD ＝ 本年迄今'
     '相加（直接相加、不作调整 —— 合并这一列可加总，与 20-F 年度值对得上）。'
     '⚠️ 汇总表这两行的 <b>m/m 与 3Y %ile 两列一律留空，那是口径原因不是缺数</b>：'
     'm/m 只是「上月累计 ＋ 当月营收」这个算术恒等式，正负号只反映日历位置；'
     '分位则由「本月是期内第几个月」决定，与经营好坏无关。这两行的可比读数是 y/y'
     '（拿<b>同样月数</b>去比上年同期的<b>前同样月数</b>）。'),

    ('占 TTM 比重',
     f'<code>当月合并营收 ÷ 最近 {_G_TTM} 个月合计</code>，用来做<b>季节性定位</b>'
     f'（这个月在一年里算大月还是小月），<b>不是增速</b>。'
     f'⚠️ <b>分母自己也在动</b>：某个月的绝对额一格没变、而过去 {_G_TTM} 个月的'
     f'合计涨了，这个比重照样往下走 —— 掉了不等于那个月变差了。'
     f'它要 {_G_TTM} 个月的历史才算得出，所以序列头 {_G_TTM - 1} 个月没有值；'
     f'跨口径断点的那几期，分子分母不在同一个合并范围里。'),
]
...
SPEC = {
    ...
    # 上面那一块注释里（本页与别家共用底座，但**词不共用** —— 七家的口径各不相同）。
    'glossary': _GLOSSARY,
