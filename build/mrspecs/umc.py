...
与仓库现有描述不一致的两处，在对应的 `_*_NOTE` 里点了名（见 ② 与 ⑤）。
  ...
  ④ **月营收 = 合并数**（不是母公司数），可加总，逐季逐年对得上：
     ...
     （逐年 diff 在**构建期现算**，不写死）。
     ⇒ `value.summable = True` 有实测撑着，季度桥 / YTD / TTM 合计在本页全部合法。
...
if _NYR and _GAP_MAX is not None:
    _ADDITIVE_NOTE = (
        f'<b>本页的季度聚合、YTD 与 TTM 滚动合计都有实测撑着，不是推定。</b>'
    ...
    _ADDITIVE_NOTE = (
        '<b>本页的季度聚合、YTD 与 TTM 滚动合计都是合法的。</b>联电的功能货币与表达货币'
...
def _first_clean(break_month):
    """给一个断点月，算出各口径「用到的每一格都落在断点右侧」的第一期。

    单月同比 = 当月 ÷ 去年同月            ⇒ 分母 ≥ 断点 ⇒ m ≥ 断点 + 12；
    季度同比 = 3 个月比 3 个月、去年同季   ⇒ 断点所在季 + 4 个季；
    汇总表「占 TTM 比重」= 当月 ÷ 近 12 个月合计 ⇒ 窗口首月 = m − 11 ⇒ m ≥ 断点 + 11。

    ⚠️ 这里原来还有一项 `'ttm': 断点 + 23`（12 个月滚动合计**同比**）。2026-09 起
    月营收柱右轴改成单月同比，页上没有任何一条滚动同比线了 —— 留着它会让页面把一个
    不存在的口径列进「按口径各不相同」那句话里。真正还要跨断点的滚动量是汇总表那一行
    12 个月合计，它只回溯 11 个月，不是 23 个月，所以换成 `ttm_share` 重算。
    """
    y, m = int(break_month[:4]), int(break_month[5:7])
    t = y * 4 + (m - 1) // 3 + 4
    return {'yoy': _shift(break_month, 12),
            'ttm_share': _shift(break_month, 11),
            'qtr': f'{t // 4}Q{t % 4 + 1}'}
...
if _C:
    _WINDOW_BREAK_NOTE = (
        ...
        f'单月同比 <b>{_C["yoy"]}</b>、季度同比 <b>{_C["qtr"]}</b>，'
        f'汇总表「占 TTM 比重」那一行 <b>{_C["ttm_share"]}</b>'
        '（分别要回溯 12 个月、4 个季，和 11 个月的滚动合计窗口）。'
    ...
    _WINDOW_BREAK_NOTE = (
        ...
        '单月同比要回溯 12 个月、季度同比要回溯 4 个季、'
        '汇总表「占 TTM 比重」那一行要回溯 11 个月。')
