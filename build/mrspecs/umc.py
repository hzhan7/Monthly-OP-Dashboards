...
#
# ━━ 与页尾 notes / 图注的分工 ━━
# notes 与图注说的是「这一张图**这个月**该怎么读」（含当月读数、当月实测的口径差）；
# 这一块说的是「这些词是什么意思」，一年到头是同一段 ⇒ 这里**不写当月读数**，
# 也不写任何随月份变的数（覆盖月数、最新一期、当月同比一个都不出现）。
# 出现的数只有两类：把定义钉住的**结构性**量（公告的单位、核对表保几位小数、
# 1/12 这个季节性基准线）与由**一次可查证的公司行为**唯一确定的日期
# （2015-06-01 Topcell 并入茂迪、2019-10-01 USJC 交割 —— 两者都已是本文件顶上的
# `_BRK_*` 常量，出处见文件头 ③ / ④，这里直接引用常量，不另抄一份）。
# ⚠️ 页尾 notes 里那些现算的读数（逐年 diff、三段同比均值、分部两栏、汇率区间高低点）
# **一个都不搬进来**：它们跟着 CSV 走，搬过来就是同一件事两处各写一份，
# 而改口径时只改得动一处。
#
# ━━ 为什么是这 11 个词（选词判断）━━
# 判据只有一条：这个词出现在本页的图题 / 纵轴 / 汇总表行头 / 核对表标题 / 图注里，
# 而且**不看定义就会读错**。按「读错会出什么事」分五类：
#   ① 这一页唯一那个官方字段和它的单位链 —— 合并净销售额 / 官方原始单位 /
#      累计差反算。读错的代价：把母公司数当合并数、把千元当百万（差三个数量级）、
#      或者以为核对表那三位小数是我们算出来的（它们是公告原文的千元位）。
#      「累计差反算」尤其要点破：页上印的月值与公告某一行印的单月数**可能不等**，
#      这是有意的（公司印错过），不点破就是一个查得到、解释不了的对不上。
#   ② 汇总表里三条**不是「本月 vs 上月」**的行 —— 3 个月移动平均、QTD / YTD、
#      占 TTM 比重。QTD/YTD 的分子分母随「本季已公布几个月」滑动，占 TTM 的分母是
#      滚动窗口、而且它根本不是增长率。这两类读成同比就会得出方向相反的结论。
#      三月均值那一行则是**同一张表里的第二种 y/y**：`ds.ma3` 是含当月在内的尾部
#      三月均值，它那一格 y/y 是**均值比均值**、不是单月同比，两者在同一列里并排印着
#      （页顶 brief 还把两个数直接放在一起比），不点破就会被当成同一口径的两个读数。
#   ③ 本页两条时间腿的口径差 —— 季度值 / 季度同比。月度那一侧是**单月**同比，
#      季度那张图是**3 个月比 3 个月**，两条线并排出现在同一页上；
#      不点破，读者会把两条同比线当成同一条序列的粗细两版互相印证。
#   ④ 让「同比」在某几段失效的两件公司行为 —— USJC、新事业分部。
#      它们是本页仅有的两条口径断点的**实质内容**（红线只给月份，不给内容）；
#      不知道 USJC 是什么，就只会看到一段没有理由的加速。
#   ⑤ 汇率那条线的身份 —— NTD per USD、便利折算。这一页上有汇率、**没有美元营收**，
#      而 20-F 里确实印着一列美元 —— 不说清那列是年末单一牌价折出来的便利数，
#      读者会以为页上少画了一条本来有官方值的线，或者自己拿本币除牌价补一条。
#
# **有意不收**：
#   · m/m、y/y、3Y %ile、pp/bp —— 全站通用的读图约定，本页 summary.note 已逐条讲过，
#     释义板再讲一遍就是两处各写一份（/cost/ 那 9 条里一条都没有，是同一条判断）。
#   · 「口径断点」「红色竖虚线」这两个词本身 —— 页尾「口径断点与截轴」一条讲的是
#     它们在本页的具体落点（哪张图上有、画在哪一期左缘、heat_matrix 为什么没有），
#     那是逐图的事实、且由 payload 现读生成；这里只解释断点的**内容**（④ 那两条）。
#   · 「单月同比」不单列 —— 它是全站默认口径（CONTRACT §6.1 第 1 条），页上唯一需要
#     提防的是它与季度同比并存，所以写在 ③ 那条里，不另立一条讲「同比是什么」。
#   · 月营收、季节性这类没有本页特定口径的常识词。
#   · 页面上根本没出现的词（分部营收、产能利用率、ASP —— 月度公告里没有这些字段，
#     页尾「本页没有指引桥，也没有分部线」讲的正是它们为什么不在页上）。
# ══════════════════════════════════════════════════════════════════════════════
_GLOSSARY = [
    ('合并净销售额',
     '本页<b>唯一</b>的官方披露字段（公告行名 <code>Net sales</code>）：联电按台湾法规在'
     '次月 10 日前公告的<b>合并</b>月度净销售额，<b>未经会计师查核</b>。'
     '它是<b>合并数、不是母公司数</b> —— 季报分部附注的'
     '「Wafer Fabrication + New Business = Consolidated」栏与本表相邻三个月相加逐字相等。'
     '联电的功能货币与表达货币<b>均为新台币</b>，月值是原生记账数、不是折算值，'
     '所以月值可以相加，季度／YTD／TTM 合计都合法'
     '（逐年、逐季的对账见页尾「本页的季度聚合、YTD 与 TTM 滚动合计…」一条）。'
     '⇒ 本页各图与两张表由这一个字段<b>加一条月均汇率序列</b>派生（口径与页尾'
     '「数据源」一条相同）：汇率那张图与核对表的汇率列<b>就是那条外部序列</b>，'
     '<b>不由本字段派生</b>，其余各处都由它派生。'),

    ('官方原始单位',
     '公告的金额单位是 <b>NT$ 千元</b>（抬头逐字 '
     '<code>1) Sales volume (NT$ Thousand)</code>），<b>不是</b>百万、更不是十亿。'
     '本页序列存 NT$mn（= 公告千元 ÷ 1,000），各图按 NT$bn 显示；'
     '核对表的金额列标着「官方原始单位、未换算」，所以那一列<b>保留三位小数</b> —— '
     '那三位就是公告的千元位，舍掉等于一边宣称未换算、一边丢掉三位官方数字。'),

    ('累计差反算',
     '本页每个月的金额<b>不是</b>直接取公告印的那个单月数，而是由公司公布的'
     '<b>本年累计</b>逐月相减得到（序列因此存两列：当月净销售额与公司公布的本年累计）。'
     '理由是公告本身出过错：单月数印错过一次，行标签与 “for the period of …” 也串过月份 —— '
     '实例与四路对账见页尾「数据源与落库口径」一条。'
     '⇒ <b>判月只认累计链，不认标签</b>，月值取的也是累计链算出来的数。'
     '已知<b>只有一处</b>（2016-05）与公告印的单月数不等，那一处的四路对账里三路指向'
     '累计差，页上取的是累计差值 —— 这是<b>个案</b>，<b>不是</b>「凡不等就是公告印错」'
     '的通则（逐条见页尾「数据源与落库口径」）。'),

    ('3 个月移动平均',
     '汇总表 <code>3-month moving avg.</code> 一行：<b>含当月在内</b>的近 3 个月算术平均'
     '（<b>尾部</b>窗口，不居中；序列头两个月不足窗口，留空）。它把单月噪音抹平，'
     '看的是水平的趋势位置。'
     '⚠️ 这一行的 y/y 是<b>均值比均值</b>（近 3 个月均值 ÷ 去年同 3 个月均值 − 1），'
     '与上一行 Monthly revenue 的<b>单月</b>同比<b>不是同一个口径</b> —— '
     '两个数不必相等，也<b>不能</b>拿来互相印证。'),

    ('QTD / YTD',
     '汇总表的两行<b>期内累计</b>：QTD 是本季<b>已公布</b>月份的合计，YTD 是本年已公布'
     '月份的合计，都由页内月值逐月累加，<b>没有把未公布的月份外推</b>。'
     '两行的 y/y 比的是「同一批日历月对同一批日历月」（本季已公布 n 个月 '
     'vs 上年同季<b>前同样 n 个月</b>），所以季内进度可以直接读。'
     '这两行的 m/m 与分位两列一律留空 —— 口径原因见汇总表下方的表注。'),

    ('占 TTM 比重',
     'TTM = trailing twelve months，<b>含当月在内</b>的近 12 个月滚动合计。'
     '这一行是 <code>当月营收 ÷ 近 12 个月合计</code>，量的是这个月在一整年里占多重'
     '（<b>季节性定位</b>），<b>不是增长率</b>：月份平均分布时读数是 1/12 ≈ 8.3%，'
     '高于它就是旺月。⚠️ 分母是<b>滚动窗口</b>，每往后一个月就换一批 12 个月，'
     '所以两个不同月份的读数并不共用同一个分母。'),

    ('季度值 / 季度同比',
     '季度那张图的柱是<b>该季已公布月份的营收直接相加</b>，不做任何调整；'
     '末季不满 3 个月时画成<b>浅蓝</b>柱。右轴是严格的 <b>3 个月比 3 个月</b>'
     '（当季 3 个月合计 ÷ 去年同季 3 个月合计 − 1），分子或分母任一边不满 3 个月'
     '就整格留空，不印一个「n 个月比 3 个月」的数。'
     '⚠️ 月度那一侧的同比是<b>单月</b>（当月 ÷ 去年同月 − 1）：月度柱图的右轴金线、'
     '热力矩阵、核对表的 y/y 列与汇总表 <code>Monthly revenue</code> 一行都是它'
     '（汇总表三月均值那一行是均值比均值，见上面那条），它与这条季度线'
     '<b>不是同一个口径、不能对读</b>。另外，季报口径含其他收入项，与本表的月营收合计'
     '<b>不完全相等</b> —— 这张图是用月营收抢跑季报，不是季报的替代。'),

    ('USJC',
     'United Semiconductor Japan Co., Ltd.，原<b>三重富士通半导体</b>'
     '（Mie Fujitsu Semiconductor，MIFS），联电与富士通的 300mm 合资晶圆厂，'
     '联电自 <b>2019-10-01</b> 起 100% 并表。'
     f'它是本页两条口径断点之一：{_BRK_USJC} 起的 12 个月，同比里含一块'
     '<b>并表带进来的无机增量</b>，不是原有业务自己的增长（三段并排的均值见页尾那一条，'
     '那里也写明了为什么不能拿「中间减两边」当作无机增量的估计）。'),

    ('新事业分部',
     '月营收公告是合并数，2015 年 6 月以前一直含着与晶圆代工无关的「新事业」'
     '（<code>New Business</code>）分部 —— 太阳能／LED 这类业务。'
     'Topcell Solar 于 <b>2015-06-01</b> 并入茂迪 Motech 后不再合并，'
     f'这就是本页另一条口径断点（{_BRK_NEWBIZ}）。'
     '⚠️ 它只对应 <b>Topcell 一家子公司</b>的除列，<b>不是</b>整个新事业分部在那一个月'
     '消失：该分部 2016 年仍有对外营收（页尾那条给了数）。'
     '另一头，FY2025 20-F 附注 12 写明公司<b>只剩晶圆代工一个报告分部</b>，'
     '而月度公告从来只有一个数 —— 所以本页没有任何分部行、分部线。'),

    ('NTD per USD',
     '汇率那张图的纵轴：1 美元兑多少新台币的<b>月均</b>牌价（美联储 H.10 台湾地区'
     '日度牌价在该月营业日上的算术平均）。<b>数字变大 = 新台币贬值</b>。'
     '⚠️ 它是一条<b>宏观序列</b>，本站挂同一份汇率的每一页上逐点相同，'
     '<b>不是联电的任何营收量</b>；本页没有把它与营收线相乘或相除，'
     '这张图上也<b>没有断点红线</b> —— 联电自己的合并范围变化不会改一条外部牌价的口径。'),

    ('便利折算',
     'convenience translation：20-F 里那一列美元<b>不是逐月实绩</b>，是把整列新台币按'
     '<b>年末单一即期牌价</b>折出来的便利数（附注 4(7) 明写 for the reader’s '
     'convenience，并声明不作任何陈述）。联电<b>不按月披露美元营收</b>，'
     '所以本页只有汇率本身、<b>没有美元营收腿</b>：拿页内的新台币月值去除外部月均牌价'
     '确实折得出一条「US$ 营收」曲线，但那是<b>分析师构造值</b>，'
     '没有任何官方月度美元数可以对账。⇒ 不画（逐条理由见页尾那两条）。'),
]
...
SPEC = {
    ...
    # 页顶「名词释义」。选词判断与「有意不收哪些词」写在 _GLOSSARY 上面那一块注释里。
    # 用静态 list 而不是 callable(ds)：这 10 条里没有一个数需要从 CSV 现算 ——
    # 结构性的那几个（公告单位、核对表的三位小数、1/12 的季节性基准线）是定义本身，
    # 两个日期是文件顶上的 `_BRK_*` 常量（出处见文件头 ③ / ④）。
    # 凡是跟着数据走的读数一律留在页尾 notes 里现算，不在这里抄第二份。
    'glossary': _GLOSSARY,
