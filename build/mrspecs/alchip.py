...
0.000008%，逐年全中。底座的季度桥 / QTD / YTD / TTM / 12 个月滚动同比全要过加总，
⇒ `value` 只能是美元列，新台币列进 `alt`（`summable: False`，只进核对表）。

━━ Ex5 / Ex6 / Ex8：本家是七家里唯一**两条腿都是官方值**的一家 ━━━━━━━━━━━
TSM 那张 Ex5 的美元线是外部牌价折的推导值；本家两条线来自官方同一张表，
所以这张图是七家里质量最高的一张。因果方向也相反，图注里写明：
...
_ALT_NOTE = (
    ...
    '所以它不进季度桥、不进 QTD/YTD、不进 12 个月滚动同比。'
...
def _win_note():
    ...
    return ('<b>短窗口图自 2016-01 起是<u>主动选的窗口</u>，不是数据边界</b>：'
            ...
            '不是没有数 —— 本轮七家统一取 2016-01 起，为的是跨家看同一段时间轴。'
...
SPEC = {
    ...
    'window': {
        # 本轮七家统一自 2016-01 起，为的是跨家看同一段时间轴。
