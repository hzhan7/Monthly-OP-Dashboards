...
━━ 与 /tsm/ 图列的对照（本家出八块，缺的两张是**结构性**缺，不是偷懒）━━━━━━━
    TSM 页              本页        本页的处置
    Ex1  汇总表         Ex1         同，且**多两行**：官方逐月给的 Turnkey / NRE 拆分
    Ex2  gs_bar         Ex2         同（右轴 12 个月滚动合计同比）
    Ex3  qtr_bar        Ex3         同
    Ex4  gs_line m/m    Ex4         同
    Ex5  NT$ vs US$     —           **显式 skip**：要一条美元营收腿，本家没有官方美元数
    Ex6  汇率贡献       —           **显式 skip**：在本家它退化成汇率自己的重画（见 D）
    Ex7  全历史 lines   Ex5         同，且**多两条线**：Turnkey / NRE 与合并线同图
    Ex8  月均汇率       Ex6         **照出**：画的是汇率本身，不需要公司披露任何东西（见 D）
    Ex9  heat_matrix    Ex7         同
    Ex10 核对表         Ex8         同，且**多两列**（Turnkey / NRE 的官方原始单位）
                                    与一列 NTD/USD（Ex8 那条线的逐月读数）
...
D. **有汇率线（Ex8），没有美元腿（Ex5/Ex6 显式 `skip`）—— 这是两件事，本轮把它们拆开了。**
   本页此前把 `fx` 整个留空，于是底座把三张图捆在一起跳掉。那对 Ex5/Ex6 是对的，
   对 Ex8 是错的：**Ex8 画的是汇率本身**，一条宏观序列（`series/tsm_fx.csv`），
   挂同一份汇率的每一页上逐点相同，不需要本家披露任何东西。判据见底座 §1.5。

   Ex5/Ex6 该跳的理由要说准，不是「公司没有外币敞口」——
   ...
   · 于是 `NT$ ÷ 牌价` 折出来的那条「美元营收」暗含一个**100% 以美元计价**的假设。
     ...
     **既对不了账、也定不了标**，Ex6 的「汇率贡献 pp」会按这个错误占比被整体放大。
...
_NOTE_SOURCE = (
    ...
    '<b>⚠️ 上面「数据源」那条说「本页各图与两张表全部由这一个字段…派生」'
    '（月均汇率那张图是它自己点名的例外，来源另写在该图图脚），在本家要这样读</b>：'
...
#      现在页上多了一条汇率线（Ex8），底座会在页尾自己讲清「有汇率线、没有美元腿」这件事，
...
_NOTE_HEAT = (
    ...
    '本家的单月同比分布被 NRE 里程碑拉得很宽（某个月三分之二营收来自一次性委托设计，'
    '下个月几乎全是量产晶圆），而引擎的色阶是<b>线性</b>插值、没有 log 入口 —— '
...
SPEC = {
    ...
    # ── 汇率：序列照给（Ex8 画得出来），美元腿那两张显式跳掉（文件头 D）───────────
    # 这两件事**不是同一件事**，捆在一起跳是本轮修掉的那个错：
    #   · Ex8 画的是汇率**本身**，一条宏观序列，挂同一份汇率的每一页逐点相同，
    #     不需要本家披露任何东西 ⇒ 该出；
    #   · Ex5/Ex6 画的是**本家的美元营收**，本家没有官方美元数 ⇒ 该跳，
    #     理由逐条写在 skip_note 里（底座硬性要求：skip 的每个 slug 都要有一句理由）。
    'fx': {
        # 全站共用的那一条 NTD/USD（tsm / alchip 等页用的是同一份文件、同一列），
        # 所以本页的 Ex8 与那几页逐点相同 —— 这正是它不需要本家披露任何东西的原因。
    ...
    'format_source':
        ...
        '与 /tsm/ 同一套图列，本页因没有官方美元数而少「本币 vs 美元」「汇率贡献」'
        '两张图、编号顺次前移；月均汇率那张照出（画的是汇率本身）',
