...
def _mm_pct(s):
    """{'YYYY-MM': 环比%}。序列自己算，不另开一份 CSV 解析。"""
    ks = sorted(s)
    return {ks[i]: (s[ks[i]] / s[ks[i - 1]] - 1) * 100.0
            for i in range(1, len(ks)) if s[ks[i - 1]]}


def _mm_std(lo, hi):
    """`lo`…`hi` **与序列的交集**上环比的样本标准差 → (std pp, n, 首月, 末月)。

    取交集而不是照抄区间，是因为本文件原先写死的那段「2015-02…2017-12 的环比标准差
    25.7pp」两半都不实：`series/mtk.csv` 自 2016-01 起，2015-02 根本不在序列里；
    而按交集（24 个月）实测是 18.6pp。两个数只差一次实算，却没有任何东西会报错 ——
    所以这里现算，不再存一个可以被抄走的常数。
    """
    v = [(k, x) for k, x in sorted(_MM.items()) if lo <= k <= hi]
    if len(v) < 2:
        return None
    m = sum(x for _, x in v) / len(v)
    sd = (sum((x - m) ** 2 for _, x in v) / (len(v) - 1)) ** 0.5
    return sd, len(v), v[0][0], v[-1][0]


def _feb_mar():
    """逐年 Feb→Mar 的环比 → (2017 年的值, 年数, 2017 从低到高的名次, 最低那年, 最低值)。

    原先写死的是「2017-03 的环比 +22.8% 是 11 年里 Feb→Mar 最低的一年」。
    「11 年」每过一年就长一岁，而「最低」在 2025 那一格进来之后就已经不成立
    （2025 年 Feb→Mar 更低）。名次与年数一律现算。
    """
    ys = {k[:4]: v for k, v in _MM.items() if k[5:7] == '03'}
    if '2017' not in ys:
        return None
    lo_y = min(ys, key=lambda y: ys[y])
    rank = sorted(ys.values()).index(ys['2017']) + 1
    return ys['2017'], len(ys), rank, lo_y, ys[lo_y]
...
#: 五条断点逐条量化出来的台阶（②那一格有两支，各算一条）。**最大的那一条现取**，
#: 不写死：本文件原先写的「五条里最大的一条也只有 2.9%」被同一页下文的
#: 「奕力出表约当当年合并营收的 4.3%」当场证伪 —— 两处读同一批常量却各写各的。
_BRK_SIZES = [(n, v) for n, v in (
    ('2016-06 奕力并表', _ILI_IN_FY_PCT), ('2017-03 傑發出表', _AUTOCHIPS_PCT),
    ('2017-03 絡達并表', _AIROHA_FY_PCT), ('2020-12 奕力出表', _ILI_PCT),
    ('2021-02 星宸出表', _SIGMA_PCT), ('2024-07 IC+ 并表', _ICPLUS_PCT),
) if v is not None]
_BRK_MAX = max(_BRK_SIZES, key=lambda kv: kv[1]) if _BRK_SIZES else None
#: 头两条断点所在那一段的环比标准差（起点取序列自己的首月，见 `_mm_std` 的 docstring）。
_MM_EARLY = _mm_std(_M0 or '0000-00', '2017-12')
#: 逐年 Feb→Mar 环比里 2017 排第几 —— 名次与年数都随数据变，不写死。
_FM = _feb_mar()
#: 对照组世芯-KY（3661）的「12 个月相加 vs 官方本年累计」缺口 —— **现算，不写死**。
#: 本页原先写死「+0.378%」，那只是世芯最近一个完整年度的读数（逐年在 −0.3% ~ +0.7%
#: 之间摆动），年一过就过期；build/mrspecs/umc.py 上一轮已把同一句改成现算，这里照办。
...
if _AL_GAPS:
    ...
    _AL_TXT = '在这一项上对不上（本轮读不到 series/alchip.csv，故不给数）'
...
if _RECON:
    ...
    _N_ADD = (
        '<b>本页的季度聚合、YTD 与 12 个月滚动合计是实测过的，不是推定。</b>'
        ...
        f'那家同一检验{_AL_TXT}（功能货币是美元、新台币月营收是逐月折算值），'
        '季度聚合与 YTD 在那一页是非法的。'
    ...
    _N_ADD = (
        '<b>本页的季度聚合、YTD 与 12 个月滚动合计是合法的。</b>月度加总与官方年度营收'
...
_N_BRK = (
    ...
    '不是水平值本身。'
    '<br>⚠️ <b>先说一句读法：这些台阶在图上是看不见的。</b>'
    + ((f'本序列 {_MM_EARLY[2]}…{_MM_EARLY[3]}（{_MM_EARLY[1]} 个月）的环比标准差 '
        f'<b>{_MM_EARLY[0]:.1f}pp</b>，而下面逐条量化出来的台阶里最大的一条'
        f'（{_BRK_MAX[0]}）也只有 <b>{_BRK_MAX[1]:.1f}%</b> —— 约 '
        f'{_BRK_MAX[1] / _MM_EARLY[0]:.2f} 倍标准差，被农历年错位的噪声盖住。')
       if (_MM_EARLY and _BRK_MAX) else
       '这些台阶的量级比本序列自己的环比波动小一个量级，被农历年错位的噪声盖住。')
    + '<b>月份来自申报原件，不来自图形</b>；'
    ...
    + ((f'。<b>絡達比傑發大 {_AIROHA_FY_PCT / _AUTOCHIPS_PCT:.1f} 倍，'
        '相抵后净口径变动是正的</b>')
       if (_AIROHA_FY_PCT and _AUTOCHIPS_PCT) else
       '。<b>絡達比傑發大，相抵后净口径变动是正的</b>')
    + '：2017-03 当月约 +0.5%'
      '（傑發只剩约 1 天、絡達只进 18 天）'
    + (f'，2017-04 起稳态约 <b>+{_NET_2017_PCT:.1f}%</b>' if _NET_2017_PCT else '')
    + '。⚠️ 只写「傑發出表、口径下降」<b>方向是错的</b>；'
    + ((f'而 2017-03 的环比 {_FM[0]:+.1f}% 在本序列 {_FM[1]} 个能算 Feb→Mar 的年份里'
        f'排倒数第 {_FM[2]}'
        + ('（最低的一年）' if _FM[2] == 1 else
           f'（最低的是 {_FM[3]} 年的 {_FM[4]:+.1f}%）')
        + '，看着很像「傑發走了所以 3 月弱」')
       if _FM else '而 2017-03 的环比在历年 Feb→Mar 里偏弱，看着很像「傑發走了所以 3 月弱」')
    + '—— 这正是上面那句「别拿图形倒推并表事件」要挡的。'
...
_N_START = (
    ...
    '<br><b>这两条断点的月份来自申报原件，不来自图形。</b>'
    + ((f'本序列 {_MM_EARLY[2]}…{_MM_EARLY[3]} 的环比标准差是 <b>{_MM_EARLY[0]:.1f}pp</b>，'
        + '而'
        + '、'.join(f'{n[8:]}（{v:.1f}%，{v / _MM_EARLY[0]:.2f} 倍）'
                    for n, v in _BRK_SIZES if n.startswith(('2016-06', '2017-03')))
        + '的台阶都远在它之下 —— 单月序列上根本不可分辨。')
       if (_MM_EARLY and _BRK_SIZES) else
       '这几条台阶都远小于本序列自己的环比波动 —— 单月序列上根本不可分辨。')
    + '别把图上的起伏读成并表事件：'
    + ((f'2017-03 的环比 {_FM[0]:+.1f}% 在 {_FM[1]} 个能算 Feb→Mar 的年份里排倒数第 '
        f'{_FM[2]}，看着像「傑發走了」，')
       if _FM else '2017-03 的环比在历年 Feb→Mar 里偏弱，看着像「傑發走了」，')
...
if _D:
    _N_DAYS = (
        ...
        '算成「经营性走弱」。'
        # ⚠️ 这里原来接着写「这也是月营收柱图右轴走 12 个月滚动合计同比、而不是单月
        #    同比的理由之一：任意连续 12 个月覆盖同样的日历，农历年错位自己抵消掉」。
        #    2026-09 起那条右轴按页面所有者的指令改成单月同比，这半句当场变成假话
        #    （它为一个页面上已经不存在的选择辩护）。滚动口径确实能抵消日历错位，
        #    但本页不再用它 —— 所以改成说清楚**读者现在要自己做什么**。
        '<b>换句话说，日历效应在本页是留在图上的</b>：月营收柱图右轴是单月同比'
        '（全站统一的口径），农历年错位与 2 月天数都还在那条线里，'
        '所以 1、2 月的读数要合起来看，或者用同月对同月的纵向对照'
        '（热力矩阵那张就是按这个排的），不要拿单独一个 2 月去说趋势。')
else:
    _N_DAYS = (
