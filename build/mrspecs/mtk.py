...
# ══════════════════════════════════════════════════════════════════════════════
# 五条断点单列出来，不再写在 SPEC 字面量里 —— 下面的释义要用 `len(_BREAKS)` 说
# 「本页登记了几次合并范围变动」。写死一个 5，哪天补登第六次（2013–2015 那两条一旦核到
# 原文就会补，见 `_N_START` 末段）释义就开始说假话，而没有任何东西会响。
# ══════════════════════════════════════════════════════════════════════════════
_BREAKS = [
...
#
# ━━ 与页尾 notes / 图注的分工 ━━
# 图注与 notes 说的是「这一张图这个月该怎么读」（含当月读数、当月实测的毛刺量、
# 这一轮核对的份数）；这一块说的是「这些词是什么意思」，一年到头是同一段
# ⇒ 这里**不写当月读数、不写「最新一期」、不写覆盖月数**（覆盖月数每月 +1）。
# 出现的数只有三类，且**一个都不写死**：
#   (a) 恒等式本身（QTD = 已公布月份直接相加；占 TTM 比重 = 当月 ÷ 最近 12 个月合计）；
#   (b) 把定义钉住的结构性量 —— 逐年对账残差、断点条数、世芯的折算缺口区间，
#       全部在 import 期从 `series/*.csv` 现算（`_RECON` / `len(_BREAKS)` / `_AL_GAPS`）；
#   (c) 官方定义里的门槛常数 —— 只有 MOPS 备注栏的 ±50%（出处：MOPS 月营收申报表
#       脚注第 6 条，与 `fetch` 侧算 `triggered` 用的是同一条）与法定的「次月 10 日前」
#       （台湾《证券交易法》）。这两个不是数据，改不了、也算不出来，所以照写。
#
# ━━ 为什么是这 10 个词（选词判断）━━
# 判据只有一条：这个词出现在本页的图题 / 序列名 / 纵轴 / 汇总表行头 / 核对表列头 /
# 图注里，而且**不看定义就会读错**。按「读错会出什么事」分四类：
#   ① 这一页到底有几个独立事实   合并营收、功能货币 —— 全页除汇率线外由**一个**
#      官方字段派生；而「月值可以相加」不是自明的，它靠「功能货币＝表达货币＝新台币、
#      月值是原生记账数」撑着。不点破，读者要么以为 3MMA／QTD／占比是另外几个公告数，
#      要么不知道同一批图在世芯那一页是非法的。
#   ② 同一个词在本页有几个口径   单月同比 / 季度同比、QTD / YTD、占 TTM 比重 ——
#      页上同时印着四种「增速」（单月、3 比 3 的季度图、3MMA 比 3MMA、累计比上年前
#      同样月数）与一个**比重**，读串的代价是拿一条线去解释另一条线，而图形完全正常、
#      看不出来。⚠️ 汇总表的 y/y 是**逐行**按各行自己的序列算的（build/mrbase.py 的
#      summary：ds.disp / ds.ma3 / ds.qtd / ds.ytd / ds.share_ttm 各算各的），
#      所以「汇总表的 y/y 列」整列都归给单月是错的 —— 释义里只点 Monthly revenue 那一行。
#   ③ 本页自己轧出来、或只是上界的数   并表 / 出表、并表贡献额 / 规模上界 ——
#      断点**图注**里那些百分比有三种来源（并表期间贡献额＝准数、子公司自身营收＝上界、
#      备考数反推的全年规模＝估计值），读者最容易做的动作正是拿它去减增速，而那是错的。
#      ⚠️ 不能按「写等号还是约等号」分：本页并表那几条里，只有 IC+ 写等号（① 类），
#      奕力 2016 并表与絡達都写 ≈（③ 类）。图上的断点标签只有月份、没有百分比。
#   ④ 不是公司披露值的那几列   MOPS 備註 / 觸發腿、月均汇率（NTD/USD）、美元营收实绩
#      —— 核对表标题已经点名「3 列不是公司披露值」，这三条解释的就是「那它们是什么」：
#      备注是达标才必填的合规文字（本家常年未达标却每月照填，**是**披露值）、
#      而「觸發腿」是本页按 ±50% 自己判出来的**分类列**，两列必须拆开写；
#      汇率是外部宏观牌价（不是公司申报的那个季度汇率）、
#      美元营收则是本页**拒绝**画的构造值。
# **有意不收**：
#   · m/m、y/y、3Y %ile、pp/bp —— 全站通用的读图约定，`summary.note` 已逐条讲过，
#     释义板再讲一遍就是两处各写一份（口径分叉的那两个 y/y 另当别论，见 ② 类）。
#   · 「口径断点」「未满季浅蓝柱」「通栏 / 标签抽稀」「热力矩阵色阶」—— 这些是**图型
#     约定**，各自的图注与页尾 notes 里连着本页实测的数一起讲了；释义板只留断点线的
#     **语义**（并进「并表 / 出表」那条），不重讲一遍落点与理由。
#   · 3 个月移动平均（**这条线本身**）、无晶圆厂、合并营收以外的常识词 —— 本页对它们
#     没有特殊口径。⚠️ 但 3MMA 在汇总表里那条 **y/y 腿**有：它比的是 3MMA 对 3MMA，
#     页上别处没解释过，所以并进「单月同比 / 季度同比」那条的逐行警告里，不另立词条。
#   · 指引区间 / 指引桥 —— 本页根本没有这张图（见 `_N_NOGUIDE`），页面上没出现的词不收。
# ══════════════════════════════════════════════════════════════════════════════
if _RECON:
    _G_ADD = (f'{_RECON[0]} 个完整年度逐年对得上官方年度营收，最大只差 '
              f'<b>{abs(_RECON[1]):,.3f} NT$mn</b>（{_RECON[2]:.5f}%），'
              f'那点残差全部来自「公司只公告到百万元整数」的逐月舍入。')
else:
    _G_ADD = '月度加总与官方年度营收逐年对得上，残差只有逐月舍入的量级。'

if _AL_GAPS:
    _G_ALCHIP = (f'⚠️ 同组页面里的世芯-KY（3661）相反：功能货币是美元、新台币月营收是'
                 f'<b>逐月折算值</b>，同一检验逐年在 {min(_AL_GAPS.values()):+.2f}% ~ '
                 f'{max(_AL_GAPS.values()):+.2f}% 之间摆动 —— 那一页的季度聚合与 YTD '
                 f'是非法的，本页的是合法的。')
else:
    _G_ALCHIP = ('⚠️ 同组页面里的世芯-KY（3661）相反：功能货币是美元、新台币月营收是'
                 '<b>逐月折算值</b>，同一检验逐年对不上 —— 那一页的季度聚合与 YTD '
                 '是非法的，本页的是合法的。')

_GLOSSARY = [
    ('合并营收',
     '本页<b>取用的唯一</b>官方字段：合并个体（联发科<b>及其子公司</b>）的当月营收，'
     '<b>新台币百万元整数、未经会计师查核</b>，按台湾《证券交易法》在次月 10 日前公告'
     '（遇假日顺延，所以本页的更新节奏由法定披露日定，不由抓取排程定）。'
     '本页的营收类读数 —— 月营收、3 个月移动平均、QTD／YTD、占 TTM 比重、各处同比 —— '
     '<b>全部</b>是这一个字段的派生量，<b>没有第二个独立事实</b>，也不掺任何券商预测'
     '或外部估计；页上另外两个输入（汇率序列、MOPS 备注列）各有各的来源，'
     '都<b>不进</b>营收的任何计算。'
     '⚠️ 「本页只取一个字段」<b>不等于</b>「公司只公告了一个字段」：月度新闻稿正文表格'
     '与 IR 的年度月营收表还印着 y/y 与累计，只是它们都是这一列金额的<b>派生量</b>、'
     '不是第二个独立事实，存进 <code>series/</code> 等于把同一个事实存两遍。'),

    ('功能货币',
     '编制财报时用来计量交易的货币。联发科合并财报附注写明母公司的功能货币与表达货币'
     '<b>均为新台币</b>，月营收因此是<b>原生记账数</b>而不是折算值 —— 这是本页敢把月值'
     '相加成 QTD／YTD／季度柱的<b>全部依据</b>，也是它被实测过的地方：' + _G_ADD
     + _G_ALCHIP),

    ('单月同比 / 季度同比',
     '本页<b>图上</b>的同比只有这两种，<b>口径不同、不能互相对读</b>。<b>单月同比</b>'
     '（当月 ÷ 去年同月 − 1）用在月营收柱图的右轴金线、热力矩阵、汇总表 '
     '<b>Monthly revenue</b> 那一行与核对表的 y/y 列，以及页顶导读里标明「单月」的读数，'
     '这几处彼此同口径；它要 12 个月历史，'
     '所以那条线<b>比柱短 12 期</b>，序列头一年只有柱没有线。'
     '<b>季度同比</b>（当季 3 个月合计 ÷ 去年同季 3 个月合计 − 1）只有季度柱图右轴一处，'
     '<b>严格 3 个月比 3 个月</b> —— 分子或分母有一边不满 3 个月就整格留空，'
     '不印「n 个月比 3 个月」。'
     '⚠️ 汇总表的 y/y 是<b>逐行按各行自己的那条序列</b>算的，<b>不是整列都走单月</b>：'
     '3 个月移动平均那一行比的是 3MMA 对 3MMA（3 个月均值比 3 个月均值，与单月不是同一个'
     '数）、QTD／YTD 两行见下一条、占 TTM 比重那一行是<b>百分点差</b>。'
     '照单月的定义去核那几行一定对不上。'
     '⚠️ 本页<b>不做日均化</b>：农历年错位与 2 月天数都还留在单月同比里，'
     '1、2 月的读数要合起来看，或者用同月对同月的纵向对照（热力矩阵就是按这个排的）。'),

    ('QTD / YTD',
     '<b>季内累计</b>（quarter-to-date）与<b>年内累计</b>（year-to-date）：把本季／本年'
     '<b>已公布</b>月份的营收直接相加，<b>不把未公布的月份外推进来</b>。'
     '它们的 y/y 比的是上年同期的<b>前同样几个月</b>（同一批日历月对同一批日历月），'
     '<b>不是</b>上年的整季／整年，所以季初就能读、也不会因为「1 个月比 3 个月」砸出假坑。'
     '⚠️ 本页的累计由百万元整数逐月相加而来，与公司自己公告的累计数可能差 1 NT$mn'
     '（公司是先加千元再舍入），量级在 0.0005% 以内。'),

    ('占 TTM 比重',
     '汇总表 Seasonality 那一行：<code>当月营收 ÷ 最近 12 个月合计（含当月）</code>，'
     '量的是当月在自己这一年里的<b>季节位置</b>。'
     '⚠️ 它是一个<b>比重</b>，不是增速 —— 别把「trailing-12-month」读成滚动同比：'
     '<b>本页一条 12 个月滚动合计的同比线都没有</b>，页内所有月度同比都是单月口径。'),

    ('并表 / 出表',
     '合并范围变动：把一家子公司纳入（<b>并表</b>）或不再纳入（<b>出表</b>）合并财务报表的'
     f'编制主体。本页登记了 {len(_BREAKS)} 次，都是<b>当期起生效、比较期不重编</b> —— '
     '所以被污染的是跨过那一期的 12 个月<b>同比</b>，水平值本身没有被重述。'
     '图上的红色竖虚线就是这几次变动，语义是「<b>从这一期起与左侧不可比</b>」、'
     '画在该期左缘（热力矩阵没有连续横轴，所以不画线）。'
     '⚠️ 这些台阶都远小于本序列自己的环比波动，<b>在图上看不见</b>：'
     '月份来自申报原件，反过来拿图形去倒推并表事件一定会读错。'),

    ('并表贡献额 / 规模上界',
     '断点<b>图注</b>里那些百分比（图上的竖虚线旁只竖排月份，不写数）有<b>三种</b>来源，'
     '按<b>来源</b>分、不按符号分：① <b>并表期间贡献额</b> —— 企业合并附注直接给的'
     '「自收购日至年末对本公司产生之收入」，是<b>准数</b>；② <b>子公司自身营业收入</b>'
     ' —— 出表那几条只能取年报「关系企业营运概况」表里该子公司自己的营收，'
     '<b>未扣除对集团内其他公司的销货冲销</b>，而合并营收里本来就没有那部分，'
     '所以它是<b>上界</b>：真实缺口 ≤ 这个数，公司从未披露过净贡献额，'
     '<b>不要拿它去减增速</b>；③ <b>按备考数反推的全年规模</b> —— 拿附注里'
     '「若合并发生于年初」的备考营收与实际数相减、再加回并表期间贡献额，'
     '倒推出该主体<b>全年</b>的体量，是<b>估计值</b>。'
     '⚠️ ① 与 ③ 都出现在<b>并表</b>那几条上，量的却不是同一件事（并表那几个月的贡献额 '
     'vs 该主体一整年的规模），<b>不能互相替代</b>；只有 ② 才是上界。'),

    ('MOPS 備註 / 觸發腿',
     'MOPS 是台湾公开资讯观测站。核对表<b>最后一列</b>「本月備註」是公司在它的月营收'
     '申报表「備註／營收變化原因說明」栏填的<b>文字</b>：<b>原文逐字、繁体不转简</b>，'
     '只出现在这一列与页顶导读里，<b>不进任何一张图、不参与任何计算</b>。'
     '它前面那一列<b>「觸發腿」不是公司填的</b>，是本页按同一条脚注的门槛'
     '<b>自己判出来的分类</b>（核对表标题已把它列进「不是公司披露值」的那 3 列）：'
     '官方门槛（申报表脚注第 6 条）是<b>单月同比或累计同比的绝对值达 50%</b>'
     '（页上写作 ±50%）才必须填说明，过线的是哪条腿就印哪条，两条都没过时印「—」。'
     '⚠️ 联发科每月都填同一句折算口径注，而门槛<b>常年没有触发</b> —— '
     '那句话<b>不是</b>公司对当月增减的说明。'),

    ('月均汇率（NTD/USD）',
     '汇率图与核对表汇率列画的那条线：美联储 H.10 台湾地区<b>日度</b>牌价的'
     '<b>月度算术平均</b>（与 FRED EXTAUS 同定义、同上游）。'
     '它是一条<b>外部宏观序列</b> —— 本站挂同一份汇率的每一页上逐点相同，'
     '公司披不披露美元营收与它无关。'
     '⚠️ 它<b>不是</b>公司申报的汇率：公司申报的那个是<b>季度</b>平均汇率，'
     '印在季度业绩新闻稿合并损益表的末行，与月度营收公告无关。'),

    ('美元营收实绩',
     '公司实际达成的美元营收<b>金额</b>。联发科<b>从不公告</b>它 —— 法说会讲稿里的美元'
     '只有三种，没有一种是实绩：<b>增速</b>、<b>下季指引区间</b>（还不是每季都给）与'
     '<b>全年目标</b>。⇒ 拿新台币月营收除以外部牌价折出来的是<b>分析师构造值</b>，'
     '没有任何官方数可以对账，所以本页不出「本币 vs 美元」与「汇率贡献」两张图，'
     '页上任何一处（抬头、页顶导读、核对表）都没有美元营收的数字。'
     '汇率线<b>本身</b>照出：它不需要公司披露任何东西就成立。'),
]
...
SPEC = {
    ...
    # 逐条内容在上方 `_BREAKS`（抽出去是为了让释义能 `len()` 它，不写死一个 5）。
    'breaks': _BREAKS,
