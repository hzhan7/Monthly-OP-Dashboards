...
1. **汇率腿**。TSMC 以美元计价、以新台币入账，所以 Ex5（NT$ vs US$ 单月同比）、
   ...
   另外六家（联电 / 联发科 / 日月光 / 南亚科 / 世芯 / 创意）**没有**这条腿：
   月度公告只有新台币，年报里那列美元是「便利折算」（全年一个汇率整列除下来），
   拿本币 ÷ 外部牌价折出来的月度美元只能是分析师构造值，没有官方数可以对账。
   ⇒ 那六家的 spec 里 `fx` 留空，底座整体跳过这三张图并把编号顺次前移。
...
━━ 断点 ━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━
`breaks` 为空。**为什么**为空由下面的 `breaks_absent_zh` 自己说 —— 底座不替任何一家
断言「口径连续、未发生并表或重述」，那句话在联电（2019-10 USJC 并表、2015-06 新事业
退出合并）等至少四家上是假话。
"""
import numpy as np
import pandas as pd
...
SPEC = {
    ...
    'fx': {
        ...
        # 页尾说明里引用同一个串时不该跟着印一遍。
        'src': 'FRED series EXTAUS，月均 NTD/USD（该月全部营业日美联储 H.10 台湾牌价的算术平均）',
