...
_GLOSSARY = [
    ('合并营收',
     ...
     '汇总表上半张、Exhibit ⟨ex:rev_bar,qtr,mom,fx_lines,fx_contrib,hist,fx_rate,heat⟩（Exhibit ⟨ex:fx_rate⟩ 是汇率线除外）与核对表的金额列<b>全部由它'
     '这一个披露字段</b>（加一条月均汇率序列）派生，不引入任何券商预测或外部估计。'
     '⚠️ Exhibit ⟨ex:capex-approvals,capex-ytd,fx-forwards,hedge-ratio,debt-cost,bond-wall,guarantees,arizona-guarantees⟩ 与汇总表下半张<b>不在此列</b>：它们读的是公司另外几张'
     '月度申报表，与月营收<b>没有派生关系</b> —— <b>唯一的例外</b>是 Exhibit ⟨ex:hedge-ratio⟩ 与'
     '汇总表 <code>Hedge book / avg. monthly revenue</code> 一行，'
     '它们的<b>分母</b>取的正是本页月营收的 12 个月均值。'
     '（公告同时给出一个同比原值，本页凡标「公告同比」的地方用的都是它，不是我们除出来的。）'),

    ('季度值（月加总）',
     'Exhibit ⟨ex:qtr⟩ 的柱：该季<b>已公布月份</b>的新台币月营收直接相加，<b>不做任何调整</b>。'
    ...
    ('月均汇率',
     'Exhibit ⟨ex:fx_rate⟩ 那条线与核对表 <code>NTD per USD</code> 那一列：美联储 H.10 台湾地区'
     ...
     '本页凡是<b>由新台币折出来</b>的美元数（Exhibit ⟨ex:fx_lines⟩ 的美元线、核对表 '
     '<code>Implied revenue</code>、Exhibit ⟨ex:arizona-guarantees⟩ 的两条腿、汇总表 Arizona 那一行）'
     '都建立在这条<b>外部</b>牌价上；'
     '⚠️ 但 Exhibit ⟨ex:capex-approvals⟩／⟨ex:capex-ytd⟩ 的 US$ 是董事会 6-K 的<b>申报原值</b>，<b>不经</b>这条牌价。'),
    ...
    ('美元营收（推导值）',
     ...
     'Exhibit ⟨ex:fx_lines⟩ 的美元线与核对表的 <code>Implied revenue (US$mn)</code> 都是它。'),

    ('汇率贡献',
     'Exhibit ⟨ex:fx_contrib⟩ 的柱：<code>新台币同比 − 美元同比</code>，单位是<b>百分点</b>，'
    ...
    ('董事會核准資本支出',
     ...
     '更不能按年换算。Exhibit ⟨ex:capex-approvals⟩ 是逐次会议、Exhibit ⟨ex:capex-ytd⟩ 是同一批额度的年内累计，'
    ...
    ('背書保證',
     ...
     'Form 6-K）是眼下实际还挂着的担保，Exhibit ⟨ex:guarantees⟩ 与汇总表画的是它；'
     '<b>董事會核准數</b>（MOPS 的「至本月份累計餘額」）是董事会批出的<b>额度</b>，'
     '含尚未动用的部分 —— 两者<b>不是同一个东西</b>，'
     '缺口有多大是随月变的读数，见 Exhibit ⟨ex:guarantees⟩ 图注与汇总表。Exhibit ⟨ex:arizona-guarantees⟩ 把亚利桑那那一腿的两套数<b>折成美元</b>并排画'
    ...
    ('边际成本 / 平均成本',
     'Exhibit ⟨ex:debt-cost⟩ 两条线的分工。<b>边际成本</b> = 最近一档新发 5 年期公司债的票面'
    ...
    ('宝岛债',
     '在台湾发行、以<b>外币</b>计价的债券。本页的债券口径（Exhibit ⟨ex:debt-cost⟩ 的两条线、'
     'Exhibit ⟨ex:bond-wall⟩ 的到期墙与汇总表 NT$ bond funding 那两行）'
    ...
    ('到期墙',
     'Exhibit ⟨ex:bond-wall⟩：把某一个月末的在外新台币本金按<b>实际还本时程</b>排进日历年。'
     '⚠️ <b>不是按到期日排</b> —— '
