# -*- coding: utf-8 -*-
"""LPL Financial (LPLA) 月度经营指标 —— 把 build/build_lpla.py 的 matplotlib deck
逐张移植成网页看板 payload，写出 data/lpla.js。

模版来源：Goldman Sachs (Alexander Blostein 团队)「LPL Financial Holdings (LPLA):
          April metrics…」的 Exhibit 1，以及同系列 11 月期。该表的三条口径规矩本站全部照搬：
   1) **流量类（NNA）不算环比/同比百分比**，改用「年化有机增长率」= 当月 NNA x 12 / 上月末资产；
   2) **比率类差异一律用 bp / pp**，不用百分比变化；
   3) 存量分两个业务口径（Advisory / Brokerage）+ Total + 占比行。
   另采用 GS「SCHW First Take」Exhibit 2 的恒等式滚存桥（期初 + 净新增 + 市值变动 = 期末）。
数据源：LPL Financial IR 月度经营指标新闻稿。季末月（3/6/9/12）无独立月报，取自当季季报。

⚠️ 并购导入：两笔整体并表会把 as-reported 序列打断 —— 2024 年 10 月 Atria（$88.3bn）与
   2025 年 8 月 Commonwealth Financial Network（$275.0bn）。两个月都不是有机流入，
   凡是画 as-reported 客户资产 / 现金 / NNA 的图都以红色竖虚线标出（见 ACQ_BREAKS）。

输入（只读，一律来自 series/）：
    series/lpla.csv       月度经营指标（2016-01 起；2018-07 以前由 fetch/lpla.py --backfill 补入）
    series/fee_rates.csv  季度费率与季度实际收入（company = LPLA）
输出：
    data/lpla.js          window.DASH = {...}

幂等：payload 里不写构建日期（只写首行注释），不使用随机数，窗口一律从数据最新月倒推。
"""
import datetime
import json
import math
import os

# 「全站同比只有单月」这句话在页面上必须带 §6.2 那五张例外 —— 不带的话，读者从本页翻到
# /exchanges-apac/ Exhibit 5（标题就写着「12 个月滚动合计的同比」）会当场撞上一句假话。
# 这一段三页共用同一份措辞，改一处要三处一起改（本仓没有跨文件的共用图注模块）。
EXC_ZH = (
    '本页与全站绝大多数页只有这一种口径 —— 页面所有者定的（CONTRACT §6 抬头引了原话）。'
    '⚠️ <b>「全站」有五张明文例外</b>（§6.2 点名保留 12 个月滚动合计口径，都不是折线：'
    '<code>/exchanges-apac/</code> 的 Exhibit ⟨ex:exchanges-apac/rolling-yoy-3y⟩ 与 ⟨ex:exchanges-apac/value-bridge⟩、<code>/exchanges12/</code> 的 Exhibit ⟨ex:exchanges12/rolling-yoy-band⟩、⟨ex:exchanges12/contracts-vs-notional⟩、⟨ex:exchanges12/contract-shrink⟩）'
    ' —— 翻到那两页时口径与本页不同，不要跨页比高低。'
    '本页上一条 12 个月滚动合计的同比都不画。'
)

import numpy as np
import pandas as pd

import brief as B       # 顶部 brief 的规则库（R1-R6），只算事实、不出文字
import glossary as gloss  # 名词释义的版式层与护栏，全站共用
import mrwin            # 通栏 / x 标签抽稀的裁决层，与 single.py 及其余 11 个生成器共用
import payload_guard
import exhibits         # 图号：ORDER 定图序、正文 ⟨ex:…⟩ 占位符（build/exhibits.py）
import pctile           # 汇总表 3Y %ile 的唯一实现，不在本文件里另写一套
import yoy              # 同比口径的唯一实现（build/yoy.py）：本文件不再自己写一份滚动同比

D = os.path.dirname(os.path.abspath(__file__))
ROOT = os.path.dirname(D)
SERIES = os.path.join(ROOT, 'series')

SRC = ('Source: LPL Financial monthly activity and quarterly reports; '
       'format after Goldman Sachs GIR')
# $275bn 是公司披露值，不是估算：LPL 2025Q3 报告原文
# "This included $275 billion of acquired net new assets resulting from the acquisition
#  of Commonwealth" —— 曾经这里写过 ~$285bn（来自原 deck docstring 的约数），
# 而同一事件在 wealth 页又写成 $277.0bn，同一个数在站内出现三种写法。
QNOTE = ('Quarter-end months have no standalone monthly report; those values come from '
         'the quarterly release')

# 官方同页披露的 Acquired NNA（$bn）。**逐条都有官方出处**，一条估算都没有：
#   · 2019-08 起：官方在 Historical File 里同时印 "Total NNA" 与 "Total Organic NNA"
#     两行，acquired = 前者 − 后者，逐月现成。
#   · 2017-12…2018-04（NPH）：那一代文件没有 organic 行，印的是影子块
#     "Net New Assets Prior to NPH"，acquired = as-reported − prior to NPH。
#     **只取并表进行中的那 5 个月**：2018-05 起两者之差转负（−0.5 / −0.2），
#     那已经不是并表导入而是 NPH 存量顾问自己的净流出，属有机口径，不许扣。
#     交叉校验：这 5 个月合计 72.3bn，而同一份 2018-04 期文件里
#     "Assets Served Prior to NPH" 与正表的总资产之差是 72.2bn（652.3 − 580.1），
#     两条独立披露互相印证，误差 0.1 是四舍五入。
#   · 2019-08 Allen & Company 用官方脚注的原话：「$2.9 billion of total brokerage and
#     advisory assets …, of which $1.8 billion were brokerage assets and $1.0 billion
#     were advisory assets」。1.8 + 1.0 = 2.8，官方自己写 2.9（各自四舍五入），这里取
#     官方给的合计 2.9。
# 2026 年至今官方逐月印的 "Total Acquired NNA" 都是 0.0（2026-05 期原件可查），所以没有条目。
#
# ⚠️ **2024-08 保留 0.3 不改**：现行文件算出来是 0.2（6.8 − 6.6，两行各自已四舍五入，
#    真值落在 0.15–0.25 之间，两个写法都对）。它是既有真值，改它属于「拿今天的四舍五入
#    盖历史」，不在本轮范围内。
#
# ⚠️ **build/wealth.py 有一份同名表，两份必须逐条相同**：那边的 _sync_lpla() 会用 ast
#    读这张表逐条比对，不一致就 SystemExit（"wealth 与 lpla 的断点/并购登记表已分叉"）。
#    所以这里加一条，wealth 那份就得同步加一条 —— 不是「补不补都行」，是构建期硬拦。
#    （2026-08-21 订正：此处原写「那一份仍是 2023-01 起的旧版」，早已不是事实，
#      而且是 _sync_lpla() 结构上禁止存在的状态。）
#
# ⚠️ **新增一笔并购时，只补这张表是不够的**：它是**手写**的，官方 Acquired NNA 行并不进
#    series/lpla.csv（列表是冻结的，见 fetch/lpla.py 的 VALUE_COLS），所以漏登一笔时
#    nna_ex = nna - 0，"有机"图会**静默**印成 as-reported，没有任何机器判据会响。
#    2026-06 的 0.5bn 就是这么漏的（线上印了一天 11.8，官方 11.3）。加一笔之前先照
#    官方 Historical File 的 "Total Acquired NNA" 行核一遍当月值。
ACQ = {'2017-12': 34.1, '2018-01': 2.5, '2018-02': 29.8, '2018-03': 3.7, '2018-04': 2.2,
       '2019-08': 2.9,
       '2020-10': 1.5, '2020-11': 2.5,
       '2021-04': 67.1, '2021-06': 1.8, '2021-09': 2.3,
       '2023-01': 3.2, '2023-03': 0.5, '2024-04': 5.0, '2024-08': 0.3, '2024-09': 0.3,
       '2024-10': 88.3, '2024-11': 0.8, '2024-12': 0.3, '2025-01': 0.1, '2025-02': 0.7,
       '2025-03': 7.1, '2025-07': 0.1, '2025-08': 275.0, '2025-12': 2.0,
       '2026-06': 0.5}

# ── 结构性断点 ①：整体并表 ──────────────────────────────────────────────────
# 判据写在这里，别靠记忆：**当月 Acquired NNA ≥ 当月末客户资产的 5%** 就登记为断点 ——
# 那种量级的并表会把存量序列整体抬一个台阶，断点两侧不可直读。按这条判据当前有四笔：
#   NPH          2017-12 起分批，合计 $72.3bn ÷ 2018-04 的 652.3 = 11.1%
#   Waddell&Reed 2021-04  $67.1bn ÷ 958.3（上月末）= 7.0%
#   Atria        2024-10  $88.3bn ÷ 1,673.3 = 5.3%
#   Commonwealth 2025-08  $275.0bn ÷ 2,263.5 = 12.1%
# 不属于这四笔的条目全在 1% 以下（最大的 2025-03 的 7.1bn 只占 0.4%），只进 ACQ、不画线。
# 这条判据在 main() 里**机器复核**：有月份达到 5% 却没登记，构建直接停更。
# CONTRACT.md §5.2：口径断点必须画出来，不能靠图注提一句就算数。
#
# NPH 的线画在 **2017-12**（第一笔导入落地的那个月），不画在并表完成的 2018-04：
# break_at 的语义是「从这一期起与左侧不可比」，而 2017-12 那根柱本身就已经含并表。
#
# 曾经这里只有 Commonwealth 一条，于是同一条 LPL as-reported 序列在本页被判成
# 「Oct-24 可比」、在 /wealth/ 被判成「Oct-24 不可比」，两页互相矛盾。
ACQ_BREAKS = [(pd.Period('2017-12', 'M'), 'NPH'),
              (pd.Period('2021-04', 'M'), 'Waddell & Reed'),
              (pd.Period('2024-10', 'M'), 'Atria'),
              (pd.Period('2025-08', 'M'), 'Commonwealth')]
#: 每条断点实际覆盖的 ACQ 月份。NPH 是唯一一笔**跨月分批**导入的：线画在第一笔落地的
#: 2017-12，但 ACQ 里 2017-12…2018-04 五条都属于这一笔。判据复核与「其余小额并购」的统计
#: 都按这张表算 —— 否则 2018-02 那 29.8bn 会被当成「一笔没登记的 4.6% 并购」，
#: 判据复核当场误报停更。
ACQ_BREAK_MONTHS = {'2017-12', '2018-01', '2018-02', '2018-03', '2018-04',
                    '2021-04', '2024-10', '2025-08'}
BRK_TXT = {'NPH': 'Dec-2017 NPH（分批导入至 Apr-2018，合计 $72.3bn）',
           'Waddell & Reed': 'Apr-2021 Waddell & Reed ($67.1bn)',
           'Atria': 'Oct-2024 Atria ($88.3bn)',
           'Commonwealth': 'Aug-2025 Commonwealth ($275.0bn)'}

# ── 结构性断点 ②：官方改了定义（不是并表）──────────────────────────────────
# 这两条是**真值表里早就存在、以前没人标出来**的口径断点。窗口从「近 25 个月」放宽到
# 2016-01 之后它们第一次进到图里，不标就是让读者拿两把尺子量同一条线。
# 两条的证据都在 fetch/lpla.py 的「口径坑 10」里，都是官方原件里的原话。
#
# 分成 'nna' / 'cash' 两族，是因为它们各自只影响一族序列：把现金那条画到 NNA 图上、
# 或者反过来，等于告诉读者「这里不可比」而其实那条线一点没受影响。
CAL_BREAKS = {
    # 2020-04 期起 Total NNA = Asset Inflows minus Outflows + Dividends plus Interest
    # minus Advisory Fees。真值表 2019-04 存老定义、2019-05 起存新定义，断点落在两者之间。
    'nna': [(pd.Period('2019-05', 'M'), 'NNA 定义',
             'May-2019 起 Total NNA 改为「净流入 + 股息利息 − 投顾费」'
             '（官方 2020-04 期脚注），左侧是只含净流入的老定义')],
    # 2019-04 期把 Total Cash Sweep Balances 改名 Total Client Cash Balances 并开始
    # 计入 Purchased Money Market Funds（当月 0.4bn，到 2020-01 已是 2.5bn）。
    'cash': [(pd.Period('2019-04', 'M'), '现金口径',
              'Apr-2019 起客户现金改用 Total Client Cash Balances（含 purchased money '
              'market funds），左侧是只含 ICA + DCA + MMA 的 Total Cash Sweep Balances')],
}

#: 时序主窗口的左端。2026-08-18 从写死的「近 25 个月」改成「2016-01 起」，与
#: build/single.py 的 WIN_FROM、build/cboe.py / build/cme.py 的同名常量、build/msci.py 的
#: WIN0 同一个口径、同一个理由：数据回补到 2016 而窗口停在近两年，等于回补给谁看。
#: 序列比它短就用序列自己的起点（**只往右让、不往左借**）。
WIN_FROM = '2016-01'
WIN_S = 13          # gsx.stack_share / bridge_bar 的窗口（近端对比图，保持 13 个月）

# ── 图序：挪图只改这一张表（机制见 build/exhibits.py 的模块头）──────────────
# 排第几项就是 Exhibit 几（从 2 起；Exhibit 1 是汇总表，核对表自动接在最后）。
# ⚠️ main() 里 bar(2, …) / 'n': _S(5) 与下面 N_ADVISORY 这些是**建图时的号**（exhibits.Seq，
# 也是本文件登记簿的钥匙），不是页面上的图号；页面上的号由这张表决定，印进正文的一律是占位符。
ORDER = [
    'assets',           # Total client assets
    'nna',              # Organic net new assets
    'organic',          # Annualised organic growth rate
    'mix',              # Client assets: advisory vs. brokerage
    'bridge',           # What moved client assets: flows vs. markets
    'nna-channel',      # Net new assets by channel
    'assets-history',   # Total client assets since 2016
    'cash',             # Client cash balances
    'cash-pct',         # Client cash as % of client assets
    'nna-q',            # Net new assets by quarter
    'advisory',         # Advisory assets
    'cash-rev',         # Implied client-cash revenue
    'cash-rev-bridge',  # Bridge check: implied vs. reported client-cash revenue
    'brokerage',        # Brokerage assets
    'advisory-share',   # Advisory share of client assets
    'cash-history',     # Client cash since 2016
    'organic-years',    # Organic growth path by year
    'organic-heat',     # Annualised organic growth rate — 单月年化（热力矩阵）
]
_S = exhibits.Seq
EX_ID = {_S(k): i for k, i in enumerate(ORDER, start=2)}   # 建图时的号 → id（缺省图序 = 建图顺序）

# 图注里被交叉引用的 Exhibit 编号一律走常量，不写字面数字：本页在末尾追加过图、
# 也拆分过图，散在正文里的「Exhibit 12」会集体指错一张，而那种错没有任何自动化能发现。
N_ADVISORY, N_BROKERAGE, N_ADV_SHARE = _S(12), _S(15), _S(16)
WIN_Q = 14          # gsx.qtr_bar / implied_vs_actual 的窗口（季度）


def _w(obj):
    """把一条 Series / DataFrame 按 WIN_FROM 截左端（取代原先写死的 `.iloc[-25:]`）。

    只往右让、不往左借：序列比 WIN_FROM 晚就原样返回。写法与 build/hkex.py 的同名助手一致。
    """
    idx = obj.index
    keep = [i for i, p in enumerate(idx) if f'{p.year}-{p.month:02d}' >= WIN_FROM]
    return obj.iloc[keep[0]:] if keep else obj


def _w_finite(df_, col):
    """同 `_w()`，但左端再往右推到 `col` **第一个算得出来的月**。

    派生列（环比 / 年化增速 / 滚动比率）在序列首月定义上不存在 —— 年化有机增速要用
    上月末资产，序列第一个月没有上月。窗口一放宽到序列起点，那一格就成了 null：
    柱图上是一个不画的空位，`cap_pack` / 量程按 null 处理不报错，但 x 轴第一格永远空着，
    读者只会以为「那个月没数据」。**丢掉那一格，不补零、不前向填充** —— 补出来的值
    在数据里不存在。
    """
    s = df_[col]
    ok = [i for i, v in enumerate(s.values) if v is not None and np.isfinite(float(v))]
    w = _w(df_)
    if not ok:
        return w
    first = df_.index[ok[0]]
    return w if w.index[0] >= first else df_.loc[first:]

DRAWN = []          # 真正画出了**并购**断点线的 exhibit 编号，供 notes 现算文案用
DRAWN_CAL = []      # 真正画出了**口径**断点线的（两族分开记：两条 notes 各说各的，
                    # 合成一张名单会让「有机口径的图不画并购线」那句自相矛盾）
CAPPED = []         # 真正截了轴的 (编号, 说明)，同理


def cap_pack(n, desc, vals, hi=None, lo=None, cap_note='axis capped — true values shown in red'):
    """只在窗口里**真的有点越界**时才截轴，返回可展开进 exhibit 的字段。

    与断点同一条道理：ycap 一给上，引擎就无条件把「axis capped」那行红字画出来。
    窗口往前滚、离群月掉出窗口之后，图上一个红圈都没有却还写着「已截轴」，
    就成了第二句假话。所以这里现算：没有越界点就不截、也不写那行字。
    """
    fin = [float(v) for v in vals if v is not None and np.isfinite(float(v))]
    over = [v for v in fin if (hi is not None and v > hi) or (lo is not None and v < lo)]
    if not over:
        return {}, ''
    CAPPED.append((n, desc))
    out = {'cap_note': cap_note}
    if hi is not None:
        out['ycap'] = hi
    if lo is not None:
        out['yfloor'] = lo
    return out, len(over)


CAP_K = 6.0         # cap_bounds() 的 Tukey 栅栏倍数；图注里那句「Q3 ± k×IQR」引它，
                    # 不写字面 6 —— 调了 k 而图注还写 6，就又是一句会过期的假话。


def cap_bounds(vals, k=CAP_K, pad=0.12, step=5.0):
    """截轴上下界**从数据现算**，不手挑常数。返回 (lo, hi)，都以 `step` 为单位取整。

    为什么不能写死：Ex7 上一版写的是 `-6.0, 35.0`，那是 **25 个月窗口**下手挑的 ——
    当时窗口里的日常上沿只有 $15bn 左右，35 离它一倍多，「离群」与「日常」分得开。
    窗口放宽到 2016-01 之后，日常上沿已经是 $30.1bn（Oct-24 的 Advisory），35 离它
    只剩 16%，于是 Apr-21 的 38.0 / 35.9（只比日常上沿高 26%）也被当成离群点截掉、
    各配一个红色竖排真值标签，而真正顶出量程的其实只有 Atria 与 Commonwealth
    那两个月（66.5 / 81.7 / 211.1，是日常上沿的 2–7 倍）。窗口每月往前滚，
    手挑的界必然再次过期 —— 那只是把「过期」这件事往后推，所以这里现算。

    算法：对给进来的所有有限值做 Tukey **极端**离群栅栏 Q3 + k×IQR / Q1 − k×IQR；
    栅栏之外的算离群。有离群的那一侧按「栅栏内极差 × pad」留白后向外取整到 step；
    没有离群的那一侧不额外留白，只向外取整到 step（那一侧本来就不需要为红圈让位）。

    · k=6 不是拍脑袋：本页实测 k 从 6 一路取到 10，划出来的都是同一批 3 个离群点
      （66.5 / 81.7 / 211.1），结论对 k 不敏感；k=4 才会把 Oct-24 的 30.1 也划进去。
    · **一侧截了、另一侧也必须给界**：`lines_endlabels` 的默认下界是 mn − 0.20×极差，
      极差被 $211bn 的尖峰撑开之后下界会掉到 −$46bn，只给 ycap 会留下一大片空白负区。
    · 一个离群点都没有时返回 (None, None) —— 不截轴，交回引擎自己的留白，
      `cap_pack()` 那边也就不会写「已截轴」那行字。
    """
    fin = np.array([float(v) for v in vals
                    if v is not None and np.isfinite(float(v))], dtype=float)
    if fin.size < 8:                       # 样本太少，四分位没有意义
        return None, None
    q1, q3 = np.percentile(fin, [25, 75])
    iqr = q3 - q1
    fence_hi, fence_lo = q3 + k * iqr, q1 - k * iqr
    inb = fin[(fin <= fence_hi) & (fin >= fence_lo)]
    if inb.size == 0 or not ((fin > fence_hi).any() or (fin < fence_lo).any()):
        return None, None
    span = float(inb.max() - inb.min()) or 1.0
    hi = (math.ceil((inb.max() + pad * span) / step) * step if (fin > fence_hi).any()
          else math.ceil(inb.max() / step) * step)
    lo = (math.floor((inb.min() - pad * span) / step) * step if (fin < fence_lo).any()
          else math.floor(inb.min() / step) * step)
    return float(lo), float(hi)


def brk_pack(idx, n=None, acq=True, cal=()):
    """把结构性断点映射到某张图窗口里的 x 索引，返回 (exhibit 字段, 该图的断点图注)。

    acq=True   画整体并表断点（ACQ_BREAKS）。画 **as-reported** 存量 / 流量的图要开；
               有机口径的图（已按公司披露把并购导入扣掉）关掉 —— 那条线本来就没有台阶，
               画一条「这里不可比」的线反而是假话。
    cal        额外要画的**口径断点**族名，取自 CAL_BREAKS 的键（'nna' / 'cash'）。
               引擎每张图只吃一条 break_at 列表，所以两类断点在这里合并、按 x 位置排序。

    窗口盖不到的断点自动省略，一条都盖不到时返回 ({}, '') —— 图上不画、图注也不会
    声称画了。**断点滚出窗口是常态而不是错误**：窗口每月往前滚，2025-08 迟早会掉出
    13 个月窗口。这里原先写的是「不在窗口里就 raise SystemExit」，那意味着 LPL 的
    2026-09 数据一入库整页就永久停更（monthly_run 每天 `lpla FAIL`、页面冻结在旧月）。
    build/schw.py 的 brk_idx() 与 build/wealth.py 的 brks() 早就是这个写法，本文件跟上。

    idx 可以是月度也可以是季度索引：断点按目标索引的频率折算（月度 2025-08 → 2025Q3）。
    """
    lst = list(idx)
    if not lst:
        return {}, ''
    fr = lst[0].freq
    hits = []                       # (x 位置, 短标签, 长说明, 是不是并表)
    seen = set()
    src = ([(p, lab, BRK_TXT[lab], True) for p, lab in ACQ_BREAKS] if acq else [])
    for fam in cal:
        src += [(p, lab, tx, False) for p, lab, tx in CAL_BREAKS[fam]]
    for p, lab, tx, is_acq in src:
        q = p.asfreq(fr) if fr != p.freq else p
        if q not in lst:
            continue
        i = lst.index(q)
        if i in seen:               # 同一根柱上两个断点：引擎只画得下一条线
            continue
        seen.add(i)
        hits.append((i, lab, tx, is_acq))
    if not hits:
        return {}, ''
    hits.sort()
    if n is not None:
        if any(h[3] for h in hits):
            DRAWN.append(n)
        if any(not h[3] for h in hits):
            DRAWN_CAL.append(n)
    a_tx = [h[2] for h in hits if h[3]]
    c_tx = [h[2] for h in hits if not h[3]]
    bits = []
    if a_tx:
        bits.append('Red dashed line' + ('s' if len(a_tx) > 1 else '') + ' = '
                    + ' and '.join(a_tx)
                    + ' — whole-firm onboardings, not organic flow; readings to the left of '
                      'each line are not directly comparable with those to the right')
    if c_tx:
        bits.append('红色竖虚线还标出<b>官方改定义</b>的月份：' + '；'.join(c_tx)
                    + '。这类断点不是并表，序列本身没有台阶，但线两侧量的不是同一个东西')
    return {'break_at': [h[0] for h in hits], 'break_label': [h[1] for h in hits]}, '；'.join(bits)


# ────────────────────────────── 读数 ──────────────────────────────
def mlab(p):
    return p.strftime('%b-%y')


def source_day(month):
    """series/source_dates.csv 里 lpla 这个月的官方发布日；没有就返回 None。

    不能裸 import source_dates：本文件是 `python3 build/lpla.py` 跑的，sys.path 上只有 build/。
    """
    import importlib.util
    spec = importlib.util.spec_from_file_location(
        'source_dates', os.path.join(ROOT, 'source_dates.py'))
    mod = importlib.util.module_from_spec(spec)
    spec.loader.exec_module(mod)
    return mod.lookup(SERIES, 'lpla', str(month))


def load():
    df = pd.read_csv(os.path.join(SERIES, 'lpla.csv'))
    df['month'] = pd.PeriodIndex(df['month'], freq='M')
    df = df.set_index('month').sort_index()
    for c in df.columns:
        df[c] = pd.to_numeric(df[c], errors='coerce')
    need = ['total_assets_usdbn', 'advisory_assets_usdbn', 'brokerage_assets_usdbn',
            'nna_total_usdbn', 'nna_advisory_usdbn', 'nna_brokerage_usdbn',
            'client_cash_usdbn']
    missing = [c for c in need if c not in df.columns]
    if missing:
        raise SystemExit(f'series/lpla.csv 缺列: {missing}')
    # 逐月连续性：断档会让「相隔数月的两点」被画成相邻柱（规矩 3）
    idx = list(df.index)
    for i in range(1, len(idx)):
        if (idx[i] - idx[i - 1]).n != 1:
            raise SystemExit(f'series/lpla.csv 月份不连续: {idx[i-1]} → {idx[i]}')
    return df


def rate_series(metric, to=None):
    """series/fee_rates.csv 里 LPLA 的季度序列，索引 PeriodIndex(freq='Q')。"""
    d = pd.read_csv(os.path.join(SERIES, 'fee_rates.csv'))
    d = d[(d['company'] == 'LPLA') & (d['metric'] == metric)].copy()
    if not len(d):
        raise SystemExit(f'fee_rates.csv 里没有 LPLA/{metric}')
    d['q'] = pd.PeriodIndex(d['period'].str.replace('-', '', regex=False), freq='Q')
    out = d.set_index('q')['value'].astype(float).sort_index()
    if to:
        units = set(d['unit'].dropna())
        if len(units) != 1:
            raise SystemExit(f'LPLA/{metric} 单位不唯一: {units}')
        scale = {('USD_k', 'mn'): 1e-3, ('USD_mn', 'mn'): 1.0, ('USD_bn', 'mn'): 1e3}
        u = units.pop()
        if (u, to) not in scale:
            raise SystemExit(f'LPLA/{metric} 单位 {u} 无法换算到 {to}')
        out = out * scale[(u, to)]
    return out


# ────────────────────────────── 格式化零件 ──────────────────────────────
def _nz(v, dec):
    """四舍五入到 dec 位后落在零上的值一律归正零 —— 否则 -0.04 会印成「-0.0」/「-0bp」，
    读者会当成一个（很小的）负数，而它其实是「按这个精度就是 0」。"""
    v = float(v)
    return 0.0 if round(v, dec) == 0 else v


def pm(v, dec=1, sfx=''):
    """带正负号的差异文本。四舍五入后为 0 时不带符号 —— 「+0.0%」和「-0bp」
    都会被读成一个方向明确的小变化，而它们其实是「按这个精度没变」。"""
    z = _nz(v, dec)
    sign = '' if z == 0 else ('+' if z > 0 else '-')
    return f'{sign}{abs(z):,.{dec}f}{sfx}'


def num(v, dec=1, money='', pct=False):
    if v is None or not np.isfinite(v):
        return '—'
    return f'{money}{_nz(v, dec):,.{dec}f}' + ('%' if pct else '')


def usd_txt(v, dec=0):
    """美元金额（单位词由调用处接）。负号放在 `$` 之前、用 U+2212（`−$5`），与引擎 usd*
    格式器同规（docs/CHART_KINDS.md §2）；按 dec 位印出来是 0 的不带符号。"""
    z = _nz(v, dec)
    return ('−' if z < 0 else '') + f'${abs(z):,.{dec}f}'


def L(a):
    """序列 → JSON 数组，非有限值写 None（页面自动断开，不画假线）。"""
    return [None if (v is None or not np.isfinite(float(v)))
            else (round(float(v), 6) or 0.0) for v in a]


def yoy_txt(s, pct_series=False):
    """gs_bar 气泡里的 y/y 文案。比率序列用 pp（GS 规矩 2），水平值用百分比。"""
    a, b = float(s.iloc[-1]), float(s.iloc[-13])
    if not (np.isfinite(a) and np.isfinite(b)):
        return None
    if pct_series:
        return f'{_nz(a - b, 1):+.1f}pp y/y'
    if b == 0 or a * b < 0:
        return None
    return f'{_nz((a / b - 1) * 100, 0):+.0f}% y/y'


def yoy_series(s, idx, pct_series=False, lag=12):
    """次轴 y/y 折线的取值，逐行照搬 gsx.lvl_bar 的口径（build/gsx.py:279-289）：

      · 比率序列（pct_series）→ **百分点差**，不是「百分比的百分比变化」（GS 规矩 2）；
      · 水平值 → 百分比变化，但基数过小（|base| < 0.15 × 全序列 |值| 中位数）或
        与当期异号时放弃 —— 那种同比只是把一个接近零的分母放大成三位数噪音。

    口径判断一律在 Python 侧做完，引擎不替我们算（见 build/engine_kinds.md §8）。
    """
    s = s.dropna()
    v = s.values.astype(float)
    scale = float(np.nanmedian(np.abs(v))) or 1.0
    out = pd.Series(np.nan, index=s.index, dtype=float)
    for i in range(lag, len(v)):
        a, b = v[i], v[i - lag]
        if not (np.isfinite(a) and np.isfinite(b)):
            continue
        if pct_series:
            out.iloc[i] = a - b
        elif abs(b) < 0.15 * scale or a * b < 0:
            continue
        else:
            out.iloc[i] = (a / b - 1) * 100
    return out.reindex(idx)


# ────────────────── 同比口径：数值实现一律走 build/yoy.py ──────────────────
# ⚠ **2026-09 的口径改动**：本页流量类（Exhibit 3 有机 NNA、Exhibit 4 年化有机增速）
# 次轴的金线从「12 个月滚动合计同比」改成**单月同比**（当月 ÷ 去年同月 − 1；
# Exhibit 4 是比率，取百分点差）。理由是一句可核对的事实 ——
# **页面所有者要求全站统一成单月口径**（CONTRACT §6 抬头引了原话），不是「看着更
# 灵敏」那种 §6.1 第 3 条点名禁止的说法。改完之后 CONTRACT §6 本身也翻了面：
# §6.1 第 1 条现在写的就是「流量序列用单月同比，走 yoy.mom_yoy(s, yoy.FLOW)」，
# 并把「柱与线取自同一列、读者能自己核对」列为它的决定性好处；
# §6.2 那张「所有者点名保留滚动」的例外表里**没有本页**。
# 要照办的是 §6.1 第 3 条：每一张画**流量**同比的图都得印出单月口径的**代价**
#（该条自己把范围限定在流量：只有流量列换了口径，存量与比率没有可对照的滚动口径）
# （逐月标准差、相邻月最大跳变带月份、符号相反的月份数），由 mom_cost_note() 现算。
# 图例名与 ylab2 里带「单月」（tools/check_yoy_caliber.py 的 R4 只认
# title / ylab2 / legend / 序列名这四处）。
#
# 代价不隐瞒也不复述：单月同比的分母是**去年那一个月**，NNA 是流量，月度分布带季节性与
# 大额单笔的时点噪音，分母越小同一笔绝对变化被放大得越狠 —— 这不是业务信号，是除法的
# 性质。⚠ 具体的标准差倍数、读数区间、相邻月最大跳变、符号相反的月份数**一律由
# mom_cost_note() 拿本序列现算写进图注**，本注释里不写死数字（写死一次就过期一次）。
# 滚动口径仍然算，但只作**对照**（roll_yoy_series），图上不画。
#
# ⚠ 一条更正（2026-08-07，仍然成立）：早先本文件写过「存量不许做滚动合计，因为 12
# 个月末值相加不是任何东西」。前半句对、**后半句推不出「存量只能点对点」**：Σ12/Σ12′
# 里的除数约掉，12 个月滚动**合计**比恒等于 12 个月滚动**均值**比（共享模块 build/yoy.py
# 实测两者差 2.3e-14），而「去年一整年的平均客户资产 vs 前年」是个真实存在的量。
# 错的只是**「合计」这个名字**。所以存量**可以**平滑（走 yoy.ttm_mean_yoy，
# 文案必须写「12 个月均值同比」），本页仍保留点对点，**主因是结构性的**：均值口径按构造
# 滞后约半年、回答的是「去年一整年的平均水平」而不是「现在相对去年此刻」。
# ⚠ 这里从前写的是「客户现金就是最硬的一条：换成均值口径标准差反而变大」——**那是假的**，
# 而且被同一页 Exhibit 9 自己的图注当场证伪（图注是现算的，印出来是均值口径更平滑）。
# 哪条序列换了口径更吵、哪条更稳，一律由 stock_keep_note / caliber_stats 现算，
# 注释与散文里都不写死方向。
# 比率序列另说：12 个月的比率做算术平均本身没有意义（每个月分母不同），
# yoy.ttm_mean_yoy 对 RATIO 直接抛 CaliberError，那是一条真的硬约束。
def mom_yoy_series(s, idx):
    """单月同比（%）—— 流量类**图上画的就是这一条**。实现走共享模块，本文件不另写一份。"""
    return yoy.mom_yoy(s, yoy.FLOW).reindex(idx)


def roll_yoy_series(s, idx):
    """12 个月滚动合计同比（%）—— 流量类的**对照**口径，2026-09 起本页不再画它。

    留着不是摆设：mom_cost_note() 要拿它量出「换成单月口径的代价是多少」，
    汇总表表注与页尾口径说明里的并排读数也来自它。数值实现走共享模块。
    """
    return yoy.ttm_yoy(s, yoy.FLOW).reindex(idx)


def pp_yoy_series(s, idx):
    """**比率**序列的单月同比 = 百分点差（pp）。数值实现走共享模块，本文件不另写一份。

    `yoy.mom_yoy(kind=RATIO)` 的定义就是 v − v.shift(12)（按索引对齐，缺月留 NaN）。
    ⚠ 这个薄壳是**必需的**，不是顺手包的：下面 `bar()` 有一个叫 `yoy` 的关键字参数，
    在那个函数体里 `yoy` 是个 bool，模块名被遮住 —— 同一页的 mom_yoy_series /
    roll_yoy_series / mean_yoy_series 三个壳也是为这件事存在的。
    ⚠ 也**不能**改用本页的 `yoy_series(..., pct_series=True)` 代替：那个函数先 dropna
    再按位置取 lag=12，序列中间缺月时「12 期之前」就不再是「去年同月」。
    """
    return yoy.mom_yoy(s, yoy.RATIO).reindex(idx)


def mean_yoy_series(s, idx):
    """12 个月滚动**均值**同比（%）—— 存量类唯一说得通的平滑口径。

    数值上与滚动合计比完全相同（除数约掉），差别只在**说法**：对存量，
    「12 个月合计」不指代任何真实的量，「去年一整年的平均余额」才是。
    本文件只拿它做反事实对照，图上画的仍是点对点。
    """
    return yoy.ttm_mean_yoy(s, yoy.STOCK).reindex(idx)


def flow_stats(s, idx):
    """**流量列**的两口径实测对比 —— 统计量整段由 `yoy.caliber_diff()` 出，本页不自己算。

    CONTRACT §6.4 白纸黑字：「`yoy.caliber_diff()` 已经先取交集再比，别自己写」。
    对齐、逐月标准差、相邻月最大跳变、符号相反的月份，四样全部取自它返回的那一个
    dict —— 与 `build/single.py::mom_cost_zh()`、`build/yoy.py::describe()` 拿的是
    同一个来源，页面上这几处印的「逐月标准差」因此是同一个统计量的同一个定义。

    ⚠ caliber_diff 的相邻月跳变只量「相邻两个月**都在交集里**」的那些对（它的
    docstring 明写：跨过一个空洞的「跳变」不是跳变）。下面 `caliber_stats()` 里那份
    自写的 `_jump` 做不到这一点 —— 交集有洞时它会把洞两侧接起来当成一次跳变。

    caliber_diff 不返回读数区间与当期并排值（它是诊断器，不是取数器），这两样在下面
    按它给出的**同一批月份**补齐 —— 补的是取值，不是统计量。
    """
    d = yoy.caliber_diff(s, yoy.FLOW, win=list(idx))
    keep = list(d['months'])
    if len(keep) < 3:
        return None
    A = mom_yoy_series(s, keep).astype(float)
    B = roll_yoy_series(s, keep).astype(float)

    def _at(j):
        """caliber_diff 的 (跳变值, 前一月, 后一月) → 本页图注要的 (值, 「X → Y」)。
        CONTRACT §6.1 第 3 条要求带月份 —— 一个光秃秃的「最大跳变 N pp」读者没法
        回到图上去核，带上月份就能。"""
        return (float(j[0]), f'{mlab(j[1])} → {mlab(j[2])}') if j else (float('nan'), '')

    _jm, _jm_at = _at(d['maxjump_mom'])
    _jr, _jr_at = _at(d['maxjump_ttm'])
    return {'n': d['n'], 'sd_m': d['std_mom'], 'sd_r': d['std_ttm'],
            'jump_m': _jm, 'jump_r': _jr, 'jump_m_at': _jm_at, 'jump_r_at': _jr_at,
            'flips': [(mlab(p), float(a), float(b)) for p, a, b in d['opposite']],
            'lo_m': float(A.min()), 'hi_m': float(A.max()),
            'lo_r': float(B.min()), 'hi_r': float(B.max()),
            'cur_m': float(A.iloc[-1]), 'cur_r': float(B.iloc[-1]),
            'first': keep[0], 'last': keep[-1]}


def caliber_stats(mono, roll, idx):
    """两种口径的实测对比。返回 None 表示可比月份不足 3 个。

    ⚠ **必须先对齐到两种口径都算得出的同一批月份**：滚动口径天然少掉头 12 个月，
    不对齐就是拿两个不同样本比波动，样本效应会伪装成口径效应。

    ⚠ **为什么这一份没有并进 `yoy.caliber_diff()`**：它只对 `kind=FLOW` 做两口径对比，
    其余两种 kind 的滚动侧一律返回 None（存量做滚动**合计**非法、比率连合法的对照
    口径都没有）。而本页要比的另外两对恰恰不是 FLOW：
      · 存量对 12 个月滚动**均值**同比（`stock_keep_note`，走 yoy.ttm_mean_yoy）；
      · 由流量推导的比率，对「分子分母各自滚动 12 个月再相除」的那条比率
        （`bar(pct_series=True, roll_col='organic_growth_roll')`）—— 那条不是 12 个月
        比率的算术平均，共享模块里没有它。
    所以这两对只能在这里算。**但估计量必须与共享模块同一个**：下面用的是样本标准差
    （`ddof=1`）。共享模块那边写成 `np.nanstd(x, ddof=1)`，这里写成 pandas 的
    `.std(ddof=1)` —— keep 已经把非有限值滤干净，两种写法逐值相同（实测差 0.0）——
    这里曾经写的是 `ddof=0`，于是同一个词「逐月标准差」在站上有了两种定义。两者之比
    恒为 sqrt((n−1)/n)，与数据无关。纯流量列不走这里，走上面的 flow_stats()。
    """
    A, B = mono.reindex(idx), roll.reindex(idx)
    keep = [p for p in idx if np.isfinite(A.loc[p]) and np.isfinite(B.loc[p])]
    if len(keep) < 3:
        return None
    A, B = A.loc[keep].astype(float), B.loc[keep].astype(float)

    def _jump(x):
        """相邻月最大跳变 **与它落在哪两个月**。CONTRACT §6.1 第 3 条要求带月份 ——
        一个光秃秃的「最大跳变 N pp」读者没法回到图上去核，带上月份就能。"""
        if len(x) < 2:
            return float('nan'), ''
        d = np.abs(np.diff(x.values))
        i = int(np.argmax(d))
        return float(d[i]), f'{mlab(keep[i])} → {mlab(keep[i + 1])}'

    _jm, _jm_at = _jump(A)
    _jr, _jr_at = _jump(B)
    return {'n': len(keep), 'sd_m': float(A.std(ddof=1)), 'sd_r': float(B.std(ddof=1)),
            'jump_m': _jm, 'jump_r': _jr, 'jump_m_at': _jm_at, 'jump_r_at': _jr_at,
            'flips': [(mlab(p), float(A.loc[p]), float(B.loc[p]))
                      for p in keep if A.loc[p] * B.loc[p] < 0],
            'lo_m': float(A.min()), 'hi_m': float(A.max()),
            'lo_r': float(B.min()), 'hi_r': float(B.max()),
            'cur_m': float(A.iloc[-1]), 'cur_r': float(B.iloc[-1]),
            'first': keep[0], 'last': keep[-1]}


def mom_cost_note(st, unit='%'):
    """「本图的次轴为什么是单月同比、代价是多少」——数字全部现算自本页序列。

    CONTRACT §6：全站同比只有单月一种，页面所有者定的（§6 抬头引了原话），
    页上一条 12 个月滚动合计的同比都不画。§6.1 第 3 条要求**每一张画流量同比的图**
    都印出单月口径的代价，用这条序列自己实测，报三样：逐月标准差、相邻月最大跳变
    （**带月份**）、两种口径符号相反的月份数 —— 这里三样都给，另加读数区间。
    同一条还明令**不许写「看着更灵敏」**，也**不许说「滚动口径更好但我们没用」**：
    图注要说的是代价，不是替页面上不存在的那条线背书。所以下面只报数。
    ⚠ **「同源」这个词以前在这里是含混的，现在把两件事分开写**：
      · **措辞**与 build/hood.py 的同名函数、build/single.py 的 mom_cost_zh() 同源 ——
        三处照着 §6.1 第 3 条同一份要求写，句子结构一样，但字是各写各的。
      · **统计量**：`st` 从哪儿来，决定它同不同源。纯流量列走 `flow_stats()`，数字整段
        出自 `yoy.caliber_diff()`，与 `mom_cost_zh()` / `yoy.describe()` **同一个来源**；
        由流量推导的比率（`pct_series=True`，对照侧是 roll_col 那条自己滚出来的比率）
        caliber_diff 根本不比，走 `caliber_stats()` 自己算，但用的是同一个估计量
        （样本标准差 ddof=1）。上一版 caliber_stats 用的是 ddof=0，措辞同源、
        统计量不同源，那时这句话只是半句真话。
    """
    how = ('本月读数 − 去年同月读数，出<b>百分点差</b>' if unit == 'pp'
           else '本月 ÷ 去年同月 − 1')
    head = (f'右轴金色折线为 <b>单月同比</b>（{how}）。'
            + EXC_ZH +
            f'§6.1 第 1 条同时给了它一个可核对的好处：<b>柱与线取自同一列</b> —— '
            f'拿这根柱和 {yoy.LAG} 根柱之前那根一比，就是线上这一点，读者能自己验算。')
    if st is None:
        return head + '本序列两种口径都算得出的月份不足 3 个，此处暂不给代价数字。'
    ratio = st['sd_m'] / st['sd_r'] if st['sd_r'] else float('nan')
    t = (head + f'<b>代价按 §6.1 第 3 条用本序列自己实测印出来</b>：把两种口径'
         f'<b>对齐到同一批月份</b>后（{mlab(st["first"])}–{mlab(st["last"])}，'
         f'{st["n"]} 个月；滚动那一侧只作对照、本页不画），'
         f'单月同比标准差 {st["sd_m"]:,.1f}pp、'
         f'滚动口径 {st["sd_r"]:,.1f}pp（放大 {ratio:,.1f} 倍），'
         f'读数区间 {st["lo_m"]:+,.0f}{unit}~{st["hi_m"]:+,.0f}{unit} vs '
         f'{st["lo_r"]:+,.0f}{unit}~{st["hi_r"]:+,.0f}{unit}，'
         f'相邻月最大跳变 {st["jump_m"]:,.0f}pp（{st["jump_m_at"]}）'
         f' vs {st["jump_r"]:,.0f}pp')
    if st['flips']:
        w = max(st['flips'], key=lambda f: abs(f[1] - f[2]))
        t += (f'，其中 {len(st["flips"])} 个月两种口径<b>符号相反</b> —— '
              f'最极端的 {w[0]} 单月 {w[1]:+,.0f}{unit}、滚动 {w[2]:+,.0f}{unit}，'
              f'相差 {abs(w[1] - w[2]):,.0f}pp，一个说在萎缩、一个说在扩张。')
    else:
        t += '，本窗口内两种口径没有符号相反的月份。'
    t += (f'当期并排：单月 {st["cur_m"]:+,.1f}{unit}、滚动 {st["cur_r"]:+,.1f}{unit}'
          f'（差 {abs(st["cur_m"] - st["cur_r"]):,.0f}pp）。柱本身是当月读数，没有改。'
          f'⇒ <b>这条金线要连着柱高一起读</b>：低基数月份它会被放大，'
          f'单看它挑月份能把结论说成两个方向。')
    return t


#: 每张存量图的两口径实测结果，键是那张图画的列名。页尾口径条要引其中一条时，
#: **引的就是这个对象**，不是照着同一个公式再算一遍 —— 上一轮页尾与 Exhibit 9 当场
#: 打架（页尾说客户现金换成均值口径「标准差反而更大」，同一页的图注印的正好相反），
#: 起因就是两处各说各的。同源之后，图窗一改、数据一动，两处只能一起变。
STOCK_CAL: dict = {}


def stock_keep_note(s, idx, what, col=None):
    """**存量**序列保留点对点（单月）同比的理由 —— 事实要说对，数字要现算。

    对照口径用 12 个月滚动**均值**同比（yoy.ttm_mean_yoy），不是「合计」——
    对存量，「12 个月合计」不指代任何真实的量，是个假名字；而均值口径
    （去年一整年的平均余额 vs 前年）是合法的，所以不换口径必须给实测理由。

    `col` 只用来登记到 STOCK_CAL：页尾要复述哪一条，就从那里取同一个 st。
    """
    st = caliber_stats(yoy.mom_yoy(s, yoy.STOCK), mean_yoy_series(s, idx), idx)
    if col is not None:
        STOCK_CAL[col] = st
    base = (f'右轴仍是<b>点对点（单月）同比</b>。{what}'
            '存量并非不能平滑 —— 合法的平滑口径是 <b>12 个月滚动均值同比</b>'
            '（去年一整年的平均余额 vs 前年；数值上等同于滚动合计比，除数约掉了），'
            '<b>但不能叫「12 个月合计同比」</b>：12 个月末余额相加不指代任何真实的量。'
            '本图不换的理由不是「不能换」，是实测下来换了没有收益：')
    if st is None:
        return base + '本序列两种口径都算得出的月份不足 3 个，暂时给不出对照数字。'
    ratio = st['sd_m'] / st['sd_r'] if st['sd_r'] else float('nan')
    verdict = ('<b>均值口径在这条序列上反而更吵</b>' if ratio < 1 else
               '均值口径确实更平滑，但按构造滞后约半年、回答的是另一个问题'
               '（「去年一整年的平均水平」而非「现在相对去年此刻」）')
    return (base + f'对齐到同一批月份后（{st["n"]} 个月），点对点同比标准差 '
            f'{st["sd_m"]:,.1f}pp、12 个月均值同比 {st["sd_r"]:,.1f}pp（{ratio:,.2f} 倍），'
            f'相邻月最大跳变 {st["jump_m"]:,.1f}pp vs {st["jump_r"]:,.1f}pp，'
            f'两种口径符号相反的月份 {len(st["flips"])} 个 —— {verdict}。'
            '噪声问题用轴范围解决。')


def ratio_keep_note(what):
    """**比率**序列保留点对点同比的理由 —— 硬约束，不是选择。

    12 个月的比率做算术平均没有意义（每个月的分母不同），要「一年的平均占比」
    必须用分母加权（Σ分子 ÷ Σ分母），那需要两条序列。yoy.ttm_mean_yoy() 对 RATIO
    直接抛 CaliberError，正是为了拦住这一步。
    """
    return (f'右轴是<b>点对点（单月）同比的百分点差（pp）</b>。{what}是<b>比率</b>，'
            '12 个月滚动均值同比在这里是<b>非法</b>口径：把 12 个月的比率做算术平均'
            '没有意义（每个月的分母不同），要「一年的平均水平」必须用分母加权'
            '（Σ分子 ÷ Σ分母），那要两条序列而不是这一条 —— 共享模块 '
            '<code>build/yoy.py</code> 的 <code>ttm_mean_yoy()</code> 对比率序列直接抛 '
            '<code>CaliberError</code>。所以比率只有点对点这一个口径，差异一律用 pp / bp。')


def avg_prior12(s):
    """「Prior 12mo Avg.」= 最新月之前的 12 个月均值（不含最新月）。"""
    v = s.iloc[-13:-1].astype(float)
    if not np.isfinite(v.values).any():
        raise SystemExit('avg12 无有效值')
    return round(float(np.nanmean(v.values)), 6)


def compose_brief(months, tot, adv, bro, cash, cashp, og, nna_ex, nnaa, nnab):
    """LPLA 页顶部的 ~300 字数据总结（payload 的 `brief` 字段）。

    规则库在 `build/brief.py`（R1 峰值扫描 / R2 基数护栏 / R4 单位恒等 / R5 标注 /
    R6 有效位），那边只算事实，句子在这里拼 —— 措辞是口径的一部分，属于各家自己。
    每个数字都当场从序列算，**一处硬编码都没有**：新高月计数、名次、「N 个月最低」、
    峰值停在哪个月、并表后经过几个月，下月重跑全部自己变。

    ═══ 与同比口径的关系（CONTRACT §6；2026-09 全站统一成单月之后）═══
    本页 Exhibit 3 / 4 的次轴金线 2026-09 按页面所有者的指令从 **12 个月滚动合计同比**
    改成 **单月同比**；存量图（Ex2/9/12/15）与比率图（Ex10/16）本来就是单月。
    汇总表的 m/m、y/y 列一直是单月，所以 brief、汇总表、图上的金线现在**三者同口径**，
    读者可以互相对照。措辞不因此放松：凡出现同比字样仍一律写明「单月」
    （CONTRACT §6 + tools/check_yoy_caliber.py 的 R4 都要求口径写明）——
    页面上还印着滚动口径的**对照读数**（Exhibit 3 / 4 的
    图注、汇总表表注、页尾口径条），不标口径的话读者一样会拿错数去对。
    单月读数只作**位置与基数**陈述（排名 / 反号 / 恒等式），不作趋势断言 ——
    R4 恒等式里的两个同比逐一标「单月」：恒等式两边必须同口径，混一个滚动进去
    恒等式本身就破了；落点句锚定「较去年同月」，趋势归年化有机增长率。
    （df 上的派生列 organic_growth_roll 现在只作图注里的对照 —— 它是比率不是存量，
    本函数按位置收显式参数、不扫列名，R1 峰值扫描按构造就碰不到它。）

    ═══ 定性词一律由当场算出的量决定分支（写死措辞 + 算出来的数字 = bug）═══
    「只有 / 信息量低 / 内部搬家 / 被摊薄」这些词全部挂在 if 上，而不是预设方向：
      · 计数类的「只有 N 个」一律走 `B.quant()`，量词跟着数字走；
      · 拼句子前用 `B.need()` 挡缺值 —— 缺了就**这一句不写**，不是整页构建失败；
      · 缺分支会印出自相矛盾的句子（「四条里只有四条落后」），历史重放能逮到。

    ═══ LPLA 独有，别家不能照抄 ═══
      · **并购并表是本页最大的误读源。**Atria（2024-10）与 Commonwealth（2025-08）
        是整体并表（见 ACQ / ACQ_BREAKS），当月既不是有机流入、并表后的存量也与左侧
        不可直读。所以本段的「区间」一律以**最近一个并表月之后**为起点现算 ——
        这是 lpla / wealth 两页专有的护栏。
      · **R3 日历护栏在这里根本不成立**：本页没有交易日列，资产/现金是月末时点值，
        NNA 是整月流量、公司也不按日均披露。硬套 calendar_split 会造出一个不存在的
        「日均修正」（brief.py 开头点名的那个坑）。
      · **T3 的处理与 IBKR 相反**：三条资产存量在长历史上并没有单调到被
        `is_monotonic()` 挡掉（2022 熊市那一年跌了大半年），所以 peak_scan 会老老实实
        报「三条同创新高」。给这句话降权用的是**历史新高月的频率本身**
        （`tot >= np.maximum.accumulate(tot)` 的计数），不是「上涨月频率」——
        两者是两个量：一条序列完全可以七成月份在涨、同时连续 18 个月低于前高
        （LPLA 2022-01→2023-06 正是如此），拿后者去证明前者是论据错位。
      · **分部 NNA 的主语必须钉死。**Brokerage NNA 长期为负，但 Advisory NNA 与
        全公司有机 NNA 同期为正、Brokerage 占资产同期在降 —— 那是公司战略目标的
        brokerage→advisory 内部搬家，不是客户流失。所以「失血」这类词只能在
        **全公司口径也为负**时才印，且三条对手方数字（Advisory NNA / 全公司有机 NNA /
        Brokerage 占比）必须同句给出，否则读者会把分部读成全公司。
      · **有机口径**：本页的 NNA 一律先减去公司同页披露的 Acquired NNA（R5 的
        「还原口径」在 LPLA 这里表现为「已扣披露的并购导入」），名次也按还原后的序列排。
        但**分部 NNA 公司只披露 as-reported**（ACQ 表只有总额、不按 Advisory/Brokerage
        拆分），所以分部那句必须自己标 as-reported 并把窗口内的并购导入金额写出来，
        不能让读者以为它和总量一样已经还原。
      · **现金占资产是 A/B 推导值**（公司只披露客户现金与客户资产两条），按 R4 只报
        「分子增速 ÷ 分母增速」这一个恒等关系，绝不写成「几成来自分子、几成来自分母」。
        s1 的基准是历史峰值、s4 的基准是去年同月，两句同段出现会撞车；解法是**把两个
        增速的数字直接摆进 s4**（样板 ibkr 的「是客户现金 +25.1% 除以账户 +34.3% 的商」
        就是这个写法），读者自己看得见现金同比是正是负。原先另加一句「客户现金按同比
        看反而在涨：」的过渡语已删 —— 有了数字它就是同一句话说两遍。
    """
    A = lambda s: np.asarray(s, float)
    tot, adv, bro, cash = A(tot), A(adv), A(bro), A(cash)
    cashp, og, nna_ex, nnaa, nnab = A(cashp), A(og), A(nna_ex), A(nnaa), A(nnab)
    i, n = len(months) - 1, len(months)
    sg = lambda v: '+' if v >= 0 else '-'
    bn = lambda v: f'{sg(v)}{B.usd(abs(v), 1)}bn'

    # ── R1：存量峰值扫描。skip_monotonic 保持默认开着 —— 本页四条都通不过
    #    is_monotonic（2022 年跌了大半年），所以一条都不会被剔，「同创新高」照实报，
    #    但紧接着用**历史新高月的频率**把这句话降权（T3 的相对表述）。
    ser = [('总资产', tot), ('Advisory', adv), ('Brokerage', bro), ('客户现金', cash)]
    pk = B.peak_scan(months, ser, i)
    smap = dict(ser)
    # 「新高」的信息量由 ATH 频率本身决定，不能拿上涨月频率代替（两者不是一个量）。
    # NaN 不能进 np.maximum.accumulate（会把后面整段污染成 NaN），所以逐点扫。
    def ath_flags(a):
        out, m = np.zeros(len(a), bool), -np.inf
        for t, v in enumerate(np.asarray(a, float)):
            if np.isfinite(v) and v >= m:
                out[t], m = True, v
        return out

    ft = ath_flags(tot)
    m_ath = int(np.sum(np.isfinite(tot)))
    ath = int(np.sum(ft))
    # 定性词跟着算出来的比例走：过半月份都在新高上，「创新高」才是噪音。
    v1 = '<b>新高本身信息量低</b>' if m_ath and ath / m_ath >= 0.5 else '<b>创新高并非常态</b>'
    ath_txt = f'总资产{m_ath}个月中{B.quant(ath, m_ath, "个月")}在新高上'
    # 计数只能落在 peak_scan **真的扫过**的那几条上（缺值月与被单调剔除的列不算），
    # 否则会印出「四条存量无一在峰值上」而其实只扫了两条。
    scan = [(nm, s) for nm, s in ser if nm not in set(pk['skipped']) and np.isfinite(s[i])]
    # 两个分支都要给计数：一轮回调会让四条同时掉出峰值、「同创新高」那半句消失，
    # 只剩一串落后名单时整段会掉到 render() 的 240 字下限以下，页面同样停更。
    if not scan:
        s1 = ''
    elif pk['at_peak']:
        s1 = f'{B.mo(months[i])}月末{B.cn(len(pk["at_peak"]))}条存量同创新高，但{ath_txt}，{v1}；'
    else:
        s1 = f'{B.mo(months[i])}月末{B.cn(len(scan))}条存量无一在峰值上，{ath_txt}，{v1}；'
    if s1 and pk['skipped']:
        s1 += f'{"、".join(pk["skipped"])}已按单调序列剔除；'
    if s1 and pk['off_peak']:
        # 落后的只详写差得最多的那条，其余只报个数 —— 一轮回调可能让四条同时掉出峰值，
        # 逐条写「峰值停在 X、现低 Y%」会把整段顶穿 render() 的 400 字上限，页面停更。
        # 「只有一条 / 有两条 / 多达三条」由 B.quant 按占比决定，不写死。
        gaps = sorted(((abs(smap[nm][i] / np.nanmax(smap[nm]) - 1), nm, k)
                       for nm, k in pk['off_peak']), reverse=True)
        g, wn, wk = gaps[0]
        # 个数只在「有的在峰值、有的没跟上」时才报；开头那句若已经写成
        # 「N 条存量无一在峰值上」，这里再报一遍个数就是重复，而且 scan 只剩一条时
        # 会印出「没跟上的多达一条」这种量词与语义打架的句子。
        s1 += ((f'没跟上的{B.quant(len(gaps), len(scan), "条")}：' if pk['at_peak']
                else '其中')
               + f'{wn}峰值停在{wk}、低{B.pct(g, sign=False)}。')
    elif s1:
        # 几条同时站在峰值上是个稀有事件，报它的历史频次比干说「无一落后」有信息量，
        # 而且这一支若只写四个字，整段会掉到 render() 的 240 字下限以下、页面停更。
        # 频次只对**本月扫过的那几条**算，否则「四条同处峰值」会把没扫的列也算进去。
        allpk = np.logical_and.reduce([ath_flags(s) for _, s in scan])
        k4 = int(np.sum(allpk))
        prev4 = [t for t in range(i) if allpk[t]]
        s1 += (f'{B.cn(len(scan))}条同处峰值的月份{n}个月里{B.quant(k4, n, "个")}'
               + (f'，上一次是{months[prev4[-1]]}。' if prev4 else '，本月是第一次。'))

    # ── R2：有机 NNA 的基数护栏。名次按**年化有机增速**排而不是按 $bn 排：
    #    资产基数八年翻了近四倍，同样的 $bn 在 2018 与今天不是一回事。
    #    R2 明写：m/m 与 y/y 反号、或上月处在全样本前三高时，**必须**同句给基数说明。
    #    那一句要挂在两个分支上 —— 只接「谷底反弹」分支的话，下个月环比一转负就会
    #    印出一句光秃秃的「环比 -X%」，正是 R2 点名要拦的误读，而且不会报错。
    #    这里的 m/m 与 y/y 都是**单月**口径（B.base_effect），与 Exhibit 1 汇总表、
    #    以及 Exhibit 3 的次轴金线（2026-09 起也是单月）同口径、可逐格对上。
    #    凡「同比」措辞仍必须标「单月」（CONTRACT §6）：页面上还印着滚动口径的对照读数，
    #    不标口径读者一样会拿错数去对。
    be = B.base_effect(nna_ex, i)
    nf = int(np.sum(np.isfinite(og)))
    r_prev = B.rank_of(-og, i - 1) if i >= 1 else None
    r_cur = B.rank_of(-og, i)
    mult = (be['mm'] + 1) if (be['mm'] is not None and i >= 1 and nna_ex[i - 1] > 0) else None
    g_high = (('（上月是全样本 NNA 最高月，环比属基数效应）' if be['prev_is_max'] else
               f'（上月 NNA 排全样本第{be["prev_rank"]}高，环比属基数效应）')
              if be['prev_rank'] is not None and be['prev_rank'] <= 3 else '')
    g_conf = (f'（环比与单月同比反号、上月 NNA 排第{be["prev_rank"]}，是基数不是趋势）'
              if be['conflict'] else '')
    # 「谷底反弹」那一支自己已经把「上月倒数第 N、回升后仍是倒数第 M」写全了，R2 要的
    # 基数说明在那里就已经给到；再挂一遍完整补注等于同一件事说两遍，还把整段顶出 350 字
    # （历史重放里 2025-12 撑到 356 字、2026-02 撑到 352 字，都是这么来的）。
    # 所以那一支只留「反号」这一个它自己讲不出来的事实 —— 删的是重复，不是分支判断。
    g_conf_lite = '（环比与单月同比反号）' if be['conflict'] else ''
    lead = '有机 NNA（已扣披露并购导入）'
    if not B.need(r_cur, be['mm']):
        s2 = ''
    elif be['mm'] > 0 and r_prev is not None and r_prev <= nf * 0.25:
        # 「像拐点、实为谷底反弹」这两句只在上月真的处在低位四分之一时才成立。
        # 「环比升至上月的 N 倍」而不是「环比 N 倍」：后者在中文里会被读成「增长 N 倍」。
        # 这一支已经把上月的名次（倒数第 N）写在句子里了，R2 对「上月处在前三高」的
        # 补注要求自动满足，且与本支的前提互斥，所以只接反号那一条。
        s2 = (lead + (f'环比升至上月的{mult:.1f}倍' if mult else f'环比{B.pct(be["mm"])}')
              + f'像拐点，实为谷底反弹：上月年化增速排{nf}个月倒数第{r_prev}，'
              + f'<b>回升后仍是倒数第{r_cur}</b>{g_conf_lite}。')
    else:
        s2 = (lead + f'环比{B.pct(be["mm"])}，年化增速排{nf}个月倒数第{r_cur}'
              + (f'、上月倒数第{r_prev}' if r_prev is not None else '')
              + f'{g_high or g_conf}。')

    # ── 口径背离：分部流量 vs 全公司流量。起点现算（不写死 2025-08），
    #    并表月本身不计入窗口 —— 那个月的 NNA 是并表导入，不是流量。
    #    **算不出来要降级，不能硬失败**：本文件历史上正是栽在 `.index()` 找不到就
    #    ValueError、整页永久停更（build/ibkr.py 的 Exhibit 18 注释点名的就是这件事）。
    #    样本里一笔并表都没有时（只会发生在被截短的历史上）退回「近 12 个月」的窗口，
    #    并把「并表月」相关的字样一并省掉 —— 只许声称真存在的东西。
    bidx = [(months.index(str(p_)), l_) for p_, l_ in ACQ_BREAKS if str(p_) in months]
    prior = [b for b in bidx if b[0] < i]
    jb, lab = prior[-1] if prior else (None, None)
    if bidx and bidx[-1][0] == i:
        # 本月就是并表月：流量窗口无从谈起（当月 NNA 是并表导入不是流量），
        # 这一句改成交代并表本身 —— 那才是这个月唯一该讲的读法。
        s3 = (f'本月{bidx[-1][1]}整体并表导入{B.usd(ACQ.get(months[i], 0.0), 1)}bn，'
              '既不是有机流入，并表后的存量也与左侧不可直读；'
              '本段的有机口径已把它扣除，分部 NNA 仍是 as-reported。')
    else:
        j = jb if jb is not None else max(0, i - 12)
        if i - j < 1 or not B.need(bro[i], bro[j], tot[i], tot[j]):
            s3 = ''
        else:
            f_bro = float(np.nansum(nnab[j + 1:i + 1]))
            f_adv = float(np.nansum(nnaa[j + 1:i + 1]))
            f_firm = float(np.nansum(nna_ex[j + 1:i + 1]))      # 全公司有机口径
            sh0, sh1 = bro[j] / tot[j] * 100, bro[i] / tot[i] * 100
            # R5：分部 NNA 是 as-reported，窗口内若含已披露的并购导入必须写出来 ——
            # 公司不按分部拆分，所以这一段的分部数里可能混着它（方向保守：剔掉只会更负）。
            # 逐月列会随窗口变长而顶穿 400 字上限，所以只报窗口合计与月数。
            aw = [(m, ACQ[m]) for m in months[j + 1:i + 1] if ACQ.get(m)]
            aws = sum(v for _, v in aw)
            cal = (f'（as-reported，含未拆分的{B.usd(aws, 1)}bn并购导入）'
                   if aw else '（as-reported）')
            # 结论按当场算出的三个方向定分支，不预设「失血」：
            if f_firm <= 0:
                v3 = '<b>全公司有机口径同为净流出</b>'
            elif f_bro < 0 <= f_adv and sh1 < sh0:
                v3 = '<b>是转投顾的内部搬家非客户流失</b>'
            else:
                negs = [nm for nm, v in (('Advisory', f_adv), ('Brokerage', f_bro)) if v < 0]
                v3 = ('<b>两个分部同为净流入</b>' if not negs else
                      f'<b>净流出只在{"、".join(negs)}，全公司有机口径仍为正</b>')
            # 一句一个意思（F1）：分号前只讲 Brokerage 自己（占比与流量同一个主语、同一个
            # 方向），口径标注跟在数字后面而不是插在「NNA」与「累计」中间；分号后摆两条
            # 对手方数字再落点。原先六个层次挤在一串逗号里，读起来像脚注不像导读。
            s3 = ((f'{lab}并表后{i - j}个月，' if jb is not None else f'近{i - j}个月，')
                  + f'Brokerage 占资产{"降" if sh1 < sh0 else "升"}{abs(sh1 - sh0):.1f}pp、'
                  + f'NNA 累计{bn(f_bro)}{cal}；'
                  + f'同期 Advisory {bn(f_adv)}、全公司有机 {bn(f_firm)}，{v3}。')

    # ── R4：现金占资产是 A/B 推导值。只报「分子增速 ÷ 分母增速」这一个恒等关系。
    #    原先这里还写了「同比降幅的 N% 出在并表那一个月的机械稀释」，为压字数删掉了：
    #    它是第二层分解，而这一句真正要拦的误读（占比创新低 ≠ 客户把钱搬走）由
    #    「分子分母同比双涨 → 被摊薄」这半句就已经拦住，删的是补充不是论据。
    #    这里的两个同比也是**单月**口径（B.per_unit，本月 ÷ 去年同月），措辞逐一标
    #    「单月」（CONTRACT §6）：恒等式两边必须同口径，混一个滚动进去恒等式就破了。
    #    本页存量图（Ex2/9）次轴本来就是单月同比，这两个数与图上读数同口径；
    #    落点句锚定「较去年同月」，只讲位置不作趋势断言 —— 趋势归 Exhibit 3 / 4。
    pu = B.per_unit(cash, tot, i, scale=100.0)
    ny, dy = pu.get('num_yoy'), pu.get('den_yoy')
    both_up = B.need(ny, dy) and ny > 0 and dy > 0
    mc = int(np.sum(np.isfinite(cashp)))
    if not B.need(pu['value']):
        s4 = ''
    else:
        # 「第 N 低」在 N 等于样本量时其实是最高，写死「低」字会把方向说反。
        lvl = (f'{mc}个月最低' if pu['is_min'] else
               f'{mc}个月最高' if pu['is_max'] else
               f'{mc}个月里第{pu["rank_low"]}低')
        # 恒等式照样板写法：两个增速的数字直接摆出来，后面接一句短落点。落点按当场
        # 算出的两个符号分三支，三支穷尽了 both_up 之外的全部情形，一个都不写死：
        #   分子分母同涨 → 摊薄；分子在缩 → 占比走低含真实收缩；分子涨而分母缩 → 分母推高。
        idt = (f'，是客户现金单月同比{B.pct(ny)}除以客户资产单月{B.pct(dy)}的商'
               if B.need(ny, dy) else '')
        # 同比算不出来时（样本不足 12 个月）不能装作能判断摊薄还是撤资，直接说明白 ——
        # 「该句不写」在这里不适用：读者看到占比创新低，最需要的恰恰是「凭什么还判断不了」。
        if not B.need(ny, dy):
            tail = f'，样本仅{mc}个月还算不出同比，无从判断是摊薄还是被搬走。'
        elif both_up:
            tail = idt + '，属摊薄而非撤资。'
        elif ny <= 0:
            tail = idt + '，占比走低含客户现金本身较去年同月的收缩。'
        else:
            tail = idt + '，占比是被较去年同月缩水的客户资产推高的。'
        s4 = '现金占资产（推导值）为' + lvl + tail

    return B.render([s1, s2, s3, s4])


# ══════════════════════════════════════════════════════════════════════════════
# 名词释义（payload 的 `glossary`，排在所有 exhibit 之前）
#
# ━━ 与 brief / 页尾 notes / 图注的分工 ━━
# brief 与图注说的是「**这个月**这组读数该怎么读」（含当月读数、当月实测的毛刺量），
# 每月重写；这一块说的是「**这些词**是什么意思」，一年到头是同一段
# ⇒ 这里**不写当月读数、不写「最新一期」**。出现的数只有两类：把定义钉住的结构性量
# （并购导入登记了多少笔、断点判据的 5% 门槛）与恒等式本身（有机 NNA = 总 NNA −
# Acquired NNA；年化增速 = 当月有机 NNA × 12 ÷ 上月末资产）—— 且**一个都不写死**：
# 笔数与覆盖区间从上面的 ACQ 现算，并表名单从 ACQ_BREAKS 现算，明年多一笔自己就变。
# 会随月/季变的数一律不进来（净收益率的 bp 读数、桥的平均绝对误差、各图的实测毛刺量），
# 它们在 FEE_Q_NOTE 与各图图注里逐期现算，写进释义就是一年 12 次的旧闻。
#
# ━━ 为什么是这 14 个词（选词判断）━━
# 判据只有一条：这个词出现在本页的图题 / 序列名 / 纵轴 / 汇总表行头 / 核对表列头 /
# 图注里，而且**不看定义就会读错**。按「读错会出什么事」分四类：
#   ① 流量 vs 存量，以及流量的分母   NNA、有机 NNA、Acquired NNA、年化有机增长率 ——
#      本页最密集的坑：NNA 是整月流量、资产与现金是月末存量，不能并列成一个量级读、
#      也不能拿存量当它的分母（唯一合法的相加是资产变动桥的恒等式：期初存量 + 整月
#      流量 + market gains = 期末存量 —— 释义里把这个例外明写出来，否则与同一块里
#      「Market gains」那一条以及那张桥图的图注当场打架）；而 NNA
#      的趋势又只能读年化增速（GS 规矩 1）。不点破，读者会把 NNA 与资产并起来看，
#      或者把 as-reported 的并购月当成有机流入。
#   ② 同一件事的两套口径   Advisory / Brokerage、Advisory 占比、客户现金 ——
#      两条口径都在页面上：分部 NNA 在本页只有 as-reported（本页的 ACQ 常量表只有总额、
#      series/lpla.csv 也没有按渠道的 Acquired NNA 列 —— 是**本页**拿不到，不是公司
#      没披露：官方 ≥2026-04 期的分节表里 Acquired NNA 一节下面就有 Advisory /
#      Brokerage 两行，见 fetch/lpla.py 口径坑 4），
#      客户现金 Apr-2019 换过官方行名与外延。读串的代价是整条线系统性偏高或偏低，
#      而图形完全正常，看不出来。
#   ③ 推导值，不是公司披露的数   现金占资产、隐含客户现金收入、Market gains ——
#      三条都是本页自己轧出来的（CONTRACT 要求推导值必须标 Implied）。不点破，
#      读者会拿它们去和公司公告对账，然后以为是本页算错了。
#   ④ 这一页的两类断点与两条披露节奏   整体并表、净收益率、季末月、现金搬家 ——
#      并表把存量抬一个台阶（红色竖虚线的语义是「从这一期起与左侧不可比」），
#      费率按季披露而月度数字每月往前走，季末月取自季报。
# **有意不收**：
#   · m/m、y/y、单月同比、3Y %ile、pp/bp —— 全站通用的读图约定，汇总表的 summary.note
#     与页尾第 7 条已经逐条讲过（还带着本页现算的口径代价），释义板再讲一遍就是
#     两处各写一份，改一处必漏一处。
#   · 「口径断点」这四个字本身与它的两处落点（May-2019 的 NNA 定义、Apr-2019 的现金
#     口径）—— 页尾第 3 条讲的就是这两处在本页的具体落点；这里只在 NNA 与客户现金
#     两条释义里点一句「线两侧量的不是同一个东西」，细节不复述。
#   · 客户资产、市值、成交量这类常识词 —— 本页对它们没有特殊口径。
#   · 「通栏 / x 标签抽稀」「截轴」—— 那是版式判据，不是名词，写在各图图注里。
# ══════════════════════════════════════════════════════════════════════════════
GLOSSARY = [
    ('NNA',
     'Net new assets，<b>净新增资产</b>：当月客户资产的净流入，是<b>整月的流量</b>，'
     '与客户资产 / 客户现金那几条<b>月末存量</b>不是一回事：'
     '<b>不能</b>与月末存量并列成一个量级去读，也<b>不能</b>拿存量当它的分母。'
     '⚠️ 唯一合法的相加是资产变动桥那个恒等式：'
     '<code>期初资产 + 当月 NNA + market gains = 期末资产</code> —— '
     '加的是<b>期初</b>存量与一整月的流量，不是两个月末存量并排。'
     '⚠️ <b>GS 规矩 1「流量不算百分比」</b>说的就是它：拿上个月或去年那一个月当分母'
     '都没有经济含义。本页仍按页面所有者的指令画出单月同比金线，'
     '但那条线<b>必须连着柱高一起读</b>，<b>趋势判断以「年化有机增长率」为准</b>。'
     '⚠️ 官方口径 May-2019 起 = 净流入（官方原文 <code>Asset Inflows minus Outflows</code>）'
     '+ 股息利息 − 投顾费，此前只含净流入那一行，'
     '<b>线两侧量的不是同一个东西</b>。⚠️ 本页凡<b>没标「有机」</b>的 NNA 都是 '
     '<code>as-reported</code>，<b>含</b>并购导入。'),

    ('Acquired NNA',
     '公司同页披露的一行：当月 NNA 里由并购 / 整体并表<b>导入</b>的那部分，'
     '<b>不是客户带来的新钱</b>，所以本页的有机口径把它逐月扣掉。'
     f'本页共登记 {len(ACQ)} 笔（覆盖 {min(ACQ)} … {max(ACQ)}），'
     '<b>逐笔都有官方出处、一笔估算都没有</b>（2019-08 起取官方并排印的 Total NNA 与 '
     'Total Organic NNA 之差，更早的 NPH 分批导入取官方影子块，逐笔出处见页尾'
     '「有机口径」一条）。⚠️ 它<b>不是 <code>series/lpla.csv</code> 的一列</b>'
     '（官方只在正文注里给），所以硬编码在 <code>build/lpla.py</code> 里。'),

    ('有机 NNA',
     '<code>有机 NNA = 总 NNA − 公司同页披露的 Acquired NNA</code>，'
     '按公司披露的拆分<b>逐月扣除，不整月置零</b>。'
     '⚠️ <b>本页的分部 NNA 没有有机口径</b>：本页的 Acquired NNA 常量表只登记<b>总额</b>，'
     '<code>series/lpla.csv</code> 也没有按渠道的 Acquired NNA 列'
     '（官方 ≥2026-04 期的分节表里 Acquired NNA 一节下面是有 Advisory / Brokerage 两行的，'
     '<b>拿不到渠道有机口径是本页的限制、不是公司没披露</b>）。'
     '所以本页「按渠道」的那两条 NNA 一律是 <code>as-reported</code>，'
     '拿它与有机总量并排读之前要先看窗口内有没有并表月。'),

    ('年化有机增长率',
     '<code>当月有机 NNA × 12 ÷ <b>上月末</b>客户资产</code>（GS 惯例），'
     '把整月的流量换算成对存量的年化速度 —— 这是本页读 NNA 趋势的<b>首选</b>指标，'
     '因为 NNA 是流量，拿上个月或去年那一个月当分母都没有经济含义。'
     '⚠️ 它要用<b>上月末</b>资产，所以序列的第一个月算不出来：那一格在数据里不存在，'
     '本页<b>不补零、也不前向填充</b>，图的左端因此比主窗口起点晚一期。'),

    ('Advisory / Brokerage',
     '客户资产与 NNA 的两个业务口径：<b>Advisory</b> 是收投顾费的账户，'
     '<b>Brokerage</b> 是佣金账户。两者互转是<b>内部搬家</b>：同一笔资产转过去会同时'
     '产生一笔负的 Brokerage NNA 与一笔正的 Advisory NNA，<b>全公司总 NNA 不变</b> —— '
     '所以 Brokerage NNA 为负<b>不等于</b>客户流失，判断流失要看全公司口径。'
     '官方原表另注 "Totals may not foot due to rounding"：两者相加与总额差 0.1 属正常，'
     '本页不做「相加必须等于总额」的硬校验。'),

    ('Advisory 占比',
     'advisory 资产 ÷ 总客户资产（<b>两个月末存量之比</b>），本页写作 % Advisory / '
     'Advisory share。它是<b>比率</b>：差异一律用 pp / bp，而且<b>不能</b>把 12 个月的'
     '占比做算术平均（每个月分母不同）。⚠️ 每一笔整体并表都直接改变构成'
     f'（共几笔见下面「整体并表」一条，各笔偏 advisory 还是偏 brokerage '
     f'见 Exhibit {N_ADV_SHARE} 的图注），'
     '<b>断点两侧的占比不可直读</b>。'),

    ('客户现金',
     '月末客户现金余额，取官方的 <code>Total Client Cash Balances</code>'
     '（含银行 sweep 与 purchased money market funds），'
     '<b>不是</b> Total Bank Sweep、也<b>不是</b>第三方持有的那一行。'
     '⚠️ <b>Apr-2019 是口径断点</b>：此前官方印的是 <code>Total Cash Sweep Balances</code>'
     '（只含 ICA + DCA + MMA、不含 purchased money market funds），'
     '<b>线两侧量的不是同一个东西</b>。⚠️ 官方在 sweep 与 purchased money market 之间'
     '做过转换，那几个月这条会出现<b>非资金流动</b>造成的变动'
     '（<code>fetch/lpla.py</code> 口径坑 7），不是客户提现。'),

    ('现金搬家',
     'cash sorting：客户把 sweep 里的低息现金搬去货基、定存等更高收益的去处。'
     '⇒ 余额下降的是<b>这个渠道</b>，<b>不必然是</b>客户把钱从公司拿走 —— '
     '与「客户流失」是两回事，本页判断后者要看 NNA 而不是现金余额。'),

    ('现金占资产',
     '客户现金 ÷ 客户资产（<b>两个月末存量之比</b>），本页写作 % of client assets。'
     '<b>推导值</b>：<code>series/lpla.csv</code> 只有现金与资产两条，没有这一列。'
     '它是净利息收入的主要驱动项。⚠️ 分子分母<b>跨并表同时跳</b>，'
     '断点两侧不可直读；比率的差异一律用 pp / bp。'),

    ('净收益率',
     '公司<b>按季</b>披露的客户现金年化净收益率（单位 bp，'
     '<code>series/fee_rates.csv</code> 的 <code>client_cash_net_yield</code>），'
     '是隐含收入那两张图唯一的费率来源。⚠️ 费率按季更新而月度数字每月往前走，'
     '所以某个月所属季度尚未披露费率时，该月<b>沿用上一可得季度</b>的费率 —— '
     '当期用的是哪一季、窗口里有多少个月是这么算的，逐期现算印在那两张图下的'
     '「费率期间」一行里。'),

    ('隐含客户现金收入',
     '<b>推导值，不是公司披露的月度数</b>（图上一律标 Implied）：'
     '<code>月末客户现金 × 公司披露的季度净收益率 ÷ 12</code>。'
     '它名义上是「每月多少收入」的流量，但规模基数是<b>月末存量</b>，'
     '所以跨并表跳升、也承接客户现金 Apr-2019 的口径变更。'
     '⚠️ 公司赚的是<b>平均</b>余额上的收益，而这里用的是<b>月末</b>余额 —— '
     '这个近似误差正是桥式核对那张图（Exhibit ⟨ex:cash-rev-bridge⟩）要量的东西。'),

    ('Market gains',
     '资产变动桥的<b>配平项</b>：<code>当月客户资产变动 − 当月 NNA</code>，'
     '<b>不是公司披露值</b>。恒等式是 <code>期初资产 + NNA + market gains = 期末资产</code>，'
     '所以凡是没被 NNA 解释掉的那部分全落在这一项里，<b>不止有市场涨跌</b>。'),

    ('整体并表',
     'whole-firm onboarding：一整家机构的资产被并进来。登记判据写在生成器里、'
     '构建时机器复核一遍：<b>当月 Acquired NNA ≥ 当月末客户资产的 5%</b>，'
     f'本页序列覆盖到的共 {len(ACQ_BREAKS)} 笔（'
     + '、'.join(n for _, n in ACQ_BREAKS).replace('&', '&amp;') + '）。'
     '这类月份既不是有机流入、并表后的存量也与左侧不可直读，所以图上画<b>红色竖虚线</b>'
     '并标出并购名 —— 线的语义是「<b>从这一期起</b>与左侧不可比」，'
     '不是「这一个月特殊」。⚠️ 够不上这条门槛的小额并购只从有机口径里扣除、不画线。'),

    ('季末月',
     '3 / 6 / 9 / 12 月<b>没有独立的月度新闻稿</b>，这几个月的值取自当季季报。'
     '⇒ 披露时点与其余月份不同，但<b>口径一致</b>，序列上<b>不构成断点</b>。'),
]


def main():
    df = load()
    LATEST = df.index[-1]
    tot = df['total_assets_usdbn']
    nna = df['nna_total_usdbn']

    df['pct_advisory'] = df['advisory_assets_usdbn'] / tot * 100
    df['cash_pct_assets'] = df['client_cash_usdbn'] / tot * 100
    df['market_gains'] = tot.diff() - nna
    acq = pd.Series({pd.Period(k, 'M'): v for k, v in ACQ.items()}).reindex(df.index).fillna(0.0)
    df['acquired_nna'] = acq
    df['nna_ex'] = nna - acq
    df['organic_growth_ex'] = df['nna_ex'] * 12 / tot.shift(1) * 100
    # 同一指标的**滚动 12 个月口径**：滚动 12 个月有机 NNA ÷ 12 个月前的月末客户资产。
    # 分子已经是一整年的流量，不用再乘 12。比率的滚动口径必须是「分子分母各自滚动再相除」，
    # 不能把 12 个月的比率加起来 —— 每个月的分母不同，加出来的数没有含义。
    df['organic_growth_roll'] = df['nna_ex'].rolling(12).sum() / tot.shift(12) * 100
    df['total_tn'] = tot / 1000.0

    # ── 时序主窗口的**索引**先定住（下面的费率文案要引用它；切片本身要等派生列都齐了）──
    # 从写死的 df.iloc[-25:] 改成「WIN_FROM 起」。今天 series 自 2016-01 起、WIN_FROM 也是
    # 2016-01，所以拿到的就是全序列；序列哪天更短，_w() 自动退回序列自己的起点。
    WIDX = _w(df).index
    WSPAN = f'{mlab(WIDX[0])}–{mlab(WIDX[-1])}（{len(WIDX)} 个月）'

    # ── 量→收入桥：client cash revenue = 月末客户现金 x 披露净收益率 / 12 ──
    cy = rate_series('client_cash_net_yield')                    # bp, annualised
    q_of_month = pd.PeriodIndex(df.index).asfreq('Q')
    cy_m = pd.Series([cy.get(qq, np.nan) for qq in q_of_month], index=df.index).ffill()
    df['implied_cash_rev_usdmn'] = df['client_cash_usdbn'] * 1000.0 * cy_m / 10000.0 / 12.0
    BR_NOTE = ('Assumption: monthly client-cash revenue = month-end client cash x the disclosed '
               'net yield / 12. The yield is taken from the quarterly report '
               f'({cy.index[-1]} = {cy.iloc[-1]:,.4g} bp) and held flat for months after '
               'that quarter')

    # ── 费率期间的披露文案（全部现算）───────────────────────────────────────
    # 月度数字每月往前走、费率按季度更新，所以「本月的隐含值用的是哪一季的费率」是
    # 长期存在的口径事实，不是 bug —— 但读者有权知道。文案一律从 series/fee_rates.csv
    # 现算：写死季度号的话，下一季就变成假话（本仓已经踩过 schw「过去 32 个季度」、
    # cost「Exhibit 4 画了红线」两次同类的坑）。
    def rate_q_for(p):
        """月份 p 实际用到的费率季度 = 不晚于该月所属季度的最后一个可得季度（对齐 ffill）。"""
        prior = [q for q in cy.index if q <= p.asfreq('Q')]
        return prior[-1] if prior else None

    LAT_Q = LATEST.asfreq('Q')            # 本页数据最新月所在季度
    RQ_LAST = cy.index[-1]                # 最新可得费率季度
    RQ_LAT = rate_q_for(LATEST)           # 最新月真正用到的费率季度
    LAG_Q = None if RQ_LAT is None else (LAT_Q - RQ_LAT).n
    # 「过期」判据：最新可得费率季度比「数据月所在季度的上一季」还老（滞后 ≥ 2 个季度）。
    # 正常节奏下 LAG_Q 只会是 0 或 1（季初一两个月还没等到当季财报），那是口径不是异常。
    FEE_STALE = LAG_Q is not None and LAG_Q >= 2
    # 窗口内所属季度尚无披露费率、只能沿用上一季的月份
    CARRY_M = [p for p in WIDX if p.asfreq('Q') not in cy.index]
    _carry_span = ('' if not CARRY_M else
                   mlab(CARRY_M[0]) if len(CARRY_M) == 1
                   else f'{mlab(CARRY_M[0])}–{mlab(CARRY_M[-1])}')
    FEE_Q_NOTE = (
        f'<b>费率期间：</b>净收益率按季度披露，本页月度数据截至 {mlab(LATEST)}（{LAT_Q}）。'
        + (f'{mlab(LATEST)} 的隐含值取 {RQ_LAT} 公司披露的 {float(cy.loc[RQ_LAT]):,.4g} bp'
           + ('（与该月同季，无滞后）。' if LAG_Q == 0 else
              f'（{LAT_Q} 尚无披露费率，沿用上一可得季度，滞后 {LAG_Q} 个季度）。')
           if RQ_LAT is not None else '最新月早于任何一个可得费率季度，隐含值留空。')
        + (f'主窗口 {WSPAN} 里有 {len(CARRY_M)} 个月（{_carry_span}）所属季度尚无披露费率、'
           '沿用上一季费率，其余月份用的是本季费率。' if CARRY_M else
           f'主窗口 {WSPAN} 里每个月都用到了自己所属季度的披露费率。')
        + (f'<b>⚠️ 费率尚未更新至 {LAT_Q}，也未更新至 {LAT_Q - 1}</b> —— 本图仍用 '
           f'{RQ_LAST} 的读数（滞后 {LAG_Q} 个季度，通常是公司财报延后）。'
           '这期间利率环境若已变化，隐含值不会跟着动，落差要算在费率口径上、不是业务上。'
           if FEE_STALE else ''))

    # 桥的季度验证：只保留满 3 个月的季度
    imp_m = df['implied_cash_rev_usdmn'].dropna()
    qi = pd.PeriodIndex(imp_m.index).asfreq('Q')
    imp_q = imp_m.groupby(qi).sum()
    cnt = pd.Series(1, index=imp_m.index).groupby(qi).sum()
    imp_q = imp_q.loc[[q for q in imp_q.index if cnt.get(q, 0) == 3]]
    act_q = rate_series('client_cash_revenue', to='mn')

    # ── 三个窗口（W25 的左端在上面已定为 WIDX，这里才切 —— 派生列到此刻才齐）──
    W25 = df.loc[WIDX]
    # 近端对比图（堆叠 / 滚存桥）仍是 13 个月：它们要看的是「最近一年发生了什么」，
    # 拉到十年只会把 12 根有意义的柱压成一堵墙。这与「时序图放宽到 2016」不矛盾 ——
    # 放宽的是**趋势图**的窗口，不是所有图的窗口（cboe / cme 两页也是这么分的）。
    W13 = df.iloc[-WIN_S:]
    XL25 = [mlab(p) for p in W25.index]
    XL13 = [mlab(p) for p in W13.index]
    XLALL = [mlab(p) for p in df.index]
    # 四套断点组合：谁画并表、谁画口径断点，按那张图画的是哪条序列决定（见 brk_pack）。
    BK25, BN25 = brk_pack(W25.index)                            # as-reported 资产
    BKC, BNC = brk_pack(W25.index, cal=('cash',))               # 客户现金一族
    BKN, BNN = brk_pack(W25.index, acq=False, cal=('nna',))     # 有机 NNA（并表已扣）
    BKNA, BNNA = brk_pack(W25.index, cal=('nna',))              # as-reported NNA
    BK13, BN13 = brk_pack(W13.index)
    BK13N, BN13N = brk_pack(W13.index, cal=('nna',))
    BKALL, BNALL = brk_pack(df.index)
    BKALLC, BNALLC = brk_pack(df.index, cal=('cash',))

    # gsx.lvl_bar 的 docstring：「次轴画的是同比而不是滚动均线 —— 均线只是把柱子再平滑
    # 一遍、不带新信息，同比才回答『相对去年这个月是好是坏』」。本页是从 gsx deck 移植
    # 的，所以照 deck 画次轴 y/y、不画均线（cost / ibkr 两页不是从 deck 移植的，保持均线）。
    YOY_NOTE = ('右轴金色折线为同比（同原 deck 的 gsx.lvl_bar 次轴），不再画 12 个月均线：'
                '均线只是把柱子再平滑一遍、不带新信息，而且它按构造落在柱子中段，'
                '凡是当月值接近均值的月份数值标签都会被它拦腰划断。')

    def bar(n, col, title, legend, ylab, fmt, note, pct_series=False,
            window=W25, xl=XL25, breaks=None, brk_note='', extra=None,
            yoy=True, ylab2=None, flow=True, roll_col=None, keep_why=None):
        """gsx.lvl_bar → 网页 gs_bar（柱 + 每柱数值 + 次轴 y/y 折线）。

        **次轴一律画单月同比**（2026-09 起，按页面所有者的指令全站统一），
        `flow` 只决定**为什么是单月**这句话怎么写：
        `flow=True`（流量类）→ mom_cost_note：口径是所有者指定的，代价（与滚动口径的
        标准差之比、读数区间、相邻月最大跳变、符号相反的月份）由本序列现算印出；
        `flow=False` + `pct_series=True` → ratio_keep_note（比率不许做滚动均值，硬约束）；
        `flow=False` + 其余 → stock_keep_note（存量可以做滚动**均值**，不做的理由由实测给出）。
        口径判断只在调用点写一次，图例标签、图注、实测对比都由这里按同一个开关生成，
        这样「图例上写的口径」与「实际算的口径」不可能分叉。

        `roll_col` 给比率序列用：比率的滚动口径不是把比率加起来，而是分子分母各自
        滚动 12 个月再相除（本页只有 organic_growth_roll 一条）。2026-09 之后它
        **不再进图**，只作 mom_cost_note 的对照那一侧。

        yoy=False 只留给「同比本身没有意义」的序列，退回 Prior-12mo 均线 + y/y 气泡。
        """
        s = df[col].dropna()
        ex = {'n': n, 'kind': 'gs_bar', 'fmt': fmt, 'yfmt': fmt, 'xlabels': xl,
              'title': title, 'ylab': ylab, 'legend': legend,
              'values': L(window[col].values)}
        if yoy:
            # 单月同比 mono：流量类走共享模块 mom_yoy_series（口径的唯一实现）；
            # 比率取**百分点差**，对照那一侧也走共享模块（yoy.mom_yoy(kind=RATIO) 的定义
            # 就是 v − v.shift(12)）—— 这里原先手写 `src - src.shift(12)`，数值一样，
            # 但那是同一个口径的第二份实现。存量仍走本页的 yoy_series()（它比 mom_yoy
            # 多一道近零基数护栏，这次改动不碰存量）。rl 是**对照**那一侧，只进图注不进图。
            if pct_series:
                mono = yoy_series(df[col], window.index, True)
                src = df[roll_col] if roll_col else df[col]
                rl = pp_yoy_series(src, window.index)
            elif flow:
                mono = mom_yoy_series(df[col], window.index)
                rl = roll_yoy_series(df[col], window.index)
            else:
                mono = yoy_series(df[col], window.index)
                rl = roll_yoy_series(df[col], window.index)
            yv = mono
            cal = 'y/y (pp, 单月, RHS)' if pct_series else 'y/y (单月, RHS)'
            ex['ylab2'] = ylab2 or cal.replace(', RHS', '')
            ex['yoy'] = {'name': cal, 'color': 'GOLD', 'values': L(yv.values),
                         'yfmt': 'pp1' if pct_series else 'pct0'}
            u = 'pp' if pct_series else '%'
            if flow:
                # 纯流量列的统计量走共享模块（CONTRACT §6.4：caliber_diff 已经先取交集
                # 再比，别自己写）；pct_series 那一支比的是「本页自己滚出来的那条比率」，
                # caliber_diff 不比这一对，只能自己算 —— 两条路的估计量相同，见两个函数。
                _st = (caliber_stats(mono, rl, window.index) if pct_series
                       else flow_stats(df[col], window.index))
                why = mom_cost_note(_st, u)
            elif pct_series:
                why = ratio_keep_note(keep_why or title)
            else:
                why = stock_keep_note(df[col], window.index, keep_why or title, col)
            note = (note or '') + why
        else:
            ex['avg12'] = avg_prior12(s)
            y = yoy_txt(s, pct_series)
            if y:
                ex['yoy_txt'] = y
        if breaks:
            ex.update(breaks)
            if n is not None:
                _cal_lab = {l for _p, l, _t in CAL_BREAKS['nna'] + CAL_BREAKS['cash']}
                _lbs = breaks.get('break_label', [])
                if any(l not in _cal_lab for l in _lbs):
                    DRAWN.append(n)
                if any(l in _cal_lab for l in _lbs):
                    DRAWN_CAL.append(n)
        if note or brk_note:
            ex['note'] = ((note or '') + ((brk_note + '。') if brk_note else ''))
        if extra:
            ex['src_extra'] = extra
        return ex

    ex = []

    def add(exd, breaks=None, cal_only=False):
        """把断点字段并进 exhibit 再登记编号 —— notes 里「哪几张图画了断点线」那句现算，
        写死一串编号，窗口滚过断点的那个月就变成假话。

        cal_only=True：这张图画的只有口径断点，不该登记进「并购断点」那份名单。
        """
        if breaks:
            exd.update(breaks)
            (DRAWN_CAL if cal_only else DRAWN).append(exd['n'])
            if not cal_only and any(lb in {l for _p, l, _t in
                                           CAL_BREAKS['nna'] + CAL_BREAKS['cash']}
                                    for lb in breaks.get('break_label', [])):
                DRAWN_CAL.append(exd['n'])
        ex.append(exd)
        return exd

    # ── Exhibit 2：Total client assets（gsx.lvl_bar, 主窗口, dec=2, $tn）──
    # 存量：月末余额，次轴保留单月同比（flow=False）。
    ex.append(bar(_S(2), 'total_tn', 'Total client assets', 'Total client assets',
                  'Total client assets ($tn)', 'f2',
                  f'原 deck 的 gsx.lvl_bar（dec=2、单位 $tn）。柱为月末客户资产总额。'
                  f'<b>窗口 {WSPAN}</b>：原 deck 写死「近 25 个月」，序列回补到 {df.index[0]} '
                  f'之后改成日历常量 {WIN_FROM} 起（右端跟着数据最新月走）。' + YOY_NOTE,
                  breaks=BK25, brk_note=BN25, flow=False,
                  keep_why='客户资产是<b>月末存量</b>。'))

    # ── Exhibit 3：Organic net new assets（gsx.lvl_bar, 主窗口, dec=1, $bn）──
    # 这张图的同比线经历过三个状态，图注里要把话说全，不能只留最后一句：
    #   ① 更早的版本一条同比线都不画，理由是「单月同比在本窗口内三位数摆动，读不出东西」；
    #   ② 2026-08 换成 12 个月滚动合计同比，读数区间收到两位数，线就加回来了；
    #   ③ 2026-09 按**页面所有者的指令**改回单月同比（全站统一口径）。
    # 于是 ① 里那个「读不出东西」的毛病又回来了 —— 这不是没人注意到，是所有者的取舍：
    # 口径统一优先于单张图的可读性。所以图注必须同时印出**当初的理由**与**现在的代价**
    # （区间、标准差、跳变、符号相反的月份），由 mom_cost_note() 拿本序列现算，
    # 不许只写「按指令改成单月」了事。
    # 有机 NNA 是流量列，统计量因此走共享模块（CONTRACT §6.4：caliber_diff 已经先取
    # 交集再比，别自己写）—— 这张图注引的读数区间与下面那段代价里的标准差，来源同一。
    _ST3 = flow_stats(df['nna_ex'], W25.index)
    _acq_big = '、'.join(f'{lab} {mlab(p)} ${ACQ[str(p)]:,.1f}bn' if str(p) in ACQ
                        else f'{lab} {mlab(p)}'
                        for p, lab in ACQ_BREAKS if p in W25.index)
    ex.append(bar(_S(3), 'nna_ex', 'Organic net new assets', 'Organic NNA',
                  'Organic net new assets ($bn)', 'usd1',
                  'Total NNA less the Acquired NNA that LPL discloses on the same page'
                  + (f'（窗口内最大的几笔：{_acq_big}；NPH 是 Dec-17 起分批导入）。'
                     if _acq_big else '。')
                  + '并购导入按公司披露的拆分逐月扣除，不整月置零。'
                  '<b>这条次轴金线的口径变过两次，两次都记在这里</b>：更早的版本一条同比线'
                  '都不画，理由是实算的单月同比在本窗口内'
                  + (f'介于 {_ST3["lo_m"]:+,.0f}% 与 {_ST3["hi_m"]:+,.0f}% 之间'
                     if _ST3 else '在三位数以上') +
                  '，一条贴零的直线加一根冲天尖峰；2026-08 曾换成 12 个月滚动合计同比，'
                  '读数区间'
                  + (f'落在 {_ST3["lo_r"]:+,.0f}%~{_ST3["hi_r"]:+,.0f}%'
                     if _ST3 else '收敛到两位数') +
                  '。<b>2026-09 按页面所有者的指令统一成单月口径</b>（全站只此一种），'
                  '上面那个宽区间因此又回到图上 —— 记在这里不是为了替谁辩护，'
                  '而是因为这正是本图该交代的<b>代价</b>（CONTRACT §6.1 第 3 条），'
                  '完整数字见下面「口径」那一段里本序列自己的实测。'
                  '<b>所以这条线必须连着柱高一起读。</b>'
                  'GS 规矩 1「流量不算百分比」针对的是<b>环比</b>与单月同比；'
                  '年化有机增长率（Exhibit ⟨ex:organic⟩）仍是读流量趋势的首选。',
                  breaks=BKN, brk_note=BNN))

    # ── Exhibit 4：Annualised organic growth rate（gsx.lvl_bar, pct_series）──
    # 派生列：年化增速要用**上月末**资产，序列第一个月算不出来。窗口左端因此比主窗口
    # 起点晚一期（_w_finite），而不是留一个空柱假装那个月没数据。
    # ⚠ 这句话原先写的是「比**其余各图**晚一格」，被同一页的下一张图当场证伪：
    #   Exhibit 5 / 6 的左端是近端 13 个月、Exhibit 11 / 14 是近端若干季度，都比它晚
    #   九年多。参照系必须是**主窗口起点**（那才是这一刀真正让开的东西），而且两端都
    #   现读、差几期现算 —— 不写「一格」这种下个月可能不成立的话。
    W4 = _w_finite(df, 'organic_growth_ex')
    XL4 = [mlab(p) for p in W4.index]
    BK4, BN4 = brk_pack(W4.index, acq=False, cal=('nna',))
    _lag4n = (W4.index[0] - W25.index[0]).n
    # 这一句在页面上出现两处（Exhibit 4 卡片 + 页尾「窗口」那一条），所以只写一遍。
    # 两处各写各的，就是下一次「只改了一处」的入口 —— 上一版正是那样烂掉的。
    # 收尾处那道停机检查会拿 payload 自己的两条 x 轴把这段字面**重建**一遍，
    # 逐字对不上就不出页（手写端点、手写期数都会当场被抓）。
    _LAG4 = ('' if _lag4n <= 0 else
             f'左端是 {mlab(W4.index[0])}，比主窗口起点 {mlab(W25.index[0])} 晚 {_lag4n} 期')
    _lag4 = ('' if not _LAG4 else
             f'本图{_LAG4}：年化增速 = 当月有机 NNA × 12 ÷ '
             f'<b>上月末</b>资产，而 {mlab(df.index[0])} 是序列第一个月、没有上月，'
             f'那一格在数据里不存在（不补零、不前向填充）。')
    ex.append(bar(_S(4), 'organic_growth_ex', 'Annualised organic growth rate',
                  'Annualised organic growth', 'Organic growth (% annualised)', 'pct1',
                  'Organic NNA x 12 / prior month-end assets, the GS convention; acquired '
                  'assets stripped out using the disclosed split。' + _lag4 +
                  '比率序列的同比用百分点差（GS 规矩 2），所以右轴单位是 pp。'
                  '<b>柱是当月年化率，右轴是同一条当月年化率的单月百分点差</b>'
                  '（本月年化率 − 去年同月年化率）。'
                  '⚠ 2026-09 之前这里画的是另一条序列：「滚动 12 个月有机 NNA ÷ 12 个月前的'
                  '月末客户资产」这条比率的百分点差；现按所有者指令改成单月，那条滚动比率'
                  '只留作图注里的对照，<b>不进图</b>。' + YOY_NOTE +
                  '本图右轴同比跨零、左轴柱值恒正，两轴零点对齐的代价可能超过阈值，'
                  '届时引擎改为两轴独立缩放并在绘图区左上角标出（红色斜体）。',
                  window=W4, xl=XL4, breaks=BK4, brk_note=BN4,
                  pct_series=True, roll_col='organic_growth_roll'))

    # ── Exhibit 5：Client assets advisory vs brokerage（gsx.stack_share, win=13）──
    # ⚠ 这张**刻意**只画近端 WIN_S 个月，不跟着 WIN_FROM 走（2026-08-19 又核过一次）：
    #   (1) 同样这三条序列的全历史本页已经各有一张全窗口的图 —— N_ADVISORY（advisory
    #       资产）、N_BROKERAGE（brokerage 资产）、N_ADV_SHARE（advisory 占比），
    #       拉长这张一个新信息都不带；具体期数与编号见下面图注里的 f-string，
    #       **注释里不写死数字、也不写 {} 占位符**（注释不是 f-string，花括号不会被求值）。
    #   (2) 这张图的全部价值在**段内那两个数值**（月末两块资产各多少 $bn）。段宽 =
    #       0.62 x 带宽：13 期半栏是 21.9px，125 期即便通栏也只剩 5.3px，而段内标签
    #       size=6.6 的「1,077」宽约 18px —— charts.js 的 thinLabels 会把四分之三抽掉，
    #       换来的是一张读不出数值的堆叠墙。
    #   (3) ⚠ 这条举证被改坏过两次，两次都是同一个毛病：**拿别页的读数当论据**。
    #       第一版写「全站同类图一致：cboe Exhibit 5 / cme Exhibit 4 也是 13 期」——
    #       2026-08-19 那一轮它们都放宽了，举证当场作废；第二版把「13」换成了另一组
    #       写死的数（cboe/cme「127 期」、hood Ex17「13」、ibkr Ex8「14」），那只是
    #       把过期时间往后推 —— 那三个数一个按月走、一个按季走，下个月照样对不上。
    #       更要紧的是：**跨页根本没有一致的做法可援引**。2026-08-19 实扫 data/*.js
    #       的 stacked_dual：cme / exchanges-eu / exchanges-na / guc / ase 长轴且不标
    #       段内数值，而 cboe Exhibit 5 同样是长轴通栏、`stacks[].label` 照标（它正是
    #       assets/charts.js 里那段压字注释举的例子）。所以「别人都怎么做」这条论据
    #       本身就是假的，写第三版也一样会被推翻 —— 这里不再写它。
    #       **本图短窗口的理由只剩 (2)，而 (2) 是本页自己量出来的**（段宽由
    #       mrwin.band_px 按引擎的量边距算式复算，不引用任何别页的读数）：段内那两个
    #       $bn 数值是本图存在的唯一理由，全窗口下它们放不进段宽。要看跨页现状就现扫
    #       data/*.js 比 `full` 与 `stacks[].label`，别把当天的读数抄回这里。
    #       **失效条件写在这里**：哪天本图改成通栏、或不再要求段内可读数值，(2) 与 (3)
    #       同时失效，应当一并放宽到 WIN_FROM。
    # 页尾「窗口」那一条已对读者写明这个分法，这里再给读者一个到长历史图的指路。
    share13 = (W13['advisory_assets_usdbn'] / (W13['advisory_assets_usdbn']
                                               + W13['brokerage_assets_usdbn']) * 100)
    ymax_share = max(60.0, 10.0 * math.ceil(float(share13.max()) / 10.0))
    add({
        'n': _S(5), 'kind': 'stacked_dual', 'xlabels': XL13,
        'title': 'Client assets: advisory vs. brokerage',
        'ylab': 'Client assets ($bn)', 'ylab2': '% advisory (RHS)',
        'yfmt': 'f0c',
        'stacks': [
            {'name': 'Advisory', 'color': 'NAVY', 'values': L(W13['advisory_assets_usdbn'].values),
             'label': True, 'label_color': 'WHITE'},
            {'name': 'Brokerage', 'color': 'BLUE', 'values': L(W13['brokerage_assets_usdbn'].values),
             'label': True, 'label_color': 'NAVY'},
        ],
        'line': {'name': '% advisory (RHS)', 'color': 'GREEN', 'values': L(share13.values),
                 'ymax': ymax_share},
        'note': '堆叠柱为两个业务口径的月末资产（$bn），右轴绿线为 advisory 占比。'
                + (BN13 + '。' if BN13 else '') + QNOTE + '。'
                + f'<b>本图只画近 {WIN_S} 个月，是刻意的</b>：它要读的是段内那两个'
                  '月末金额，段宽拉到全历史就只剩几个像素、数值标签会被抽稀掉大半。'
                  f'同样三条序列的<b>全历史（{len(W25)} 期）</b>本页各有一张：'
                  f'Exhibit {N_ADVISORY}（advisory 资产）、'
                  f'Exhibit {N_BROKERAGE}（brokerage 资产）、'
                  f'Exhibit {N_ADV_SHARE}（advisory 占比）。',
    }, BK13)

    # ── Exhibit 6：What moved client assets（gsx.bridge_bar, win=13）──
    net13 = W13['nna_total_usdbn'].fillna(0) + W13['market_gains'].fillna(0)
    add({
        'n': _S(6), 'kind': 'bridge_bar', 'xlabels': XL13, 'fmt': 'f0',
        'title': 'What moved client assets: flows vs. markets',
        'ylab': 'Change in client assets ($bn)',
        'stacks': [
            {'name': 'Net new assets', 'color': 'NAVY', 'values': L(W13['nna_total_usdbn'].values)},
            {'name': 'Market gains (balancing)', 'color': 'BLUE', 'values': L(W13['market_gains'].values)},
        ],
        'net': {'name': 'Total change in client assets', 'values': L(net13.values)},
        'net_color': 'INK',
        'note': 'Identity: opening assets + NNA + market gains = closing assets. '
                'Market gains 是恒等式的配平项（当月资产变动 − 当月 NNA），不是公司披露值。'
                + (BN13N + '。' if BN13N else ''),
    }, BK13N)

    # ── Exhibit 7：NNA by channel（gsx.multi_line, 主窗口）──
    # 截轴（规矩 7）：并表月把两条线顶出量程 —— 不截轴的话整张图就是「两条压在零线上的
    # 平线 + 几根尖峰」，日常经营的那点差异一个都读不出。截轴不删点：超界的点画成
    # 空心红圈、真值竖排标出（是哪几个月由下面的 ex7_top 现算，不写死）。
    # **上下界一律由 cap_bounds() 从窗口数据现算**，不再手挑常数 —— 原先写死的
    # `-6.0, 35.0` 是 25 个月窗口下的手挑值，窗口放宽到 2016-01 之后它把 Apr-21
    # （38.0 / 35.9，只比日常上沿高 26%）也划成了离群点；理由与算法见 cap_bounds()。
    #
    # ⚠️ **Aug-25 那两个竖排真值标签（211.1 与 81.7）互相压 23.2px²，本轮判「保留」。**
    # 别再拿「调调上界就好了」去试，代价已经量过：
    #   · 根因在共用引擎，不在本页：同一个 x 上两条线都越界时，`assets/charts.js` 的
    #     `capSlot()` 把两个竖排标签横向拉开**写死的 8px**，而标签字号是 `7.2 × FS`、
    #     FS 随卡片宽度长 —— 1280 视口的通栏卡 W=1172 ⇒ FS=1.70 ⇒ 字号 12.24，
    #     实测墨迹框宽 8.97px，比 8 宽 0.97px，乘上两个标签共有的 23.8px 长度
    #     = 23.2px²。768 视口卡宽 736 ⇒ FS=1.52 ⇒ 墨迹框 8.0px，正好不压，
    #     所以这条只在 1280 出现 —— 这也反过来证明根因是「8 不跟 FS 缩放」。
    #     真正的修法是引擎里那一个常数（`8` → 随 FS 缩放的间距），
    #     那是 34 页共用的文件，不在本轮范围。
    #   · 抬上界到 $81.7bn 以上（唯一能让 Aug-25 只剩一个标签的做法）实测能清掉，
    #     但界内 $-3 ~ $38bn 的日常起伏会从占纵轴 82% 掉到 43%，2016–2022 那六年
    #     两条线一起压成贴零的一条带 —— 本图要讲的「Brokerage 长期净流出、
    #     Advisory 是它的镜像」当场读不出。拿整张图的纵向分辨率换 1px 宽的压字，不换。
    #   · 把真值标签改成整数（`label_fmt: 'f0'`）实测只降到 13.3px²，仍是 🟡，
    #     而且「截轴不删点、真值竖排标出」这条规矩当场变成标了个四舍五入值。不换。
    ex7_v = list(W25['nna_advisory_usdbn'].values) + list(W25['nna_brokerage_usdbn'].values)
    ex7_lo, ex7_hi = cap_bounds(ex7_v)
    CAP7, _ = cap_pack(_S(7), f'渠道 NNA，截 {usd_txt(ex7_lo)} ~ {usd_txt(ex7_hi)}bn' if ex7_hi else '', ex7_v,
                        hi=ex7_hi, lo=ex7_lo,
                        cap_note='axis capped — true values shown in red')
    ex7_out = sorted(
        [(p, lab, float(v)) for c, lab in (('nna_advisory_usdbn', 'Advisory'),
                                           ('nna_brokerage_usdbn', 'Brokerage'))
         for p, v in W25[c].items()
         if ex7_hi is not None and v is not None and v > ex7_hi],
        key=lambda t: (t[0], t[1]))
    ex7_top = '、'.join(f'{mlab(p)} 的 {lab} NNA {usd_txt(v, 1)}bn' for p, lab, v in ex7_out)
    # 「不截轴会怎样」也现算：拿窗口里两条线的**全段极差**跟**界内极差**比，
    # 不去复刻引擎的留白算式（那份算式在 charts.js 里，抄一遍就是第二份实现）。
    ex7_fin = [float(v) for v in ex7_v if v is not None and np.isfinite(float(v))]
    ex7_inb = [v for v in ex7_fin
               if ex7_lo is not None and ex7_hi is not None and ex7_lo <= v <= ex7_hi]
    ex7_shr = ((max(ex7_inb) - min(ex7_inb)) / (max(ex7_fin) - min(ex7_fin))
               if ex7_inb and max(ex7_fin) > min(ex7_fin) else 0.0)
    add({
        'n': _S(7), 'kind': 'lines_endlabels', 'fmt': 'f1', 'xlabels': XL25,
        'title': 'Net new assets by channel', 'ylab': 'Net new assets ($bn)',
        'series': [
            {'name': 'Advisory NNA', 'color': 'NAVY', 'values': L(W25['nna_advisory_usdbn'].values)},
            {'name': 'Brokerage NNA', 'color': 'RED', 'values': L(W25['nna_brokerage_usdbn'].values)},
        ],
        **CAP7,
        'note': 'Brokerage NNA has been persistently negative — the advisory conversion is '
                'visible as a mirror image。'
                + (f'纵轴截在 {usd_txt(ex7_lo)}bn ~ {usd_txt(ex7_hi)}bn：并表月把线顶出量程'
                   f'（{ex7_top}）。这两个界不是手挑的，是按窗口里两条线的 '
                   f'Tukey 极端离群栅栏（Q3 ± {CAP_K:g}×IQR）现算 —— 栅栏之外的 '
                   f'{len(ex7_out)} 个点算离群，界内 '
                   f'{usd_txt(min(ex7_inb), 1)} ~ {usd_txt(max(ex7_inb), 1)}bn 的日常起伏只有全段极差 '
                   f'${max(ex7_fin) - min(ex7_fin):,.1f}bn 的 {ex7_shr * 100:.0f}%，'
                   '不截轴就全部压成贴零的平线。'
                   '<b>截轴不删点</b> —— 超界的点画成空心红圈，真值竖排标在图上，'
                   '表格视图与 tooltip 里也是真值。' if CAP7 else '')
                + (BNNA + '。' if BNNA else ''),
    }, BKNA)

    # ── Exhibit 8：Total client assets（gsx.long_line，全序列；标题年份现算）──
    # zero_base + end_label = 补回 gsx.long_line 的 set_ylim(0, max*1.16) 与 n_label。
    # 不给 zero_base 时引擎走 y0 = min − 极差×5%，那是一次没有任何标注的隐性截轴，
    # 在长历史图上等于把增长幅度凭空放大（本图实测轴底原为 0.8，数据最低 0.67）。
    add({
        'n': _S(8), 'kind': 'lines', 'x': 'long', 'full': True, 'fmt': 'usd1', 'xstep': 6,
        'zero_base': True, 'end_label': True, 'label_fmt': 'usd2',
        'title': f'Total client assets since {df.index[0].year}',
        'ylab': 'Total client assets ($tn)',
        'series': [{'name': 'Total client assets', 'color': 'NAVY', 'values': L(df['total_tn'].values)}],
        'note': (BNALL + '。' if BNALL else '') +
                '纵轴从 0 起（同原 deck 的 long_line），末点标出最新读数 —— '
                '浮动基线会把这条线的增长幅度视觉上放大，而长历史图上末点数值是唯一的'
                '绝对水平锚点。原 deck 在末端画了一个红色虚线圈标出最近 3 个月，'
                '网页版没有等价元素，改由末点数值标注与表格视图直接读数。',
    }, BKALL)

    # ── Exhibit 9：Client cash balances（gsx.lvl_bar, 主窗口）──
    # 客户现金同样跨并表：2025-07 $49.5bn → 2025-08 $52.7bn（+6.5% m/m，2018 年以来
    # 8 个 8 月里最大的一个），Commonwealth 把客户现金一起带了进来。原先这一族
    # （Ex9/10/13/17）一条断点线都没画，而页面 notes 却声称「凡是跨这一期读的图都画了」。
    # 存量：月末余额，次轴保留单月同比（flow=False）。换成 12 个月均值同比之后是更稳
    # 还是更吵，由 stock_keep_note 现算印在图注里 —— 这里从前写的是「本页实测里最有说服力
    # 的一条反例：强行做滚动合计标准差反而变大」，与图注现算出来的方向相反，是假话。
    ex.append(bar(_S(9), 'client_cash_usdbn', 'Client cash balances', 'Client cash balances',
                  'Client cash ($bn)', 'f1',
                  '月末客户现金余额（含银行存款 sweep）。' + QNOTE + '。' + YOY_NOTE,
                  breaks=BKC, brk_note=BNC, flow=False,
                  keep_why='客户现金是<b>月末存量</b>。'))

    # ── Exhibit 10：Client cash as % of client assets（gsx.lvl_bar, pct_series）──
    ex.append(bar(_S(10), 'cash_pct_assets', 'Client cash as % of client assets',
                  'Client cash / client assets', 'Client cash (% of client assets)', 'pct1',
                  'Cash share is the key net-interest-revenue driver; a falling share is a '
                  f'headwind。原 deck 这张图取两位小数；网页图上取一位 —— {len(W25)} 根柱的'
                  '两位小数标签会横向叠成一片（引擎只能把它们抽稀掉一半），两位小数见本图的'
                  '「表格」视图与末尾核对表。分子分母跨并表同时跳，'
                  '断点两侧的占比不可直读。' + YOY_NOTE,
                  pct_series=True, breaks=BKC, brk_note=BNC, flow=False,
                  keep_why='客户现金 ÷ 客户资产（两个月末存量之比）'))

    # ── Exhibit 11：NNA by quarter（gsx.qtr_bar, win=14）──
    nq = nna.dropna()
    qidx = pd.PeriodIndex(nq.index).asfreq('Q')
    q_sum = nq.groupby(qidx).sum()
    q_cnt = pd.Series(1, index=nq.index).groupby(qidx).sum()
    q_yoy = pd.Series([(q_sum.iloc[i] / q_sum.iloc[i - 4] - 1) * 100
                       if i >= 4 and q_sum.iloc[i - 4] else np.nan
                       for i in range(len(q_sum))], index=q_sum.index)
    qw = q_sum.iloc[-WIN_Q:]
    qy = q_yoy.iloc[-WIN_Q:]
    partial = int(q_cnt.iloc[-1])
    qlabs = [str(p) for p in qw.index]
    BKQ, BNQ = brk_pack(qw.index, cal=('nna',))
    # 右轴同比撤掉的依据不是「看着难看」，是逐季实算：并购污染率 = 该季 Acquired NNA ÷
    # 该季总 NNA；自身 ≥10% 或去年同期 ≥10% 的季度，同比就不是可比读数。
    acq_q = acq.groupby(pd.PeriodIndex(df.index).asfreq('Q')).sum().reindex(q_sum.index).fillna(0.0)
    contam = (acq_q / q_sum.replace(0, np.nan) * 100).abs()
    bad_q = [q for q in qw.index
             if contam.get(q, 0) >= 10
             or (q - 4 in q_sum.index and contam.get(q - 4, 0) >= 10)]
    ok_yoy = qy.dropna().drop(index=[q for q in bad_q if q in qy.index], errors='ignore')
    worst = float(ok_yoy.abs().max()) if len(ok_yoy) else 0.0
    ex11_cap = 48.0
    CAP11, _ = cap_pack(_S(11), f'季度 NNA，截 ${ex11_cap:.0f}bn', list(qw.values), hi=ex11_cap)
    ex11_top = '、'.join(f'{q} ${v:,.0f}bn' for q, v in qw.items() if v > ex11_cap)
    add({
        'n': _S(11), 'kind': 'qtr_bar', 'xlabels': qlabs, 'fmt': 'f0c', 'label_fmt': 'f0c',
        'title': 'Net new assets by quarter', 'ylab': 'Net new assets ($bn)',
        'legend': 'Complete quarter',
        'values': L(qw.values), 'qtr_months': 3,
        **({'partial_months': partial} if partial < 3 else {}),
        **CAP11,
        'note': '月度 NNA 按日历季汇总。'
                + (f'纵轴截在 ${ex11_cap:.0f}bn：{ex11_top} 这几个含并表/大型机构导入的季度'
                   '不截轴会把其余常规季度全部压成矮矮一排。'
                   '<b>截轴不删点</b> —— 超界的柱画到边界并加断口符号，真值竖排标在柱旁，'
                   '表格视图与 tooltip 里是完整的季度合计。' if CAP11 else '')
                + '<b>右轴同比已撤掉</b>：撤它的理由与「单月 vs 滚动」无关 —— '
                '这条画的是 <b>as-reported 总 NNA</b>（含并购导入），'
                f'实算下来窗口内 {len(qw)} 个季度有 {len(bad_q)} 个的同比不可比'
                '（自身或去年同期的 Acquired NNA 占该季 NNA 的 10% 以上），'
                f'只剩 {len(ok_yoy)} 个可比点、彼此还不相邻，连不成一条可读的线，'
                f'而且其中最大的一个仍有 {worst:.0f}%，画出来照样是「一根尖峰加一条贴零的直线」。'
                '换成滚动口径也救不了：滚动窗口一样会把那两笔并表整整背 12 个月。'
                '季度趋势请读 Exhibit ⟨ex:nna⟩（有机 NNA 的<b>单月</b>同比，已扣除并购导入）、'
                'Exhibit ⟨ex:organic⟩ 与 Exhibit ⟨ex:organic-years⟩ 的年化有机增速，'
                '各季 NNA 合计与并表金额见本图的「表格」视图与末尾核对表。'
                + (BNQ + '。' if BNQ else ''),
    }, BKQ)

    # ── Exhibit 12：Advisory assets（gsx.lvl_bar, 主窗口）──
    ex.append(bar(N_ADVISORY, 'advisory_assets_usdbn', 'Advisory assets', 'Advisory assets',
                  'Advisory assets ($bn)', 'f0c',
                  '月末 advisory 口径客户资产。' + YOY_NOTE,
                  breaks=BK25, brk_note=BN25, flow=False,
                  keep_why='advisory 资产是<b>月末存量</b>。'))

    # ── Exhibit 13：Implied client-cash revenue（gsx.lvl_bar, 主窗口）──
    ex.append(bar(_S(13), 'implied_cash_rev_usdmn', 'Implied client-cash revenue',
                  'Implied client-cash revenue', 'Implied client-cash revenue ($mn / month)',
                  'f0c', BR_NOTE + '。<b>推导值，非公司披露</b>，验证见 Exhibit ⟨ex:cash-rev-bridge⟩。'
                  '规模基数是月末客户现金，因此与 Exhibit ⟨ex:cash⟩ 一样跨并表跳升、'
                  '也一样承接 Apr-2019 的客户现金口径变更。' + YOY_NOTE,
                  breaks=BKC, brk_note=BNC, extra=FEE_Q_NOTE, flow=False,
                  keep_why='它名义上是「每月多少收入」的流量，但 = <b>月末</b>客户现金 × '
                           '季度净收益率 ÷ 12，规模基数是存量、行为跟着存量走，'
                           '所以按<b>期末存量</b>处理。'))

    # ── Exhibit 14：Bridge check（gsx.implied_vs_actual）──
    qs = [q for q in imp_q.index if q in act_q.index][-WIN_Q:]
    imp_v = np.array([float(imp_q[q]) for q in qs])
    act_v = np.array([float(act_q[q]) for q in qs])
    err = np.where(act_v != 0, (imp_v / act_v - 1) * 100, np.nan)
    mae = float(np.nanmean(np.abs(err)))
    # 「未满 3 个月的季度」也现算 —— 这句原先写死「2026Q2 只有 2 个月」，下一季就是假话
    _part = sorted(q for q in cnt.index if int(cnt.get(q, 0)) != 3)
    PART_TXT = ('未满 3 个月的季度不参与对比（'
                + '、'.join(f'{q} 只有 {int(cnt[q])} 个月' for q in _part) + '）。'
                if _part else '本页月度序列覆盖到的季度都满 3 个月，没有季度被排除。')
    # 对比窗口里哪些季度的隐含值是用「上一季费率」算的 —— 那几季验的是费率的时滞，
    # 不是月末/季均余额这一个近似，误差读数要分开看。
    _qc = [q for q in qs if q not in cy.index]
    FEE_Q14 = FEE_Q_NOTE + ((
        f'对比窗口 {qs[0]}–{qs[-1]} 内，'
        + (f'{"、".join(str(q) for q in _qc)} 用的是上一季费率（这几季的误差同时含费率时滞），'
           if _qc else '每个季度都用到了本季披露的费率，')
        + f'公司披露的实际客户现金收入已更新至 {act_q.index[-1]}。') if qs else '')
    ex.append({
        'n': _S(14), 'kind': 'grouped_bars', 'xlabels': [str(q) for q in qs],
        'fmt': 'f0c', 'label_fmt': 'f0c',
        'title': 'Bridge check: implied vs. reported client-cash revenue',
        'ylab': 'Client-cash revenue ($mn / quarter)', 'ylab2': 'Error (%)',
        'groups': [
            {'name': 'Implied by the bridge', 'color': 'BLUE', 'values': L(imp_v)},
            {'name': 'Actually reported', 'color': 'NAVY', 'values': L(act_v)},
        ],
        'line': {'name': 'Error (RHS)', 'color': 'RED', 'values': L(err), 'yfmt': 'pct1'},
        'note': 'Reported = the client-cash revenue line in LPL results. The bridge applies the '
                'disclosed yield to MONTH-END cash while LPL earns it on AVERAGE cash — that '
                f'proxy error is what this tests. 窗口内平均绝对误差 {mae:.1f}%。'
                + PART_TXT +
                '误差线跨零而柱恒正，本引擎又要求两轴零点画在同一高度，所以左轴基线被'
                '拉到负区（收入本身不会为负）—— 这是双轴对齐的既定代价，浪费率约 25%，'
                '在引擎的 38% 阈值以内，故保留对齐而不是让两轴零点错位。'
                '原 deck 的 matplotlib 不对齐零点、误差线直接压在柱上。',
        'src_extra': FEE_Q14,
    })

    # ── Exhibit 15：Brokerage assets（gsx.lvl_bar, 主窗口）──
    ex.append(bar(N_BROKERAGE, 'brokerage_assets_usdbn', 'Brokerage assets', 'Brokerage assets',
                  'Brokerage assets ($bn)', 'f0c',
                  '月末 brokerage 口径客户资产。' + YOY_NOTE,
                  breaks=BK25, brk_note=BN25, flow=False,
                  keep_why='brokerage 资产是<b>月末存量</b>。'))

    # ── Exhibit 16：Advisory share of client assets（gsx.lvl_bar, pct_series）──
    ex.append(bar(N_ADV_SHARE, 'pct_advisory', 'Advisory share of client assets',
                  'Advisory share', 'Advisory share (% of client assets)', 'pct1',
                  'Advisory assets carry a higher payout-adjusted margin than brokerage, so the '
                  f'mix shift is a structural profit driver。原 deck 取两位小数；'
                  f'网页图上取一位 —— {len(W25)} 根柱的两位小数标签会横向叠成一片，'
                  '两位小数见本图的「表格」视图与末尾核对表。'
                  '每一笔整体并表都直接改变了业务口径的构成（NPH 与 Atria 偏 brokerage、'
                  'Waddell & Reed 与 Commonwealth 偏 advisory），断点两侧的占比不可直读。'
                  + YOY_NOTE,
                  pct_series=True, breaks=BK25, brk_note=BN25, flow=False,
                  keep_why='advisory 资产 ÷ 总客户资产（两个月末存量之比）'))

    # ── Exhibit 17：Client cash（gsx.long_line，全序列；标题年份现算）──
    # 「不给 zero_base 会放大多少倍」现算：引擎的默认下界是 min − 极差×5%，
    # 放大倍数 = 零基线下的纵向跨度 ÷ 浮动基线下的纵向跨度。写死数字的话，
    # 窗口一变（本轮从 2018 起变成 2016 起）那句话立刻是假的。
    _c17 = df['client_cash_usdbn'].dropna()
    _c17_lo, _c17_hi = float(_c17.min()), float(_c17.max())
    _c17_floor = _c17_lo - (_c17_hi - _c17_lo) * 0.05
    _c17_amp = (_c17_hi / (_c17_hi - _c17_floor)) if _c17_hi > _c17_floor else float('nan')
    add({
        # fmt 用 f1 而不是 deck 的 dec=0：末点标注与表格视图共用 fmt，f0c 会把 54.8 印成 55
        'n': _S(17), 'kind': 'lines', 'x': 'long', 'full': True, 'fmt': 'f1', 'xstep': 6,
        'zero_base': True, 'end_label': True,
        'title': f'Client cash since {df.index[0].year}', 'ylab': 'Client cash ($bn)',
        'series': [{'name': 'Client cash', 'color': 'NAVY',
                    'values': L(df['client_cash_usdbn'].values)}],
        'note': '2020 的台阶是疫情期间的现金堆积，2022-24 的回落是现金搬家（cash sorting）；'
                '这条线是 Exhibit ⟨ex:cash-rev⟩ 隐含收入的规模基数。'
                '纵轴从 0 起（同原 deck 的 long_line）、末点标出最新读数：'
                f'不给 zero_base 时引擎走「最小值 − 极差 × 5%」，本序列会把轴底浮到 '
                f'{_c17_floor:,.0f} 附近，纵向落差被放大约 {_c17_amp:,.1f} 倍 —— '
                '那是一次没有任何标注的隐性截轴。'
                + (BNALLC + '。' if BNALLC else ''),
    }, BKALLC)

    # ── Exhibit 18：Organic growth path by year（gsx.year_lines, n_years=6）──
    og = df['organic_growth_ex'].dropna()
    yrs6 = sorted({p.year for p in og.index})[-6:]
    yl_series = []
    for y in yrs6:
        vals = [None] * 12
        for p, v in og.items():
            if p.year == y:
                vals[p.month - 1] = round(float(v), 6)
        yl_series.append({'name': str(y), 'values': vals})
    # 截轴（规矩 7）：本轮把 2021-04 Waddell & Reed 的 $67.1bn 按官方披露的
    # "Total Organic NNA" 行扣进了 ACQ，那根 92.4% 的尖峰因此消失。截轴留着不删 ——
    # 它是**现算**的（cap_pack 只在窗口里真有越界点时才截），下一次大并表出现时自动生效。
    # yfloor 与截轴一起给：这几年没有负值月份，不给的话通用留白会把轴底压到零下、
    # 凭空多出一段空区。
    ex18_cap = 32.0
    y18_all = [v for s_ in yl_series for v in s_['values'] if v is not None]
    y18_max = max(y18_all)
    y18_next = max([v for v in y18_all if v <= ex18_cap], default=0.0)
    CAP18, _ = cap_pack(_S(18), f'年化有机增速，截 {ex18_cap:.0f}%', y18_all,
                          hi=ex18_cap, lo=0.0 if min(y18_all) >= 0 else None)
    if not CAP18 and min(y18_all) >= 0:
        CAP18 = {'yfloor': 0.0}          # 没有越界点也仍要零基线，但不写「已截轴」
    add({
        'n': _S(18), 'kind': 'year_lines', 'fmt': 'pct1', 'label_fmt': 'pct1',
        'xlabels': ['Jan', 'Feb', 'Mar', 'Apr', 'May', 'Jun', 'Jul', 'Aug', 'Sep', 'Oct',
                    'Nov', 'Dec'],
        'title': 'Organic growth path by year', 'ylab': 'Organic growth (% annualised)',
        'series': yl_series, 'highlight': len(yl_series) - 1,
        **CAP18,
        'note': 'Red = current year。画的是各月的年化有机增速本身（非累计，同原 deck 的 '
                'cumulative=False）。'
                + f'画 {yrs6[0]}–{yrs6[-1]} 共 {len(yrs6)} 年（同原 deck 的 n_years=6）——'
                  '逐年对照图上再多几条线就分不清哪条是哪年了，更长的历史请看⟨ex:organic-heat@+1:下一张⟩热力矩阵。'
                + (f'纵轴截在 {ex18_cap:.0f}%（其余各月最高 {y18_next:.1f}%，'
                   f'越界的最高点 {y18_max:.1f}%）—— '
                   '<b>截轴不删点</b>：越界的点画成空心红圈、真值竖排标在旁边，'
                   '表格视图与 tooltip 里也是真值；不截轴则各条线全部压成贴零的一团，'
                   '年份之间无法比较。' if CAP18.get('cap_note') else
                   f'纵轴从 0 起，本图窗口内最高 {y18_max:.1f}%，无需截轴 —— '
                   'Apr-2021 Waddell & Reed 的 $67.1bn 已按公司披露的 Organic NNA 行扣除，'
                   '此前那根 92% 的尖峰不再存在。'),
    })

    # ── Exhibit 19：Organic growth heat matrix（gsx.heat_matrix）──
    # 原 deck 写死 n_years=9。改成「与其余时序图同一个左端」：热力矩阵是本页唯一
    # 一张能把全部历史一次摆开的图，回补到 2016 之后还只画最近 9 年，等于把新补的
    # 两年藏起来。行数由 WIN_FROM 决定，跟着窗口走，不再是一个与窗口无关的常数。
    yrs9 = [y for y in sorted({p.year for p in og.index}) if y >= int(WIN_FROM[:4])]
    matrix = []
    for y in yrs9:
        row = [None] * 12
        for p, v in og.items():
            if p.year == y:
                row[p.month - 1] = round(float(v), 6)
        matrix.append(row)
    ex.append({
        'n': _S(19), 'kind': 'heat_matrix', 'full': True, 'fmt': 'pct1',
        # 标题里必须写「单月」：热力矩阵按定义是逐格月度读数，读者才知道格子里那个数
        # 是怎么算的。这张图属 CONTRACT §6.3 的图型豁免（heat_matrix 每一格本来就是
        # 一个月）—— 逐格波动与季节形状就是它的题眼。
        # ⚠ 2026-09 起本页别处的次轴也统一成单月口径，旧稿那句「不是 Exhibit 4 右轴的
        # 滚动口径」已经不成立，下面的图注跟着改掉了 —— 别再写回去。
        'title': 'Annualised organic growth rate — 单月年化 (%)',
        'rows': [str(y) for y in yrs9],
        'cols': ['Jan', 'Feb', 'Mar', 'Apr', 'May', 'Jun', 'Jul', 'Aug', 'Sep', 'Oct',
                 'Nov', 'Dec'],
        'matrix': matrix, 'legend': 'Annualised organic growth, 单月', 'row_head': '年',
        'note': '格内是<b>单月</b>年化增速的<b>水平值</b>（当月有机 NNA x 12 ÷ 上月末资产），'
                '不是同比 —— Exhibit ⟨ex:organic⟩ 的柱画的是同一条序列，它的次轴则是这条序列的'
                '单月同比（百分点差）。逐格的季节形状正是这张图要看的东西。'
                'Green = faster organic asset gathering; acquired assets removed using the '
                f'disclosed split（{min(ACQ)} 起逐笔扣除，见页尾「有机口径」一条）。'
                '色标取全部有限值的 5/95 分位，离群月不会把整表压平。'
                f'{yrs9[0]} 行只有 {sum(v is not None for v in matrix[0])} 格有数、'
                f'{yrs9[-1]} 行只有 {sum(v is not None for v in matrix[-1])} 格：'
                f'序列自 {df.index[0]} 起，而年化增速要用到上月末资产，所以首月算不出；'
                '末行是当年至今。空白格是数据不存在，不是没画。',
    })

    # ── Exhibit 1：汇总表（gsx.summary_table 的行分组范式）──
    cur, prv, yag = LATEST, LATEST - 1, LATEST - 12

    # 3Y %ile 的判据一律走 build/pctile.py，本文件不再自己写一套。
    # 从前每个生成器各写各的，结果是同一条 LPL total client assets 在本页被判成噪音留空、
    # 在 /wealth/ 却印成绿色 100，两页互相矛盾。判据是口径，口径只能有一处定义。
    blank_rows = []          # 留空的行标签，供表注现算

    def srow(label, col, dec, mode, pct=False, money='', inv=False):
        s = df[col].dropna()
        c = s.get(cur, np.nan)
        p1 = s.get(prv, np.nan)
        p12 = s.get(yag, np.nan)
        cells = [{'v': num(c, dec, money, pct)}, {'v': num(p1, dec, money, pct)},
                 {'v': num(p12, dec, money, pct)}]
        for b in (p1, p12):
            if not (np.isfinite(c) and np.isfinite(b)):
                cells.append({'v': ''})
                continue
            if mode in ('pp', 'abs'):
                v = c - b
            elif b == 0 or c * b < 0:
                cells.append({'v': ''})
                continue
            else:
                v = (c / b - 1) * 100
            # 四舍五入后落在零上的差异不带方向：印「-0bp」/「-0.0」会被读成一个很小的
            # 负数，而它其实是「按这个精度就是 0」。归零并去掉涨跌配色。
            if mode == 'pp' and abs(v) < 1:
                z, sfx, d = _nz(v * 100, 0), 'bp', 0
            elif mode == 'pp':
                z, sfx, d = _nz(v, 2), 'pp', 2
            elif mode == 'abs':
                z, sfx, d = _nz(v, max(0, dec)), '', max(0, dec)
            else:
                z, sfx, d = _nz(v, 1), '%', 1
            sign = '' if z == 0 else ('+' if z > 0 else '-')
            txt = f'{money}{sign}{abs(z):,.{d}f}{sfx}'
            if z == 0:                       # 「按这个精度就是 0」，不带方向也不上色
                cells.append({'v': txt})
                continue
            good = (v < 0) if inv else (v > 0)
            cells.append({'v': txt, 'cls': 'pos' if good else 'neg'})
        # 分位一律走 pctile.cell()：它自己判死列、自己给颜色类
        if s.index[-1] != cur:               # 该列最新月缺数，分位没有对应的当期读数
            cells.append({'v': ''})
            blank_rows.append((label, f'该列最新读数停在 {mlab(s.index[-1])}'))
            return {'label': label, 'cells': cells}
        vals = [float(x) for x in s.values]
        txtp, cls = pctile.cell(vals, inverse=inv)
        cells.append({'v': txtp, 'cls': cls} if txtp else {'v': ''})
        if not txtp:
            blank_rows.append((label, pctile.why_blank(vals) or '样本不足'))
        return {'label': label, 'cells': cells}

    # 汇总表的 y/y 列一直是单月口径：它恒等于表内算术「本月 ÷ 去年同月」，读者拿第一列
    # 除第三列就能验算。2026-09 各图次轴也改成单月之后，这一列与图上的金线**同口径**了 ——
    # 但组标题上的口径标注不能因此省掉，表注里也仍要把滚动口径的对照读数并排印出。
    _GS = '　·　y/y 列 = 单月口径（本月 ÷ 去年同月）'
    sum_rows = [
            # srow() 有副作用（往 blank_rows 里登记留空原因），所以必须在 summary 之前跑完
            {'kind': 'group', 'label': 'Assets ($bn)' + _GS},
            srow('Advisory', 'advisory_assets_usdbn', 1, 'ratio'),
            srow('Brokerage', 'brokerage_assets_usdbn', 1, 'ratio'),
            srow('Total client assets', 'total_assets_usdbn', 1, 'ratio'),
            srow('% Advisory', 'pct_advisory', 1, 'pp', pct=True),
            {'kind': 'group', 'label': 'Net new assets ($bn)' + _GS},
            srow('Advisory NNA', 'nna_advisory_usdbn', 1, 'abs'),
            srow('Brokerage NNA', 'nna_brokerage_usdbn', 1, 'abs'),
            srow('Total NNA', 'nna_total_usdbn', 1, 'abs'),
            # 这一行的列名必须是 organic_growth_ex（剔并购），不是 as-reported 的
            # nna_total —— 行标签自己写着 organic，表注又叫读者拿它对 Exhibit 4。
            # 2026-08-21 之前这里取的是 as-reported，于是并购月同一个指标在同一页
            # 给出两个数（Jun-26 表里 5.54%、Exhibit 4 里 5.31%）。schw / hood 两页
            # 同名列没有这个问题：它们的 NNA 本身就是有机口径，只有 LPL 的含并购导入。
            srow('Annualised organic growth (%)', 'organic_growth_ex', 2, 'pp', pct=True),
            {'kind': 'group', 'label': 'Client cash ($bn)' + _GS},
            srow('Client cash balances', 'client_cash_usdbn', 1, 'ratio'),
            srow('% of client assets', 'cash_pct_assets', 2, 'pp', pct=True),
    ]
    # 两种口径的当期读数：现算，供表注与页尾口径说明并排印出。
    # 表里的 y/y 与 Exhibit 3 的次轴现在同口径（都是单月），并排印的目的因此变了 ——
    # 不再是「别拿这个去核那个」，而是把被换掉的那个口径的读数一并交代清楚，
    # 让读者自己看得到「换分母能换出多大一个差」。
    _mo3 = mom_yoy_series(df['nna_ex'], df.index)
    _rl3 = roll_yoy_series(df['nna_ex'], df.index)
    # 年化有机增速是**比率**，同比取百分点差，走共享模块（yoy.mom_yoy(kind=RATIO)）——
    # 原先手写 `x - x.shift(12)`，数值与它逐月相同，但那是同一个口径的第二份实现。
    _og_m = pp_yoy_series(df['organic_growth_ex'], df.index)
    _og_r = pp_yoy_series(df['organic_growth_roll'], df.index)

    def _lastf(s):
        s = s.dropna()
        return float(s.iloc[-1]) if len(s) else None

    _CAL = [t for t in (
        (f'有机 NNA（Exhibit ⟨ex:nna⟩ 次轴画的是单月那个）：单月 {_lastf(_mo3):+,.1f}% / '
         f'滚动 12 个月（对照，本页不画）{_lastf(_rl3):+,.1f}%'
         f'（差 {abs(_lastf(_mo3) - _lastf(_rl3)):,.0f}pp）'
         if _lastf(_mo3) is not None and _lastf(_rl3) is not None else ''),
        (f'年化有机增长率（Exhibit ⟨ex:organic⟩ 次轴画的是单月那个，比率取 pp 差）：'
         f'单月 {_lastf(_og_m):+,.2f}pp / '
         f'滚动 12 个月（对照，本页不画）{_lastf(_og_r):+,.2f}pp'
         f'（差 {abs(_lastf(_og_m) - _lastf(_og_r)):,.1f}pp）'
         if _lastf(_og_m) is not None and _lastf(_og_r) is not None else ''),
    ) if t]
    # ── 页尾口径条里「客户现金那一条实测」现算 ──────────────────────────────
    # ⚠ 这里从前是一句写死的散文：「最硬的一条是客户现金（Exhibit 9）：换成 12 个月
    # 均值同比标准差反而更大」。**那是假的**，而且被同一页 Exhibit 9 自己的图注当场
    # 证伪 —— 那条图注是现算的，印出来的是「均值口径确实更平滑」。更早的版本还带着
    # 两个写死的读数（9.2pp → 13.1pp，那是 25 个月窗口时代的数），上一轮把数删了、
    # 把结论留下了，于是页面上只剩一句读者无法当场核对的假话。
    # 现在这一段与该图图注**取自同一个 st 对象**（STOCK_CAL 由 stock_keep_note 登记），
    # 两个数一并印出来，方向由数据决定：翻面了句子跟着翻。
    _cash_st = STOCK_CAL.get('client_cash_usdbn')
    if not _cash_st:
        _CASH_CAL_TXT = '<b>逐图实测印在各图图注里</b>（本轮客户现金两种口径可比的月份不足 3 个）。'
    else:
        _cash_r = (_cash_st['sd_m'] / _cash_st['sd_r']) if _cash_st['sd_r'] else float('nan')
        _CASH_CAL_TXT = (
            '<b>「换成均值就更吵」不是理由，逐图实测就印在各图图注里</b> —— 客户现金'
            f'（Exhibit ⟨ex:cash⟩）这一条与该图图注同一批月份、同一个统计量：对齐后 '
            f'{_cash_st["n"]} 个月，点对点同比标准差 {_cash_st["sd_m"]:,.1f}pp、'
            f'12 个月均值同比 {_cash_st["sd_r"]:,.1f}pp（{_cash_r:,.2f} 倍），'
            + ('<b>均值口径在这条序列上反而更吵</b>。'
               if _cash_r < 1 else
               '<b>均值口径确实更平滑</b>；本页不换它，靠的是上面那条结构性的理由，'
               '不是波动。')
            + '（本站从前在这里写的是「客户现金换成均值口径标准差反而更大」，'
              '与该图图注当场打架，现按实测改正。）')

    # 表注里「哪几行留空、为什么」现算，不写死行名 —— 序列一变，写死的名单就是假话
    _why = {}
    for lab, why in blank_rows:
        _why.setdefault(why, []).append(lab)
    _bl = '；'.join(f'{" / ".join(v)}（{k}）' for k, v in _why.items())
    summary = {
        'title': f'LPL Financial monthly metrics — {mlab(LATEST)}',
        'heads': [mlab(cur), mlab(prv), mlab(yag), 'm/m', 'y/y 单月', '3Y %ile'],
        'sep': 3,
        'rows': sum_rows,
        'note': 'Per GS convention: flow items (NNA) show an absolute change rather than a '
                'percentage, and are read through the annualised organic growth line. '
                + QNOTE + '. 3Y %ile = 当月读数在最近 36 个月里高于多少百分比的观测，'
                '判据与全站其余 13 页共用一份实现（<code>build/pctile.py</code>）：'
                '把这一列在过去 24 个月里逐月回放一遍，若 ≥ 70% 的月份分位都钉在极值'
                '（100 或 0），这一列对该行就没有区分度，留空。'
                + (f'本期留空：{_bl}。它们的强弱读 m/m 与 y/y。' if _bl else '本期没有留空的行。')
                + '<br><b>本表的 y/y 列是「单月口径」= 本月 ÷ 去年同月 − 1，'
                '与各图次轴现在<b>同一个口径</b>（2026-09 起全站统一，页面所有者指定）。</b>'
                '这一列同时恒等于表内算术（第一列 ÷ 第三列），读者可以直接验算。'
                'Exhibit ⟨ex:nna-q⟩（季度 NNA）没有同比线，本页因此不再有第二种同比口径。'
                + (f'被换掉的那个口径（12 个月滚动合计）本页只算不画，当期读数并排现算 '
                   f'—— {"；".join(_CAL)}。' if _CAL else ''),
    }

    # ── 末尾核对表（官方原始单位，未换算）──
    T13 = df.iloc[-13:]
    tcols = [['Total client assets ($bn)', 'tot'], ['Advisory ($bn)', 'adv'],
             ['Brokerage ($bn)', 'brk'], ['Total NNA ($bn)', 'nna'],
             ['Advisory NNA ($bn)', 'nnaa'], ['Brokerage NNA ($bn)', 'nnab'],
             ['Acquired NNA ($bn)', 'acq'], ['Client cash ($bn)', 'cash'],
             ['Client cash (% of assets)', 'cashp']]
    trows = []
    for p, r in T13.iterrows():
        trows.append({
            'xl': mlab(p),
            'tot': num(r['total_assets_usdbn'], 1),
            'adv': num(r['advisory_assets_usdbn'], 1),
            'brk': num(r['brokerage_assets_usdbn'], 1),
            'nna': num(r['nna_total_usdbn'], 1),
            'nnaa': num(r['nna_advisory_usdbn'], 1),
            'nnab': num(r['nna_brokerage_usdbn'], 1),
            'acq': num(r['acquired_nna'], 1),
            'cash': num(r['client_cash_usdbn'], 1),
            'cashp': num(r['cash_pct_assets'], 2, pct=True),
        })
    # 图号自查：exhibit 编号必须是 2..N 的连号，核对表接在最后一张之后。编号写死过一次
    # 代价就够大了 —— 全站审计发现别的页把核对表写死成 'n': 15，后来在末尾追加了两张图，
    # 页面就出现「…16、17、15」而没有任何东西报错。这里改成现算 + 硬拦。
    _ens = [e['n'] for e in ex]
    if _ens != list(range(2, 2 + len(_ens))):
        raise SystemExit(f'Exhibit 编号不连续: {_ens}')
    table = {
        # 标题里的月数跟着 trows 现算：T13 哪天改了，标题不会留在原地说假话。
        'n': _ens[-1] + 1,
        'title': f'近 {len(trows)} 个月月度指标核对表（官方原始单位，未换算）',
        'idx': '月份', 'cols': tcols, 'rows': trows,
    }

    # ── 抬头 ──
    # 抬头必须**同时**报好消息与坏消息。原先这一行只写「资产 +37.8% y/y、有机增速、
    # 现金 +11.4%、Advisory 占比 +5.09pp」，四项全是顺风，而同一页的汇总表里
    # Brokerage NNA 已连续为负、现金占资产比正在刷 3 年新低 —— 读者只看抬头会得到
    # 一个与页面本身相反的印象。规矩：跨并表的同比要注明含并表，并至少带上一项逆风。
    latest = df.iloc[-1]
    tot12 = float(df['total_assets_usdbn'].iloc[-13])
    y_tot = (float(latest['total_assets_usdbn']) / tot12 - 1) * 100
    # 并表口径：把窗口内（近 12 个月）已披露的 Acquired NNA 从当期资产里剔掉再比一次
    acq12 = float(df['acquired_nna'].iloc[-12:].sum())
    y_tot_ex = (float(latest['total_assets_usdbn']) - acq12) / tot12 * 100 - 100
    y_cash = (float(latest['client_cash_usdbn']) / float(df['client_cash_usdbn'].iloc[-13]) - 1) * 100
    m_cash = (float(latest['client_cash_usdbn']) / float(df['client_cash_usdbn'].iloc[-2]) - 1) * 100
    d_adv = float(latest['pct_advisory']) - float(df['pct_advisory'].iloc[-13])
    d_cashp = (float(latest['cash_pct_assets'])
               - float(df['cash_pct_assets'].iloc[-13])) * 100          # bp
    # Brokerage NNA 连续为负的月数
    nneg = 0
    for v in reversed(list(df['nna_brokerage_usdbn'].values)):
        if np.isfinite(v) and v < 0:
            nneg += 1
        else:
            break
    cp36 = df['cash_pct_assets'].iloc[-36:]
    cp_low = float(latest['cash_pct_assets']) <= float(cp36.min()) + 1e-9
    og_now = float(latest['organic_growth_ex'])
    og_avg = float(df['organic_growth_ex'].iloc[-13:-1].mean())
    # 并表只在它仍落在同比比较窗口（近 12 个月）里时才需要在抬头点名；
    # 滚出窗口之后再写「含 Commonwealth 并表」就成了假话。
    acq_in12 = [f'{p.strftime("%b-%y")} {lab} 并表 ${ACQ.get(str(p), 0):,.1f}bn'
                for p, lab in ACQ_BREAKS if p in df.index[-12:]]
    acq_txt = ('，含 ' + '、'.join(acq_in12) + '；'
               f"剔除近 12 个月已披露的并购导入约 {pm(y_tot_ex, 1, '%')}"
               if acq_in12 and acq12 > 0 else '')
    # 抬头的每个 y/y 都带口径标签：光秃秃的「y/y」等于误导（读者会拿它去核图上的次轴）。
    # ⚠ 2026-09 起有机 NNA 这一项也报**单月**同比 —— 上一版报的是 12 个月滚动合计，
    # 理由写在旁边：「它才是 Exhibit 3 右轴画的那个数」。图改成单月之后那条理由自己就把
    # 这里改了：仍报滚动就会与图上那条金线对不上，而抬头是多数人唯一会读的一行。
    # 客户资产与客户现金两项本来就是存量的点对点同比，不受影响。
    _hl_m3 = _lastf(_mo3)
    headline = (f"客户资产 ${float(latest['total_assets_usdbn']):,.1f}bn"
                f"（{pm(y_tot, 1, '%')} y/y·单月{acq_txt}）"
                f" · 总 NNA {usd_txt(float(latest['nna_total_usdbn']), 1)}bn，"
                f"有机 {usd_txt(float(latest['nna_ex']), 1)}bn"
                + (f"（{pm(_hl_m3, 1, '%')} y/y·单月，"
                   f"年化有机增速 {og_now:.1f}%，前 12 个月均值 {og_avg:.1f}%）"
                   if _hl_m3 is not None else
                   f"（年化有机增速 {og_now:.1f}%，前 12 个月均值 {og_avg:.1f}%）")
                + ' · Brokerage NNA '
                + ('-' if float(latest['nna_brokerage_usdbn']) < 0 else '+')
                + f"${abs(float(latest['nna_brokerage_usdbn'])):,.1f}bn"
                + (f"（已连续 {nneg} 个月净流出）" if nneg >= 2 else '')
                + f" · 客户现金 ${float(latest['client_cash_usdbn']):,.1f}bn"
                f"（{pm(y_cash, 1, '%')} y/y·单月，环比 {pm(m_cash, 1, '%')}），"
                f"占资产 {float(latest['cash_pct_assets']):.2f}%（{pm(d_cashp, 0, 'bp')} y/y"
                + ('，为近 36 个月最低' if cp_low else '') + '）'
                + f" · Advisory 占比 {float(latest['pct_advisory']):.2f}%（{pm(d_adv, 2, 'pp')} y/y）")
    hub = (f"客户资产 ${float(latest['total_assets_usdbn'])/1000:.2f}tn"
           f"（{pm(y_tot, 0, '%')} y/y{'，含并表' if acq_in12 and acq12 > 0 else ''}）、"
           f"有机增速 {og_now:.1f}%、"
           f"现金占资产 {float(latest['cash_pct_assets']):.2f}%"
           + ('（3 年低位）' if cp_low else ''))

    # ── 断点说明：从「真的画出来了」的清单现算 ──────────────────────────────
    # 复查规矩：图注声称画了断点线，图上就必须真有。原先这句写死「受影响的是
    # Exhibit 2、5、6、7、8、11、12、15」，而客户现金一族（9/10/13/17）同样跨并表却
    # 一条线都没画，Atria 更是整页一条都没有 —— 说明与渲染对不上。现在名单从 DRAWN 现算。
    drawn = sorted(set(DRAWN))
    drawn_cal = sorted(set(DRAWN_CAL))
    _in_win = [BRK_TXT[lab] for p, lab in ACQ_BREAKS
               if p in df.index or p.asfreq('Q') in q_sum.index]
    # ACQ_BREAKS 的登记判据（当月 Acquired NNA ≥ 当月末客户资产的 5%）**机器复核一遍**：
    # 判据写在注释里而名单靠手维护，下一次并表进来时没人会想起来对一遍。
    _share = {k: v / float(df['total_assets_usdbn'].loc[pd.Period(k, 'M')]) * 100
              for k, v in ACQ.items() if pd.Period(k, 'M') in df.index}
    _reg = ACQ_BREAK_MONTHS
    _missing = sorted(k for k, sh in _share.items() if sh >= 5.0 and k not in _reg)
    if _missing:
        raise SystemExit(f'ACQ 里 {_missing} 当月并购导入已达客户资产的 5% 以上，'
                         f'却不属于任何一条已登记的断点（判据见 ACQ_BREAKS 上方）')
    _orphan = sorted(m for m in ACQ_BREAK_MONTHS if m not in ACQ)
    if _orphan:
        raise SystemExit(f'ACQ_BREAK_MONTHS 里 {_orphan} 在 ACQ 表里没有对应条目，两表已漂')
    _small = {k: sh for k, sh in _share.items() if k not in _reg}
    _small_max = max(_small.values()) if _small else 0.0
    if drawn:
        BRK_NOTE = ('<b>⚠️ 并购断点。</b>本页序列覆盖到的整体并表共'
                    + B.cn(len(_in_win)) + '笔：' + '、'.join(_in_win) +
                    '。它们都是整体并表导入，当月既不是有机流入、并表后的存量也与左侧不可直读。'
                    '登记判据写在 <code>build/lpla.py</code> 的 ACQ_BREAKS 上方：'
                    '<b>当月 Acquired NNA ≥ 当月末客户资产的 5%</b>，构建时会机器复核一遍'
                    '（有漏登记的直接停更，不会静默少画一条线）。'
                    f'公司披露的并购导入本页共登记 {len(ACQ)} 个月，其中 '
                    f'{len(ACQ_BREAK_MONTHS)} 个月属于上面这几笔（NPH 分 5 个月导入）；'
                    f'其余 {len(_small)} 个月最大的一笔也只占当月资产的 {_small_max:.1f}%，'
                    '只从有机口径里扣除、不画线。'
                    '凡是画 as-reported 客户资产 / 客户现金 / 总 NNA 的图，都在该期柱的'
                    '左缘画了红色竖虚线并标出并购名：从这一期起与左侧不可比。'
                    '本期真正画出<b>并购</b>断点线的是 Exhibit ' +
                    '、'.join(str(i) for i in drawn) +
                    '（各图窗口不同，盖不到的断点自动不画，也不会在图注里声称画了）。'
                    'Exhibit ⟨ex:nna⟩/⟨ex:organic⟩ 走有机口径、已按公司披露的拆分把并购导入扣除，'
                    '所以不在这份名单里（它们画的是口径断点，见下一条）；'
                    'Exhibit ⟨ex:organic-years⟩/⟨ex:organic-heat⟩ 是逐年 / 逐格的类别轴，引擎没有竖线可画，'
                    '相应说明写在那两张图的图注里。')
    else:
        BRK_NOTE = ('<b>并购断点。</b>' + '、'.join(_in_win) +
                    ' 这几笔整体并表已全部滚出各图窗口，本期没有任何一张图需要画断点线。')

    # ── 口径断点说明：与并购断点分开写，因为它们是两件事 ────────────────────
    # 并表是「数变了」，改定义是「量的东西变了」。混在一句里，读者会以为 2019 年也并了表。
    _cal_in = [(p, lab, tx) for fam in ('nna', 'cash') for p, lab, tx in CAL_BREAKS[fam]
               if p in df.index]
    if _cal_in:
        CAL_NOTE = ('<b>⚠️ 口径断点（官方改定义，不是并表）。</b>窗口放宽到 '
                    f'{WIN_FROM} 之后，两处历史口径变更第一次进到图里 —— 它们在'
                    '<code>series/lpla.csv</code> 里早就存在，只是以前落在窗口外没人看见：'
                    + '；'.join(f'<b>{tx}</b>' for _p, _lab, tx in _cal_in) +
                    '。本期真正画出口径断点线的是 Exhibit '
                    + '、'.join(str(i) for i in drawn_cal)
                    + '（NNA 那条画在总 NNA / 有机 NNA 一族，现金那条画在客户现金一族，'
                      '窗口盖不到的自动不画，也不会在图注里声称画了）。'
                    '<b>本站不改写历史值去「拉平」这两处</b>：真值表存的是各期官方公告原值，'
                    '拿后来的定义回算过去，等于把一个官方从未发布过的数字印成事实。'
                    'Exhibit 1 汇总表的 y/y 与本页所有跨过这两条线的同比，'
                    '分子分母分属两套定义，读的时候要记住这一点。')
    else:
        CAL_NOTE = ('<b>口径断点。</b>已知的两处历史口径变更（May-2019 的 NNA 定义、'
                    'Apr-2019 的客户现金口径）都在本页序列覆盖范围之外，本期无需标注。')

    # ── 双轴零点对齐的代价：现算「右轴跨零而左轴柱恒正」的图，不写死编号 ──
    DUAL_NEG = []
    for e_ in ex:
        rhs = e_.get('yoy') or e_.get('line')
        if not rhs or not rhs.get('values'):
            continue
        rv_ = [v for v in rhs['values'] if v is not None]
        if e_['kind'] == 'gs_bar':
            lv_ = [v for v in e_['values'] if v is not None]
        elif e_['kind'] == 'grouped_bars':
            lv_ = [v for g in e_['groups'] for v in g['values'] if v is not None]
        else:
            continue
        # 必须是**跨零**：整条右轴都在零下（或全在零上）时引擎根本不做对齐，左轴不会被
        # 拉下去，写进来就是一句假话。窗口从 25 期放宽到 2016-01 之后，Exhibit 9/10/13 的
        # 右轴同比第一次跨到零两边（此前 Ex10 的右轴整条在零下、不入选），
        # 所以这张名单是现算的、不能写死 —— 换个窗口成员就变。
        if rv_ and lv_ and min(rv_) < 0 < max(rv_) and min(lv_) >= 0:
            DUAL_NEG.append(e_['n'])

    # ── 截轴说明：同样从「真的截了」的清单现算 ──
    if CAPPED:
        CAP_NOTE = ('<b>截轴的 ' + str(len(CAPPED)) + ' 处，一处都不删点。</b>' +
                    '、'.join(f'Exhibit {i}（{d_}）' for i, d_ in sorted(CAPPED)) +
                    '的纵轴被并表月 / 并表季单独撑爆，不截轴则其余各期全部压成贴零的一条带。'
                    '一律按本仓的规矩 7 处理：轴截住、超界的柱画到边界加断口符号、'
                    '超界的点画成空心红圈，<b>真值一律竖排标在图上</b>，'
                    '表格视图与 tooltip 里也仍是真值 —— 截的是轴，不是数据。'
                    '图右上角的红色斜体小字标明该图截过轴；离群期滚出窗口之后截轴自动撤销，'
                    '那行字与图注里的相应说明也一并消失。本站没有对数轴，也不用对数轴绕开这件事。')
    else:
        CAP_NOTE = ('<b>本期没有截轴。</b>各图窗口内都没有把其余各期压平的离群值，'
                    '所以纵轴全部按数据自然范围画；一旦再出现并表这类离群期，'
                    '会自动改为截轴 + 标出真值（规矩 7），并在图右上角标明。')

    # ── 通栏 / x 标签抽稀：必须在组 payload 之前跑 ────────────────────────────
    # 窗口从 25 期变成 100+ 期之后，半栏卡片里每期只剩 2–3px，柱图会糊成一片色带。
    # 判据不在本文件里另写一份：mrwin 是全站共用的裁决层（与 build/single.py 及
    # cboe / hkex / cme 等共用同一份量边距算式），本文件**只调用、不修改**它。
    # 它就地给 exhibit 补 full / xrot / xstep，并把「为什么通栏、为什么每 N 期标一个」
    # 追加进各自的图注 —— 那段说明是算出来的，不是写死的。
    mrwin.layout_all(ex)

    payload = {
        'ticker': 'lpla',
        'tracker': 'LPLA Monthly Metrics Tracker',
        'title': f'LPL Financial Holdings (LPLA)：月度经营指标 — {LATEST.year} 年 {LATEST.month} 月',
        'data_through': str(LATEST),
        'through_label': f'{LATEST.year} 年 {LATEST.month} 月',
        'subtitle': ('LPL Financial 月度经营指标新闻稿 + 季报（季末月无独立月报）· '
                     f'覆盖 {df.index[0]} – {LATEST}（{len(df)} 个月）· '
                     '版式照 Goldman Sachs GIR「LPLA monthly metrics」系列'),
        'headline': headline,
        # 一行数据条给读数，brief 给「这个读数该怎么读」。序列一律传全历史：
        # 名次、「N 个月最低」、并表后经过几个月都要在全样本上算，13/25 个月窗口算不出。
        'brief': compose_brief([str(p) for p in df.index],
                               df['total_assets_usdbn'].values,
                               df['advisory_assets_usdbn'].values,
                               df['brokerage_assets_usdbn'].values,
                               df['client_cash_usdbn'].values,
                               df['cash_pct_assets'].values,
                               df['organic_growth_ex'].values,
                               df['nna_ex'].values,
                               df['nna_advisory_usdbn'].values,
                               df['nna_brokerage_usdbn'].values),
        # 名词释义：排在所有 exhibit 之前。选词判断与「有意不收哪些词」写在 GLOSSARY
        # 上方那块注释里；版式与四道护栏在 build/glossary.py，本文件只交那张二元组表。
        'glossary': gloss.render(GLOSSARY, where='lpla glossary'),
        'hub_line': hub,
        'source': SRC,
        'xlabels': XL13,
        'xlabels_long': XLALL,
        'summary': summary,
        'exhibits': ex,
        'table': table,
        'notes': [
            '<b>数据源。</b>全部数值来自 <code>series/lpla.csv</code>（LPL Financial IR 月度经营指标'
            f'新闻稿，{df.index[0]} 起逐月连续）与 <code>series/fee_rates.csv</code>（LPLA 季度净收益率与'
            '季度实际客户现金收入）。页面不做任何计算，所有口径判断与格式化都在 '
            '<code>build/lpla.py</code> 里完成。'
            f'两份源的更新节奏不同：月度序列到 {mlab(LATEST)}，净收益率到 {RQ_LAST}、'
            f'季度实际客户现金收入到 {act_q.index[-1]}。费率按季度披露而月度数字每月往前走，'
            '所以某个月所属季度尚未披露费率时，该月的隐含值沿用上一可得季度的费率；'
            + (f'本期主窗口 {WSPAN} 里有 {len(CARRY_M)} 个月是这样算的。' if CARRY_M
               else f'本期主窗口 {WSPAN} 里没有这样的月份。')
            + '逐图的期间说明见 Exhibit ⟨ex:cash-rev⟩ / ⟨ex:cash-rev-bridge⟩ 图下的「费率期间」一行。',
            '<b>季末月口径。</b>' + QNOTE + '（3/6/9/12 月无独立月报，取自当季季报），'
            '因此这几个月的披露时点与其余月份不同，但口径一致。',
            BRK_NOTE,
            CAL_NOTE,
            '<b>有机口径与 Acquired NNA。</b>Exhibit ⟨ex:nna⟩/⟨ex:organic⟩/⟨ex:organic-years⟩/⟨ex:organic-heat⟩ 用的是有机 NNA = 总 NNA − '
            f'公司同页披露的 Acquired NNA。本页登记 {len(ACQ)} 笔、覆盖 {min(ACQ)} … {max(ACQ)}，'
            '<b>逐笔都有官方出处、一笔估算都没有</b>：'
            '2019-08 起官方在同一份文件里并排印 Total NNA 与 Total Organic NNA 两行，两者之差'
            '就是当月并购导入；2017-12…2018-04 的 NPH 分批导入那一代文件没有 organic 行，'
            '用的是官方印的影子块「Net New Assets Prior to NPH」，差额合计 $72.3bn，'
            '与同一份文件里「Assets Served Prior to NPH」和正表总资产之差 $72.2bn 互相印证；'
            'Aug-2019 的 Allen &amp; Company $2.9bn 取自官方脚注原话。'
            '<b>2018-05 起 NPH 影子块与正表之差转负</b>（−0.5 / −0.2），那已经是 NPH 存量顾问'
            '自己的净流出、属有机口径，所以没有扣 —— 并表只扣「导入进行中」的那几个月。'
            '这张常量表不是 <code>series/lpla.csv</code> 的一列（官方只在正文注里给），'
            '所以硬编码在 <code>build/lpla.py</code>。'
            '⚠️ <code>build/wealth.py</code> 里的同名副本仍是 2023-01 起的旧版：'
            '那一页 LPL 的窗口只到近年、今天读数不受影响，但<b>它哪天放宽窗口，'
            '必须把 2017-12…2021-09 这几条一并带过去</b>，否则同一条 LPL 有机 NNA 会在两页'
            '给出不同的值。',
            CAP_NOTE,
            '<b>GS 规矩 1：流量不算百分比。</b>这条规矩针对的是<b>环比</b>与<b>单月同比</b>：'
            'NNA 是流量，拿上个月或去年那一个月当分母都没有经济含义。'
            '汇总表里给的是绝对变化（$bn），趋势请读「年化有机增长率」= 当月 NNA × 12 / '
            '上月末客户资产。'
            '⚠ <b>Exhibit ⟨ex:nna⟩ 的次轴 2026-09 起画的正是这条规矩点名的单月同比</b> —— '
            '页面所有者要求全站统一成单月口径，这条 GS 规矩因此在本页被覆盖了一半：'
            '同比线照画，但读它必须连着柱高一起看，代价（区间、跳变、与滚动口径符号相反的'
            '月份）由该图图注拿本序列现算印出。趋势判断仍以年化有机增长率为准。'
            'Exhibit ⟨ex:nna-q⟩（季度 NNA）仍然没有同比线，但理由不同 —— 它画的是 as-reported 总 NNA，'
            f'窗口内 {len(bad_q)} 个季度被并购导入污染，换哪种口径都不可比。',

            # ── 同比口径：2026-09 全站统一成单月之后，本页只剩一种 ──
            # 「点名」不是客套：读者在同一页上看到两个都叫 y/y 的有机 NNA 读数，
            # 没人告诉他分母不同，他只会以为哪里算错了。现在分母统一了，但**为什么**
            # 统一、代价是什么，仍然要逐处交代 —— 尤其是流量那两张，它们是被改过来的。
            '<b>⚠ 同比口径：本页现在只有<b>一种</b>—— 单月同比（本月 ÷ 去年同月 − 1；'
            '比率序列取<b>百分点差</b>）。</b>'
            'Exhibit ⟨ex:assets⟩ / ⟨ex:nna⟩ / ⟨ex:organic⟩ / ⟨ex:cash⟩ / ⟨ex:cash-pct⟩ / ⟨ex:advisory⟩ / ⟨ex:cash-rev⟩ / ⟨ex:brokerage⟩ / ⟨ex:advisory-share⟩ 的次轴金线、Exhibit 1 汇总表的 '
            'y/y 列、Exhibit ⟨ex:organic-heat⟩ 热力矩阵的逐格读数、以及顶部 headline 与 brief 里标明「单月」'
            '的同比，全部是这一个口径，彼此可以逐格对上。'
            '<b>其中流量类的两张（Exhibit ⟨ex:nna⟩ 有机 NNA、Exhibit ⟨ex:organic⟩ 年化有机增速）是 2026-09 才从'
            f'「{yoy.TTM_WIN} 个月滚动合计同比」改过来的，理由是页面所有者要求全站统一成'
            '单月口径</b>（CONTRACT §6 抬头引了原话）—— 一句可核对的指令，'
            '不是「看着更灵敏」（§6.1 第 3 条点名禁止的说法）。'
            '§6.1 第 1 条同时给了单月口径一个可核对的好处：<b>柱与线取自同一列</b>，'
            f'拿一根柱和 {yoy.LAG} 根柱之前那根一比就是线上那一点，读者能自己验算。'
            '<b>代价按 §6.1 第 3 条一并印出，不只写结论</b>'
            + (f'：对齐到两种口径都算得出的同一批月份（{_ST3["n"]} 个月），有机 NNA 的'
               f'单月同比标准差 {_ST3["sd_m"]:,.1f}pp 是滚动口径 {_ST3["sd_r"]:,.1f}pp 的 '
               f'{_ST3["sd_m"] / _ST3["sd_r"]:,.1f} 倍，读数区间 '
               f'{_ST3["lo_m"]:+,.0f}%~{_ST3["hi_m"]:+,.0f}% vs '
               f'{_ST3["lo_r"]:+,.0f}%~{_ST3["hi_r"]:+,.0f}%，相邻月最大跳变 '
               f'{_ST3["jump_m"]:,.0f}pp vs {_ST3["jump_r"]:,.0f}pp，'
               f'{len(_ST3["flips"])} 个月两种口径符号相反。' if _ST3 else '。')
            + '滚动那一侧本页<b>只算不画</b>，两条图注与本条里的并排读数都出自它。'
            + '<b>存量与比率类（Exhibit ⟨ex:assets⟩ / ⟨ex:cash⟩ / ⟨ex:advisory⟩ / ⟨ex:cash-rev⟩ / ⟨ex:brokerage⟩ 与 Exhibit ⟨ex:cash-pct⟩ / ⟨ex:advisory-share⟩）不受这次改动'
            '影响</b>：单月本来就是它们唯一（或唯一合法）的口径，不是选出来的。'
            '<b>这里要更正一句本站从前的说法</b>：「存量不能做滚动，所以只能点对点」是<b>错的</b> —— '
            'Σ12/Σ12′ 里的除数约掉，12 个月滚动合计比恒等于 12 个月滚动<b>均值</b>比，'
            '而「去年一整年的平均客户资产 vs 前年」是个真实存在、可以核对的量。'
            '假的只是<b>「合计」这个名字</b>（12 个月末余额相加不指代任何东西）。'
            '所以存量<b>可以</b>平滑，本页仍用点对点，<b>主因是结构性的</b>：'
            '均值口径按构造滞后约半年、回答的是「去年一整年的平均水平 vs 前年」，'
            '而这几张图要回答的是「现在相对去年此刻」。'
            + _CASH_CAL_TXT
            + 'Exhibit ⟨ex:cash-pct⟩ / ⟨ex:advisory-share⟩ 是另一回事：它们是<b>比率</b>，12 个月的占比做算术平均'
            '本身就没有意义（每个月分母不同），要一年的平均占比必须分母加权 —— '
            '那是真正的硬约束，共享模块 <code>build/yoy.py</code> 会直接抛错。'
            '<b>Exhibit ⟨ex:nna-q⟩（季度 NNA，按日历季汇总）不画单月同比</b>：'
            '它画的是 as-reported 总 NNA，被并购导入污染到换哪种口径都不可比，理由见该图。'
            + (f'当期读数并排现算：{"；".join(_CAL)}。' if _CAL else '')
            + '<b>热力矩阵（Exhibit ⟨ex:organic-heat⟩）</b>属 CONTRACT §6.3 的图型豁免：'
            'heat_matrix 每一格本来就是一个月的读数，与这次口径改动无关；'
            '标题里已写明画的是「单月」年化增速。'
            '<b>逐年对照图（Exhibit ⟨ex:organic-years⟩）根本不画同比</b>，它画的是每个日历年年内的'
            '累计路径，不进口径这条规矩。',

            '<b>次轴同比取代了 12 个月均线。</b>原 deck 的 <code>gsx.lvl_bar</code> 在右轴画'
            '金色同比折线，其 docstring 写明「均线只是把柱子再平滑一遍、不带新信息，'
            '同比才回答『相对去年这个月是好是坏』」。网页版此前一律画成均线，本轮改回同比：'
            'Exhibit ⟨ex:assets⟩/⟨ex:nna⟩/⟨ex:organic⟩/⟨ex:cash⟩/⟨ex:cash-pct⟩/⟨ex:advisory⟩/⟨ex:cash-rev⟩/⟨ex:brokerage⟩/⟨ex:advisory-share⟩ 现在画的都是次轴 y/y（比率序列用 pp），不再画均线，'
            '口径写在各图的图例名与 ylab2 里（一律 <code>单月</code>，见上一条）。'
            '副作用是双轴要对齐零点 —— 同比跨零而柱恒正时，左轴基线会被拉进负区，'
            '代价超过引擎阈值时改为两轴独立缩放并在绘图区左上角用红色斜体标出。',
            '<b>3Y %ile 的判据。</b>与全站其余各页共用同一份实现'
            '（<code>build/pctile.py</code>），本文件不再自己写一套 —— 从前各页各写各的，'
            '同一条 LPL 客户资产序列在本页被判成噪音留空、在横截面页却印成绿色 100。'
            '判据是「把这一列在过去 24 个月里逐月回放，≥70% 的月份分位钉在极值就留空」，'
            '本期留空的行与原因印在汇总表下方。',
            '<b>GS 规矩 2：比率类差异用 pp / bp。</b>% Advisory、% of client assets、'
            '年化有机增速这三行的 m/m 与 y/y 都是百分点差；绝对值小于 1pp 时改用 bp。',
            '<b>推导值必须标 Implied。</b>Exhibit ⟨ex:cash-rev⟩ 的月度客户现金收入 = 月末客户现金 × '
            '公司披露的季度净收益率 ÷ 12，是<b>推导值</b>；Exhibit ⟨ex:bridge⟩ 的 Market gains 是恒等式'
            '配平项（当月资产变动 − 当月 NNA），同样不是披露值。Exhibit ⟨ex:cash-rev-bridge⟩ 把桥算出的季度值'
            f'与公司披露的实际值并排，窗口内平均绝对误差 {mae:.1f}% —— 误差主要来自'
            '「月末余额 vs 季度平均余额」这一个近似。' + FEE_Q_NOTE,
            f'<b>窗口。</b>时序柱图与双序列线图的左端是<b>日历常量 {WIN_FROM}</b>'
            f'（本期 {WSPAN}），右端跟着数据最新月走 —— 原 deck 写死「近 25 个月」，'
            f'而 <code>series/lpla.csv</code> 已回补到 {df.index[0]}，'
            '窗口停在近两年等于把回补的历史藏起来。序列若比这个起点晚，就用序列自己的起点'
            '（<b>只往右让、不往左借</b>），与 <code>build/single.py</code> 的 '
            '<code>WIN_FROM</code>、cboe / cme / hkex / msci 各页同一个口径。'
            # ⚠ 原文是「比其余各图晚一格」—— 同一段话下面就写着堆叠图 / 滚存桥 / 季度图
            #   用的是近端窗口，它们的左端比 Exhibit 4 晚九年多。参照系换成主窗口起点，
            #   差几期由 _lag4n 现算（与 Exhibit 4 卡片上那一句同一个变量）。
            f'Exhibit ⟨ex:organic⟩ 的{_LAG4}：年化增速要用上月末资产，'
            '序列第一个月算不出来，那一格在数据里不存在，不补零也不前向填充。'
            f'堆叠图与滚存桥仍用 {WIN_S} 个月、季度图<b>上限</b> {WIN_Q} 个季度 —— '
            '它们回答的是「最近一年发生了什么」，拉到十年只会把有意义的柱压成一堵墙。'
            # ⚠ 这里原先只写「季度图用 14 个季度」，而 Exhibit 14 的桥检验还要剔掉
            # 未满 3 个月的季度、实际比上限短一张 —— 一个数替两张图说话就会有一张对不上。
            f'本期 Exhibit ⟨ex:nna-q⟩ 画 {len(qw)} 季；Exhibit ⟨ex:cash-rev-bridge⟩ 还要剔掉未满 3 个月的季度，'
            f'画 {len(qs)} 季（哪几季被剔、为什么，写在该图图注里）。'
            '窗口一律从数据最新月倒推，不依赖构建当天的日期。'
            '断点线也随窗口现算：滚出窗口就不画，同时这段说明里的编号清单也跟着少一张，'
            '不会留下「图注说画了、图上却没有」的死文案。'
            f'{len(W25)} 期塞不进半栏卡片，通栏与 x 标签抽稀由共享判据 '
            '<code>build/mrwin.py</code> 按引擎自己的量边距算式现算（抽的是标签不是数据点），'
            '逐图结论追加在各自图注末尾。',
            '<b>与原 PDF 的已知差异。</b>'
            f'(1) 比率图（Exhibit ⟨ex:cash-pct⟩/⟨ex:advisory-share⟩）原 deck 取两位小数，网页图上取一位 —— {len(W25)} 根柱的'
            '两位小数标签会横向叠成一片；两位小数见这两张图的「表格」视图、汇总表与末尾核对表。'
            '(2) 柱顶数值标签去掉了 <code>$</code> 前缀（改用不带货币符号的 '
            '<code>f2</code> / <code>f1</code> / <code>f0c</code>），单位一律写在纵轴标题里。'
            '原因有两层：引擎的 <code>usd0</code> 没有千分位，会把 advisory 印成 $1537；'
            f'而窗口拉到 {len(W25)} 期之后，一格只有 '
            f'{mrwin.band_px(ex[0], full=True):.1f}px，'
            '<code>build/mrwin.py</code> 的 <code>label_clash()</code> 按引擎自己的量边距'
            '算式实测：带 $ 的 <code>$54.8</code> 宽 20.0px、预算只有 17.5px，'
            '末柱标签会压进右轴刻度栏（<code>$1.94</code> 与 <code>$150</code> 同样超）。'
            '去掉 $ 之后三张都回到预算内。Exhibit ⟨ex:nna⟩ 的 <code>$8.8</code> 本来就在预算内，'
            '保留 $。'
            '(3) 原 deck 长历史图末端的红色虚线圈没有网页等价元素，改由末点数值标注'
            '（Exhibit ⟨ex:assets-history⟩/⟨ex:cash-history⟩ 已补上）与表格视图读数。'
            # ⚠ 这一句被抓到过一次说假话：它写着「Exhibit 3 保留均线不画同比」，而
            #   Exhibit 3 自 2026-08 起就画同比、并且**没有**均线（`bar()` 的 avg12 分支
            #   只在 yoy=False 时才走）—— 一条被同一份 payload 当场证伪的断言。
            #   现在只留 Exhibit 11 那一半（它确实没有同比线），Exhibit 3 的口径沿革
            #   写在该图自己的图注与「同比口径」那一条里。
            '(4) Exhibit ⟨ex:nna-q⟩ 撤掉了原 deck 的右轴季度同比（理由见该图图注：'
            '并购导入把可比季度打散了）。'
            '(5) 本引擎强制两轴零点同高，原 deck 的 matplotlib 不对齐 —— 右轴序列跨零'
            '而左轴柱恒正的图，左轴基线会被拉到零下（金额/占比本身不会为负）；'
            f'本期这样的是 Exhibit {"、".join(str(i) for i in DUAL_NEG)}，'
            '其中对齐代价超过引擎阈值的改为两轴独立缩放并在绘图区左上角红字标出。'
            '(6) 原 deck 的截轴由 matplotlib 自动量程决定，网页版是显式截轴 + 标出真值，'
            + (f'本期共 {len(CAPPED)} 张（清单见上文「截轴」那一条）。' if CAPPED
               else '本期没有一张需要截轴。'),
            '<b>两处已知的渲染瑕疵（引擎层，非本页数据）。</b>'
            '(1) 红色断点竖虚线画在所有系列之上（这是刻意的，否则会被柱子盖住），'
            '于是紧贴断点左侧那根柱的数值标签会被削掉一个角，例如 Exhibit ⟨ex:assets⟩ 的「$1.94」、'
            'Exhibit ⟨ex:advisory⟩ 的「1,077」—— 真值可在该图的「表格」视图里读到。'
            f'(2) 逐柱数值标签在 {len(W25)} 根柱下必然互相压住，引擎按实测 bbox 抽稀'
            '（最新一期永远保留），被抽掉的数值同样在「表格」视图里一个不少 —— '
            '窗口拉长之后抽稀比例大幅上升，逐月读数请一律走「表格」按钮而不是柱顶标签。',
            f'<b>核对表。</b>末尾 Exhibit {table["n"]} 是官方原始单位、'
            f'未做任何换算的近 {len(trows)} 个月明细，'
            '用来与 LPL 新闻稿逐条对账。每张图右上角的「表格」按钮同样给出该图的源数值。',
        ],
        'footer': ('数据与算法源自本机 <code>monthly-op-dashboards</code> 项目 · '
                   '仅供个人研究，不构成投资建议'),
    }

    # 官方发布日：查到才写这个字段。写 None 或空串都会让抬头印出「官方发布于 」这半句空话——
    # assets/page.js 判的是字段在不在，不是值真不真。
    src_day = source_day(LATEST)
    if src_day:
        payload['source_date'] = src_day

    # ── 停机兜底：Exhibit 4「比主窗口起点晚 N 期」这句话必须与 payload 对得上 ──
    # 这一族错误（图注里的断言被同一页的另一张图当场证伪）在本页与 hood 上已经修了
    # 三轮，每一轮修的人都以为自己写对了。所以判据写在这里，而不是靠人再读一遍：
    #   · 句子里印的两个端点，要能在渲染完的 payload 里原样查到；
    #   · 差额要等于两张图 x 轴长度之差；
    #   · 「比其余各图」这个参照系是**假的**（下一张图就是反例），出现在任何正文里就停机。
    _texts = [e.get('note', '') for e in payload['exhibits']] + list(payload['notes'])
    _e4 = next(e for e in payload['exhibits'] if e['n'] == 4)
    _e2 = next(e for e in payload['exhibits'] if e['n'] == 2)   # 跟着主窗口走的那一族
    _bad = [f'「比其余各图」是假的参照系，正文里还有 {sum("比其余各图" in t for t in _texts)} 处'
            ] if any('比其余各图' in t for t in _texts) else []
    _said = [t for t in _texts if '主窗口起点' in t]
    # 拿 payload 自己的两条 x 轴把那句话**重建**一遍，再回正文里逐字查 ——
    # 只比对变量不够：把句子里的 Feb-16 手写成 Jan-16，变量比对照样全过，
    # 读者读到的却是假的。重建串必须一字不差地出现在两处。
    _frag = (f'左端是 {_e4["xlabels"][0]}，比主窗口起点 {_e2["xlabels"][0]} 晚 '
             f'{len(_e2["xlabels"]) - len(_e4["xlabels"])} 期')
    if _lag4n > 0:
        if len(_said) != 2:
            _bad.append(f'说这句话的地方有 {len(_said)} 处 —— 该是 2 处：'
                        'Exhibit 4 图注 + 页尾「窗口」那一条')
        if any(_frag not in t for t in _said):
            _bad.append(f'正文里那句话与 payload 重建出来的对不上，应逐字是「{_frag}」')
    elif _said:
        _bad.append('Exhibit 4 今天没有比主窗口起点晚，正文却还在说它晚')
    if _bad:
        raise SystemExit('Exhibit 4 左端那句话与 payload 对不上：' + '；'.join(_bad))

    path = os.path.join(ROOT, 'data', 'lpla.js')
    # 图号：补 id、正文里建图时的号换成 ⟨ex:id⟩，交出 ORDER；write_dash 编号兑号。
    exhibits.bind_ids(payload, EX_ID, table['n'], where='build/lpla')
    payload['order'] = ORDER
    # 写出前先过 CONTRACT §5.5 护栏（NaN/Infinity 一律拒写）；首行注释与序列化都在里面。
    payload_guard.write_dash(path, payload, 'lpla')

    print(f'数据截至 {LATEST} | 主窗口 {W25.index[0]} → {W25.index[-1]}（{len(W25)} 个月，'
          f'自 WIN_FROM={WIN_FROM}）| 长历史 {df.index[0]} → {df.index[-1]}（{len(df)} 个月）')
    print(f'Exhibit 1 汇总表 + Exhibit {ex[0]["n"]}-{ex[-1]["n"]}（{len(ex)} 张图）+ '
          f'Exhibit {table["n"]} 核对表')
    print(f'桥验证：{len(qs)} 个完整季度，平均绝对误差 {mae:.2f}%')
    print(f'写出 data/lpla.js ({os.path.getsize(path)/1024:.1f} KB)')
    print(headline)


if __name__ == '__main__':
    main()
