...
def compose_brief():
    ...
    66/67 个月构建成功、无自相矛盾，去标签字数 272–319，恒为 4 句、<b> 2–3 处。
    唯一失败的 2020-12 截断月死在本页 volcmp 的滚动口径诊断上（那个截断点 HKEX 的
    滚动同比只有 1 个读数，diff 全空、idxmax 抛错），与 brief 无关 —— 真跑时共同窗口
    远长于 24 个月碰不到；但若日后某成员的共同历史真回到恰好 24 个月，会在 volcmp
    响亮地炸而不是静默出错页，符合「失败要响」。

    ═══ 同比口径（2026-08-07 全站改造之后这段才存在）═══
    本页的曲线（Exhibit 3–6）与汇总表增长组已改成 **12 个月滚动合计同比**，但 brief
    排在 headline 之下、Exhibit 1（汇总表）之上，属于「本月读数」的导读 —— 它引用
    m/m 与 y/y 时与汇总表水平值行**同口径（单月）是合法的**：R2 的基数护栏抓的正是
    单月读数里的毛刺，拿滚动口径喂它等于把要抓的东西先抹掉。代价按 CONTRACT §6 付：
    句面上凡出现同比措辞必须写明**单月**（s2 的「反号」句），否则读者拿它对
    Exhibit 3–6 的滚动曲线，两边符号相反时会以为页面自相矛盾 —— 两个口径能差到
    符号相反、连排序都不同，抬头里已把两组读数并排印出。s1（峰值序数）/
    s3（品种环比拆解）/ s4（期间均值倍数）不含任何同比措辞，无需适配。
    ...
    # ── s2 / R2：三家里环比与单月同比反号的那一条，基数护栏。名次、方向全部现算。
    ...
    if not BE:
        ...
        # y/y 的符号不再写出来：与「反号」+ 环比方向重复，且抬头已把两个口径的 y/y 并排
        # 印了。此处 base_effect 的 y/y 是**单月**口径（与汇总表水平值行的 y/y 列同口径，
        # 合法但必须标）—— 所以句面写「单月同比」而不是裸的「同比」，否则读者会拿它对
        # Exhibit 3–6 的滚动曲线，两边符号相反时以为页面自相矛盾（见 docstring 口径块）。
        ...
        lead2 = (f'{B.cn(len(BE))}家{cnt2}环比与单月同比反号：'
                 f'{nm2} 环比{d2}{mag2}'
                 if rev else f'{B.cn(len(BE))}家环比与单月同比同向，{nm2} 环比{d2}{mag2}、'
