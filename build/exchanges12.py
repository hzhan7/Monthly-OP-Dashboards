...
━━ 三条与单公司页不同的硬规矩（横截面页通例；⚠ 原注写的「沿用 build/exchanges.py」
   已经指不到东西：那个文件在 e3c6f81「删除 /exchanges/ 三家横截面页」里删了）━━
...
⇒ 改完**图上只剩一种同比曲线口径**（滚动合计），加上一种明确标注的年度聚合；
  汇总表第 ① 组的 y/y 列仍是单月口径，组标题与表注都写死并印出两个口径的当期差距。
  ⚠ 别把上一句写成「唯一的单月同比在汇总表第 ① 组」——那句曾被搬进 NOTES，
    而抬头本身就印着三家的单月同比读数（「同月的单月同比前三是 …，本页不采用」），
    JPX 两单位图的图注里也现算印着单月同比。本文件构建期没有「某读数是不是单月口径」
    这条元数据，任何「唯一 / 没有任何一处」的说法都只能靠人肉维护 ⇒ 一律不写。
...
# 引擎给旋转 x 标签的底部空间只有 XB = 36px（assets/charts.js 搜 `var XB = fscale`
# —— ⚠ 不写行号：原注写的 charts.js:522 已经不是这一行，2026-08-19 实跑在 741），
# 而 45° + anchor='end' 的
...
# ── ICE 能源块只含 Brent：占比与中位数现算，UNCOVERED 那条正文引用它 ────────────
# ⚠ 原文写死「2019-01 占 34.8%（947 / 2,718 千张日均），187 个月中位 33.3%」。
#   基期那一格是常数不会变，但**月数每个月 +1、中位数跟着动** ——「187 个月中位 33.3%」
#   下一期就是假话，而这句话正是「ICE 这根柱子系统性偏低」这个结论的全部依据。
#   这里现读 series/ice.csv 算，取不到就退化成不带数字的说法，绝不留一个过期的数。
def _ice_brent_facts():
    """(基期占比%, 基期分子, 基期分母, 月数, 中位占比%) —— 拿不到就 None。"""
    p = os.path.join(SERIES, 'ice.csv')
    need = ('adv_brent_kcontracts', 'adv_energy_kcontracts')
    if not os.path.exists(p):
        return None
    d = pd.read_csv(p)
    if 'month' not in d.columns or not set(need) <= set(d.columns):
        return None
    d['month'] = pd.PeriodIndex(d['month'], freq='M')
    d = d.set_index('month').sort_index()
    r = (d[need[0]] / d[need[1]]).dropna()
    if r.empty or BASE not in r.index:
        return None
    return (float(r[BASE]) * 100, float(d[need[0]][BASE]), float(d[need[1]][BASE]),
            int(len(r)), float(r.median()) * 100)


_IB = _ice_brent_facts()
ICE_BRENT_TXT = (
    f'{BASE} 占 ICE 全能源张数的 <b>{_IB[0]:.1f}%</b>'
    f'（{_IB[1]:,.0f} / {_IB[2]:,.0f} 千张日均），{_IB[3]} 个月中位 {_IB[4]:.1f}%'
    if _IB else '只是 ICE 能源的一部分（本轮取不到 Brent / 全能源两列，占比未计算）')
...
def mlab(p):
    ...
    ⚠ 原注还并列了 exchanges.py —— 那个文件已在 e3c6f81 删除，全仓不存在。
    """
...
def fx_only_legs(key, specs):
    ...
    盖住全页 —— 这正是 build/pools.py「二、必须留痕的两条告诫」第 2 条与
    build/notional.py 开头那段立的
    规矩（⚠ 原注写的是 pools.py:49，实际在第 48 行，差一行；行号一律换成可 grep 的
    小节名 —— 图注必须写明「已剔汇率、
    未剔标的涨跌」），本页此前漏接。
    """
...
# 本页没有 brief（仅图无评论）。图注与页尾 notes 说的是「这一张图**这个月**该怎么读」
# （含当月读数、当月实测的毛刺量、当期缺常数的清单）；这一块说的是「**这些词**是什么
# 意思」，一年到头是同一段 ⇒ 这里**不写当月读数**，一个随月份变的数都不出现。
# 出现的数只有两类：
#   (a) 把定义钉住的**结构性**量（缺常数的 product_id 有几个、水平值口径含几家、
#       含 fx_only 腿的有几家）—— 全部在装配处**现算**传进来，一个都不写死；
#   (b) 官方定义里的常数（SOFR / 美债期货的面值与久期量级、JPX 的官方折算系数），
#       出处在下面逐条注明，且与页尾同一条 note 用的是同一组数、不另起一份。
# 措辞口径必须与本页既有的图注 / notes 一致 —— 释义与图注打架是这块最难被发现的错。
#
# ━━ 为什么是这 13 个词（选词判断）━━
# 判据只有一条：这个词出现在本页的图题 / 序列名 / 纵轴 / 汇总表行头与组标题 /
# 核对表列头 / 图注里，而且**不看定义就会读错**。按「读错会出什么事」分五类：
#   ① 本页自己造的主口径与它的零件   定基名义额、基期常数、产品块、紧界区间、指数化
#      —— 横截面页最要紧的一类：这五个词读者在别处见不到，而整页的「谁在跑赢」
#      全建在它们上面。读错的代价是把「构造上不给水平值」读成「漏画了几家」，
#      或把紧界区间读成置信区间/估计误差（页面上一个中心估计都没有）。
#   ② 同一件事的两个口径同页并存   fx_only、12 个月滚动合计、年度同比 ——
#      本页最密集的坑：fx_only 腿的增长里含标的涨跌，与纯合约腿不是一个口径；
#      同比有滚动与日历年两套 12 个月聚合，汇总表第 ① 组还留着单月口径。
#      读串的代价是整条线系统性偏高/偏低或跨组比出一个假差，而图形完全正常。
#   ③ 本页轧出来、不是官方披露的派生量   大合约当量、合约变小 ——
#      必须点明「不是交易所逐月发布的字段」与「恒为 0 的家是构造出来的零」。
#   ④ 读者最容易自己脑补出来的错误结论   市场份额、名义额 ≠ 风险敞口 ——
#      这两条本页一处都没给，但柱图摆在那里，不写死就会被读出来。
#   ⑤ 发布规则   共同最新月 / 短板 —— 决定了页面上为什么看不到某家已发布的最新月。
# **有意不收**：
#   · m/m、y/y、3Y %ile、pp/bp —— 全站通用的读图约定，每页 summary.note 已逐条讲过，
#     释义板再讲一遍就是两处各写一份（样板 /cost/ 的 9 条里一条都没有，是同一条判断）。
#   · notional_source 三档（definitional / official_notional / reconstructed）——
#     页尾有一整条 note 专讲，且它是「常数有多硬」的分布而不是一个读图会用到的词。
#   · 各家缺常数的**逐个原因**（HKEX_DERIV / ICE_STIR / MX_*_OPT …）—— 那是水平值
#     柱图图注里逐条列明的取证记录，属于「这一期的事实」，不是词义。
#   · 口径断点 / break_at —— 本页共同窗口内一条都没有（页尾最后一条已登记这件事）。
#   · 成交量、市值、名义额本身这类常识词 —— 本页对它们没有特殊口径；
#     真正有特殊口径的那一层已经落在「定基名义额」与「名义额 ≠ 风险敞口」两条里。
# ══════════════════════════════════════════════════════════════════════════════
def compose_glossary(band_keys, lvl_keys, fxo_keys, gap_prods):
    """`[(词, 释义), …]` —— 顺序即页面上的顺序，由 gloss.render() 拼成 HTML。

    参数全部是 build_payload 当场算出来的**结构性**清单，不是当月读数：
      band_keys  缺常数、增长只能给紧界区间的家（DISP 的 key）
      lvl_keys   常数齐备、进得了水平值那张图的家
      fxo_keys   含 fx_only 腿（只锁汇率、没锁标的涨跌）的家
      gap_prods  填不出基期常数的 product_id 清单

    排序：先讲本页自己造的主口径与它的零件（后面几条都要用到），
    再讲两个口径同页并存的那几处，最后是派生量、易被脑补的错误结论与发布规则。

    ⚠ 这里**不点 Exhibit 号**。本页图号是现算的（缺常数补齐那天第 6 张整张不画、
    差值图在 mix_keys 为空时也不画，后面的号全部前移），写死一个号，等它前移那天
    释义就会指到别的图上去 —— 而页面上不会有任何提示。图一律按内容称呼。
    """
    n_gap, n_lvl, n_fxo = len(gap_prods), len(lvl_keys), len(fxo_keys)
    band_txt = '、'.join(DISP[k] for k in band_keys)
    # 「没有水平值」与「增长只能给区间」**不是同一批家**，这里必须分开算：
    #   LEVEL_OK = 常数齐备（进得了水平值柱图）  ⇒ 缺席水平值 = MEM_KEYS − lvl_keys
    #   EXACT    = 只有一个产品块或常数全已知（增长是点值）⇒ band_keys = 非 EXACT
    # 一家可以「常数一个都没有、却只有一个产品块」——常数在增长里被完全约掉，
    # 于是它既不在 lvl_keys、也不在 band_keys（指数图图例上写作「水平值缺常数」）。
    # 拿 band_keys 当「缺席水平值的家」用会漏掉这一类，正是这里最容易犯的错。
    _lvl, _band = set(lvl_keys), set(band_keys)
    nolvl_keys = [k for k in MEM_KEYS if k not in _lvl]          # 没有水平值的家
    onlyblk_keys = [k for k in nolvl_keys if k not in _band]     # 缺常数、但增长仍精确
    onlyblk_txt = '、'.join(DISP[k] for k in onlyblk_keys)

    G = []                      # (词, 释义)

    # ① 主口径本体。恒等式写全，并把「增长率＝张数增长率」这条本页最常被用到的性质
    #    钉在这里 —— 后面 fx_only 那条正是它的例外，两条必须挨着读。
    #    「期权名义额是标的敞口不是权利金」并到最后一条「名义额 ≠ 风险敞口」里去了 ——
    #    两条讲的是同一件事（名义额不是什么），分开写就是同一处坑写两遍。
    G.append((
        '定基名义额',
        '本页的主口径（constant-basis notional）：<code>张数 × 乘数 × '
        f'{mlab(BASE)} 基期价格 × {mlab(BASE)} 基期汇率</code>。'
        '后三项都是<b>常数</b>，所以对合约腿与股数腿（规格表 kind=contract / share）'
        '而言，一条腿的定基名义额<b>增长率与它自己的张数增长率完全相同</b> —— '
        '常数只决定产品与产品、交易所与交易所之间的<b>权重</b>，'
        '不把标的涨跌与汇率波动混进增长里。'
        '（<b>唯一的例外是 fx_only 腿</b>，见下。）'))

    # ② 基期常数：本页「有的家有水平值、有的家没有」这件事的全部原因。
    #    数只有一个（缺常数的 product_id 个数），现算；缺口补齐那天括号自动消失。
    #    ⚠ 名单走 nolvl_keys（= MEM_KEYS − lvl_keys）而不是 band_keys：
    #    缺常数但只有一个产品块的家（增长仍精确）同样没有水平值，用 band_keys 会漏掉它，
    #    而指数图图例上明写着它「水平值缺常数」—— 两处会当场打架。两类分开点名。
    #    约分那一步在上一条（定基名义额）已经讲过，这里只留结论，不重讲一遍。
    G.append((
        '基期常数',
        '上式里那个不随月份变的乘子，<code>乘数 × 基期价格 × 基期汇率</code>，'
        '按<b>产品块</b>挂。关键性质：<b>它只在比水平值时才需要</b> —— '
        '比增长时它在分子分母里同时出现、<b>约得掉</b>；比水平值时<b>约不掉</b>。'
        + (f'本页有 <b>{n_gap} 个 product_id</b> 填不出这个常数'
           if n_gap else '本页每个 product_id 的这个常数都填得出')
        + ((f'，因此缺常数的家<b>都没有水平值</b>：一类是缺口分散在多个块上、'
            f'增长只能给区间的 <b>{band_txt}</b>' if band_keys else '，因此')
           + ((f'；另一类是整家只有<b>一个</b>产品块、常数被完全约掉、'
               f'增长照样精确的 <b>{onlyblk_txt}</b>'
               '（指数图图例上写作「水平值缺常数」）' if band_keys else
               f'<b>{onlyblk_txt}</b> 没有水平值，但它只有一个产品块、增长照样精确')
              if onlyblk_keys else '')
           + '。没有水平值是<b>构造上不给</b>，不是漏画；'
             '逐个缺口的取证记录写在水平值柱图的图注里。'
           if nolvl_keys else '。')))

    # ③ 产品块：块内精确 / 块间不可加 —— 这一条是「缺常数不等于看不见增长」的支点。
    #    约分的过程在上面两条里已经讲过，这里只留这一对性质本身。
    G.append((
        '产品块',
        '换算链的最小单位：一个 product_id（或若干常数已知、权重因而已知的 product_id '
        '先合成的一块，页面上标「已定基」）。基期常数按块挂，于是有互为代价的一对性质：'
        '<b>块内精确</b> —— 一个块自己的同比与常数齐备的家同等可信，'
        '拆到产品块的那张热力矩阵每一格都是实测值；'
        '<b>块间不可加</b> —— 把块加起来要用的正是那个未知常数。'
        '这就是缺常数的家<b>拆到块能给精确值、合起来只能给区间</b>的原因。'))

    # ④ 紧界区间：页面上唯一会写成 a–b 的读数。三个否定词是这一条的全部重量。
    G.append((
        '紧界区间',
        '页面上写成 <code>a–b</code> 的那种读数（汇总表标「区间」的行、'
        '同比图上的蓝带）。含义：未知的基期常数取遍<b>所有非负值</b>时，'
        '该指标的<b>全部可能取值</b>，而且<b>两端都取得到</b>。'
        '它<b>不是</b>置信区间、<b>不是</b>估计误差，也<b>没有</b>中心估计 —— '
        '所以同比图按<b>下界</b>排序（「不管常数取什么值都至少涨了这么多」），'
        '不按中点：中点会凭空造出一个并不存在的中心估计。'
        '带子越窄，说明未知常数对结论越无关紧要。'))

    # ⑤ 指数化：把「线的高低 = 各自的增长，不是体量」写死。基期锁死那半句与页尾同源。
    #    ⚠ 本页的指数图不止一张，图例括号里的东西**不一样**：跨所那张放的是水平值
    #    （US$bn/日，见 Exhibit 2 的 series 名），JPX 那张「两个单位」放的是指数末值
    #    （raw_end / lgeq_end，定基 = 100 的读数）。把「括号里是水平值」写成通则，
    #    读者会把 JPX 图例上的 146 当成一个体量读数 —— 量纲完全不同。故这里按图分开写。
    G.append((
        '指数化',
        f'把一条序列除以它自己在基期 {mlab(BASE)} 的读数再乘 100'
        f'（纵轴写作 index, {mlab(BASE)} = 100）。'
        '⇒ <b>线的高低比的是各自相对自己基期的增长，与体量无关</b>。'
        '⚠ <b>图例括号里是什么，随图而定，不是通则</b>：<b>跨所那张</b>指数图括号里是'
        '该家最新月的定基名义额（US$bn/日），可以拿来看谁大谁小'
        '（缺基期常数的家那里写的是「水平值缺常数」）；而 JPX 那张'
        '「同一批合约、两个单位」的图，括号里是<b>指数末值</b>、<b>不是</b>体量，'
        '那张图上根本没有水平值。'
        f'基期 {mlab(BASE)} 由 <code>build/notional.py</code> 的 BASE_MONTH '
        '<b>全仓锁死</b>，一经发布不再改（改了所有历史图的水平值会整体平移，'
        '而页面上不会有任何提示）。'))

    # ⑥ fx_only：第 ① 条那条性质的唯一例外，也是跨家比增长时最容易漏掉的一层。
    #    家数现算；逐家占比留在页尾那条 note 里（那是当期事实，不进释义）。
    G.append((
        'fx_only',
        '规格表 kind=notional 的腿走的口径（<code>build/notional.py</code> 里叫 '
        'deflator=<code>fx_only</code>）：源头只披露<b>本币成交额</b>'
        '（ADT / ADNV / 现货成交额这类列，逐条列在页尾口径说明里），'
        '而金额 ≡ 股数 × <b>当期</b>价格，拿不到股数就没法把价格项剥出来。'
        '⇒ 这类腿<b>只剔掉了汇率、没剔掉标的涨跌</b>，第一条里「增长率＝张数增长率」'
        f'<b>对它不成立</b>：它的增长 = 成交量增长 + 标的涨跌。本页有 <b>{n_fxo} 家</b>'
        '含这类腿（逐家占多大写在页尾口径说明里）—— 跨家比增长时，'
        '这几家与纯合约腿的家<b>不是同一个口径</b>。'))

    # ⑦ 滚动合计：本页同比的默认口径 + 汇总表第 ① 组那个必须点破的例外。
    #    「不乘交易日数」那半句与页尾 TTM_UNIT_NOTE 同源，不另起一套说法。
    G.append((
        f'{TTM} 个月滚动合计',
        '本页同比的默认口径：<code>本月往前 '
        f'{TTM} 个月的合计 ÷ 去年同月往前 {TTM} 个月的合计 − 1</code>，'
        '<b>不是</b>单月同比（改口径的实测理由写在页尾与同比图的图注里）。'
        f'合计的算法是 <b>{TTM} 个月的日均值直接相加、不乘交易日数</b>'
        '（本页所有腿在 <code>leg_units()</code> 里已统一除成日均，'
        '理由写在页尾口径说明里）。'
        '⚠ <b>汇总表第 ① 组（水平值那一组）是例外</b>：那一组的 m/m 与 y/y '
        '恒等于本行三列的算术，因而是<b>单月口径</b>，'
        '与第 ②③ 组的 y/y <b>不可比</b>。'))

    # ⑧ 年度同比：同页第二套 12 个月聚合。要点是「与滚动不是精粗之分，只是切法不同」。
    G.append((
        '年度同比',
        # 热力矩阵有两张还是一张，取决于本期有没有缺常数的家（拆到产品块那张是它的补集）。
        ('两张热力矩阵' if band_keys else '热力矩阵')
        + '用的口径：按<b>日历年</b>切，且<b>同月对同月</b> —— '
        '未满的年份只拿已有的那几个月去比上年同样的几个月，绝不拿半年比全年。'
        f'它与滚动合计口径<b>都是 {TTM} 个月量级的聚合、都不受单月毛刺影响</b>，'
        '只是切法不同（一个按日历年、一个按最近 12 个月）；'
        '完整年份的 12 月那一格，两者在数学上恰好相等。'
        # 色标取本图 5/95 分位、空格 = 没有可比历史 —— 这两条读图约定在热力图自己的
        # 图注里已经逐条写过，释义板再写一遍就是两处各写一份，故不收。
        '热力图的色标只在<b>同一张图内部</b>可比（判据写在图注里），'
        '不要拿两张图对望。'))

    # ⑨ 大合约当量：本页唯一一条「官方系数 + 本仓自建序列」的列，必须把这条界划清。
    #    折算系数来自 JPX IR 脚注（同页尾 note 与本文件 docstring 引的是同一组）。
    G.append((
        '大合约当量',
        'JPX 那张「同一批合约、两个单位」图里的第二条线。'
        '<b>不是 JPX 逐月发布的字段</b>：折算系数是官方的'
        '（JPX IR 脚注：日経225mini ÷10、日経225マイクロ ÷100 等），'
        '<b>逐月序列是本仓按这些系数自建的</b>（<code>fetch/jpx.py</code>，'
        '并用 IR 的季度 Financial Derivatives 合计校准到 ~2% 以内）—— 两件事不能混为一谈。'
        '该列<b>不进换算链</b>（乘数已经被折算掉，再乘一次基期价就是重复计数），'
        '只在那张图里出现；那张图只含<b>金融</b>衍生品（股指 + 利率）、不含商品。'))

    # ⑩ 合约变小：差值图的读法 + 两处「读成别的意思就错了」。
    G.append((
        '合约变小',
        '「张数口径同比 − 定基名义额口径同比」这个差（单位 pp）为<b>正</b>时的读法：'
        '张数涨得比它代表的<b>敞口</b>快，成交往更小的合约迁移（拆细、mini / micro 化）；'
        '为负则相反。两侧必须<b>同口径</b>（都取滚动合计），'
        '否则算出来的是噪音差不是结构差。⚠ 它量的是<b>品种结构在一年里往哪边挪</b>，'
        '不是「这家有没有 micro 合约」：把成交全迁进 micro 而品种间比例没变，读到的仍是 0。'
        '⚠ 只配了<b>一个</b>合约产品的家差值<b>恒为 0</b>（构造出来的零，'
        '不是「这家没把合约做小」），所以<b>不画</b>；'
        '缺常数、名义额那一侧算不出来的家同样不画。'))

    # ⑪ 市场份额：本页一处都没给，但柱图摆在那里，不写死读者一定会自己除。
    #    两个原因（覆盖边界 + 缺席几家）与页尾「口径边界」那条 note 同源。
    G.append((
        '市场份额',
        '<b>本页一处都没有给，任何一张图都不能这么读。</b>两个原因：其一，'
        '本页的「某家名义额」= 该家<b>已进换算链的那几条腿之和</b>，不是它的全部业务'
        '（哪些列在范围内却没进腿，页尾逐条列明）；其二，水平值那张图'
        f'<b>只含常数齐备的 {n_lvl} 家</b>'
        # ⚠ 这里**不给缺席的家数**。缺席水平值的是 MEM_KEYS − lvl_keys，与
        # band_keys（增长只能给区间的家）不是同一批 —— 早先写的 len(band_keys) 少算
        # 了「缺常数但只有一个产品块」的那一类。水平值柱图的图注本来就用中性说法
        # （「缺席的几家里有两家体量都不小」），照抄它既不写死数、也不与页尾 notes 打架。
        + ('，缺席的那几家里体量不小的也有。' if nolvl_keys else '。')
        + '⇒ 那张图左上角的合计<b>只是图上这几家的和，不是市场总量</b>，'
          '拿一根柱子除以它得到的是<b>池内占比</b>，<b>不是</b>份额。'
          '另有一道硬规矩：混了 deflator 的池不许算份额'
          '（<code>build/notional.py</code>）。'
          '比<b>增长</b>不受这些影响 —— 每家都是拿自己和自己的去年比。'))

    # ⑫ 共同最新月：解释「为什么本页看不到某家已经发布的最新月」。
    G.append((
        '共同最新月 / 短板',
        '发布门槛：本页所有图与表一律截到 <b>12 家中最慢的那家的最新月</b>，'
        '这个月称共同最新月，卡住它的那几家称短板。'
        '理由是 12 家的披露节奏差好几天到两周，若各画各的最新月，'
        '读者会拿一家的 7 月去比另一家的 6 月，'
        '看到的「谁跑赢」里就有<b>一整个月是口径造出来的</b>。'
        '⇒ <b>跑在前面那些家的最新一个月不在本页任何一张图、任何一行表里</b>，'
        '要看它们请去各自的单公司页。'))

    # ⑬ 名义额 ≠ 风险敞口：面值与久期量级是**合约定义里的常数**（notional_source
    #    的 definitional 一档），不随月份变；与页尾同名 note 引的是同一组数，
    #    出处见 series/contract_specs.csv 的 evidence 列。
    G.append((
        '名义额 ≠ 风险敞口',
        '一条必须留痕的告诫。利率池尤其危险：3 个月 SOFR 期货面值 100 万美元、'
        '久期约 0.25 年；10 年期国债期货面值 10 万美元、久期约 8 年 —— '
        '按名义额短端是长端的 10 倍，按 DV01（真正的风险敞口）却相反，'
        '<b>两者能差一个数量级</b>。能源同理（不同能源品的热值不同）。'
        '所以本页的名义额只能读作<b>被交易的名义规模</b>，'
        '<b>不可</b>读作谁承担了更多利率风险；要回答风险口径必须 DV01 加权，'
        '而月度成交报表里没有久期字段。'
        # 期权那一半原先挂在「定基名义额」条末：与这里讲的是同一件事（名义额不是什么），
        # 分开写就是同一处坑写两遍，故并到这一条来。
        '同理，期权的名义额是<b>标的敞口</b>（张数 × 乘数 × 基期指数点位），'
        '<b>不是</b>权利金，与交易所按张收费的 RPC 口径不可互推。'))
...
def build_payload(raw, specs, fx, kconst):
    ...
    # 真值竖排标在图外（assets/charts.js 搜注释「超出截轴范围的柱画到边界」——
    # ⚠ 原注写的 charts.js:919-923 已经不是这段代码，2026-08-19 实跑在 1296，
    #   行号会漂，字符串不漂）。判据写成规则而不是写死一个数：
    ...
    ex.append({
        ...
        'note': ('<b>本图是年度同比，不是单月同比，也不是滚动同比。</b>'
                 ...
                 # ⚠ 原文「所以本页<b>没有单月同比曲线</b>」——今天为真，但它是人肉维护的
                 ...
                 + (f'{"、".join(str(y) for y in part_yrs)} 年只到 {mlab(LATEST)}，'
                    ...
                    if part_yrs else '')
                 + ((f'<b>{"、".join(f"{DISP[k]} {y}" for k, y in gap5)} 那一格是'
                     f'<b>11 个月</b>对 11 个月，不是完整 12 个月</b>：该家有一个月的官方数'
                     f'被主动留空（缘由见 Exhibit 2 的图注），而本图是同月对同月，'
                     f'缺的那个月被分子分母同时剔掉了 —— 数值仍然可比，但它比同一列'
                     f'其余各格少一个月的样本。'
                     + (f'紧接着的 {"、".join(str(y + 1) for _k, y in gap5)} 年那一格因此'
                        f'<b>空着</b>：它要拿只剩 11 个月的上一年做分母，同月对同月凑不齐。'
                        if any(y + 1 in yrs for _k, y in gap5) else ''))
                    if gap5 else '')
    ...
    notes = [
        ...
        '完整年份的 12 月那一格，两个口径在数学上恰好相等。'
        # ⚠ 原文这里是两句全称：「所以本页的图上没有任何一处是单月同比。」
        #   「唯一的单月同比在汇总表第 ① 组的 y/y 列」。后一句被**同一页的抬头**当场证伪 ——
        #   抬头印着「同月的单月同比前三是 …（连排序都不一样），本页不采用」；前一句也站不住，
        #   JPX 那张两单位图的图注里就现算印着单月同比读数（「同月的单月同比是张数 …」）。
        #   两句都不是现算的，而本文件构建期没有「某个读数是不是单月口径」这条元数据 ⇒
        #   (A) 做不到。收窄成只说汇总表那一列自己（读者对着表就能验），全称一律去掉。
        ...
        + ('相邻期可直读。' if not GAPS else
           '上一条登记的缺月是另一回事 —— 那是同一口径下某个月没有可用的官方数，'
           '不是口径变了；除了跨过缺口的那一对，相邻期仍可直读。')
    ...
    payload = {
        ...
        # 名词释义：排在所有 exhibit 之前。选词的判断与「有意不收哪些词」写在
        # compose_glossary 上面那块注释里。参数全是现算的结构性清单，不写死。
        'glossary': gloss.render(compose_glossary(band_keys, lvl_keys, fxo_keys, gap_prods),
                                 where='exchanges12 glossary'),
