...
def ttm_cover(vals):
    ...
    页上不画滚动同比（CONTRACT §6），这条只用来量**对照侧的覆盖度** —— Exhibit 2
    的图注拿它说明「本图为什么给不出两种口径并排的实测对照」。判据要现算不能写死：
    序列每月长一个月，覆盖度会自己变好，写死的数字迟早变成假话。"""
...
# ── 本页的同比口径与「为什么拿不出两口径对照」的现算判据 ────────────────────
# 【2026-09 改】这一段从前写的是「为什么本页**偏离**契约默认的 12 个月滚动合计」。
# 那个前提没了：CONTRACT.md §6 现在规定全站同比一律单月（§6.1 第 1 条流量、
# 第 2 条存量），本页与全站同口径，不需要任何理由。
# 下面这几个数留着，用途换了 —— 它们现在回答的是 §6.1 第 3 条那笔代价账的一个前提：
# **本页这条序列给不出干净的对照侧**。
#   (1) 口径断点：滚动窗口会把 2025-12「剔除 event contracts」一次抹到 12 个月上，
#       拿它当对照量到的是断点不是口径（单月口径至少还能逐月点名哪几个读数跨了口径）；
#   (2) billed issuance 从来不给绝对面值，Ex3/4/7 手里根本没有可加总的水平值序列，
#       滚动口径在那几张上无从算起 —— 全页只有 Ex2 一处能算出对照侧。
...
ex2 = {
    ...
    'note': (f'柱为公司披露的 SPDJI 交易所交易衍生品日均成交量；金线（右轴）是同月对'
             ...
             f'<b>这就是全站的口径，不是本图的偏离</b>：CONTRACT.md §6.1 第 1 条把流量'
             f'序列定为单月同比，页上一条 {Y.TTM_WIN} 个月滚动合计的同比都不画。'
             f'柱与线取自同一列 —— 拿这根柱除以 12 根柱之前那根就是线上这一点，'
             f'读者能自己核，不必信我们。'
             f'本图还有两件事跟着这条规矩一起成立：① <b>口径断点看得见</b>：'
             ...
             f'一次性事件与季节性会被放大。'
             f'本图不给两种口径并排的实测对照，理由是这条序列给不出干净的对照侧：'
             f'本页序列自 {mlab(MONTHS[0])} 起共 {len(MONTHS)} 个月，'
             f'这 {W2} 个月的窗口里单月口径能画 {MOM_HAVE} 个月、'
             f'{Y.TTM_WIN} 个月滚动口径只画得出 {TTM_HAVE} 个月，'
             f'其中 {_ttm_after_brk} 个还落在 {mlab(BRK_M)} 的断点之后 —— '
             f'滚动窗口把这次断点一次抹到 {Y.TTM_WIN} 个月上，拿它当对照量到的是断点'
             f'不是口径。'
...
ex8 = {
    ...
    'note': ('色标取全部有限值的 5/95 分位，绿 = 发行量增速更快。'
             ...
             '热力矩阵按 CONTRACT.md §6.3 属<b>图型豁免</b>：每一格本来就是一个月的读数，'
             '与全站那次改口径无关（全站同比一律单月，页上一条滚动合计的同比都不画）——'
             '逐格的月度波动就是这类图的题眼，抹平了这张图就没有内容了。'),
...
_EXEMPT_EX = [(e['n'], _EXEMPT_KINDS[e['kind']]) for e in EXHIBITS
              if e.get('kind') in _EXEMPT_KINDS]
_exempt_txt = '、'.join(f'<b>Exhibit {n}</b> 是{lab}' for n, lab in _EXEMPT_EX)
...
if _a5 and _a5['zero_aligned'] and _a5['left_min'] < 0:
    ex5['note'] += ('左轴量程延到 0 以下是双轴零点对齐的副作用（右轴同比有负值）；'
                    'ADV 本身恒为正，零线下方是空的，所以引擎在那一段不画左轴刻度与网格线。')
...
NOTES = [
    ...
    ('<b>双轴图的零点对齐</b>：本引擎默认把「左右两轴的零点画在同一条水平线上」，'
     ...
     + (f'{_exl(AX_DIP)} 走对齐，左轴下界被压到 0 以下；'
        f'那一段一定是空的，不是数据（ADV 与指数都恒为正），'
        f'所以引擎在零线以下不画左轴刻度与网格线。' if AX_DIP else '')
    ...
    (f'<b>同比口径：本页的同比分两种，别混着读</b>（名单由代码逐图归类算出）：'
     ...
     f'<b>这与全站一致，不是本页的偏离</b> —— CONTRACT.md §6 规定全站同比一律单月'
     f'（§6.1 第 1 条流量、第 2 条存量），页上一条滚动合计的同比都不画。'
     f'<b>本页给不出两种口径并排的实测对照</b>（§6.1 第 3 条要的那笔代价账），'
     f'原因逐图如下，都不是「日均量不能加总」'
     f'（同比是比值，分子分母同权，交易日在比值里直接约掉）：'
     f'(1) {_billed_txt} —— 画的是公司<b>直接披露</b>的同比百分比'
     ...
     f'比出来的是断点不是口径。'
     f'（序列长度已经<b>不是</b>理由：自 {mlab(MONTHS[0])} 起共 {len(MONTHS)} 个月，'
...
def compose_brief(months, adv_raw, advy_raw, biy_raw, derived_raw):
    ...
    远端写这一段时 Exhibit 2 / 3 的标题还没有「单月」标注；本地已在两图标题写明
    「单月同比 / single-month y/y」（§6.6 的判据认标题域里的这两个字），并在页尾
    口径说明里把每张图归到「单月同比」或「季度（3 个月比 3 个月）」两类之一 ——
    那份名单由 `YOY_KIND` 现算，不在散文里写死。
    ⚠ 这里从前写着「偏离默认口径的理由是数据长度」：那句话的两半现在都不成立 ——
    §6 已把**单月定为全站口径**（本页无处偏离），而长度那条理由在 2026-08 的历史
    回填之后本来也失效了。页尾那条说明现在讲的是另一件事：为什么本页拿不出
    §6.1 第 3 条要的两口径实测对照。
    适配照 cboe / ibkr 的先例：brief 里凡引用同比读数，措辞一律带「单月」标签，
    与两图标题、页尾点名逐字对得上；页尾点名的名单里也补上了 brief。
    与 ibkr 不同，本页**没有**滚动口径的读数可并排印 —— 断点与缺面值让对照那一侧
    算不干净（Exhibit 2 图注现算过覆盖度），所以不造一个「单月 / 滚动并排」的假对照；
