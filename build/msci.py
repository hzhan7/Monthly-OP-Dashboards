...
def compose_brief(months, EOP, AVG, brk):
    ...
    ═══ 与同比口径（CONTRACT §6）的关系 ═══
    远端写这一段时页尾还没有「同比口径逐处点名」那条 note；本页**每一处都是点对点
    （单月）同比**，没有一张图用 12 个月滚动口径 —— 这与 2026-09 定下的全站规矩
    （§6：流量走单月同比、存量走点对点同比，页上不画滚动同比）一致，本页无处偏离。
    页尾那条 note 逐条印的是**这个口径的代价**（§6.1 第 3 条），不是选它的理由。
    适配照 schw / ibkr 的先例：
      · s2 引用的同比标「单月」（存量走 §6.1 第 2 条的点对点）—— 本页图上、汇总表、
        核对表全是同一口径，标注不是为了区分两种口径（页上只有一种），
        是让 brief 与全页措辞统一、并满足 §6.6 的判据；
        读者拿它对 Exhibit 2 的金线与汇总表 y/y 列能逐格对上。
      · 「只看环比会误读成趋势反转」说的是**环比的读法**（R2 的本职），不是拿单月
        同比另立一条趋势 —— 对存量序列，点对点同比就是 §6.1 第 2 条定的口径
        （页尾 note ①：滚动均值只在拐点上滞后半年，把窗口拉到 2016 之后实测出的
        反号月份全部落在 2018 / 2020 那几个拐点上 —— 那是代价的量，不是改口径的信号）。
      · 本页核心算术「费收 ≈ 平均 AUM × 有效费率」要求**费收与 AUM 同口径**
        （两者各走各的，Exhibit 10 的缺口会把口径差读成费率压缩）。本段引的只有
...
# brief 说「**这个月**这组读数该怎么读」、每月重写；图注与 notes 说「这一张图这个月
# 该怎么读」（含当月读数、当月现算的落点：几个月落在估计段、断点线画在哪几张图上）；
# 这一块只说「**这些词**是什么意思」，一年到头是同一段。
# ⇒ 这里**不写当月读数**，凡是随月份变的量（最新费率、最新一期 AUM、几个月沿用、
#   窗口有多少个月、隐含 vs 实际的逐季偏差）一律不进来，只指路去现算它的那一处。
# 所以这一块是模块级常量而不是函数：**没有一个数需要现算**。出现的数只有三个，
# 都不随月份变：
#   · 挂钩 ETN 占 AUM &lt;1%  —— MSCI 官方在 AUM 页上对每期数据的同一句限定
#     （同页尾 notes 第 3 条、汇总表表注 "include linked ETNs (<1% of AUM)"）；
#   · ÷ 12                 —— 有效费率是**年化**的，月度桥按月摊即除以 12（恒等式本身）；
#   · 4/1–4/25 与 4/26–4/30 —— 2019-04 那个缝合月两家数据源的分界日，
#     出处是本文件的 BRK_SRC / 页尾 notes 第 4 条（MSCI 供应商切换的口径说明）。
#
# ━━ 为什么是这 11 个词（选词判断）━━
# 判据只有一条：这个词出现在本页的图题 / 序列名 / 纵轴 / 汇总表行头 / 核对表列头 /
# 图注里，而且**不看定义就会读错**。按「读错会出什么事」分四类：
#   ① 这一页画的到底是什么   ETF AUM —— 第三方客户端产品的资产规模，不是 MSCI 的营收。
#      不点破，整页会被当成一张收入图来读，这是本页最贵的一次读错。
#   ② 同一件事的两个口径   月末 AUM / 当月平均 AUM / 月末 − 月均，以及有效费率 vs
#      期末 ETF 基点费率 —— 两条口径都印在页面上（headline 与汇总表都以月末打头，
#      而费基跟的是月均），读串的代价是把费基读反、或把两档费率当成同一个数。
#   ③ 哪些数不是公司给的   Implied（隐含）、asset-based fee、分摊与估计 ——
#      本页的费收桥是自己搭的，且同一条线上前后两段的性质不同（分摊 ≠ 估计）。
#      不点破，读者会拿隐含值当披露值去对账，或把分摊段的精度按估计段打折。
#   ④ 图上那两处「看起来是数据、其实是标记」   未满季（浅蓝柱 + 作废的右轴同比）、
#      缝合月（红色竖虚线的那一格）。不点破，浅蓝柱会被当成断崖、竖虚线会被当成
#      「这一点有问题」而不是「从这一期起与左侧不可比」。
# **有意不收**：m/m、y/y、点对点同比、3Y %ile、pp/bp（全站通用的读图约定，本页
# summary.note 与页尾 notes 第 10 条已经逐条讲过，释义板再讲一遍就是两处各写一份）；
# 「12 个月滚动」（本页一条都不画，页尾 notes 第 10 条讲的是不画它的代价，不是一个
# 本页出现过的口径）；以及 AUM / 资产规模 / 收入这类本页没有特殊口径的常识词。
# ══════════════════════════════════════════════════════════════════════════════
GLOSSARY = [
    # ① 这一页画的到底是什么 —— 与页尾 notes 第 1、3 条同源
    ('ETF AUM',
     '本页画的是<b>第三方</b>挂钩 MSCI 指数的 ETF 的资产规模（客户端产品），'
     '<b>不是 MSCI 自己的营收</b>。它由 MSCI 官方按月披露（每月中旬发上一月），'
     '又直接决定 asset-based fee 收入，所以能当抢在季报之前的高频量用。'
     '所有数字都是 <b>MSCI 的估算值</b>，且<b>含</b>挂钩 ETN（占 AUM &lt;1%）。'),

    # ② 两个口径：月末 / 月均 / 两者之差
    ('月末 AUM',
     '当月最后一天的 AUM <b>快照</b>（month-end）—— 页顶 headline 与汇总表第一行'
     '报的都是它。它是<b>存量</b>：比的是两个时点的资产，不含「今年这个月比去年'
     '多开几天市」这类日历效应。但<b>费不是按它计提的</b>，见下一条。'),
    ('当月平均 AUM',
     '当月的<b>平均</b>资产规模（average AUM for the month），MSCI 与月末值并列'
     '披露的另一列。<b>asset-based fee 按平均资产计提，不是按月末快照</b>，'
     '所以它才是费率相关的那条口径：季度平均图是它按季再取一次平均，'
     '本页的隐含费收桥也建在它上面、而不是建在月末那条线上。'),
    ('月末 − 月均',
     '汇总表的一行、核对表的一列：同一个月的月末快照<b>减</b>当月平均。'
     '这是<b>本页自己相减得到的</b>，不是公司披露的第三条序列。'
     '它在零附近<b>变号</b>，百分比变化无意义，所以这一行的 m/m 与 y/y 报的是'
     '<b>绝对额（$bn）</b>而不是百分比。'),

    # ③ 哪些数不是公司给的：收入项本身 → 费率 → 桥 → 桥上前后两段的性质
    ('asset-based fee',
     'MSCI 的一项收入：按挂钩产品的<b>平均</b>资产规模计提的费用'
     '（核对表列头缩写作 <b>ABF</b>）。公司<b>按季</b>披露实际金额；'
     '本页把它<b>按月</b>拆开画成隐含序列，见下面两条。'),
    ('有效费率',
     '<code>有效费率 = 该季披露的 asset-based fee 收入 ÷ 该季平均 AUM</code>，'
     '年化后以 <b>bp</b> 计，<b>一个季度只有一个值</b>。它是从收入<b>反解</b>出来的，'
     '<b>不是</b>公司直接给的一档报价。费率按季、AUM 按月，两者天然错位，'
     '所以核对表里同属一个季度的那几个月填的是<b>同一个</b>费率值'
     '（季度值下挂到月，<b>不是</b>月度披露）。'),
    ('Implied（隐含）',
     '凡标题带 <b>Implied</b> 的图都<b>不是公司披露值</b>，是本页搭的桥：'
     '<code>月度 asset-based fee = 当月平均 AUM × 有效费率 ÷ 12</code>'
     '（费率是年化的，按月摊故除以 12）。桥搭得准不准不看嘴上说 —— '
     '已收官季度的「隐含 vs 实际披露」逐季列在季度隐含费收那张图的图注里。'),
    ('分摊与估计',
     '隐含费收这条线上有性质不同的两段。最新<b>已披露</b>费率的那个季度<b>及之前</b>，'
     '费率是从实际收入反解的，月度值只是把季度实际值<b>分摊</b>到月，'
     '<b>不是估计</b>；那之后的月份<b>沿用</b>最新已知季度的费率，'
     '<b>那一段才是估计</b>。分界落在哪一季、当前有几个月在估计段，'
     '页尾「费率的期间口径」那条按最新数据现算写明。'),
    # ⚠️ 主语别写反：这一档按定义就是 **ETF 专属**（取自新闻稿「AUM in ETFs Linked to
    # MSCI Equity Indexes」表的 Period-End Basis Point Fee），它不可能「覆盖非 ETF」；
    # 含非 ETF 的是**本页的有效费率**——分子是 Index 分部整条 Asset-based fees 行
    # （fetch/rates_msci.py 的 _parse_index_revenue 锚的就是这一行），分母只有 ETF AUM。
    # 两档口径的差异见 fetch/rates_msci.py 模块 docstring（期末 Run Rate ÷ 期末 AUM，
    # 是时点数，与「实际收到的钱 ÷ 实际平均规模」口径完全不同）。
    ('期末 ETF 基点费率',
     '公司另行披露的另一档费率（period-end basis point fee）：口径是'
     '<b>期末 run rate ÷ 期末 AUM</b>，是个<b>时点</b>数，而且<b>只对 ETF</b>。'
     '本页用的有效费率比它<b>高</b>，因为那是另一套算法 —— 分子是 Index 分部'
     '<b>整条</b> asset-based fee 收入（<b>含</b>非 ETF 的挂钩授权），'
     '分母却只有 ETF 的平均 AUM。两个口径<b>不可互换</b>，'
     '本页的桥自始至终用的是有效费率。'),

    # ④ 图上那两处「看起来是数据、其实是标记」
    ('未满季',
     'AUM 按月来、季度却要走满三个月，所以季度图<b>最右一根柱</b>可能只含该季的'
     '<b>部分月份</b>。这种柱画成<b>浅蓝</b>，且它的右轴同比<b>已作废</b>，'
     '<b>不能</b>与完整季直读。⚠️ 柱高会不会因此变矮，要看那张图怎么把月合成季：'
     '<b>合计</b>型（季度隐含费收 = 当季各月相加）少一个月柱就矮一截；'
     '<b>平均</b>型（季度平均 AUM = 当季各月取平均）柱高<b>不受</b>月数影响 —— '
     '它只是 1–2 个月的平均、噪声大，同样不能与完整季直读。'),
    ('缝合月',
     '专指 <b>2019-04</b> 这一格：MSCI 在 2019 年 4–5 月把数据供应商由 Bloomberg '
     '换成 Refinitiv，而这一格<b>本身就是两家拼出来的</b> —— 月末值已是 Refinitiv，'
     '月均值是 4/1–4/25 Bloomberg 加 4/26–4/30 Refinitiv，2019-05 起才全程 Refinitiv。'
     '页上的红色竖虚线因此画在 2019-04（季度图画在它所在的那一季），语义是'
     '<b>「从这一期起与左侧不可比」</b>，<b>不是</b>「这一格有问题」。'),
]
...
def main():
    ...
    # ── 费率的期间披露（Exhibit 7 / 8 / 9 / 10 与核对表的「有效费率」列共用）──
    ...
    # Exhibit 6 是逐年路径图（x 轴是 Jan..Dec，窗口由「最近 6 年」定义，没有连续时间轴）。
    #
    # 费率派生的四张（Exhibit 7 / 8 / 9 / 10）以前够不到 2016 —— 有效费率序列只回溯到
    # 2019Q1。2026-08 把 fetch/rates_msci.py 的抓取起点从 2020-04 下压到 2016-04 并补上
    # 老版式解析（Table 5 三列收入 + Table 7 老 AUM 表），费率序列现已自 2015Q1 起，
    # 这四张因此和其余各图一样真的从 2016 起。2015 那四季不是多余的：Exhibit 8 / 9 的
    # 同比要往前借 4 个季度、Exhibit 10 的同比要借 12 个月，没有它们 2016 年会是空的。
    ...
    def add_brk(exd, keys, at, cn):
        ...
        （费率派生的图若整段落在断点右侧，同样跳过）。
        src_extra 用**追加**：Exhibit 7 / 8 / 9 自己已经有一段费率期间说明，覆盖会丢。
        """
    ...
    ex.append(add_brk({
        ...
        # 引擎按 Xc(n-4) 定位气泡、Xc(n-2) 定位箭头（charts.js 的 gs_bar 分支，搜 `ex.mom_txt`；
        # **不写行号** —— 上一版写的 charts.js:1286 本轮复核已指偏），那是照短窗口设的：
    ...
    AQW = [q for q in aqk if q >= QWIN0]      # 同 Ex9：2016Q1 起 ∩ 可得 = 全序列
    aq_yoy = []
    for q in AQW:
        j = aqk.index(q)
    ...
    ex.append(add_brk({
        ...
        # 就查这个（CONTRACT.md §6.6 把「单月同比没写进标题」列为 🟡，2026-08-19 之前
        # 本图是全站唯一一条 🟡）。为什么两条线必须**同**口径，写在下面 note 里 ——
        # 注意那不是「为什么用单月」：单月是全站口径（§6），本图无处偏离，也无需辩护。
        ...
        'ylab': '% y/y',
        'series': [
            # NAVY = 本图主角（隐含费收），MBLUE = 平均 AUM —— 与汇总表 / 核对表里
            # 「当月平均 AUM」那一列指的是同一条量，读者不用再去猜哪条是哪条。
        ...
        'note': ('<b>两条线的垂直距离就是有效费率那一项</b>：本页的核心算术是'
                 ...
                 # 这里要说的是另一件事 —— **两条线为什么必须同口径**：中蓝那条的底层是
                 # AUM，存量走 §6.1 第 2 条的点对点；而本图的全部命题是「两条线的垂直距离
                 # = 费率那一项」，要能逐格相减，深蓝那条就必须跟着走同一个口径。
                 # 一边滚动一边点对点，相减出来的东西不对应任何量。
                 '<b>为什么两条线必须是同一个口径</b>：中蓝那条是<b>平均 AUM</b> 的同比，'
                 'AUM 是存量，走的是点对点同比（12 个月末快照相加既不是一年的量、'
                 '也不是平均水平，「滚动合计」对它是个假名字，CONTRACT §6.1 第 2 条）；'
                 '而本图唯一的命题是「两条线的垂直距离 = 费率那一项」，要能逐格相减，'
                 '深蓝那条费收就必须跟着走同一个点对点口径。'
                 '两条线各走各的口径，相减得到的差不对应任何真实存在的量。'
                 '（口径本身不在这里论证 —— <b>全站同比一律单月</b>，本图没有偏离。）'
                 ...
                 # （本轮 -10.9pp vs -8.0%），印错一个读者会拿去对 Exhibit 9 却对不上。
    ...
    # ── 两张已删的图，以及本页图序的两次调整（2026-08）────────────────────────────
    #
    # ① 原「Average AUM since Dec-08」（月均全历史线）已删。
    #    它与 Exhibit 4 画的是同一个量的两种时间口径（月末 vs 月均），在**全历史尺度上
    #    是同一张图**：实测两条 path 的垂直距离中位数 0.74px、p90 3.44px、最大 8.17px，
    #    而绘图区高 230px、线宽本身就 1.6px —— 过半的月份两条线连线宽都拉不开
    #    （相关系数 0.9993，18 年 21.4x vs 22.0x）。留月末不留月均：本页所有长视角图与
    #    headline、汇总表都以月末打头，且月均恰是被 2019-04 缝合污染更重的一条；
    #    费基的长视角由 Exhibit 5 的季度平均承担。也没有合并成一张双线全历史图 ——
    #    按上面的像素数，画出来会是一条线却声称两条，比删掉更糟。
    #
    # ② 原「Month-end vs. average AUM」（月末与月均两条线，25 个月窗口时代的图）已删。
    #    **删掉它等于本页不再有月末/月均口径背离的图形**，这一层现在只在三个地方：
    #    汇总表的「月末 − 月均（$bn）」一行、核对表同名列、页顶 brief 的第三句
    #    （那句会在两者环比反向时点名）。页尾 notes 第 2 条已改成指向这三处 ——
    #    图没了但话不能没了，否则读者会以为本页只有月末一条口径。
    #    起因：窗口拉到 2016 之后两条线在多数月份贴在一起（差额中位数只占月末值的 1.7%），
    #    一张 127 个月的图上看不出那个差，真要读差得回表里读。
    #
    # ③ 图序两次调整（阅读路径而非图号本身）：逐年路径图提到第 6 位（紧跟季度平均 AUM，
    #    先把「量」讲完再进「量→收入桥」）；隐含 ABF（季）提到有效费率**前面**，于是
    #    桥的顺序变成 月度隐含 → 季度隐含 → 有效费率 → 费收同比，先给结果再给驱动项。
    #    图号按新的阅读顺序重排（GS deck 的图号就是阅读顺序，不留空号）。
    ...
    # ── 「哪几张是季度图」现算 ────────────────────────────────────────────
    # 页尾同比口径那条要点名季度图，而季度与月度在本页的唯一机器判据就是 x 轴刻度步长：
    # 季度图按 QSTEP 每年标一次 Q1、月度图按 MSTEP 每年标一次 1 月。
    # 手写图号的话，加一张季度图就多一句假话，而没有任何东西会报错。
    if MSTEP == QSTEP:
        raise SystemExit('MSTEP 与 QSTEP 相等，页尾就分不出哪几张是季度图了')
    QTR_EX = [e['n'] for e in ex if e.get('xstep') == QSTEP]
    _QTR_EX_TXT = ' / '.join(str(n) for n in QTR_EX)

    # ── 2026-08 回补当轮那一次「与官网季度均值对账」的**固定**范围 ──────────────
    # 这一对季度端点记的是一件**已经发生过的事**，所以写死；页上引用时必须连范围一起印。
    # 反面教材：上一版只写「46 季全部落在 ±0.35% 内」，而同一句里 len(qs) 也是 46 ——
    # 两个数看着一致，实则一个是历史记录、一个随季度增长，下个季度就会互相打架。
    # ⚠️ 季数**不写死**，从两个端点现算：写死的季数与端点一旦对不上，
    #    页上那句「X–Y 共 N 季」就是句没人会发现的假话，所以这里算完当场自验。
    _XCHK_Q0, _XCHK_Q1 = '2015Q1', '2026Q2'
    _XCHK_QN = qi(_XCHK_Q1) - qi(_XCHK_Q0) + 1
    if _XCHK_QN <= 0 or _XCHK_Q0 not in qs or _XCHK_Q1 not in qs:
        raise SystemExit(f'对账范围 {_XCHK_Q0}–{_XCHK_Q1} 不在本页的费率季度序列里'
                         f'（{qs[0]}–{qs[-1]}），页上那句会指向不存在的季度')

    # ── 同比口径盘点：本页全部用点对点同比（§6.1 第 2 条），代价要拿本页自己的序列实测 ──
    # ⚠️ 不许写「存量不能做滚动」这种一般性说辞：12 个月滚动**均值**同比对存量在数值上
    # 完全正确（Σ12/Σ12′ ≡ 均值比），不能说的只是把它叫「合计」（12 个月末快照相加不指代
    # 任何东西）。所以这里真的把两种口径都算出来、对齐月份后比一遍 —— 但那是为了
    # **印代价**（§6.1 第 3 条），不是为了「再决定用哪个」：口径由 §6 定死，页上只画单月，
    # 滚动那一侧只以数字出现。
    def _cal(seq_map, keys, kind, win):
        ...
        **流量列（kind=FLOW）整段交给 `Y.caliber_diff()`**：CONTRACT §6.4 白纸黑字
        「`yoy.caliber_diff()` 已经先取交集再比，别自己写」。本页不用 pandas，序列是
        按 allk 排好的一个 list，caliber_diff 拿到的因此是 0..n−1 的位置索引 ——
        窗口按位置传进去、月份按位置映回来（那一支里的 `allk[i]`）。

        **存量列（kind=STOCK）只能在这里自己算**：caliber_diff 对非 FLOW 的 kind 不做
        两口径对比（存量做滚动**合计**非法，它的滚动侧一律回 None），而本页要的对照
        恰恰是 12 个月滚动**均值**同比 —— 共享模块里没有这一对。估计量取样本标准差
        （`ddof=1`），与 `build/yoy.py::_std()` 的 `np.nanstd(x, ddof=1)` 是同一个写法，
        所以本页两处「逐月标准差」和全站其余各页是同一个定义。
        """
        allk = sorted(seq_map)
        s = [seq_map[k] for k in allk]
        if kind == Y.FLOW:
            pos = {k: i for i, k in enumerate(allk)}
            d = Y.caliber_diff(s, Y.FLOW, win=[pos[k] for k in win if k in pos])
            return {'n': d['n'], 'sd_mom': d['std_mom'], 'sd_ttm': d['std_ttm'],
                    'mj_mom': d['maxjump_mom'][0] if d['maxjump_mom'] else float('nan'),
                    'mj_ttm': d['maxjump_ttm'][0] if d['maxjump_ttm'] else float('nan'),
                    'opp': [(allk[i], float(x), float(y)) for i, x, y in d['opposite']]}
        # 走到这里的只剩存量：FLOW 已经在上面提前返回，比率没有合法的对照口径
        # （Y.ttm_mean_yoy 对 RATIO 直接抛 CaliberError，那正是该有的行为）。
        a = Y.mom_yoy(s, kind).values.astype(float)
        b = Y.ttm_mean_yoy(s, kind).values.astype(float)
    ...
    notes = [
        ...
        f'{mlab(WM[0])} 以来 {len(WM)} 个月里，差额中位数只占月末值的 '
        f'{sorted(abs(EOP[k] - AVG[k]) / EOP[k] for k in WM)[len(WM) // 2] * 100:.1f}%，'
        '画成一张十年期的线图两条线基本贴在一起，真要读那个差还是得回表里读。',
        ...
        '新补的季度逐季与 MSCI 官网月度 AUM 页的季度均值对过账 —— '
        f'<b>核过的范围是回补当轮的 {_XCHK_Q0}–{_XCHK_Q1} 共 {_XCHK_QN} 季</b>'
        '（全部落在 ±0.35% 内，典型 ±0.05%）。'
        + ('⚠ 费率序列此后又长了 %d 季，那几季不在这次对账里 —— 这里写死范围而不是'
           '写「全部 N 季」，就是为了不让这句话随序列变长自己变成假话。'
           % (len(qs) - _XCHK_QN) if len(qs) > _XCHK_QN else '')
        + '既有季度的数值一格未动。',
        ...
        (f'<b>同比口径：本页每一处都是点对点同比</b>（月度图是当月对去年同月，'
         ...
         f'流量按 §6.1 第 1 条走单月同比、存量按第 2 条走点对点同比，'
         f'页上一条 {Y.TTM_WIN} 个月滚动合计的同比都不画。'
         f'下面逐条印的<b>是这个口径的代价，不是选它的理由</b>（§6.1 第 3 条：'
         f'拿本页自己这几条序列实测，对照的 {Y.TTM_WIN} 个月滚动只以数字出现、页上不画线）。'
         f'顺带更正一句本站从前常见的说辞：<b>「存量不能做滚动」这句话本身是错的</b>'
         ...
         + (f'放大倍数确实到了全站流量序列的中位水平（2.08 倍），'
            ...
            f'滚动均值在拐点上滞后半年、于是与点对点反号 —— 这几个月正是这笔代价'
            f'最该被读者看见的地方；')
         ...
         + ('单月口径确实更吵，但窗口内没有一个月方向相反 —— '
            '这笔账照实印在这里，供读者自己折价读那条线。<br>'
    ...
    payload = {
        ...
        # 上方的注释块里（判据：本页图题 / 序列名 / 纵轴 / 行头 / 列头 / 图注里出现过，
        # 且不看定义就会读错）。与 brief 的分工：那一段每月重写，这一块一年到头不动。
        'glossary': gloss.render(GLOSSARY, where='msci glossary'),
