...
_NOTE_TTM_CASH = (
    ...
    '产线彼此独立。它同时画在⟨ex:lines-turnover_xetra_eurbn@<:上面⟩「Xetra 电子盘成交额」那张折线里 —— '
    + ((f'与 Xetra 合计只差 <b>{_LEDMED:.1f}%</b>（{_LEDN} 个可比月的中位，最大 {_LEDMAX:.1f}%），'
       ...
       + '两条线几乎重合是对的，不是同一列画了两遍：那道缝正是法兰克福场内，'
         '⟨ex:lines-turnover_fwb_eurbn@lines-turnover_xetra_eurbn+1:紧随其后的那张⟩'
         '「法兰克福场内成交额」就是把这道缝单独放大来画。')
...
# ══════════════════════════════════════════════════════════════════════════════
# 名词释义（页顶「名词释义」板块，排在所有 exhibit 之前）
# ══════════════════════════════════════════════════════════════════════════════
# 与 notes / brief 的分工：brief 说「**这个月**这组读数怎么读」、每月重写；
# notes 讲「这一页是怎么画出来的」；glossary 只说「**这些词**是什么意思」，一年到头
# 同一段。⇒ 这里**不写当月读数**，出现的数只有两类：把定义钉住的结构性量，与恒等式
# 本身；且一律取本文件已有的现算变量（`_LEDMED` / `_RATE*` / `_EQ*`），
# 与图注同源 —— 释义与图注打架是这块板最容易犯的错，同源就打不起来。
#
# ── 为什么是这 16 个词 ────────────────────────────────────────────────────────
# 本页 43 张图、三个官方源缝在一起，读错的入口集中在四类，逐类各取几个：
#   ① 缩写与行话：ADV、未平仓（OI）、ICSD/CSD/IFS、GSF、EurexOTC Clear、360T、EEX、
#      产品代码（FGBL/FESX/ODAX…）—— 图题与汇总表行头里到处是它们，不看定义读不动。
#   ② 同一个词在本页有两套口径：台账口径 / 工作簿口径（永不互校）、股息衍生品
#      （Eurex 单列 vs 台账摊入股指与单股）、Xetra ≠ 现货全部、授权指数衍生品
#      （不是 Eurex 自己的成交）—— 这几条是「把两套数相加 / 互相验算」的唯一防线。
#   ③ 加不加得起来：存量 / 流量（本页 22 列是存量，单独成图）。
#   ③' 只在标签里出现、页面从不解释的口径限定语：现金余额那一列的「（含受制裁冻结
#      账户）」在页上出现 8 次、全是标签，读者会把它当客户活跃度读 —— 收进来一条。
#   ④ 页面自己的用语：慢腿 / 快腿（决定「—」是缺数还是正常）、单边计（跨家比的前提）。
# 刻意**不收**：m/m、y/y、3Y %ile、pp/bp —— 全站通用的读图约定，summary.note 已逐条
# 讲过；也不收「成交量」「市值」这类本页没有特定口径的常识词。
_GLOSSARY = [
    ('ADV',
     '日均成交量（average daily volume）：<b>当月合计 ÷ 当月交易日数</b>。'
     '但本页 Eurex 那一批 ADV <b>不是</b>本仓算出来的商 —— 是官方月度工作簿自己发的 '
     '"Daily average" 字段（<code>adv_eurex_*</code>），本页照抄。'
     '⚠ 现货侧（Xetra / 法兰克福）本页<b>只有月度总额、没有 ADV</b>：'
     '算现货 ADV 要拿 <code>trading_days_cash</code> 当除数，而那一列是慢腿，'
     '除完整条线就跟着变成慢腿。'
     # ⚠ 这里原本写「衍生品的量一律以合约张数计」，是个假的全称量词：本页 EEX 三条
     #   能源列（*_mwh）按 MWh 计、EurexOTC 两条按名义 EUR bn 计，它们都是衍生品。
     #   页尾「合约张数类列全在衍生品侧」那句是按列名后缀 _contracts 做的普查
     #   （_column_census()：money=turnover_*/_eurbn/_eurmn，contracts=*_contracts），
     #   *_mwh 两个桶都不在 —— 所以量词只能限到那两套「合约张数」台账上。
     '本页 Eurex 工作簿与集团台账两套<b>衍生品成交量</b>都以合约<b>张数</b>计'
     '（EEX 三条能源列按 MWh、EurexOTC 两条按名义 EUR bn，见它们各自的条目），'
     '本页没有与之配对的名义金额列，'
     '张数换不成金额（页尾「量价分解」那条讲的就是这件事）。'),

    ('未平仓（OI）',
     '月末仍未了结的合约张数（open interest）。它是<b>存量</b> —— 某一天的截面，'
     '不是当月发生的量：与 ADV、当月成交量这类流量<b>不能相加</b>，'
     '也不能把 12 个月末的数加起来当年度量（那个和不指代任何真实的量，'
     '所以存量没有「滚动合计」口径可选，同比只能是月末对月末的点对点比）。'
     '本页每一条未平仓列都单独成图，不与流量共轴。'),

    ('存量 / 流量',
     '本页每一列非此即彼：<b>流量</b>是一段时间里累计发生的量（日均或当月合计），'
     '<b>存量</b>是某一天的截面（各条未平仓、托管资产、担保品在外量、现金余额、'
     'OTC 名义未平仓）。两者不能相加；跨币种换算时流量配月均汇率、存量配月末汇率'
     '（本页是 EUR 本币页，换算不在这里做）。'
     '⚠ 标成存量的那几条 Clearstream / EurexOTC 列，官方原文其实是<b>月内平均值</b>'
     '（average value / average outstandings）而不是期末时点 —— '
     '这处冲突记在页尾，换算前必须先解决。'),

    ('慢腿 / 快腿',
     '本页缝了三个官方源、两种发布节奏：Eurex 与 FWB 两份工作簿次月头几天就出'
     '（<b>快腿</b>），集团 IR 台账要等次月约第 10 天（<b>慢腿</b>）。'
     '慢腿列<b>不参与</b>本页数据月的门槛判定 —— 否则一条天生晚发的腿会把整页'
     '拖住整整一个月。所以慢腿列最新月留空是<b>正常状态</b>：'
     '图画到自己序列的最新月为止，汇总表那一格显示「—」，'
     '<b>不是 0，也不是抓取失败</b>。'),

    ('台账口径 / 工作簿口径',
     '同一件事本页有<b>两套并行口径，永不互校</b>：<code>vol_fd_*</code> 出自集团 IR '
     '台账（当月成交张数，含 ETC / 农产品 / 贵金属，官方脚注自己写明「总数不等于'
     '分项之和」）；<code>adv_eurex_*</code> / <code>oi_eurex_*</code> 出自 Eurex '
     '月度工作簿。'
     # 两处实测都取本文件已在图注里用的那两个现算变量，分母每月长 1，不写死。
     + ((f'实测：台账利率与 Eurex 利率组小计 {_RATEN} 个月里 <b>{_RATEDIFF}</b> 个不等；'
         if _RATEN else '实测两侧逐月对不上；')
        + (f'按官方脚注把股息衍生品摊回股指+单股之后，{_EQN} 个月里 <b>{_EQDIFF}</b> '
           f'个仍然不等。' if _EQN else ''))
     + '⇒ 两套<b>不要画进同一条线</b>，也不要拿一套去验算另一套。'),

    ('股息衍生品',
     'Eurex 官方大组之一（Dividend Derivatives，2008-06 起才有这一组）。'
     '本页写「股指衍生品（<b>不含</b>股息）」「单股衍生品（<b>不含</b>股息）」，'
     '正是因为 Eurex 把它单列；而集团台账口径反过来 —— 按官方脚注把股息'
     '<b>摊进</b>股指与单股两项。⇒ 同名的「股指 / 单股」在两套口径下<b>外延不同</b>，'
     '看到这两个词先认它归在哪一套。'),

    ('单边计',
     '成交额按<b>单边</b>计（single-counted）：一笔成交的买卖两方只算一次。'
     'FWB 工作簿的 "Explanation Report" 表与 IR 台账 PDF 的现货行脚注都写明了。'
     '本页现货各图与汇总表里的 EUR bn 全是这个口径 —— <b>跨家比要当心</b>，'
     '别的交易所报的可能是双边（如 HKEX 的南向 ADT）。'),

    ('Xetra / 法兰克福场内',
     '德国现货的两个场所：<b>Xetra</b>（XETR）是电子盘，<b>法兰克福场内</b>（XFRA）'
     '是交易所场内。⚠ <b>Xetra 不等于德意志交易所现货的全部</b>：'
     '集团台账的现货合计 = 两者之和，'
     + ((f'实测两条线只差 <b>{_LEDMED:.1f}%</b>（{_LEDN} 个可比月的中位）'
         if _LEDMED is not None else '实测两条线只差百分之几')
        + '，那道缝正是法兰克福场内。')
     + '两条场所线出自 FWB 现货工作簿、合计那条出自集团 IR 台账，是<b>两条独立产线</b>，'
       '逐月能对上是回填时闭合检验的结论，不是定义上的恒等。'),

    ('ICSD / CSD / IFS',
     'Clearstream 的三套口径，官方把它们混在同一张表里：「结算笔数」<b>只含 ICSD</b>'
     '（国际中央证券存管），<b>不含</b>德国本土 CSD 的那部分（官方稿里 CSD 的笔数'
     '约是 ICSD 的 2 倍，页尾有两期实证）；而同一份文件的「托管资产（AuC）」'
     '却是 <b>ICSD + CSD 合并</b>数 —— 同表两行两个口径。'
     '<b>IFS</b> 是官方在 FUND SERVICES 分部下单列的基金服务'
     '（Assets under custody IFS / Settlement transactions IFS），'
     '与前两者是<b>另一套数</b>，不要相加。'),

    ('GSF 担保品',
     'Global Securities Financing —— Clearstream 的担保品管理业务。'
     '本页那一条是担保品的<b>在外量</b>（官方列名 "Collateral management '
     '(average outstandings in €bn)"）：某个时点还在被占用的担保品规模，'
     '<b>不是</b>当月新增的量；而且是<b>月内平均</b>，不是月末时点。'),

    # 本页唯一一条「限定语出现了 8 次、却没有任何一处解释它」的列：
    # 「（含受制裁冻结账户）」在汇总表行头、组标题、图题、页尾慢腿/存量清单里反复出现，
    # 全是标签。出处是 fetch/db1.py 的列口径表（cash_balances_eurmn，kind=stock_avg）。
    ('现金余额（含冻结）',
     '汇总表行头与图题里的「日均现金余额（含受制裁冻结账户）」那一列'
     '（<code>cash_balances_eurmn</code>，单位 EUR mn）：Clearstream 账上持有的现金余额，'
     '本身就是<b>月内日均</b>而不是月末时点 —— 本页把它标成存量，'
     '与上面「存量 / 流量」那条记的口径冲突是同一件事。'
     '⚠ 括号里那句限定语是<b>口径的一部分</b>，不是注解：这一列'
     '<b>含被制裁冻结的账户</b>（出处是本仓 <code>fetch/db1.py</code> 的列口径表），'
     '即水平值里有一截与当期经营活动无关 ⇒ 这条线<b>不能</b>直接当业务活跃度读。'),

    ('EurexOTC Clear',
     'Eurex 的场外衍生品清算服务。本页两列单位都是 EUR bn，但一存一流、'
     '<b>不可比</b>：「名义未平仓」是存量（官方口径为<b>月内平均值</b>），'
     '「当月清算名义量（含压缩）」是当月流量 —— 官方列名写作 "Notional cleared '
     'volumes (<b>incl. compr.</b>)"，即压缩（compression，把互相抵消的存续头寸'
     '批量注销）产生的名义额也计在这一列里。'),

    ('授权指数衍生品',
     '官方列名 "Exchange licenses Index derivatives (traded contracts)"，'
     '列在 <b>Investment Management Solutions</b>（指数与投资管理）分部下：'
     '按<b>指数授权</b>口径统计的指数衍生品成交张数，与 Eurex 自己那套 ADV / OI '
     '是两回事，<b>不要</b>与本页 Eurex 各组相加。'
     '同一分部下的存量对应物是「挂钩 STOXX/DAX 的 ETF 资产」—— '
     '那一列的时点口径<b>官方未言明</b>，本页按存量标注只是推断（见页尾）。'),

    ('360T',
     '集团的外汇交易平台；本页那一条是它的日均名义额'
     '（EUR bn/day，官方列名 "360T (ADV) FX market"）。'
     '⚠ 自 <b>2018-07 起这一列含 GTX</b>，跨那个月的同比里有一截是并进来的量、'
     '不是纯内生增长。这是<b>列级</b>口径变化而不是全页断点，所以页面上没有画红竖线。'),

    ('EEX 电力 / 天然气',
     '官方台账 TRADING &amp; CLEARING 分部下的三行能源成交量'
     '（电力现货 / 电力衍生品 / 天然气）。⚠ 单位是 <b>MWh 不是 TWh</b>：'
     '官方工作簿表头写的 "(in TWh)" 是笔误，差 10<sup>6</sup>'
     '（页尾有与官方 PDF 的逐格互证）。本页<b>照原值画、不做换算</b>，'
     '列名带 <code>_mwh</code> 就是这个原因。'),

    ('产品代码（FGBL 等）',
     '图题括号里那些四字母是 Eurex 的产品代码，标出来是为了与官方工作簿逐行对账：'
     'FGBL / FGBM / FGBS = 10 / 5 / 2 年德债期货，FBTP（意大利）、FOAT（法国）、'
     'FEU3（3 个月 EURIBOR）。同一标的的期货与期权只差首字母 —— '
     '<b>F 开头是期货、O 开头是期权</b>（FESX / OESX 是 EURO STOXX 50 的期货与期权，'
     'FDAX / ODAX 是 DAX 的），FVS 是 VSTOXX 期货。'),
]
...
SPEC = {
    ...
    # 名词释义：排在所有 exhibit 之前。选词的判断写在 _GLOSSARY 上方的注释里
    #（为什么是那 16 个词、为什么不收 m/m 与 y/y 这类全站读图约定）。
    'glossary': _GLOSSARY,
