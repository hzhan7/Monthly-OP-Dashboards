...
在台股不是问题。这一页的全部难点在**哪一段月度数据可以当同一个东西读**，逐条实测：
  ...
  · **2015-06 太阳能/LED「新事业」分部退出合并也是真断点**，同样登记进 `breaks`。
    月营收公告是**合并数**，里面一直含着新事业分部；20-F 分部附注（对外营收，NT$ 千元）：
    2013 7,030,171 → 2014 10,563,149 → 2015 3,685,289 → 2016 425,859。
    可指认的公司行为是 TOPCELL（太阳能电池）与茂迪 Motech 换股合并，**合并基准日
    2015-06-01**，UMC 当日除列 TOPCELL 的资产负债（FY2015 20-F 附注 25）。后果落在同比上：
    2015 合并同比 +3.4% 而晶圆代工分部 +9.0%（129,448,927 → 141,145,132，FY2015 20-F），
    2016 合并 +2.1% 而晶圆代工 +4.1%（141,705,196 → 147,444,265，FY2016 20-F）。
    也就是说 2015 有约 5.6pp、2016 有约 2.0pp 的「同比拖累」来自口径缩减而不是代工景气。
...
# 太阳能/LED「新事业」分部退出合并。可指认的公司行为：TOPCELL 与茂迪 Motech 换股合并，
# 合并基准日 2015-06-01，UMC 当日除列 TOPCELL 资产负债（FY2015 20-F 附注 25）。
_NEWBIZ_BREAK_MONTH = '2015-06'
# 新事业分部**对外**营收（NT$ 千元），取自各年 20-F 分部附注。外部对照常量。
_NEWBIZ_FY = {
    2013: 7_030_171, 2014: 10_563_149, 2015: 3_685_289, 2016: 425_859,
}
# 晶圆代工分部对外营收的同比对子（年, 上年, 当年, 出处）。必须取**同一份 20-F** 内的
# 两栏才可比：FY2016 20-F 把 2015 的分部划分重述过（wafer 141,145,132 → 141,705,196），
# 跨年报混用会算出一个不存在的同比。
_WAFER_YOY_PAIRS = (
    (2015, 129_448_927, 141_145_132, 'FY2015 20-F'),
    (2016, 141_705_196, 147_444_265, 'FY2016 20-F'),
)
...
# ── ③b 太阳能/LED「新事业」分部退出合并（2015-06）────────────────────────────
def _cons_yoy(s, year):
    """本表口径的整年同比（%）。两年月份不齐返回 None。"""
    cur = [f'{year}-{m:02d}' for m in range(1, 13)]
    pre = [f'{year - 1}-{m:02d}' for m in range(1, 13)]
    if not all(m in s for m in cur + pre):
        return None
    return (sum(s[m] for m in cur) / sum(s[m] for m in pre) - 1.0) * 100.0


_NB_ROWS = []
for _y, _w0, _w1, _src in _WAFER_YOY_PAIRS:
    _c = _cons_yoy(_S, _y)
    if _c is None:
        _NB_ROWS = []
        break
...
if _NB_ROWS:
    _NEWBIZ_NOTE = (
        f'<b>{_NEWBIZ_BREAK_MONTH} 起太阳能/LED「新事业」分部退出合并，'
        f'2015–2016 的同比里有一块口径缩减。</b>'
        f'月营收公告是<b>合并数</b>，里面一直含着与晶圆代工无关的新事业分部 —— '
        f'20-F 分部附注的对外营收（NT$ 千元）是 '
        + '、'.join(f'{y} {v:,}' for y, v in sorted(_NEWBIZ_FY.items()))
        + f'。可指认的公司行为是 TOPCELL（太阳能电池）与茂迪（Motech）换股合并，'
          f'<b>合并基准日 2015-06-01</b>，UMC 当日除列 TOPCELL 的资产负债'
          f'（FY2015 20-F 附注 25「Deconsolidation of Subsidiaries」）；'
          f'其余部分是 2015–2016 年太阳能/LED 业务本身的收缩。后果落在同比上：'
        + '；'.join(
            f'{y} 年本表合并同比 <b>{c:+.1f}%</b>，而同期晶圆代工分部 '
            f'<b>{w:+.1f}%</b>（{src}），差 {abs(c - w):.1f}pp'
            for y, c, w, src in _NB_ROWS)
        + '。数据本身没有问题（合并数就是这么多），不可比的是这两年的同比 —— '
          f'图上 {_NEWBIZ_BREAK_MONTH} 的红色竖虚线与 {_BREAK_MONTH} 那条同义：'
          '「从这一期起与左侧不可比」。')
else:
    _NEWBIZ_NOTE = (
        f'<b>{_NEWBIZ_BREAK_MONTH} 起太阳能/LED「新事业」分部退出合并，'
        f'2015–2016 的同比里有一块口径缩减。</b>'
        'TOPCELL（太阳能电池）与茂迪 Motech 换股合并，合并基准日 2015-06-01，'
        'UMC 当日除列其资产负债（FY2015 20-F 附注 25）；新事业分部对外营收自 2014 年的 '
        'NT$10,563,149 千元降到 2016 年的 NT$425,859 千元。'
        '数据本身没有问题，不可比的是那两年的同比。')
...
SPEC = {
    ...
    # 断点写成字面量而不是指向 series/umc_breaks.csv：本家只有两条断点，
    # 每条都由一次公司行为唯一确定（2015-06-01 TOPCELL 除列 / 2019-10-01 USJC 交割），
    # 不会跟着数据长。
    'breaks': [{'month': _NEWBIZ_BREAK_MONTH,
                'zh': '太阳能/LED「新事业」分部退出合并（TOPCELL 2015-06-01 并入茂迪除列），'
                      '2015/2016 的同比含口径缩减'},
               {'month': _BREAK_MONTH,
                'zh': 'USJC（原三重富士通半导体 MIFS）100% 并表，'
                      '2019-10~2020-09 的同比含无机增量'}],

    'notes': [_ADDITIVE_NOTE, _START_NOTE, _NEWBIZ_NOTE, _BREAK_NOTE,
