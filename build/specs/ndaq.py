...
def _from_zh(m0, alt=''):
    """「YYYY-MM 起」；算不出就整半句不写（同上面 _span_zh 的规矩）。

    ⚠ 定义放在这里而不是释义那一节（2026-09-16 挪上来的）：`_NOTE_VS_PREV`
    在释义之前就要用它，留在原处会在 import 期 NameError、整批构建炸掉。
    """
    return f'{m0} 起' if m0 else alt
...
def _mlab(m):
    """'2026-08' → 'Aug-26'（与 build/single.py 的 mlab 同款）。

    这半句话指的是**页面上印着的那两个月份标签**（核对表末行、图的横轴末端），
    读者会拿它去对表，所以必须用页面上的写法，不能用本文件其它地方那种
    「2010-10 起 N 个月」的 YYYY-MM —— 那些指的是数据覆盖，不是屏幕上的字。
    """
    try:
        y, mm = m.split('-')
        return f'{_MONS[int(mm) - 1]}-{y[2:]}'
    except (ValueError, IndexError, AttributeError):
        return m


_A_COLS = ('vol_us_options_mmcontracts', 'vol_nordic_derivs_mmcontracts',
           'vol_us_cash_matched_mnsh', 'vol_nordic_cash_value_usdbn')


def _fast_leg_zh():
    """A 组（快腿）比 B 组（头条）多走的那一个月 —— 那半句话，**现算**。

    2026-09 之前这里是一句写死的「在末尾核对表里会多出一行、其余列显示「—」」。
    那句话只说对了一半：底座的 `ex_single` 让**每一张图各自画到自己那一列的最新月**，
    所以 A 组的图同样会多出一根柱 —— 2026-08 这个月就是四张通栏图画到 Aug-26、
    而汇总表（Exhibit 1）与头条仍锁在 Jul-26。读者看到同一个指标在卡片上是 402、
    在图上最后一根柱是 363，只被告知了核对表那一处，找不着另一处的解释。

    ⚠ 这一版有三处是上一版（2026-09-05 当天）写错、复审逮回来的，别再写回去：
      · **条数要现算**，不许写死「四条」。四列各自有各自的末月，掉队一列这句话就假了。
      · 不许说那一个月「在页面上出现两次」。核对表一行 + 每列一根柱 ⇒ 至少五处，
        再加四张图的图注各自把它写成一遍散文。说「两次」是数错了。
      · **不许说「两处各自标着自己的月份」。** 图的 x 轴刻度是抽样打的
        （`build/mrwin.py` 的 MAX_XLABS=20 ⇒ 128 个点 xstep=7），索引 127 正好被跳过：
        四张图上最后一个**刻度**写的都是 Jul-26，压在倒数第二根柱下面，多出来那根柱是裸的。
        月份只出现在图注里，不在轴上。
    """
    a_last = sorted({m for m in (_span(c)[1] for c in _A_COLS) if m})
    if not a_last or not _B1:
        return '在末尾核对表里会多出一行、其余列显示「—」'
    top = a_last[-1]
    if top <= _B1:                         # 慢腿追平了，这个月没有多出来的一格
        return f'本月两组都到 {_mlab(_B1)}，没有多出来的月份'
    n_ahead = sum(1 for m in (_span(c)[1] for c in _A_COLS) if m and m > _B1)
    # 四列齐步走 ⇒「A 组 4 条序列各自的图」；有掉队的 ⇒ 只说领先的那几条，别多报。
    who = (f'A 组 {n_ahead} 条序列各自的图' if n_ahead == len(_A_COLS)
           else f'A 组 {len(_A_COLS)} 列里领先的那 {n_ahead} 条，它们的图')
    return (f'本月 A 组已到 <b>{_mlab(top)}</b>、B 组止于 <b>{_mlab(_B1)}</b>，'
            f'于是 {_mlab(top)} 那一个月散在页面上好几处：末尾核对表多出一行'
            f'（该行里 B 组那几列显示「—」），{who}各多画一根柱，'
            f'那根柱的读数写在图注里。'
            f'（图的 x 轴刻度是抽样打的，多出来那根柱底下通常没有刻度字，'
            f'所以别拿轴上的月份去认它。）'
            f'汇总表（Exhibit 1）与头条<b>不</b>跟过去 —— 它们整行锁在数据月 '
            f'{_mlab(_B1)}，所以同一个指标在卡片上与在图上最后一根柱会差一个月')
...
# ── 「美股 matched 成交股数」那一组的组名（拼接列那张 gs_bar）──────────────────
# 提成常量的唯一理由：2026-09-16 起 `level_yoy` 用 `after_group` 锚在这一组上
# （页面所有者「ex14 插到 ex7 后面」）。两处各写一份字面量的话，哪天改了组标题措辞、
# 锚点就对不上 —— 底座会当场 SpecError（不会静默掉回页尾），但那是一次本来不必发生的
# 构建失败。共用一个常量，改措辞时锚点自动跟着走。
_GZ_MNSH = (('美股 matched 成交股数（%s 起 IR 原值、更早为 nasdaqtrader 三盘口和；'
             '当月总量，次轴：单月同比）' % _SPLICE_AT) if _SPLICE_AT else
            '美股 matched 成交股数（当月总量，次轴：单月同比）')
...
# ── 这张图与它<b>前一张</b>（三盘口合计成交股数）差在哪 —— 逐个数现算 ─────────────
# 2026-09-16 起两张图在页面上紧挨着（页面所有者「ex14 插到 ex7 后面，并给说明这两个
# 图的差异」）。挨着之后差异必须写出来：两条线的形状、窗口、次轴口径完全一样，
# 显示出来的读数还恰好同量级（前一张按「百万股 ÷ 1,000」= 十亿股，本张直接按十亿股），
# 差的只是分子里多不多 NTX 与 PSX 两个尾部盘口 —— 不说破，读者会以为其中一张是重复的。
def _venue_gap():
    """(近 12 个月差幅 min%, max%, 末月标签, 末月差幅%, 末月差额, 末月两条读数) —— 单位一律十亿股/月。

    差幅 = 三盘口合计 ÷ 单盘口 − 1。两列在 CSV 里量纲不同（前者百万股/月、后者裸股数/月），
    所以先各自换算到十亿股/月再比 —— 这一步做在这里而不是图注里，免得页面上出现一个
    没有来历的倍数。算不出返回 None。
    """
    rs = []
    for r in _rows():
        a = _num(r, 'vol_us_cash_matched_mnsh')       # 百万股/月（三盘口合计）
        b = _num(r, 'vol_us_cash_matched_nasdaq_sh')  # 裸股数/月（单盘口）
        if a is None or b is None or b <= 0:
            continue
        a_bn, b_bn = a / 1e3, b / 1e9
        rs.append((r['month'], (a_bn / b_bn - 1.0) * 100.0, a_bn - b_bn, a_bn, b_bn))
    if len(rs) < 12:
        return None
    tail = rs[-12:]
    m, pct, gap, a_bn, b_bn = tail[-1]
    return (min(t[1] for t in tail), max(t[1] for t in tail),
            _mlab(m), pct, gap, a_bn, b_bn)
...
_NOTE_VS_PREV = (
    ...
    + (f'<b>柱高更是几乎分不出来</b>：两条序列在 CSV 里量纲不同（前一张百万股/月、'
       ...
       f'正是 NTX + PSX 那两个尾部盘口）；近 12 个月的差幅一直在 '
       f'{_GAP[0]:.1f}%–{_GAP[1]:.1f}% 之间。'
       if _GAP else
...
_NOTE_TTM = (
    ...
    + '本图用的这一列没有接缝，所以不受影响 —— 这正是留在它上面的价值。'
      ...
      '<b>为什么这一列不在「美股 matched 市占率（nasdaqtrader 口径，本页脊梁）」'
      '那一组里。</b>它的单位是裸股数/月、量级 10¹¹，'
      '唯一能读的显示方式是 ×1e-9 换成「十亿股/月」；而底座给带 scale 的列生成的'
      '<b>核对表脚注</b>写死成「源表是 0–1 的小数比率，本页统一按百分数显示」——'
      '对这种非比率列会在页面上印出一句假话。'
      '这类「水平值 + 单月同比」的图<b>不走核对表那条路径</b>，'
      '所以在这里用 scale 是安全的；'
      '等底座把那句脚注一般化之后，这一列可以一并加回那一组。'
    + (f'<b>⚠️ 柱是<u>当月合计</u>，所以交易日数这一层留在同比里。</b>'
       ...
       # ⚠ 这里原来写的是「本页 ADV 那几张是日均口径，天然不含这一层」—— 那半句是假的：
       #   本页一张日均口径的图都没有（各 exhibit 的 unit 只有 % / mn contracts/month /
       #   mn shares/month / USD bn/month / days/month / bn shares/month / pp y/y，
       #   没有任何 per-day 的量纲），而且它与释义「ADV / 当月总量」那条
       #   （本页所有量列给的都是当月合计）当场打架，同页两句读者必然认为有一句在骗人。
       f'（本页<b>一张日均口径的图都没有</b>，所以这一层在图上减不掉，'
       f'只能对着交易日数那张图自己扣。）'
...
# A 组（IR Monthly Reporting Sheet PDF，4 列）：常规月次月第 2–6 个日历日发布、季末月
#     最晚第 8 天（`fetch/ndaq.py` 的「发布节奏」一节逐条打开新闻稿实测，12 期：
#     常规月 10 期落在第 2–6 天，季末月 2 期都是第 8 天）。
#     ⚠ 这里从前写的是「第 6~8 个日历日」。它错在**把常规月说晚了**：10 期常规月里
#     只有 1 期（2019-07→08-06）落进「6~8」，另外 9 期都更早。两个季末月倒是都在里面，
#     所以别说成「全都对不上」——错的是常规月那一半。也别说它「把方向解释反了」：
#     即便按第 8 天算，A 组仍早于 B 组（B 组唯一实测是次月日历第 13 天），
#     它只是把领先幅度从 ~8–11 天压成 ~5 天。series/source_dates.csv 已记的两期
#     （2026-07→08-05 第 5 天、2026-08→09-03 第 3 天）也都在 2–6 内。别再改回去。
#     （2026-08 那期原记 09-02，是电头 "Sept." 没解析出来、回落了 PDF creationDate；2026-09-12 已按电头改正。）
#     那份 PDF 每月**原地替换**同一个 uuid，所以 **IR 自己**只有 2025-01 起 19 个月的窗口。
...
# 出现的数只有两类：把定义钉住的结构性量（各盘口的起点月、美股每月的开市天数区间）
# 与恒等式本身，且**一个都不写死** —— 全部在 import 期从 series/ndaq.csv 现算
# （同本文件其余图注的做法）。唯一不是现算的是「六家期权所」那串所名与
# 「含指数期权」「单边计数」这两条口径断言：它们来自官方文件而不是数据
# （IR Monthly Reporting Sheet 第 1 页与其脚注 1，逐字出处记在
#  fetch/ndaq.py 的「每一列的确切口径」A 组一节），不会随月份变。
#
# ━━ 为什么是这 13 个词（选词判断）━━
# 判据只有一条：这个词出现在本页的图题 / 序列名 / 纵轴 / 汇总表行头 / 核对表列头里，
# 而且**不看定义就会读错**。按「读错会出什么事」分四类：
#   ① 分子与分母   matched、consolidated、三盘口合计 —— 本页的脊梁是四条份额线，
#      而它们的分子是「Nasdaq 自家撮合」、分母却是「含场外的全行业总量」。
#      不点破，读者会把它读成「在交易所之间占几成」，再去和别家只用场内做分母的
#      份额比高低 —— 两个分母不是一个宇宙，图形却完全正常。
#   ② 分母与单位   ADV / 当月总量、当月交易日数 —— 本页所有量列都是**当月合计**，
#      而 CME / Cboe / HKEX 发的是 ADV；不先除交易日就并排，本页会大出一个月的
#      开市天数倍。北欧那两列在本仓连分母都没有（IR 不给北欧交易日；交易所公告的北欧
#      月报里有，但本仓刻意不落库，见 fetch/ndaq.py「未找到」一节）。
#   ③ 覆盖范围     北欧 + 波罗的海、北欧现货成交额、北欧期权与期货、
#      美股期权（六所合计）—— 本页最容易画出误导图的一处全在这里：官方 PDF 标题
#      写的是 "European" 而实际不是泛欧；现货那条是 Nasdaq **自己**撮合的金额而不是
#      全市场；期权那条含指数期权且是单边计数。范围读错，整条线系统性偏高或偏低。
#   ④ 改名与合成   Nasdaq NTX（原 BX）—— 改的是名字不是盘口，图上前后是一条线；
#      三盘口合计是**本仓合成**的列，Nasdaq 的月度文件里没有这一行。
#   ⑤ 两条头条指标之间的关系（2026-09-16 补，页面所有者点名要收的两条）
#      市占率（三盘口合计）、市占率（The Nasdaq）—— 分子分母本身在 ①④ 里已经讲透，
#      这两条讲的是**它们之间**：差额恰好等于 NTX + PSX 两个尾部盘口（所以两条线常年
#      贴在一起，读者会以为其中一条是重复的）、分母完全相同因而可以直接相减、
#      共同历史决定本页的数据月。这三件事在上面四条里一件都没有。
#      ⚠ 词条名是缩写（页面上的全名超 dt 宽度上限，会硬失败），全名写在释义第一句。
# **有意不收**：
#   · m/m、y/y、单月同比、3Y %ile、pp/bp、同月常态、通栏 ——
#     全站通用的读图约定，summary.note 与各图注已逐条讲过，释义板再讲一遍
#     就是两处各写一份、且迟早对不上（样板 /cost/ 的 9 条里一条都没有这类）。
#   · 分位带（P10/P90）—— 2026-09-16 起本页 `headline_style='line_only'`，
#     那两条辅助线**整页一次都不出现**了（页面所有者指令）。它现在不收的理由不再是
#     「别处已讲过」而是「本页根本没有这个东西」，收了反而是给一个不存在的元素做注解。
#     （汇总表的「3Y %ile」列还在，读法由底座自动生成的那条页注交代。）
#   · 「A 组 / B 组 / D 组」与「发布更快的腿」—— 页尾 notes 第 2 条整条就是讲这件事，
#     连发布日证据都摆在那里，这里重讲只会稀释它。
#   · 「拼接列 / 接缝」—— 组标题已经把两段来源写在图题上，页尾 notes 另有一整条
#     用现算的重叠月对账（含唯一那个离群月的逐日裁定），不需要第三处。
#   · 成交量、市值这类本页没有特殊口径的常识词。
#     ⚠ 「份额」这个**泛指词**同样不收（它在本页没有特殊口径）；上面 ⑤ 收进来的是
#       抬头行那两个**具名指标**，收的理由是两者之间的关系会被读错，不是「份额」难懂。
#       这两件事别混为一谈 —— 混了下一个人会照这一行把 ⑤ 那两条删掉。
# ══════════════════════════════════════════════════════════════════════════════

# 释义里要用到的结构性量 —— 逐个现算，一个都不写死（起点月会随回补前移，
# 交易日区间会随年份变；写死的那天就是它开始变旧的那天）。
_NTX0 = _span('vol_us_cash_matched_ntx_sh')[0]      # NTX（原 BX）盘口起点
_PSX0 = _span('vol_us_cash_matched_psx_sh')[0]      # PSX 盘口起点（三家里最晚）
_GRP0 = _span('share_us_cash_matched_group')[0]     # 合成份额列的起点 = 三家齐的那个月
_GRPN = _span('share_us_cash_matched_group')[2]     # 两条头条列的共同历史 = 短的那条的月数
_NAS0 = _span('share_us_cash_matched_nasdaq')[0]    # 单盘口**份额**列的起点（不是成交股数列的）
_CONS0 = _span('vol_us_cash_consolidated_sh')[0]    # 份额分母（全美 consolidated）的起点


def _tail_max():
    """NTX 与 PSX 两个尾部盘口的历史最大份额（%）—— 现算，不写死。

    这两个数是把「两条头条线为什么常年贴在一起」钉住的结构性量（差额的上限），
    不是当月读数，所以可以进释义（判据见本节抬头那段）。算不出返回 (None, None)。
    """
    out = []
    for col in ('share_us_cash_matched_ntx', 'share_us_cash_matched_psx'):
        vs = [v for v in (_num(r, col) for r in _rows()) if v is not None]
        out.append(max(vs) * 100.0 if vs else None)
    return tuple(out)


_NTX_MAX, _PSX_MAX = _tail_max()


_GLOSSARY = [
    ('matched（撮合量）',
     '<b>Nasdaq 自家盘口撮合成交</b>的股数（官方行名 <code>NASDAQ Matched Volume</code> '
     '一类），<b>不含</b>券商内化与 TRF 场外报盘。本页凡带 matched 的行、图与份额都是它。'
     '⚠ 份额的<b>分子</b>是它、<b>分母</b>却是含场外在内的 consolidated（下一条）⇒ '
     '这几条线量的是「Nasdaq 撮合了<b>全市场</b>的百分之几」，'
     '<b>不是</b>「在各交易所之间占几成」—— 与只拿场内做分母的份额不能直接比高低。'),

    ('consolidated（分母）',
     '官方同一张表里的<b>全美股当月合计成交股数</b>（<code>Consolidated Volume</code>，'
     '含所有交易所<b>与</b>场外报盘），是本页四条 matched 市占率的分母'
     + (f'（{_from_zh(_CONS0)}）' if _CONS0 else '') +
     '。这一列本身<b>本页不画</b>（单位是裸股数/月、量级 10¹¹，按裸数显示读不动，'
     '理由见页尾）—— 但份额怎么读全看它：分母是<b>全行业</b>而不是「交易所合计」。'),

    ('三盘口',
     'Nasdaq 在美股的三个自营撮合盘口：<b>The Nasdaq Stock Market</b>、'
     '<b>Nasdaq NTX</b>、<b>Nasdaq PSX</b>'
     + (f'（在本页 CSV 里分别 {_B0}、{_NTX0}、{_PSX0} 起）'
        if _B0 and _NTX0 and _PSX0 else '') +
     '。本页凡写「三盘口」的行与线都是这三家相加，<b>不含</b> Nasdaq 的六家期权所，'
     '也不含北欧那一侧 —— 那两块在本页是另外的图，单位也不同。'),

    ('Nasdaq NTX（原 BX）',
     '<b>改的是名字不是盘口</b>：Nasdaq BX 迁址后自 2026 年起叫 Nasdaq Texas（NTX），'
     '官方 xlsx 的列头也从 <code>BX Matched Volume</code> 改成 '
     '<code>NTX Matched Volume</code>，而序列是<b>同一根</b>、不断裂也不重述。'
     '⇒ 图上改名前后是一条线不是两条；本页统一用新名字回标全历史，'
     '早年的读数仍是当期官方原值。'),

    ('三盘口合计（合成列）',
     '<b>本仓合成的一列</b> = 三个官方份额相加；Nasdaq 的月度文件里<b>没有</b>这一行'
     # ⚠ 这行季度指标的出处是 **IR 月报 PDF 第 2 页的季度面板**（fetch/ndaq.py 的
     #   IR_QUARTERLY_SPEC 从段落 S_USCSH 解析它到 series/ndaq_q.csv），不是季度 8-K。
     #   原来写「季度 8-K 里那行」是串了源 —— 仓内没有任何一处证据说 8-K EX-99.1 有这一行，
     #   读者拿着 8-K 去核会找不到。措辞照 fetch/ndaq.py 的列说明。
     #   （同段下面「北欧现货成交额」那条引的 8-K 那行是另一件事，出处没错，见 fetch 口径坑 16。）
     '（<b>IR 月报第 2 页季度面板</b>那行 "Nasdaq on-exchange matched market share '
     'as a % of total industry volume" 与它同口径，<code>fetch/ndaq.py</code> 逐季核过）。'
     '⚠ 三家都有数才算得出来，所以它只能从最晚那个盘口上线的月份'
     + (f'（{_GRP0}）' if _GRP0 else '') +
     '起算，比单盘口那条'
     + (f'（{_from_zh(_B0)}）' if _B0 else '') +
     '晚一大截 —— 图上左边那段空白是<b>合成口径的起点</b>，不是份额为 0。'),

    # ── 下面两条 2026-09-16 补：页面所有者「『美股 matched 市占率（三盘口合计）』、
    #    『The Nasdaq Stock Market 市占率』，也放在名词释义里」。
    #    它们是**抬头行那两个数**的名字。收词判据（本页出现过 + 不看定义会读错）过得去：
    #    两个词在抬头行、汇总表头条行、两张开篇折线的图题与序列名、核对表列头里都印着，
    #    而「读完上面四条仍然会读错的那件事」是**两条之间的关系** ——
    #    差额恰好等于两个尾部盘口、分母完全相同因而可以直接相减、共同历史决定本页数据月。
    #    分子分母本身不在这里重讲（上面 matched / consolidated / 三盘口 / 三盘口合计
    #    四条已经讲透），只按词条名点回去。
    #    ⚠ 词条名是**缩写**：页面上印的全名（21 / 27 个字符）超过 build/glossary.py 的
    #      dt 宽度上限 170px，会构建期硬失败；全名写在释义正文的第一句里。
    ('市占率（三盘口合计）',
     '页顶抬头行<b>左边</b>那个数，页面上的全名是「<b>美股 matched 市占率（三盘口合计）</b>」，'
     '本页两条头条指标之一'
     + (f'（这条线本身 {_from_zh(_GRP0)}）' if _GRP0 else '') +
     '。分子 = 三盘口的撮合股数之和，分母 = 全美 consolidated ——'
     '这两头分别见上面「matched（撮合量）」「consolidated（分母）」「三盘口合计（合成列）」三条，'
     '这里不重讲（⚠ 分母自己的历史比这条线长，起点不是同一个月）。'
     '⚠ 它与下一条的差<b>只有</b> NTX 与 PSX 两个尾部盘口'
     + (f'（历史最大也不过 {_NTX_MAX:.2f}% 与 {_PSX_MAX:.2f}%）'
        if _NTX_MAX and _PSX_MAX else '') +
     '，所以两条线常年贴在一起、形状几乎重合 —— <b>那不是同一条线画了两遍</b>。'),

    ('市占率（The Nasdaq）',
     '页顶抬头行<b>右边</b>那个数，全名「<b>The Nasdaq Stock Market 市占率</b>」，'
     '本页另一条头条指标：三个盘口里<b>只算最大的那一个</b>'
     + (f'（这条线 {_from_zh(_NAS0)}）' if _NAS0 else '') + '。'
     '与上一条<b>共用同一个分母</b>（都是全美 consolidated）⇒ '
     '两条可以<b>直接相减</b>，差出来就是 NTX + PSX 的份额之和，单位仍是百分点；'
     '换了分母的份额（例如只拿场内做分母的那种）不能这么减。'
     '⚠ <b>本页的数据月由这两条的共同历史定</b>'
     + (f'（短的是合计那条，{_GRPN} 个月）' if _GRPN else '') +
     '：发布门槛要 24 个月共同历史，而发布更快的 IR 那组够不着，'
     '所以本页跟着发布更慢的 nasdaqtrader 走、比 Nasdaq IR 晚一期（页尾另有一条专讲）。'),

    ('美股期权（六所合计）',
     '六家 Nasdaq 期权所 —— <b>NOM / PHLX / ISE / GEMX / MRX / NTX</b> —— 的'
     '<b>当月总成交张数</b>，<b>含指数期权</b>（IR 月报脚注 1 的 capture 口径明说含）。'
     '张数是交易所口径的<b>单边计数</b>（一手成交记 1 张），与 Cboe / CME 的合约数'
     '同口径，可以直接比 —— 但要比的是对方的<b>总数</b>那一列，'
     '不是「不含指数」的 multilist 那一列（页尾另有一条专讲这个坑）。'),

    ('ADV / 当月总量',
     '本页所有量列（期权张数、matched 股数、北欧两列）给的都是<b>当月合计</b>，'
     '不是日均 —— 官方段落标题里没有 average daily 字样。'
     '<b>ADV</b>（average daily volume，日均）= <code>当月合计 ÷ 当月交易日数</code>，'
     '是与 CME / Cboe / HKEX 那几家（它们发的就是 ADV）唯一可比的口径；'
     '不先做这步除法就并排，差的正是当月的开市天数'
     + (f'（美股每月 {_DMIN:.0f}–{_DMAX:.0f} 天）' if _DMIN and _DMAX else '') +
     '。<b>本页一条 ADV 都不画</b>（各图的纵轴全是「每月」量纲），要 ADV 得自己除。'
     # ⚠ 这里原来写「Nasdaq 不发北欧的交易日数」—— 假的，而且被本页自己引用的源文件打脸：
     #   D 组那份 Nasdaq Nordic 交易所公告「Derivative Volumes per Month」每张 sheet
     #   就有 "Trading Days" 列，fetch/ndaq.py 口径坑 17 还拿它做过 8-K 对账（60 / 62 天）。
     #   真实情况是「IR 不给 + 本仓刻意不落库」，原话见 fetch/ndaq.py「未找到」一节。
     '⚠ 北欧那两列<b>做不了</b>这步换算：<b>IR 那份月报</b>不给北欧的交易日数'
     '（交易所公告的北欧月报里倒是有，本仓刻意不落库 —— 落一列没人用的交易日'
     '只会给出「可以除」的错误暗示），所以本页北欧两条只画当月总量；'
     '美股那一列<b>不是</b>它们的分母。'),

    ('当月交易日数',
     '官方 xlsx 直接印的<b>美股</b>当月开市天数（<code>Trading Days</code>）。'
     '本页把它单列成一张图，因为它是上面那步换算<b>唯一</b>的分母'
     + (f'，而它自己并不是常数：本页历史里每月 {_DMIN:.0f}–{_DMAX:.0f} 天、'
        f'两端相差 {_DSPR:.0f}%' if _DSPR is not None else '') +
     '。⇒ 任何一条「当月合计」的同比里都含着「今年这个月比去年多开 / 少开了几天市」'
     '这一层，要先把它减掉再判断量本身的方向。'
     '⚠ 它来自 nasdaqtrader 那份文件、比 IR 的月报晚一期，所以 IR 最新那一期'
     '当期换不出 ADV。'),

    ('北欧 + 波罗的海',
     '本页两条北欧序列的<b>覆盖范围</b>：Nasdaq 在北欧与波罗的海<b>本地市场</b>的交易所。'
     'IR 那份 PDF 的段落标题写的是 "European"，<b>但它不是泛欧</b> —— NDAQ 季度 8-K '
     'EX-99.1 的脚注逐字列的是瑞典、丹麦、芬兰、冰岛的现货市场。'
     '⇒ 与 Cboe Europe 的<b>泛欧</b>口径不是同一个分母，两家的欧洲份额禁止并排比；'
     '可比的只有绝对值，且量级关系与份额给人的印象正好相反（页尾有本机实算）。'),

    ('北欧现货成交额',
     '上面那个范围内的<b>当月</b>现货成交金额，单位是<b>美元</b>十亿 —— IR 直接印美元'
     '（官方北欧原件是欧元的，折回去与 IR 系统性对不上，页尾有一条专讲）。'
     '⚠ 这是 <b>Nasdaq 自己</b>撮合的金额，<b>不是</b>整个北欧市场的成交额：'
     '季度 8-K 里那行 "Total average daily value of shares traded" 才是含 MTF 的'
     '<b>全市场</b>分母，两个数直接并排会读出「IR 少报一半」这种错结论。'),

    ('北欧期权与期货',
     '同一范围内的<b>衍生品当月合约张数</b>'
     + (f'；{_from_zh(_ND0)}的历史' if _ND0 else '；早年的历史') +
     '是按 Nasdaq Nordic 交易所公告的 "Derivative Volumes per Month" 月报重建的，'
     '重建口径 = 各<b>权益类</b>产品族的 "Cleared volumes / No. of Contracts" 求和'
     '（与 IR 印刷值在重叠月上一位小数全等）。'
     '⚠ 它与上面那条现货成交额<b>不是一对</b>：一个是张数、一个是金额，标的也不同 '
     # ⚠ 不写序号：量价分解那条在 SPEC 的 notes 里排第一，但底座在它前面还插了 7 条
     #   通用说明，读者看到的是页尾第 8 条 —— 序号会随底座条数漂移，按标题点名才稳。
     '—— 本页因此做不了量价分解，理由见页尾「本页不具备量价分解的数据条件」那一条。'),
]
...
SPEC = {
    ...
    # 在核对表尾部会多出一行（该行里 B 组那几列显示「—」），**并且 A 组各列自己的图
    # 也各多画一根柱** —— 后半句别再漏掉，页注 `_fast_leg_zh()` 的整段就是为了补它。
    # 这是底座内建的正常形态。
    ...
    # ── 开篇画法：页面所有者 2026-09-16 指令第 1、2 条 ────────────────────────
    #   「ex2/3 里面近 36 个月的线全部删除」「ex4/5 删除，这两个指标没意义」。
    # 两条合起来正好是底座的 `'line_only'`：① 那两张全历史折线照出、但不画
    # 近 36 个月 P10/P90 两条辅助线；② 那两张单月同比柱整段不出。
    # 代价（与 `'bar_yoy'` 同一笔）：分位带离开页面。汇总表的「3Y %ile」列不受影响
    # —— 它直接从序列算，不读这张图；页尾「汇总表读法」那句由底座按真画出来的图
    # 自动换成「本页没有画分位带那张图」，不会留下一句指着不存在的线的话。
    # ⚠ 撤掉 ② 不与「全站同比统一成单月」（CONTRACT §6，所有者 2026-09 定）冲突：
    #   §6 管的是同比**只能是哪一种口径**，不是「必须画几张同比图」。本页撤掉这两张
    #   之后仍有五张画着单月同比的金线（四张 gs_bar + level_yoy 那张）。
    'headline_style': 'line_only',
    ...
    'groups': [
        # ⚠ **组的先后就是页面上的先后**（底座 ③ 按 spec 的顺序出图）。B 组份额那一组
        #   排在最前，是页面所有者 2026-09-16 指令第 3 条「ex10 插在 ex3 后面」——
        #   ① 只剩两张折线（见上面 headline_style），③ 的第一张紧跟其后，正是所有者
        #   要的位置。不要随手把它挪回原位。
    ...
    # 尾部多出一行（该行里 B 组那几列显示「—」），**同时各自的图也多画一根柱**。
    # 若将来 IR 那份 PDF 攒够 24 个月历史，
    ...
    'level_yoy': [{
        # 页面所有者 2026-09-16 指令第 4 条「ex14 插到 ex7 后面，并给说明这两个图的差异」。
        # 原 Exhibit 14 缺省排在页尾（底座 ⑦），原 Exhibit 7 是下面这个锚点组的那张
        # 三盘口合计成交股数 —— 两张图的形状、窗口、次轴全一样，隔着七张图读者认不出
        # 差别在哪。`after_group` 把它就地挪到那一组之后；两张的差异写在 `zh`
        # （标题里的「单盘口」）与 `_NOTE_TTM` 开头那一段里。
        'after_group': _GZ_MNSH,
        'zh': 'The Nasdaq Stock Market 美股 matched 成交股数（单盘口）',
    ...
    'notes': [
        ...
        'Monthly Reporting Sheet PDF，<b>常规月次月第 2–6 个日历日</b>发布、'
        '季末月最晚第 8 天（<code>fetch/ndaq.py</code> 逐条打开新闻稿实测 12 期，'
        '其中季末月只有 2 期），是官方权威值；'
        '<b>B 组</b>（成交股数、份额、交易日）来自 nasdaqtrader.com 的月度市占率 xlsx，'
        '历史深但发布晚得多 —— 官方自述次月第 10 个工作日，本仓唯一实测是 2026-06 那份的 '
        'Last-Modified 2026-07-13（即当月第 8~9 个工作日，比官方自述还早一两天），'
        '而 IR 的 07 月数据 08-05 就发了；'
        '<b>D 组</b>（只用于历史，不用于当期）是 Nasdaq Nordic 交易所公告的衍生品月报。'
        '<b>本页以 B 组为脊梁</b> —— 只有它够 24 个月历史、过得了发布门槛，'
        '因此页面数据月比 Nasdaq IR 官方晚一期；A 组反而是「发布更快的腿」。'
        + _fast_leg_zh() + '。这是刻意的取舍，不是抓取失败。',
        ...
        # ── 本轮历史账（docs/SINGLE_SPEC.md §2 表下判例第 4 条：任何让图号位移的改动，
        #    页尾都要向读者交代新旧号怎么对应，且**只写「原 Exhibit N」与相对位移，
        #    一个当前号都不写**）。当前号会随下一次增删图再变，写进来的那天就开始过期。
        '<b>2026-09-16 版式变更（页面所有者四条指令），与上一版的对应关系。</b>'
        '① <b>原 Exhibit 2、3</b>（两条头条指标各一张全历史折线）<b>还在原位</b>，'
        '但图上近 36 个月 P10/P90 那两条辅助线撤掉了 —— 撤的是线，图本身与窗口没动。'
        '② <b>原 Exhibit 4、5</b>（同两条指标各一张单月同比柱）<b>整张删除</b>。'
        '③ <b>原 Exhibit 10</b>（四条 matched 市占率序列对比）<b>提前</b>到原 Exhibit 3 之后，'
        '成为组图的第一张。'
        '④ <b>原 Exhibit 14</b>（单盘口 matched 成交股数的「水平值 + 单月同比」）<b>提前</b>到'
        '原 Exhibit 7（三盘口合计那张成交股数）之后 —— 两张图长得几乎一样，'
        '差在哪写在后一张的图注开头。'
        '⇒ 删了两张、又有两张换了位置，<b>原 Exhibit 6 及其后各图的编号与上一版不再一一对应</b>'
        '（有的前移、有的号数不变）。对着旧截图找图请<b>按标题找，别按号找</b>。'
        '每一格的数、每条线的口径、每张图的窗口都一个字没改。',
