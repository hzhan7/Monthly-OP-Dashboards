...
# **定义在 SPEC 之外**：页尾那条「单月口径为什么保留」要说「趋势看页尾哪几张滚动图」，
# 而这个名单只有从这里现数才说得准 —— SPEC 是一个 dict 字面量，notes 那一项拿不到
# 同一个字面量里的 ttm_yoy。上一版就是在 notes 里写死「趋势看页尾两张滚动图」，
# 而两张都是**证券市场**的、衍生品那一侧一张都没有；同一条注往下几句自己又写着
# 「衍生品那一侧本页没有滚动图」—— 一条注里两句话互相打架。
_TTM_YOY = [{
    'zh': '证券市场成交股数',
...
def _ttm_names_zh():
    """页尾滚动图叫什么 —— 逐条现列（底座给它们的标题就是 '<zh>：水平值与 N 个月滚动同比'）。"""
    if not _TTM_YOY:
        return '本页没有滚动图'
    return '、'.join('「%s：水平值与 12 个月滚动同比」' % t['zh'] for t in _TTM_YOY)


def _ttm_deriv_zh():
    """衍生品那一侧有没有滚动图 —— **现判**，判据是 level 列的单位里有没有 contract。

    本页衍生品口径一律以张数计（contracts/*），证券口径是 S$ 与股数，
    所以单位串就是最省事也最不会骗人的判据：加一张衍生品滚动图，这句话自己翻面。
    """
    deriv = [t['zh'] for t in _TTM_YOY
             if 'contract' in ((t.get('level') or {}).get('unit') or '').lower()]
    if deriv:
        return ('衍生品那一侧也有（%s），趋势看它'
                % '、'.join('「%s」' % z for z in deriv))
    return ('<b>⚠️ 衍生品那一侧本页一张滚动图都没有</b>，'
            '所以「趋势看滚动图」这句话对 DDAV 不适用')
...
SPEC = {
    ...
    'groups': [
        ...
        # 而 CONTRACT §6.1 第 2 条要求单月口径写进标题、ex_single 的标题 = 组名 + 列名
    ...
    'notes': [
        ...
        # 底座给不出的那半 —— 为什么这几张该用单月。分歧数字全部现算
        # （_GT / _GB / _madd / _ttm_names_zh / _ttm_deriv_zh），没有一个写死的数据数字。
        #
        ...
        '「一整年在不在长」看后两条自己的滚动口径 —— 页尾 ' + _ttm_names_zh() + '，'
        '两种口径分歧有多大由那几张的图注按本序列实测。'
        '<b>「衍生品日均成交」那张（DDAV）</b>与 SDAV 那张同型：'
        '官方月报 At-A-Glance 印的日均数，与页顶数据条第二格同一条序列、同一个读数，'
        '留单月是为了两处对得上；' + _ttm_deriv_zh() + ' —— '
        '跨年趋势请回到「衍生品成交与未平仓」那张当月合计的折线图上看。'
        f'<b>「GIFT Nifty 50 期货」那张</b>：{_NIFTY_BREAK} 计数口径断点让滚动口径的'
        ...
        + (f'<b>「FTSE 台湾指数期货」那张</b>：全期实测单月与滚动符号相反 {_GT[1]} 个月'
           f'（{_GT[0]} 个可比月），分歧最大的 {_GT[2]} 单月 {_GT[3]:+.1f}% vs '
           f'滚动 {_GT[4]:+.1f}%；单月视图读逐月的竞争动态（与同页 GIFT、A50 那两条线'
           f'同窗对照），方向判断以滚动口径为准 —— {_GT[5]} 的滚动同比 '
           f'{_GT[6]:+.1f}%（现算）。'
           if _GT[0] else
           '<b>「FTSE 台湾指数期货」那张</b>：单月视图读逐月的竞争动态，'
           '方向判断以滚动口径为准。')
        + (f'<b>「当月新债券挂牌数」那张</b>：逐月的发行事件计数，单月同比天生毛刺大 —— '
           f'全期实测与滚动符号相反 {_GB[1]} 个月（{_GB[0]} 个可比月）；'
           f'方向判断以滚动口径为准 —— {_GB[5]} 的滚动同比 {_GB[6]:+.1f}%（现算）。'
           if _GB[0] else
           '<b>「当月新债券挂牌数」那张</b>：逐月的发行事件计数；'
           '方向判断以滚动口径为准。')
