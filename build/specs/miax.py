...
SPEC = {
    ...
    'groups': [
        ...
        {'zh': '多挂牌期权（IR 月报口径，2025-01 起）', 'cols': [
            ...
            # 所以 Exhibit 9 的金线画的是 (a/b−1)×100，而紧挨着的 Exhibit 8
            # （同组份额列，pct1）走差值 —— 同一组里两种口径，差别只在 fmt。
            # ⚠️ **2026-09-03：这个差的单位是钱，不是百分点。** 分子是美元，
            # 差出来仍然是 USD/contract —— Exhibit 9 的右轴标题今天是
        ...
        {'zh': 'MIAX Pearl 股票（IR 月报口径，2025-01 起）', 'cols': [
            ...
            # 实测 Exhibit 12：Jul-26 曾印 **−36.4%**（0.7% vs 去年同月 1.1%）。
            # 同页对照 `share_multilist_options_pct`（15.5~19.4，过得了量纲体检、
            # 配 pct1）在 Exhibit 8 一直走 pp —— 两列同为份额、同一页、同一种量，
            # 唯一的差别是量纲体检那句「> 1.5」。
            ...
            # fmt 仍然留 f2，量纲体检那条路一步没动。Exhibit 12 的 Jul-26 现在是
            ...
            # 读不出方向 —— Exhibit 13 的 Jun-26 曾印 **−92.9%**，而真实发生的事是
        ...
        {'zh': 'MIAX Futures（IR 月报口径）', 'cols': [
            ...
            # 被底座合成一张 lines（Exhibit 17），lines 没有次轴 —— 所以 2026-09 的
