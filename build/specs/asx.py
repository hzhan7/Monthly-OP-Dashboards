...
━━ 分解图按**月**分桶（2026-09 按页面所有者的指令改），并前移到它的输入之后 ━━
这张图的横轴改过两回，两回都是所有者指令：2026-08-07 由 ASX 财年（Jul–Jun）7 根柱
改成「4 个完整日历年 + 当年 YTD」；**2026-09 再改成「一格 = 一个月，基期去年同月」**
（原话：「ex40 如果有数据的话，做成月度的」）。判据是实测：`value_cash_total_audbn` 与
`trades_cash_total` 在本表 128 个月上全有值、零空洞、全为正，单月同比算得出 116 格，
重标定权重 w 实测 0.78–1.48（无 0/0 退化），三道闭合残差 ≤ 4e-14。
底座的开关是 `decomp` 的 `bucket='monthly'`（缺省仍是 `'year'`，另外 5 页不受影响）。
⇒ **日历年 / year_label / YTD 那一整套在本页已经作废**，`year_start_month` 与 `years`
两个键随之删掉（月度桶下写着它们是硬失败：留一个再也不起作用的声明比当场报错糟）。

同一轮还按所有者指令把这张图**前移到紧跟它的输入之后**（`after_group` 指向
「成交笔数与单笔金额」那一组），于是它落进「二、现货市场」那一节，排在当月成交额、
成交构成、日均笔数 / 当月笔数 / 平均每笔金额之后 —— 分解的两条输入就在它上面三格。
⚠️ 这么排的前提是**本文件与页尾文案里没有任何硬编码的「当前」图号**（底座自己生成的
「见 Exhibit k」都是现算的，跟着走不会错）。判据：`grep -n 'Exhibit [0-9]' 本文件`
命中的每一处都必须属于下面两类之一 ——
  · 汇总表 <b>Exhibit 1</b>：它永不移动（底座把汇总表钉在 1 号）；
  · 页尾那条历史账里的「<b>原</b> Exhibit N」：那是**冻结的上一版编号**，
    是历史事实、不随本页增删图变化，正因为如此才写得死。
**除这两类之外一个当前图号都不许出现**。加图删图之前先重跑这条 grep，
逐条按上面两类归位；归不进去的就是会过期的引用。

FY 口径与官方新闻稿的对账证据**没有丢**，它独立留在下方 notes 里那一条
（「二次融资合计」那条：FY26 窄口径 A$37.849bn 对上新闻稿的 A$37.8bn，
含换股对价 A$58.428bn）—— 图不再按年分桶，`_fy_probe()` 那个探针就没有调用方了，
一并删掉；证据靠 notes 那一条，不靠一个没人调的函数。
...
_NOTE_SFE = (
    ...
    + '<b>左边那一大段空白是本仓这条取数路径的边界，不是产品那时不存在。</b>'
      '2016-07 起的 MAR 同样印了这条链接（2016-06 及更早的 MAR 正文里根本没有它），'
      '但那批链接指向已下线的老站点路径 <code>/data/market-reports/…</code>；'
      '首月那一期起改指 DAM，从此一直在。SPI 200 与国债期货本身在本页首月之前很久'
      '就在跑了 —— 想看那段时间的期货活动，用同页的「ASX 24 期货与期货期权 ADV」。'
      # ── 2026-09 按页面所有者的指令逐条实测过「能不能拓到 2016」，结论与证据都印出来 ──
      # 这一段不是背景介绍，是**对一个具体问题的回答**：所有者问「如能有到 2016 年
      # 开始的数据就拓时间维度」。答案是官方源到不了，而「到不了」必须带证据 ——
      # 否则下一个人只会看到一段空白，无从判断是没做还是做不到。
      '<b>2026-09 逐条实测过「能不能把这两组图拓到本页首月」，答案是官方源到不了。</b>'
      '（一）2016-01…2020-05 那 47 条正文链接<b>逐条实发</b>：47/47 全部返回 '
      'HTTP 200 + <code>text/html</code>（不是干净的 404），落地在官网的 404 页 —— '
      '这种 soft-404 骗得过「状态码是不是 200」那种判据，所以抓取器同时校验 '
      'Content-Type 与 <code>%PDF-</code> 魔数。'
      '（二）老站点的目录索引返回 403，官网 <code>sitemap.xml</code> 里一条 PDF 都没有，'
      '在 DAM 上按十几种旧文件名形状拼过 —— 全是真 404。'
      '（三）官网另有一份<b>结构相同的孪生件</b>（同抬头、同合约代码、同「当月量 + 月末未平仓」'
      '两列），但它只回到 2020-04，而且自称 <i>preliminary</i>：与本页的终值逐月<b>系统性偏低</b>'
      '（实测 10 年期国债最大差 −350,969 张、−9.0%）。把它接到终值序列上就是在一条线里'
      '混两个 vintage，<b>本页不做</b>。'
      '（四）每期报告都印去年同月那一列，拿它可以往前推 12 个月 —— 但那是<b>后期回看值</b>'
      '而不是当期原值，与上面「一律用当期原值」那条规矩冲突，而且最远也只到 2019 年年中，'
      '<b>够不着 2016</b>。'
      '<b>「存档天花板不等于产品上线」这句话现在有活的官方凭据</b>：同一个目录下'
      '<code>MonthlyWebNonTradedVolume1601S.htm</code>（2016 年 1 月）今天仍然打得开，'
      '逐品种列着 <code>AP - SPI 200</code> 与 <code>YT - 3 Year Bonds</code> —— '
      '2016 年官方确实在按品种披露 ASX 24，只是那批<b>成交量</b>报告的原件已经不在网上了。'
      '（那份是 Non Traded Volume、不是成交量，只能当存在性凭据，不能当数据源。）'
...
# ⚠️ 上面这段原来还有第三节，讲「本图按日历年 Jan–Dec 分桶，与 ASX 自家 7 月制、
# 按结束年命名的报告年度不是一套日历」，并引 `_fy_probe()` 现算的 FY 对账读数。
# 2026-09 分解图改成**月度桶**之后，那一整节讲的是一张不存在的图（本图既不按日历年
# 也不按财年分桶，一格就是一个月），所以连同 `_fy_probe()` 一起删。
# **FY 对账的证据没有丢**：它独立印在页尾 notes 里「二次融资合计」那一条
# （FY26 窄口径 A$37.849bn 对上新闻稿的 A$37.8bn，含换股对价 A$58.428bn）。
...
# 「口径断点」与「慢腿」这两个**读图约定词本身**（页尾那两条底座现算的 notes ——
# 讲「慢腿」的那条与讲「口径断点」的那条 —— 说的是这两件事在本页的具体落点，
# 释义板再讲一遍就是两处各写一份）、以及成交量 / 市值这类本页没有特殊口径的常识词。
# ⚠️ 这里**按内容点名、不按序号**：页尾条目的序号随增删条目滚动，
#    2026-09 页尾插进一条历史账，spec 侧全部条目就整体后移了一位。
# ⚠️ 但断点**两侧那两个具名口径**要各收一条（挂牌市值 / 募资额、
# 表内现金 / 保证金总额）：收的是「这两个官方行名各指什么、外延差在哪」，
# 不是断点机制本身 —— 页尾讲「口径断点」的那条与讲「上市融资换口径」的那条
# 都只点了名字，没说改的是名字还是外延，
...
_GLOSSARY = [
    ...
    ('平均每笔金额',
     ...
     '会让整条线系统性偏低而图形完全正常'
     + ((f'（本页那张<b>量价分解</b>图把同一个量自算了一遍对账：用含场外口径当分子，'
...
# 这条循环是 9 家 spec 页共用的。为一家去改它，代价是另外 8 家的图号整体平移。
...
# ⚠️ 原来还有一个 `_SEC_DEC = '九、现货成交额的量价分解'`：分解图从前排在页尾、
# 自成一节。2026-09 按所有者指令前移到它的输入之后（decomp 的 `after_group`），
# 它落进「二、现货市场」，与那一组的其余图共用 `_SEC_CASH` ——
# 于是这个常量只剩一个用处：给一个不再存在的第九节起标题。删掉而不是留着不用：
# 留一个没人引用的分节名，下一个人会以为页面上还有那一节。
# （底座的 `_mark_section` 收口逻辑对此是安全的：分解图想要的 section 与它前一张图
# 相同 ⇒ 不重复起标题，也不会凭空多出一个空标题。）
...
SPEC = {
    ...
    'groups': [
        {'zh': '现货日均成交额', 'cols': [
         ...
         'ratio_rhs': {
             ...
             # 「两者为什么还差那么一点」是**底座证不了的事实断言**（底座只能量出差有
             # 多大），所以由这里承载：本页这一处的差确实来自口径 —— 比值线那一侧的两列
             # 是日均（官方先除以交易日数再按披露位数取整），堆叠图那一侧是当月合计。
             # 同一句话由底座印在两处（比值线图注与占比堆叠图注），不会各自漂。
             'dup_note': '两者口径不同（比值线这一侧是日均、堆叠图那一侧是当月合计，'
                         '各自按官方披露取整），',
         }},
        ...
        {'zh': '现货成交额与成交构成', 'cols': [
         ...
         'mix': {
             ...
             'share_note': '<b>这张答的是「结构往哪边走」，不是「有多大」</b> —— '
                           '规模看⟨ex:mix-value_cash_total_audbn@-1:上一张⟩的柱高。四段的分母是官方自己印的当月成交额合计，'
    ...
    # ── 开篇两条头条列：柱 + 次轴单月同比，一条列一张图（所有者 2026-09 指令 1、2）──
    # 缺省 'band_yoy' 是四张：两条列各出一张「全历史折线 + 近 3 年 P10/P90 分位带」，
    # 再各出一张「单月同比柱」。所有者要的是「仅保留主标的（柱状图）与 YOY（折线）」，
    # 而底座早有这条现成通路（`build/single.py` 的 HEADLINE_STYLES，tmx 2026-09 已在用）：
    # `ex_head_bar()` 把水平值柱与单月同比折线并成一张 gs_bar，纯同比那两张随分支消失。
    # ⚠️ **代价：近 3 年 P10/P90 分位带没了。** 引擎没有「柱 + 两条带 + 次轴线」这种图型，
    # 带的上下沿与柱同量纲、画上去会被读成第三、第四根柱。汇总表（Exhibit 1）的
    # 「3Y %ile」列**不受影响**（它不靠这张图），页尾那句指向分位带的话由底座按
    # 「这张图在不在」自动收放，不会留下一句指着不存在的图的话。
    ...
    'decomp': [{
        # ── 位置：紧跟它的两条输入之后（2026-09 所有者指令）────────────────────
        # `after_group` 让底座在 groups 循环里就地出图，而不是缺省的页尾。
        # 值必须逐字等于某个 group 的 zh，且在本页唯一 —— 打错字底座当场 SpecError
        # （不校验的话会静默掉回页尾：图还在、闸门全过，只有数图号的人看得出来）。
        # 挑这一组而不是「现货成交额与成交构成」：分解的**两条**输入分别是当月成交额
        # （那一组的合计柱）与当月成交笔数（这一组的第二张），排在两者中较后的那个
        # 之后，读者往上一格就是笔数、往上四格就是金额。
        'after_group': '成交笔数与单笔金额（次轴：单月同比）',
        # 于是它落进「二、现货市场」，不再自成一节（原 _SEC_DEC 已删）。
        'section': _SEC_CASH,
        'zh': '现货成交额',
        ...
        # ── 一格 = 一个月，基期去年同月（2026-09 按所有者指令从「日历年 + YTD」改来）──
        # 判据是实测，不是偏好：两条输入列在本表 128 个月上全有值、零空洞、全为正，
        # 单月同比算得出 116 格，重标定权重 w 实测 0.78–1.48（无一格 0/0 退化），
        # 三道闭合残差 ≤ 4e-14（上限 1e-9）。柱的构成、留空格、各项统计
        # 全部由底座按 CSV 现算并打进自检行，这里一个数都不写死。
        # ⚠️ 月度桶下 **year_start_month / year_label / years 三个键一写就硬失败**
        #   （build/single.py 的 _norm_decomp）：它们只管年度分桶，留着会让下一个人
        #   以为月度图的横轴还受它们管。日历年 / FY / YTD 那一整套在本页已经作废。
        'bucket': 'monthly',
        'note': _NOTE_DECOMP,
    }],
    ...
    'notes': [
        # ── 本轮的历史账（页面所有者 2026-09 的六条指令）────────────────────────
        # 判例第 4 条（git show 057ae0c = /cme/、cf74786 = /cboe/）：删图与重排必须在
        # 页面上向读者交代。读者手里可能还留着上一版的图号（本页图注、站外引用都按号
        # 说话），不写明白就只剩「怎么少了几张、号还全变了」。
        # ⚠️ 本页是声明式 spec 页，**图号由底座按渲染顺序现算** —— 这一条里
        #    一个新号都不许写死，只用「原 Exhibit N」（冻结的上一版编号）与相对位移。
        #    同理它是历史账、不随数据变 ⇒ 一个当月读数都不含，全是静态字面量。
        '<b>2026-09：开篇四张并成两张，量价分解由页尾前移到中段，全页重排一次编号。</b>'
        ...
        '「单月同比柱」）合并成两张「全历史水平值柱 + 次轴单月同比折线」，一条头条列一张。'
        '<b>并图一个数都没改</b>：柱是原折线图那条深蓝线的同一段、同一个'
        '「首个有值月 → 末个有值月」窗口，金线是原同比柱的同一条序列 —— '
        '变的是画法，不是数据。'

        '（二）<b>并图的代价：近 3 年 P10/P90 分位带从页面上消失了。</b>'
        '引擎没有「柱 + 两条带 + 次轴线」这种图型，而带的上下沿与柱同量纲，'
        '画上去会被读成第三、第四根柱。<b>页顶汇总表的「3Y %ile」列不受影响</b> —— '
        '那一列直接从序列算，从来不读那张图；消失的只是「把常态区间画出来」这一种呈现，'
        '不是这个口径本身。'

        '（三）<b>「现货日均成交额」那张加了一条右轴比值线</b>'
        '（日均成交额（仅场内）÷ 日均成交额（含场外报告）），并因此由两条平滑折线'
        '改成两根并排柱 —— 引擎的折线图型<b>没有次轴</b>，而一条 0–100 的无量纲比值'
        '与 A$bn/day 同轴会把两条主线压成贴底的一条'
        '（这条比值今天的实测区间印在那张图自己的图注里，本条不复述）。'
        '<b>这条比值与本页「现货成交额与成交构成」那张 100% 堆叠图里的'
        '「场外成交事后报告」那一段互补</b>（两者相加恒为 100%），差多少由构建期'
        '逐月复算并印在那张图自己的图注里。两处分工不同：堆叠图答「四块构成怎么此消彼长」，'
        '这条线答「两根柱之间的缺口有多大」。'

        '（四）<b>量价分解由「4 个完整日历年 + 当年 YTD」改成月度桶，并由页面末位'
        '前移到它的输入之后</b>（当月成交额、当月成交笔数、平均每笔金额那三张之后）。'
        '两件事是同一件事的两半：改月度桶让它与本页其余时序图同一个刻度、'
        '同一个基期（去年同月），前移让「输入 → 分解」在阅读顺序上连成一段。'
        '⚠️ <b>因此它与全站其余仍按年度桶的分解图不再同口径，跨页不要比。</b>'
        '年度版图注里「每个端点都是整整 12 个月的合计，不是某一个月的点值 —— '
        '拿单月当端点，挑到一个异常月就能把归因整个说反」那段话<b>随之作废</b>：'
        '月度桶的端点<b>就是</b>一个月。换来的是每一格都能与本页其余月度图逐格对读；'
        '换掉的正是那层保护，所以要连着相邻几格一起读，别拿单独一格下结论。'
        '同时作废的还有「本图任何一根日历年柱都对不上官方 FY 新闻稿的年度读数」那一句 —— '
        '页面上已经没有年度柱。'

        '（五）<b>「二次融资」与「上市融资·旧口径」两张截了纵轴。</b>'
        '这两张各有一两个把整条线压平的单月尖刺（并购换股对价那一笔的量级是常态月的'
        '一二十倍甚至上百倍），不截轴的话其余上百个月全被压在贴近零线的一条窄带里。'
        '<b>截轴不删点</b>：超界的点钳到轴顶、画成空心红圈、真值红字竖排标在旁边。'
        '截在哪一档、判据是什么、哪几期出界，写在这两张图<b>各自的图注</b>里。'

        '（六）<b>ASX 24 分品种那两组图的左端没有跟着拓到本页首月。</b>'
        '<b>官方直链这条路确实到不了</b>，实测证据印在本页「分品种那两组图不出自 MAR」那一条里。'
        '拿 ASX 24 期货合计按固定比例劈成四条、或者拿官网那份 preliminary 孪生件接上去，'
        '都能画出一张看着完全正常的图 —— 那不是官方在那些月份披露的值，'
        '所以本页宁可让左边留着那段空白。'
        '⚠️ <b>但话要说准：还有一条路没走，不是没有路。</b>'
        '那批原件在<b>网页存档</b>里还找得到相当一部分，抽查三期用未改动的解析器全部读得出，'
        '且各自的报告合计与同月 MAR 的期货 + 期权合计逐期相等。'
        '本仓对存档源有明确分工：<b>一次性回补脚本允许用，常驻抓取路径一律禁止</b>。'
        '走那条路能补到本页首月，代价是仍有若干期只能取「后期报告里回看去年同月」那一列，'
        '于是一条线里混两种取数时点 —— 与本页「一律用当期原值」正面冲突。'
        '<b>要不要开这个口子是页面所有者的决定，本轮挂起未做</b>；'
        '在他拍板之前，左边那段空白就是如实呈现。'

        '（七）<b>编号怎么动。</b>原 Exhibit 2 / 3 与原 Exhibit 4 / 5 已并成开篇那两张；'
        '<b>原 Exhibit 6 至原 Exhibit 12 整体前移 2 号</b>；量价分解（原 Exhibit 40，'
        '原本是全页最后一张）插到这一段之后，于是<b>原 Exhibit 13 至原 Exhibit 39 '
        '各前移 1 号</b>；末尾核对表前移 2 号。'
        '<b>为什么这一轮非重排不可</b>：删掉的那两张在页面<b>最前面</b>，后面还压着'
        '三十几张 —— 不重排的话页面按顺序渲染出来就是一串跳号，'
        '再加上分解图插进中段造成的一次倒序，读者只会理解成「漏了图」。'
        '<b>本页每一个图号都是底座按渲染顺序现算的</b>，正文与图注里没有写死任何一个新号，'
        '所以下一次增删同样只会移动号，不会造出一句指错图的话。',
        ...
        + '、以及 <b>四条</b> contracts_*_total 月总量列'
          '（期货 / 期货期权 / 单股期权 / 指数期权，= ADV × 对应交易日，与 adv_* 重复）。'
          '⚠️ 同前缀的第五条 <code>contracts_futures_total</code>（ASX 24 期货合计）'
          '<b>不在此列，它上了页面</b>（分品种结构那两张图的分母、汇总表与核对表各一处）—— '
          '而且它<b>不是</b>「ADV × 交易日」算出来的，是官方自己印的一列：'
          '实测某月 748,243 × 21 = 15,713,103，而官方那一列印的是 15,713,097，差 6 张。'
          '本页用的一律是官方原列。',
