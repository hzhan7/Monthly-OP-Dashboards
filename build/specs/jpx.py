...
SPEC = {
    ...
    'decomp': [
        {'zh': '东证内国株现货成交额',
         ...
         # 「东证内国株成交额」那张的金线逐格相等）。
         # **代价要说清楚**：连带失去的是底座那道「日均 × 立会日数 vs 官方合计列
         # 逐月对账、对不上硬失败」的闸门。本页原先拿它当「两列共用同一套立会日数、
         # 因而同口径」的自动证明 —— **这个自动证明本轮取消了**。同口径的证据退回
         # 到 fetch/jpx.py 那一侧（两列出自同一个 dom_* 累加器，见下面 note 的第一段），
         # 那是人核的，不是机器每次构建都复核的。要把闸门找回来，得在年度桶下另画
         # 一张，或者给底座补一条「与桶无关的口径对账」——两者都不在本轮范围内。
