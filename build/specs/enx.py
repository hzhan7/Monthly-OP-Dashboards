...
def _quarter_pulse(col, m0):
    ...
    ⚠ 这个倍数 2026-09 之前在释义里写死成「5–7 倍」，抄自 `fetch/enx.py` 口径坑 2 的
    旧断言（那是样本更短时的一次快照，释义里既没注出处也没现算）。实测已经掉出下沿：
    以并表后非季末月的中位数为分母，最低的那个季末月只有 4 倍出头。每进一个新季末月
    这个区间还会被顶宽 —— 与 `_athex_share()` 记下的是同一种烂法，所以同样改成现算。

    分母取并表后**非季末月的中位数**（均值会被个别高月拉动）；分子逐个季末月各取一次。
    起点非用并表月不可：并表之前这一列是 legacy Euronext 的单股期货，量级差两个数量级，
    混进来算出的倍数不是要说的那件事（与 `_athex_month()` 那条 ⚠ 同一个理由）。
    算不出全部返回 None。
    """
    if not m0:
        return (None,) * 4
    ...
    for r in _rows():
        ...
        if not v:
            continue
        (q if m[5:7] in ('03', '06', '09', '12') else base).append(v)
    if not q or not base:
        return (None,) * 4
    b = statistics.median(base)
    if not b:
        return (None,) * 4
    ratios = [x / b for x in q]
    return min(ratios), max(ratios), len(q), len(base)


_QP_LO, _QP_HI, _QP_NQ, _QP_NB = _quarter_pulse(
    'adv_singlestock_futures_kcontracts', _ATH_M0)
#: 端点同样走 `_band()`（下界向下取、上界向上取）—— 四舍五入会把区间收窄到装不下自己的数据。
...
# ══ 名词释义要用到的两个现算量 ═════════════════════════════════════════════
def _day_cols():
    """series/enx.csv 里有几列「日数」、其中几列不是交易日 —— 现算，不写死。

    官方给每个板块各配一列 `Nb of trading days`（fetch/enx.py 口径坑 10），
    本仓逐列入库成 `trading_days_*`，唯独 Nord Pool 现货电力那一列是**自然日**，
    列名因此叫 `days_power_calendar`。释义里「本页各条 ADV 的分母不是同一个」
    这句话的算术底就是这两个数 —— 官方哪天多并一个板块，它自己会变。
    """
    rs = _rows()
    if not rs:
        return None, None
    cols = [c for c in rs[0] if c.startswith('trading_days_') or c.startswith('days_')]
    return len(cols), len([c for c in cols if not c.startswith('trading_days_')])


_NDAYCOL, _NCALCOL = _day_cols()

#: 雅典并进主列的月份（`_athex_month()` 从 series/enx_breaks.csv 读）。备注列那条释义
#: 的语义在这个月翻转，所以月份只能现算 —— 官方改脚注时它跟着改。
_ATH_M0_ZH = _ATH_M0 or '雅典并表月'

#: 台账里被官方标成「已重述」的那些断点月 —— 与「不重述」的并表断点是相反的两件事，
#: 释义里要举例，例子从 CSV 驱动的 `_BREAKS` 里取，不在这里抄月份。
_RESTATE_MS = sorted({b['month'] for b in _BREAKS if '重述' in b.get('zh', '')})
...
SPEC = {
    ...
    # 选词的判断（为什么是这 15 个）：
    #   这一页 38 张图、一张 71 列的官方 xlsx，读错的方式基本只有三类，释义就按这三类选：
    #   ① **同一个词在这一页有多个口径**：ADV 的分母有好几个（现货/衍生品/固收/商品/FX
    #      各一列交易日，电力还是自然日）；「单边计 / 双边计」在同一张官方表里并存，
    #      而双边计那一列还含 reported trades（这两件事是**两层**：一层是同一笔成交
    #      数两次、一层是这一列根本不只装撮合成交，所以拆成两条）；
    #      「未平仓」是存量而其余是流量。这几条是全页 38 张图共用的读图前提。
    #   ② **Euronext 特有的、不看定义就不知道在说什么的专名**：MTS、TAADV、MATIF、
    #      Nord Pool、电力衍生品、CSD、清算笔数/手数、发行人家数 vs 挂牌只数。
    #      它们全部出现在图题、序列名或汇总表行头上。
    #   ③ **这一家靠并购长起来所特有的两个陷阱**：「重述 ≠ 断点」（一个把历史重算了、
    #      一个没有）与 athex 备注列（语义在并表月翻转）；外加最危险的那一列单股期货。
    #   刻意不收：m/m、y/y、3Y %ile、pp/bp（全站通用读图约定，summary.note 已逐条讲过）；
    #   ETC / 超额配售 / 结构化产品（这三个在页面上各出现在两三处 —— 行头、序列名、
    #   页尾注 —— 上一版这里写的「只各出现一次」是错的；不收的真实理由是它们都按
    #   **字面意思**读就不会读错方向，而释义板收的是「不看定义会读错」的词，不是所有专名）；
    #   FX 即期（「⚠ 美元，单边」已经写在组名与序列名上，页尾另有一整条注）。
    #
    #   数怎么写（上一版这里写着「数只有两类」，而释义里当时还写死着 5–7 倍的季度脉冲、
    #   股权比例与几个日期 —— 那句话正好把「唯一一个该现算却写死的数」盖了过去，
    #   起的是反作用。改成如实枚举）：
    #   **能现算的一律现算**，import 期从 series/enx.csv 与 series/enx_breaks.csv 读 ——
    #     · ADV 的恒等式与「官方给了几列交易日」（`_day_cols()`）；
    #     · 雅典占单股期货的比重、并表月（`_athex_share()` / `_athex_month()`，
    #       与序列中文名共用同一个字符串，这里不抄）；
    #     · 并表后季末月的脉冲倍数（`_quarter_pulse()` —— 上一版写死成「5–7 倍」，
    #       抄的是 fetch/enx.py 口径坑 2 的旧快照，实测下沿已经掉到 4 倍出头）；
    #     · 台账里标着「已重述」的断点月（`_RESTATE_MS`）。
    #   **写死的只有申报／官方脚注里的历史常数**，每一个在页尾 notes 里都有对应的一条：
    #     · Nord Pool 66% / 34% 股权与 2020-01-15 完成收购、2020-01-16 起并表
    #       —— 页尾「左半边空白是业务史」那条；
    #     · Borsa Italiana 2021-04-29 交割 —— 同条，并见「2022-01 是披露起点」那条；
    #     · 2021-05 发行人家数改算法、2023-11 清算扩容、2025-06 新增挂牌口径扩大
    #       —— 都是台账里的断点月，页尾「口径断点」那条逐月列着。
    #   除这两类之外一个数都不写：拿不准就不写，少一个数不是缺陷。
    'glossary': [
        ('ADV',
         '日均（average daily volume / value）：<b>当月合计 ÷ 当月天数</b> —— '
         '「这个月多开了几天市」这一层已经被除掉了。'
         '⚠ 本页各条 ADV 的<b>分母不是同一个</b>：官方给每个板块各配了一列日数'
         + ((f'（现算 series/enx.csv 里有 {_NDAYCOL} 列）') if _NDAYCOL else '')
         + '，现货、股权衍生品、固收、商品、FX 各用各的；其中 Nord Pool 现货电力那两条'
           '除的是<b>自然日</b>（30/31）而不是交易日。'
           '带「未平仓」「只数」「市值」「托管资产」的列，以及「发行人家数（<b>月末</b>）」，'
           '是月末时点值，<b>不除天数</b>，也就不叫 ADV；'
           '⚠ 同样带「家数」的「当月新增股票挂牌家数」<b>不在其列</b>，它是当月流量'
           '（页尾「存量与流量分开读」那条逐列点了名，以那条为准）。'
           '现货金额那一条官方自己写作 ADNV（日均名义成交额）。'),

        ('单边计 / 双边计',
         '同一张官方表里两种计数惯例并存，本页<b>逐列</b>标注：现货成交额<b>单边计</b>'
         '（表头 "single counted"）、现货成交笔数<b>买卖双边计</b>'
         '（表头 "Transactions (buy and sell)"，且含 reported trades）、'
         '股票清算单边、债券清算双边、Nord Pool 电力双边。'
         '双边计相当于把整列乘一个常数 ⇒ <b>同比不受影响</b>，两种口径的增长率可以直接对读；'
         '但拿双边计的笔数去除单边计的金额，得到的「每笔平均成交额」<b>不可读</b> —— '
         '本页因此不画量价分解图（见页尾 📌 那一条）。'),

        ('reported trades',
         '官方在现货成交笔数那一列的口径里明写 <code>reported trades included</code>'
         '（Q2 业绩稿原文）：这一列<b>不只是交易所撮合成交</b>，还含向交易所'
         '<b>报备</b>的成交。⇒ 同一个月里它<b>只多不少</b>于撮合笔数，'
         '不能当作撮合活跃度的干净读数，也不能拿去与只报撮合笔数的同业直接对比。'
         '（本仓没有核到官方对 reported trades 的进一步展开 —— 哪些场景算报备、'
         '门槛是什么 —— 所以这里不给一个猜的定义。）'),

        ('未平仓（月末）',
         '未平仓合约（官方表头 "Open Interest (in lots)"）：<b>月末仍然存续、尚未平仓或交割</b>'
         '的合约张数；电力那条计的是名义量（"Notional Open Interest (in GWH)"）。'
         '它是某一天的<b>截面</b>（存量），<b>不是</b>当月成交量，也<b>不除天数</b>；'
         '与本页其余「日均 / 当月合计」的流量列不能相加。'),

        ('重述 / 断点',
         '两件<b>相反</b>的事，本页都有。<b>重述</b>＝官方把历史按今天的口径重算过'
         + ((f'（台账里标着「已重述」的断点月：{"、".join(_RESTATE_MS)}）')
            if _RESTATE_MS else '')
         + '：序列内部自洽，但与当年新闻稿印出来的数字对不上。'
           '<b>断点</b>＝并表当月<b>不重述</b>被并进来的那段历史（都柏林、奥斯陆、米兰、'
           '雅典逐个并进主列），所以红色竖虚线<b>左右两侧不可比</b>，'
           '跨线的同比读出来的是口径不是增长。断点<b>逐列限定</b>，'
           '只画在真正用了那一列的图上 —— 现货的断点月与衍生品的不是同一批。'),

        ('备注列（Athex）',
         '官方在每个主指标右侧配的一列雅典数（本页 <code>athex_*</code> 各列，'
         '官方把它回填到远早于并购的年份）。它的语义<b>在并表月翻转</b>：'
         f'<b>{_ATH_M0_ZH} 之前</b>主列<b>不含</b>雅典、这一列是雅典单独数 ⇒ '
         '主列＋备注列＝官方 pro-forma；'
        ...
        ('单股期货',
         ...
         + ((f'雅典占并表后的 {_ATH_ZH}（{_ATH_M0_ZH} 起现算）') if _ATH_LO is not None
            else '雅典占了并表后的绝大部分')
         + '，那一格是<b>口径跳变、不是增长</b>。'
           '并表之后这条线还会在 3 / 6 / 9 / 12 月出现脉冲'
         + ((f'（{_ATH_M0_ZH} 起现算：{_QP_NQ} 个季末月各是 {_QP_NB} 个非季末月'
             f'中位数的 {_QP_ZH}）') if _QP_ZH else '')
         + ' —— 希腊的单股期货被当作融券／回购的替代品按季滚动，'
           '那是<b>换月</b>不是活跃度。'),

        ('MTS',
         '随 Borsa Italiana Group 并进 Euronext 的<b>欧洲主权债电子交易平台</b>'
         '（那笔交易 2021-04-29 才交割，而官方把序列回填到更早的月份，见页尾起点那条）。'
         '本页两条：MTS 现券 ADV（单边计）与 MTS 回购 TAADV。'
         '另有一条「<b>MTS 以外</b>的债券成交 ADV」，那是 Euronext 各地债券市场'
         '（2025-11 起含雅典），量级差两三个数量级（EUR mn/日 vs EUR bn/日），'
         '所以两者<b>不画在同一根轴上</b>。'),

        ('TAADV（MTS 回购）',
         'Term Adjusted ADV —— 按回购<b>期限</b>调整之后的日均量，官方季报印的就是这条，'
         '本页画的也是它。⚠ 同一张官方表里另有一条<b>未经期限调整</b>的 MTS Repo 日均，'
         '两条同时存在、数值不同 —— 当成同一条序列的两次读数，就会读出一个不存在的跳变。'),

        ('MATIF',
         '巴黎的商品衍生品市场。本页「商品期货 / 商品期权 ADV」是它的<b>农产品</b>合约'
         '（小麦 / 玉米 / 菜籽），<b>不是能源</b> —— Euronext 的能源侧是另一块'
         '（Nord Pool 电力）。跨家配对时它的对手是 CME 的农产品合约，配能源是错的。'),

        ('Nord Pool 电力',
         'Euronext 于 2020-01-15 完成收购 66% 股权与表决权（余下 34% 由原股东输电系统'
         '运营商持有）、自 2020-01-16 起并表的北欧电力交易所。'
         '本页「日前市场 / 日内市场 ADV」是它的<b>现货</b>电力：官方口径<b>买卖双边计</b>，'
         '且日均的分母是<b>自然日</b>而不是交易日 —— 与本页其余 ADV 不是同一个分母。'),

        ('电力衍生品',
         '与现货电力分开的另一块（官方 FICC Markets 脚注 (5) 原文 "Power derivatives '
         'market became fully operational on 16 March 2026"），本页三列：'
         '系统价格期货 ADV、EPAD 期货 ADV 与名义未平仓。'
         '计量是<b>名义量</b>（GWh）不是张数，日均的分母是<b>交易日</b>'
         '（与现货电力那条自然日不是同一列）。'
         '⚠ 这块的历史极短：样本攒够之前同比与 3Y %ile 都是空的 —— '
         '那<b>不是</b>数据缺失，2026-03 之前官方没有这个市场。'
         '（EPAD 沿用官方表头 "EPADs futures" 的写法；本仓没有核到官方给的展开，'
         '就不在这里给一个猜的。）'),

        ('清算笔数 / 手数',
         'Euronext Clearing 清算的股票交易量（官方标签 "Shares (nb of contracts)"，'
         '季报写作 number of transactions and lots cleared），<b>单边计</b>。'
        ...
        ('CSD 托管与结算',
         ...
         'Euronext Securities 旗下波尔图 Interbolsa、奥斯陆 VPS、哥本哈根 VP Securities、'
         '米兰 Monte Titoli 四家，官方 Total 列<b>2025-11 起</b>才含雅典成五家。'
         '「CSD 托管资产」是月末托管规模（<b>存量</b>），「当月结算指令笔数」是当月发生的量'
         '（<b>流量</b>），两者不是同一类数。两条都自 2022-01 起 —— 那是官方的'
         '<b>披露起点</b>，不是任何一个并购事件的日期。'),

        ('发行人家数 / 新增挂牌',
         '「股票发行人家数」是月末的<b>公司</b>家数（存量；2021-05 官方改过它的计算方法）；'
         '「挂牌 ETF / 债券 / 基金只数」数的是<b>产品只数</b>，一家发行人可以挂很多只 —— '
         '两者<b>不是同一个量纲</b>，不要相减或相除。'
         '「当月新增股票挂牌家数」是<b>当月流量</b>：2025-06 起官方把它的口径扩到'
         '<b>所有类型的挂牌</b>（含私募配售、直接上市、市场间转板、反向并购、de-SPAC、'
         '二次上市）并已重述。'),
    ],
