...
# ── 那两张「合计柱 + 单月同比」的图注（原 ttm_yoy 专图搬过来的）─────────────
...
SPEC = {
    ...
    'groups': [
        ...
        {'zh': 'MX 衍生品 ADV' + _since('mx_adv_contracts'), 'cols': [
            ...
            # 柱是当月日均（官方直接发布），次轴是这一列自己的单月同比
            # （本列除本列，一步还原都不需要 —— 所以这里不再有 total_col）。
            'note': _NOTE_TTM_MX,
    ...
    # 「水平值柱 + 12 个月滚动同比」。2026-09 改版之后这两张图由
    # `groups[].mix` 的第一张（合计柱）产出 —— **图型与列相同，口径不同**：
    # 次轴由 12 个月滚动同比改成单月同比（全站统一，页面所有者指定），
    # 两条图注（_NOTE_TTM_MX / _NOTE_TTM_SPOT）也因此整段重写过。
