...
这个文件里没有任何一家公司的知识，也没有任何图注文案。它只回答两个问题：
  ...
  ① **这张图的左端该停在哪一格？**（`resolve`）
     ...
     突然变得可见：环比要 1 个月、单月同比要 12 个月、「同比的同比」要 24 个月
     （汇率贡献那张图的右轴就是它）。谁能带前导 null、谁必须截断，由**图型**决定，
     不由写图的人记性决定。
     ⚠️ 本文件对「要几个月历史」一无所知，也不该知道：每条腿的 lag 由调用方在
     `Leg(..., lag_zh=...)` 里用中文写出来，裁决只看数组里第一个有值的下标。
     所以 2026-09 把月营收柱的右轴从「12 个月滚动合计同比（24 个月历史）」换成
     「单月同比（12 个月历史）」，这里一行都不用改 —— 派生线晚起几期、图注里那句
     「比柱短 N 期」的 N，都是从新数组自己数出来的。
...
def _selftest():
    ...
    w = resolve('gs_bar', [Leg('a', '柱', bar, 'primary'),
                           Leg('b', '同比的同比', lag24, 'derived', '要 24 个月')], lab, 0)
    ...
    dead = [None] * N
    w = resolve('gs_bar', [Leg('a', '柱', bar, 'primary'),
                           Leg('b', '次轴同比', dead, 'derived', '历史不够')], lab, 0)
