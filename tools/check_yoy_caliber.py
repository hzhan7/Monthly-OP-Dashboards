...
# 0.05，`tmx` 的 Alpha-X & Alpha DRK **成交股数**那张会认到 `enx.csv:athex_mktcap_eurtn`
# 头上」复现不出来 —— 0.02 / 0.05 / 0.5 三档下它一律命中
# `tmx.csv:alphax_drk_volume_shares`（误差 ~5e-07、没有第二个候选），而
# `athex_mktcap_eurtn` 在任何一档上都没当过任何一条序列的最佳匹配。
# **没跑过的例子不许写**，这条规矩就是被它换来的。
# ⚠️ **按内容点名、不写 Exhibit 号**（与 `build/specs/tmx.py` 自己立的同一条规矩）：
# 上一版这里写的是「`tmx` Ex23」，而 2026-09 那一轮改版**在同一个月里把 /tmx/ 的图号
# 动了两次**（先是开篇柱与同列的 mix 合计柱并成一张，全页左移一位；再是删掉一张量价
# 分解、新增一张锚点图、两张分解各前移到自己的输入之后），那个写死的号每动一次就指到
# 一个不同的东西上。这是一条**照着重跑就该复现**的例子（不是带时刻的存档快照，
# 那种见本文件 R8 那一段），所以它必须永不过期：说清楚是哪一列，
# 别写会随增删图整体位移的号 —— 这里也就不再补一句「它现在指着谁」，
# 那句话下一次增删同样会过期。
#
