...
2) 季末月无独立月报 → 见"发布节奏"。本模块只认 Historical File，**不从季报推算**。
   季报只给季度合计 NNA，要拿季末月得用"季合计 − 前两月"倒挤，实测有 ±0.1 的四舍五入误差
   （2026Q1 倒挤 Mar brokerage = −1.5，官方 Historical File 是 −1.6），
   为了不把倒挤值当真值写进唯一真值表，默认不做；真要抢那一个月见 quarter_end_estimate()。
3) **PDF 行名 2026 年 4 月期改过版**，两套词表都得认，否则静默取不到值：
...
4) 表尾脚注区里还有一张 "Organic NNA from Large Institutions" 小表，行名同样是 Advisory/Brokerage。
   解析前必须在第一条脚注 "(1) " 处截断，否则小表会覆盖正表。
5) client_cash_usdbn 取 **Total Client Cash Balances**（含 CCA），不是 Total Bank Sweep、
   也不是 Total Client Cash Sweep Held by Third Parties。三者在表里上下相邻，很容易拿错。
6) 官方会把 sweep 货基转成 purchased money market（2025-11 转 1.6B、2026-02 转 0.5B）。
   这会让 client cash 出现**非资金流动导致的**下降，看图别当成客户提现。
7) 官方原表明写 "Totals may not foot due to rounding"，advisory + brokerage 与 total 差 0.1 属正常，
   所以本模块**不做 total = adv + brk 的硬校验**，只做"列必须存在"的硬校验。
8) 2025 年的 organic NNA 里含 OSJ（misaligned large OSJ）分离造成的流出，官方在脚注里逐月列出。
        ...
        if not vals:                                  # 无数字 = 节标题
            for pat, tag in (('client assets', 'assets'),
                ...
                if low.startswith(pat):
                    ...
                    break
            continue
        label = re.split(r'\s+\(?-?\$?[\d,]', s)[0].strip().lower()
...
def _lookup(rows, col, rule, path):
    ...
    for label in rule['loose']:
        ...
        if len(hits) == 1:
            return hits[0]
        if len(hits) > 1:
            raise ParseError('列 %s 的行名 %r 在 %s 里出现 %d 次，无法消歧'
...
def quarter_end_estimate(series_dir, cache_dir, quarter=None):
    ...
    def first(*keys):
        for k in keys:
            if k in rows:
                return rows[k][0]                       # 季报第一列 = 本季
        raise ParseError('季报里找不到行 %r' % (keys,))
    ...
    vals = {
        'advisory_assets_usdbn':  first(('assets', 'advisory')),
        'brokerage_assets_usdbn': first(('assets', 'brokerage')),
        'total_assets_usdbn':     first(('assets', 'total client assets')),
        'client_cash_usdbn':      first(('cash', 'total client cash balances')),
    }
    for col, key in (('nna_advisory_usdbn', ('total_nna', 'advisory')),
                     ('nna_brokerage_usdbn', ('total_nna', 'brokerage')),
                     ('nna_total_usdbn', ('total_nna', 'total nna'))):
        vals[col] = round(first(key) - sum(existing[p][col] for p in prior), 1)
