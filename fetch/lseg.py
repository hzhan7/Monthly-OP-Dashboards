...
════════════════════════════════════════════════════════════════════════════
（「实测覆盖」这一列的日期 2026-08-19，是**逐列**量出来的 —— 同一路里各列的起点
可以不同，写成一个区间会骗人。判据：series/lseg.csv 里该列首个/末个非空月 + 中间空洞）
| leg         | 官方源                                   | 列数 | 实测覆盖            |
|-------------|------------------------------------------|------|---------------------|
| `orderbook` | LSEG Monthly Market Report PDF（第 1 页 MTD 表） | 17 | 17 列都是 2016-01 → 2026-07，无洞 |
| `primary`   | LSE Main Market / AIM factsheet xlsx + xls | 24   | **两段列起点不同（见 ※）**：`aim_*` 11 列 2016-01 → 2026-07 缺 2019-09；`mm_*` 13 列 2018-05 → 2026-07 缺 2022-12 |
| `tradeweb`  | Tradeweb Historical ADV xlsx（monthly activity reports）| 27 | 27 列都是 2017-01 → 2026-07，无洞 |
| `lch`       | lseg.com LCH SwapClear / ForexClear / RepoClear volumes 页 | 18 | forexclear 4 列 2024-08 起、swapclear 8 列 2025-08 起、repoclear 6 列 2025-06 → 2026-05（官方滚动窗口，各 12–24 期） |

※ `primary` 两段列起点差 28 个月，**两侧的原因完全不同，别当成同一件事**：
   AIM 的 2016 那 12 期是**老版式**（2016-01…2016-10 是 .xls/BIFF，2016-11/12 是
   老版式 xlsx），由 part 模块里另一套解析器吃下来，口径用「家数 = 上月家数 + 新上市
   − 退市」这条恒等式跨版式验过；Main Market 的 .xls 官方同样有（回到 2009-01、
   格式也读得动），是**口径接不上所以主动不做** —— 13 列里 4 列无源、1 列数法不同，
   逐列理由见 fetch/lseg_primary.py 的口径坑 11。

各路的口径坑、故障注入结果、单位守卫写在各自模块的 docstring 里，本文件不复述。
唯一要在这里说清楚的是**跨路的三件事**：

口径坑 A：**起点不是四个，是七个** —— 宽表必然是「左上角空一大片」。
    2026-08-19 实测的填充度（86 个数据列）：2016-01 那一行有 28 列
    （orderbook 17 + AIM 11），2017-01 起 55 列（+ tradeweb 27），
    2018-05 起 68 列（Main Market 那 13 列才进来），2024-08 起 72 列，
    到 2026-07 是 80 列（LCH 三块各有各的滚动窗口）。
    连「一路一个起点」都不成立：primary 内部两个市场差 28 个月，lch 内部三个法人
    差一年。这是官方档案深度的真实形状，不是抓漏。**空格就是空格** —— 本模块永不写 0、
    永不写 NaN、永不用前值/后值/插值填补（README 铁律 2 的直接后果）。
    build 侧画图时必须按**列**自己的起始月裁窗口，不能拿「宽表有 127 行」
    当成每列都有 127 个观测，也不能拿一路的起点当它每一列的起点。
