...
直链   : https://live.euronext.com/sites/default/files/statistics/ir/
         ...
         219,581 bytes（2026-08-06 实测），5 个 sheet，2012-01 → 2026-06 共 174 个月。
伴生   : .../euronext_latest_month_volumes.xlsx
         49,225 bytes，只有最新月/上月/去年同月/本季/YTD 五档，**并且给出官方算好的 ADV**。
...
━━ 发布节奏 ━━
每月都发，**没有季末月例外**（不像 SCHW / LPLA 要等季报）。本机把新闻列表页翻了 22 页
（440 条稿件），对 **2019-01 → 2026-06 共 90 个数据月逐月找它自己那期稿子，命中 90/90**：
...
**10. `Nb of trading days` 有 7 个，各管各的分母，且其中一个根本不是交易日。**
   Equity Markets 两个（现货 C3 / 股权衍生品 C16，实测 174 个月全部相等，但官方保留成
...
━━ 产出的两个文件 ━━
· `series/enx.csv` —— 72 列 × 174 行（2012-01 → 2026-06）。列序是
...
· `series/enx_breaks.csv` —— 口径断点台账，**由本模块从官方脚注原文自动抽取**，
  ...
  ⚠ 这个文件**目前没有 build 读它**（build/enx.py 还没写）。按 README 的说法，
  「没有 build 读它」不等于孤儿：删了它，2025-11 那条红色竖线该画在哪几张图上
  就只剩人脑记忆，而人脑记不住 26 列。它同时也是「官方又并购了一家」的探测器 ——
  官方改脚注，这张表下次跑就跟着变，git diff 里看得见。
...
COLUMN_SPEC = [
    ...
    # ── Nord Pool 电力衍生品：2026-03-16 全面上线，只有 4 个月 ─────────
    # 日均名义量（GWh/日）
...
def _validate(data):
    ...
    三道检查：
      ...
      2. 恒等式 Total ≡ Equities + ETF + Structured（Athex 备注列同理，它没有
         结构化产品，所以是 Total ≡ Equities + ETF）。实测 174 个月最大相对差 2.5e-16。
...
def _record_source_date(series_dir, cache_dir, month):
    ...
    只给「本次真的新入库的那个最新月」记 —— 同一份 xlsx 里躺着 174 个月，
...
def update(series_dir, cache_dir):
    ...
    （2012-01 起 174 个月）。这不是「顺手做掉」：Euronext 的单一 xlsx 本来就带全序列，
