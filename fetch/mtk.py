...
6. **序列起点 2018-01 的理由是会计准则，不是「最早可得」。**
   联发科自 **2018-01-01 起适用 IFRS 15**，且采用**修正追溯法**（"the Company elected
   to recognize the cumulative effect of initially applying IFRS 15 at the date of initial
   application"，2018 年报附注），**比较期不重编**。所以 2017-12 及以前的月营收与
   2018-01 起的月营收不在同一套收入认列口径上（销货退回与折让从"冲减应收帐款"改为
   "认列退款负债"）。年度汇总 PDF 有 2008 年起的月度数，能取到不等于口径连续。
...
def update(series_dir, cache_dir):                                 # noqa: ARG001
    ...
    years = sorted({int(m[:4]) for m in have} | {int(newest[:4])})
    official = {}
    for year in years:
    ...
    if added:
        ...
        with open(csv_path, 'w', newline='', encoding='utf-8') as fh:
            ...
            w.writerows(body)
        for m in added:
            print(f'[mtk] +{m} {have[m]:,} NT$mn（新闻稿电头 {dates[m]}）')
