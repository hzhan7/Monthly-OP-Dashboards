...
· 台湾《证券交易法》要求次月 10 日前公告上月营收。UMC 的实际公告日（附件抬头日期）
  163 个月里 **160 个落在次月 4–10 日**，最近 12 个月全部落在这个窗口：
  ...
  三个例外都在窗口右侧、都有说得通的成因（**别写成「无一例外」**，实测过）：
    2016-03 → 2016-04-11（4/10 是周日，顺延一个工作日）
    2019-01 → 2019-02-14、2020-01 → 2020-02-12（1 月号赶上农历年，交易所准予展延）
  → 这三例对本模块没有影响：`_month_from_date` 只取抬头的**月**、不看日；
    roster 的 LAG 也不是照抬头日算的，是照 EDGAR filingDate 算的（见下一条）。
