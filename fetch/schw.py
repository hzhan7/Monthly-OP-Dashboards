...
_RECAST = [
    ...
    #   这里写死全拼名，是因为本表要的是「那一份具体的文件」，不是「那一期的任一写法」：
    #   重述的判据钉在实测过的这一份上。**通用的七月坑已经由 _MON_URL 的候选机制治了**
    #   （原注写的「_template_url() 至今仍只拼三字母缩写」在 2026-09-16 合进候选机制之后
    #   不再成立，照实改掉）。元组是 _col_sources 的返回契约（候选串），不是多个候选。
...
def _col_sources(cache_dir: str):
    ...
    ── 返回值形状：[(报告月, 候选 URL 元组)] ──
    第二个元素是**一串**候选不是一个（见 _url_candidates / _download_any）。锚点选**五月**
    顺带躲过了七月那个全拼坑（may 的三字母与全拼同形），但躲不过前缀坑：2025-04 实测只有
    schwab_ 命中，同样的事哪天落在某年的五月上，这条锚点链就会整年缺一格而**不出声**
    ...
    for y in range(2016, y_now + 1):
        out.append(((y, 12), _url_candidates('quarterly', (y, 12))))
...
def backfill_columns(series_dir, cache_dir, cols=_NEW_COLS,
    ...
    for ym, urls in _col_sources(cache_dir):
        ...
        if fp is None:
            ...
            log(f'  [cdn] {urls[0].rsplit("/", 1)[-1]} 取不到（404）—— 已落盘的历史行不受影响')
            continue
