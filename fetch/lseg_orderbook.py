...
抛出来的后果是**降级不是 FAIL**：`fetch/lseg.py` 的 `_update` 逐路 catch，这里抛只让
orderbook 这一路当天不前进，其余三路照常合流发布；自 2026-09 起，`fetch/lseg.py` 把
降级的路记进 `DEGRADED`，`monthly_run` 把 lseg 计入末行失败清单，并在末行前印明细，
所以「出声」不再只是日志中段的一行 ⚠（`⚠ orderbook 这一路本轮失败` 与
「本轮降级/报警的路：orderbook」）。
