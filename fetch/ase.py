...
2) 数值源：月度新闻稿 PDF（英文版）
   ...
   重述体检就靠这个（口径坑 6）。
   四张表的列头逐字相同、月份键逐份完全一致（99 份实测：C/A × NT/US 各 284 个
   月读数 = 99 自期 + 98 上月 + 87 去年同月，四者一个不差）——
   `_pdf_tables()` 把这条当**硬不变量**校验，四表月份键不齐就抛异常，
...
5. **ATM 有过一次追溯重述，合并没有。**
   ...
   2026-08-14 全量重扫（99 期 × 4 表 × 每月 3 次观测）实测：
     · `revenue_usd_mn`（合并 US$）：99 个月 × 3 次观测**零分歧**，与 NT$ 合并同。
...
6. **重述体检**：每份新 PDF 自带上月与去年同月两列，**四张表都有**，
   所以美元两列与新台币两列一样每月拿得到 3 次独立观测。本模块把这些读数与
...
8. **季末月与 12 月的 PDF 里还有 Q 表与 FY 表**，列头是 `Q1`/`FY` 而不是月份缩写，
   而且 **Q 表、FY 表也都是 NT$/US$ 各一张**（99 期实测：Q 表四表各 91 个读数、
   年度表四表各 6 个读数）。本模块按列头是不是月份缩写来筛，Q/FY 列直接丢掉
...
12. **美元两列是官方实绩，不是折算值 —— 而「折算值」正是最像真的那种污染。**
    ...
    99 个月落在 **27.5989（2022-01）~ 32.8371（2025-04）**，与 `series/tsm_fx.csv`
...
# NT$ 混进 US$ 槽 ⇒ 比值 1.0；反向错配 ⇒ 约 1000。实测 99 个月落在 27.60~32.84。
...
# 判据与「这张表是哪种货币」的判据。99 期实测各出现 310 次，写法零变体。
...
def _pdf_tables(blob):
    ...
    # 四表月份键必须逐字相同（99 期实测无一例外，见「数据源」第 2 条）。
